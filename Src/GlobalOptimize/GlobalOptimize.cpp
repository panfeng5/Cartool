...
namespace crtl
{
    ...
    {
        if (m == GlobalBoxScan)
            return "Global Box-Scan";
        else if (m == CyclicalBoxScan)
            return "Cyclical Box-Scan";
        else if (m == WeakestGroupBoxScan)
            return "Weakest Group Box-Scan";
        else if (m == WeakestDimensionBoxScan)
            return "Weakest Dimension Box-Scan";

        else if (m == GlobalCrossHairScan)
            return "Global Crosshair-Scan";
        else if (m == CyclicalCrossHairScan)
            return "Cyclical Crosshair-Scan";
        else if (m == WeakestGroupCrossHairScan)
            return "Weakest Group Crosshair-Scan";
        else if (m == WeakestDimensionCrossHairScan)
            return "Weakest Dimension Crosshair-Scan";

        else if (m == GlobalNelderMead)
            return "Global Nelder-Mead";
        else if (m == CyclicalNelderMead)
            return "Cyclical Nelder-Mead";
        else if (m == WeakestGroupNelderMead)
            return "Weakest Group Nelder-Mead";

        else
            return IsMethodScanBox(m)         ? "Box-Scan Method"
                                       ...
                                       bool forceevaluate)
    {
        ...
        for (int v = 0; v < simplexdim; v++)
        {
            ...
            {
                simplex.BestVertex = v;
                bestv = simplex.Value(v);
            }
        }
...
#define NelderMeadShrink 0.60
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // 5.1) Reflect
        TVector<double> SimplexReflect = simplex.Centroid * (1 + NelderMeadReflect) - simplex[worsti1] * NelderMeadReflect;
        // copy to parameters
        ...
        if (reflectv < bestv)
        {
            ...
            TVector<double> SimplexExpand = simplex.Centroid * (1 + NelderMeadExpand) - simplex[worsti1] * NelderMeadExpand;
            // copy to parameters
        ...
        for (int v = 0; v < simplexdim; v++)
        {
            ...
            simplex[v] = simplex[simplex.BestVertex] * (1 - NelderMeadShrink) + simplex[v] * NelderMeadShrink;
            // copy to parameters
                                      ...
                                      TEasyStats *stat)
    {
        ...
        for (Iteration = 0; Iteration < GOMaxIterations; Iteration++)
        {
            ...
            if (IsMethodScanBox(method) || IsMethodCrossHairScan(method))
            {
            ...
            double reldiff = (log(CurrentPrecision) - log(requestedprecision)) / (log(1) - log(requestedprecision));
