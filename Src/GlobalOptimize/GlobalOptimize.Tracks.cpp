...
namespace crtl
{
    ...
    {
        if (How & FunctionsMixture)
            return EvaluateFunctionsMixture(stat);
        else
            return 0;
    }
    ...
    double TTrackAnalysis::EvaluateFunction(double x, int g)
    {
        ...
        if (kerneltype != SupportKernelNone)
        {
            ...
            case SupportKernelUniform:
                kernelw = u <= 1 ? 1 : 0;
                break;
            case SupportKernelTriangular:
                kernelw = u <= 1 ? 1 - u : 0;
                break;
            case SupportKernelQuartic:
                kernelw = u <= 1 ? Square(1 - Square(u)) : 0;
                break;
            case SupportKernelTriweight:
                kernelw = u <= 1 ? Cube(1 - Square(u)) : 0;
                break;
            case SupportKernelTricube:
                kernelw = u <= 1 ? Cube(1 - Cube(u)) : 0;
                break;
            case SupportKernelRaisedCosine:
                kernelw = u <= 1 ? (cos(u * Pi) + 1) / 2 : 0;
                break;
            }
        ...
        for (int i = minindex; i <= maxindex; i++)
        {
            ...
            if (How & MoreCompact)
            {
                for (int g = 0; g < GetNumFunctions(); g++)
                    ...
                    delta += sqrt(GetValue(g * NumFunctionParams + FunctionHeight) * GetValue(g * NumFunctionParams + FunctionWidth) / numdata) * morecompactfactor;
            ...
            if (How & KeepOrdering)
            {
                for (int g = 1; g < GetNumFunctions(); g++)
                    if (GetValue(g * NumFunctionParams + FunctionCenter) < GetValue((g - 1) * NumFunctionParams + FunctionCenter))
                        ...
                        delta += (GetValue((g - 1) * NumFunctionParams + FunctionCenter) - GetValue(g * NumFunctionParams + FunctionCenter)) / numdata * keeporderingfactor;
            }
