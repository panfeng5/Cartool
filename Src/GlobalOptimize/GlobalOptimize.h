...
#define GOStepsPrecise 2 * 5 + 1, 1, 4.0 / 5.0
    ...
    public:
        ...
        void SetValue(int type, double value)
        {
            if (HasValue(type))
                *ToValues[type] = value;
        } // force set AN EXISTING PARAMETER
#if defined(_DEBUG)
        // missing type will crash, better give some warning
        double GetValue(int type) const
        {
            if (HasValue(type))
                return *ToValues.at(type); /*assert ( HasValue ( type ) );*/
            DBGV(type, "TGlobalOptimize::GetValue: undefined type");
            return 0;
        }
        double GetMinValue(int type) const
        {
            if (HasValue(type))
                return *ToMin.at(type); /*assert ( HasValue ( type ) );*/
            DBGV(type, "TGlobalOptimize::GetMinValue: undefined type");
            return 0;
        }
        double GetMaxValue(int type) const
        {
            if (HasValue(type))
                return *ToMax.at(type); /*assert ( HasValue ( type ) );*/
            DBGV(type, "TGlobalOptimize::GetMaxValue: undefined type");
            return 0;
        }
#else
