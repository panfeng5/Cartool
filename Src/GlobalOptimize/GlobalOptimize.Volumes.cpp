...
namespace crtl
{
                                        ...
                                        TEasyStats *stat)
    {
        ...
        if (How == SagittalPlaneSymmetric || How == SagittalPlaneSymmetricT1 || How == SagittalPlaneSymmetricT1Gad)
        ...
        else if (How == TransversePlaneLongest || How == TransversePlaneBiggestBox || How == TransversePlaneBiggestSurface)
        {
            // !original NormToMRI will be lost!
        ...
        else if (IsTransversePlaneMNI(How))
        {
            ...
            if (HasValue(RotationX))
                NormToMRI.RotateX(GetValue(RotationX), MultiplyRight);
        }
    }
    ...
    double TVolumeProperties::Evaluate(TEasyStats *stat)
    {
        ...
        if (How == TransversePlaneLongest)
            return EvaluateTransversePlaneLongest(stat);
        else if (How == TransversePlaneBiggestBox)
            return EvaluateTransversePlaneBiggestBox(stat);
        else if (How == TransversePlaneBiggestSurface)
            return EvaluateTransversePlaneBiggestSurface(stat);
        else if (How == TransversePlaneMNIT1 || How == TransversePlaneMNIT2)
            return EvaluateTransversePlaneMNI(stat);
        else if (How == TransversePlaneGuillotine)
            return EvaluateTransversePlaneGuillotine(stat);
        else if (How == SagittalPlaneSymmetric || How == SagittalPlaneSymmetricT1 || How == SagittalPlaneSymmetricT1Gad)
            return EvaluateSagittalPlaneSymmetric(stat);
        else
            return 0;
    }
    ...
    inline void TVolumeProperties::EvaluateSagittalPlaneMatrix()
    {
        ...
        if (HasValue(TranslationX))
            Center[LeftRightIndex] = GetValue(TranslationX);
        if (HasValue(TranslationY))
            Center[FrontBackIndex] = GetValue(TranslationY);
        if (HasValue(TranslationZ))
            Center[UpDownIndex] = GetValue(TranslationZ);

        NormToMRI.Translate(Center[0], Center[1], Center[2], MultiplyLeft);

        if (HasValue(ShearYtoX) || HasValue(ShearZtoX))
            NormToMRI.ShearX(GetValue(ShearYtoX), GetValue(ShearZtoX), MultiplyRight);
        if (HasValue(ShearXtoY) || HasValue(ShearZtoY))
            NormToMRI.ShearY(GetValue(ShearXtoY), GetValue(ShearZtoY), MultiplyRight);
        if (HasValue(ShearXtoZ) || HasValue(ShearYtoZ))
            NormToMRI.ShearZ(GetValue(ShearXtoZ), GetValue(ShearYtoZ), MultiplyRight);

        if (HasValue(RotationX))
            NormToMRI.RotateX(GetValue(RotationX), MultiplyRight);
        if (HasValue(RotationY))
            NormToMRI.RotateY(GetValue(RotationY), MultiplyRight);
        if (HasValue(RotationZ))
            NormToMRI.RotateZ(GetValue(RotationZ), MultiplyRight);
    }
    ...
    double TVolumeProperties::EvaluateSagittalPlaneSymmetric(TEasyStats *stat)
    {
        ...
        OmpParallelForSum(sumslice, sumw)
            ...
            for (int z = MinCorner.Z; z <= MaxCorner.Z; z += Step.Z) for (int y = MinCorner.Y; y <= MaxCorner.Y; y += Step.Y)
        {
    ...
    double TVolumeProperties::EvaluateTransversePlaneGuillotine(TEasyStats *stat)
    {
        ...
        OmpParallelForSum(sumslice, numsum)
            ...
            for (int y = MinCorner.Y; y <= MaxCorner.Y; y += Step.Y) for (int x = MinCorner.X; x <= MaxCorner.X; x += Step.X)
        {
...
#define extfactor 2.00
    ...
    inline void TVolumeProperties::EvaluateTransversePlaneMatrix(TMatrix44 &normtomri)
    {
        ...
        if (HasValue(Scale))
            normtomri.Scale(GetValue(Scale), GetValue(Scale), GetValue(Scale), MultiplyRight);
        if (HasValue(ScaleY))
            normtomri.ScaleY(GetValue(ScaleY), MultiplyRight);
        if (HasValue(ScaleZ))
            normtomri.ScaleZ(GetValue(ScaleZ), MultiplyRight);
    }
    ...
    */
    ...
    inline void TFitPointsOnVolume::Transform(TPointFloat &p)
    {
        // translations
        if (HasValue(TranslationX))
            p.X += GetValue(TranslationX);
        if (HasValue(TranslationY))
            p.Y += GetValue(TranslationY);
        if (HasValue(TranslationZ))
            p.Z += GetValue(TranslationZ);
        ...
        if (HasValue(RotationX) || HasValue(RotationY) || HasValue(RotationZ))
        {
            ...
            if (HasValue(RotationX))
                mat.RotateX(GetValue(RotationX), MultiplyLeft);
            if (HasValue(RotationY))
                mat.RotateY(GetValue(RotationY), MultiplyLeft);
            if (HasValue(RotationZ))
                mat.RotateZ(GetValue(RotationZ), MultiplyLeft);
    ...
    void TFitPointsOnVolume::GetMatrix(TMatrix44 &m)
    {
        ...
        // translations
        if (HasValue(TranslationX))
            m.TranslateX(GetValue(TranslationX), MultiplyLeft);
        if (HasValue(TranslationY))
            m.TranslateY(GetValue(TranslationY), MultiplyLeft);
        if (HasValue(TranslationZ))
            m.TranslateZ(GetValue(TranslationZ), MultiplyLeft);
        ...
        if (HasValue(RotationX) || HasValue(RotationY) || HasValue(RotationZ))
        {
            ...
            if (HasValue(RotationX))
                m.RotateX(GetValue(RotationX), MultiplyLeft);
            if (HasValue(RotationY))
                m.RotateY(GetValue(RotationY), MultiplyLeft);
            if (HasValue(RotationZ))
                m.RotateZ(GetValue(RotationZ), MultiplyLeft);
        }

        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        if (HasValue(Scale))
            m.Scale(GetValue(Scale), GetValue(Scale), GetValue(Scale), MultiplyLeft);
        if (HasValue(ScaleX))
            m.ScaleX(GetValue(ScaleX), MultiplyLeft);
        if (HasValue(ScaleY))
            m.ScaleY(GetValue(ScaleY), MultiplyLeft);
        if (HasValue(ScaleZ))
            m.ScaleZ(GetValue(ScaleZ), MultiplyLeft);
    }
...
#define StepToGaussianSmooth(S) (2 * (S))
    ...
    void TFitVolumeOnVolume::EvaluateMatrices()
    {
        ...
        if (HasValue(Scale))
            ToAbs_FromAbs.Scale(GetValue(Scale), GetValue(Scale), GetValue(Scale), MultiplyLeft);
        if (HasValue(ScaleX))
            ToAbs_FromAbs.ScaleX(GetValue(ScaleX), MultiplyLeft);
        if (HasValue(ScaleY))
            ToAbs_FromAbs.ScaleY(GetValue(ScaleY), MultiplyLeft);
        if (HasValue(ScaleZ))
            ToAbs_FromAbs.ScaleZ(GetValue(ScaleZ), MultiplyLeft);
        ...
        // rotations
        if (HasValue(RotationX))
            ToAbs_FromAbs.RotateX(GetValue(RotationX), MultiplyLeft);
        if (HasValue(RotationY))
            ToAbs_FromAbs.RotateY(GetValue(RotationY), MultiplyLeft);
        if (HasValue(RotationZ))
            ToAbs_FromAbs.RotateZ(GetValue(RotationZ), MultiplyLeft);
        ...
        // translations
        if (HasValue(TranslationX))
            ToAbs_FromAbs.TranslateX(GetValue(TranslationX), MultiplyLeft);
        if (HasValue(TranslationY))
            ToAbs_FromAbs.TranslateY(GetValue(TranslationY), MultiplyLeft);
        if (HasValue(TranslationZ))
            ToAbs_FromAbs.TranslateZ(GetValue(TranslationZ), MultiplyLeft);
        ...
        if (HasValue(FitVolumeNormCenterRotateX))
            ToAbs_FromAbs.RotateX(GetValue(FitVolumeNormCenterRotateX), MultiplyLeft);
        if (HasValue(FitVolumeNormCenterRotateZ))
            ToAbs_FromAbs.RotateZ(GetValue(FitVolumeNormCenterRotateZ), MultiplyLeft);

        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // shearing

        if (HasValue(FitVolumeShearXtoY))
            ToAbs_FromAbs.ShearX(GetValue(FitVolumeShearXtoY), 0, MultiplyLeft);
        if (HasValue(FitVolumeShearXtoZ))
            ToAbs_FromAbs.ShearX(0, GetValue(FitVolumeShearXtoZ), MultiplyLeft);
        if (HasValue(FitVolumeShearYtoX))
            ToAbs_FromAbs.ShearY(GetValue(FitVolumeShearYtoX), 0, MultiplyLeft);
        if (HasValue(FitVolumeShearYtoZ))
            ToAbs_FromAbs.ShearY(0, GetValue(FitVolumeShearYtoZ), MultiplyLeft);
        if (HasValue(FitVolumeShearZtoX))
            ToAbs_FromAbs.ShearZ(GetValue(FitVolumeShearZtoX), 0, MultiplyLeft);
        if (HasValue(FitVolumeShearZtoY))
            ToAbs_FromAbs.ShearZ(0, GetValue(FitVolumeShearZtoY), MultiplyLeft);
            ...
            for (int y = 0; y < expvol.Dimension.Y; y++)
                for (int x = 0; x < expvol.Dimension.X; x++)
                {
                    ...
                    { // numsubsampling != 1
                        ...
                        if (filtertype == FilterTypeMedian)
                            median.Reset();
                        v = 0;
                        count = 0;
                        ...
                        for (int zd = 0; zd < numsubsampling; zd++)
                            for (int yd = 0; yd < numsubsampling; yd++)
                                for (int xd = 0; xd < numsubsampling; xd++)
                                {
                                    ...
                                    if (filtertype == FilterTypeMedian)
                                        median.Add(getvalue, ThreadSafetyIgnore);
