...
namespace crtl
{
    ...
    double TFitModelOnPoints::Evaluate(TEasyStats *stat)
    {
        ...
        if (How == ContainModelNorm1 || How == ContainModelNorm2)
        {
            // icosahedron / 12 vertices
            //  double              phi             = ( 1 + sqrt ( 5.0 ) ) / 2;
#define phi 1.6180339887498948482045868343656

            pm[0] = TPointFloat(0, 1, phi) + tooptimalcenter;
            Transform(pm[0]);
            pm[1] = TPointFloat(0, -1, -phi) + tooptimalcenter;
            Transform(pm[1]);
            pm[2] = TPointFloat(0, 1, -phi) + tooptimalcenter;
            Transform(pm[2]);
            pm[3] = TPointFloat(0, -1, phi) + tooptimalcenter;
            Transform(pm[3]);
            pm[4] = TPointFloat(1, phi, 0) + tooptimalcenter;
            Transform(pm[4]);
            pm[5] = TPointFloat(-1, -phi, 0) + tooptimalcenter;
            Transform(pm[5]);
            pm[6] = TPointFloat(1, -phi, 0) + tooptimalcenter;
            Transform(pm[6]);
            pm[7] = TPointFloat(-1, phi, 0) + tooptimalcenter;
            Transform(pm[7]);
            pm[8] = TPointFloat(phi, 0, 1) + tooptimalcenter;
            Transform(pm[8]);
            pm[9] = TPointFloat(-phi, 0, -1) + tooptimalcenter;
            Transform(pm[9]);
            pm[10] = TPointFloat(-phi, 0, 1) + tooptimalcenter;
            Transform(pm[10]);
            pm[11] = TPointFloat(phi, 0, -1) + tooptimalcenter;
            Transform(pm[11]);
...
#undef phi
            ...
            for (int i = 0; i < (int)Points; i++)
        {
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            else if (How == ContainModelNorm1 || How == ContainModelNorm2)
            {
    ...
    double TFitModelOnPoints::GetMaxRadius() const
    {
        ...
        else if (HasValue(ScaleX) && HasValue(ScaleY) && HasValue(ScaleZ))
            return max(GetValue(ScaleX), GetValue(ScaleY), GetValue(ScaleZ));
        else
            return 1;
    }
    ...
    double TFitModelOnPoints::GetMeanRadius() const
    {
        if (HasValue(Scale))
            ...
            return (GetValue(ScaleX) + GetValue(ScaleY) + GetValue(ScaleZ)) / 3.0;
        else
            return 1;
    }
    ...
    TDipole TFitModelOnPoints::Spherize(const TDipole &d) const
    {
        ...
        // transform vector
        ds.Direction = Spherize(d.Position + d.Direction) - ds.Position;
    ...
    void TFitModelOnPoints::Transform(TPointFloat &p) const
    {
        ...
        do
        {
            ...
            if (cos0 > cosr && cos0 > cosl)
            {
                ...
                if (cos0 > cosu && cos0 > cosd)
                    ; // pcurr += 0,            cos0    = cos0;     // remaining centered
                else if (cosu > cosd)
                    pcurr += o2, cos0 = cosu;
                else /*if ( cosd > cosu )*/
                    pcurr -= o2, cos0 = cosd;
            ...
            else if (cosr > cosl)
            {
                ...
                if (cos0 > cosu && cos0 > cosd)
                    ...
                    pcurr += (o1 + o2), cos0 = sqrt(2 - (cosr * cosr + cosu * cosu));
                else /*if ( cosd > cosu )*/
                    pcurr += (o1 - o2), cos0 = sqrt(2 - (cosr * cosr + cosd * cosd));
            }
            ...
            { // if ( cosl > cosr ) {
                ...
                if (cos0 > cosu && cos0 > cosd)
                    ...
                    pcurr -= (o1 - o2), cos0 = sqrt(2 - (cosl * cosl + cosu * cosu));
                else /*if ( cosd > cosu )*/
                    pcurr -= (o1 + o2), cos0 = sqrt(2 - (cosl * cosl + cosd * cosd));
            }
    ...
    void TFitModelOnPoints::_Transform(TPointFloat &p) const
    {
        ...
        // Various flattening
        if (HasValue(FlattenX))
            p.X += GetValue(FlattenX) * sin(p.X * HalfPi);
        if (HasValue(FlattenXPos) && p.X > 0)
            p.X += GetValue(FlattenXPos) * sin(p.X * HalfPi);
        if (HasValue(FlattenXNeg) && p.X < 0)
            p.X += GetValue(FlattenXNeg) * sin(p.X * HalfPi);

        if (HasValue(FlattenY))
            p.Y += GetValue(FlattenY) * sin(p.Y * HalfPi);
        if (HasValue(FlattenYPos) && p.Y > 0)
            p.Y += GetValue(FlattenYPos) * sin(p.Y * HalfPi);
        if (HasValue(FlattenYNeg) && p.Y < 0)
            p.Y += GetValue(FlattenYNeg) * sin(p.Y * HalfPi);

        if (HasValue(FlattenZ))
            p.Z += GetValue(FlattenZ) * sin(p.Z * HalfPi);
        if (HasValue(FlattenZPos) && p.Z > 0)
            p.Z += GetValue(FlattenZPos) * sin(p.Z * HalfPi);
        if (HasValue(FlattenZNeg) && p.Z < 0)
            p.Z += GetValue(FlattenZNeg) * sin(p.Z * HalfPi);
        ...
        if (HasValue(TopBumpX) || HasValue(TopLateralBumpX))
            alpha = RadiansToDegrees(ArcTangent(p.Y, p.Z));
        if (HasValue(TopBumpY) || HasValue(TopLateralBumpY))
            alpha = RadiansToDegrees(ArcTangent(-p.Z, p.X));
...
#define topbumpextent 45.0
        ...
        if (HasValue(TopBumpX) && p.Y < 0 && alpha > -topbumpextent && alpha < topbumpextent)
        {
        ...
        if (HasValue(TopBumpY) && p.Z > 0 && alpha > -topbumpextent && alpha < topbumpextent)
        {
...
#define latbumpxcenter 0.33
        ...
        if (HasValue(TopLateralBumpX) && (alpha >= latbumpmiddle1 - latbumpextent && alpha <= latbumpmiddle1 + latbumpextent || alpha >= latbumpmiddle2 - latbumpextent && alpha <= latbumpmiddle2 + latbumpextent))
        {
        ...
        if (HasValue(TopLateralBumpY) && (alpha >= latbumpmiddle1 - latbumpextent && alpha <= latbumpmiddle1 + latbumpextent || alpha >= latbumpmiddle2 - latbumpextent && alpha <= latbumpmiddle2 + latbumpextent))
        {
        ...
        */
        ...
        if (HasValue(SinusPinchXtoY))
            p.Y *= 1 + GetValue(SinusPinchXtoY) * sin(p.X * HalfPi);
        if (HasValue(SinusPinchXtoZ))
            p.Z *= 1 + GetValue(SinusPinchXtoZ) * sin(p.X * HalfPi);

        if (HasValue(SinusPinchYtoX))
            p.X *= 1 + GetValue(SinusPinchYtoX) * sin(p.Y * HalfPi);
        if (HasValue(SinusPinchYtoZ))
            p.Z *= 1 + GetValue(SinusPinchYtoZ) * sin(p.Y * HalfPi);
        ...
        if (HasValue(ShearXtoY))
            p.Y += GetValue(ShearXtoY) * p.X;
...
#define deltaangle 0.1
        ...
        // translate back to original space
        if (translate)
            points -= *translate; // specified by caller
        else
            points -= GetTranslation(); // center of original points
    ...
    */
    ...
    {
        if (How == PointsSagittal)
            return EvaluateSagittal(stat);
        else if (How == PointsTransverse)
            return EvaluateTransverse(stat);
        else if (How == PointsReorientTop)
            return EvaluateToTop(stat);
        else
            return 0;
    }
    ...
    void TPointsProperties::Transform(TPoints &points, bool reorient, TPoints *auxpoints) const
    {
        OmpParallelFor
            ...
            for (int i = 0; i < points.GetNumPoints(); i++) if (points[i].IsNotNull())
                Transform(points[i]);
    ...
    {
        if (How == ClosestPoints)
            return EvaluatePoints(stat);
        else if (How == MatchingPairs)
            return EvaluatePairs(stat);
        else
            return 0;
    }
