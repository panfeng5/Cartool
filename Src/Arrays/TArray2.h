...
namespace crtl
{
    ...
    public:
        ...
        void LinearIndexToIndexes(int li, int &i1, int &i2) const
        {
            i1 = li / Dim2;
            i2 = li % Dim2;
        }
    ...
    {
        if (!origin)
        { // this simply plugs in another array, clipping it if necessary
            int x;
    ...
    {
        if (!IsInsideLimits(column, 0, Dim2 - 1))
            column = 0;
    ...
    void TArray2<TypeD>::SortRows_(int column, SortDirection direction, int l, int r)
    {
        ...
        do
        {
            while (direction == Ascending && GetValue(i, column) < v || direction == Descending && GetValue(i, column) > v)
                i++;

            while (direction == Ascending && v < GetValue(j, column) || direction == Descending && v > GetValue(j, column))
                j--;
        ...
        if (l < j)
            SortRows_(column, direction, l, j);
        if (i < r)
            SortRows_(column, direction, i, r);
    }
