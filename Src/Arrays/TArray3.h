...
namespace crtl
{
    ...
    public:
        ...
        int GetDim(int axis) const { return axis == 0 ? Dim1 : axis == 1 ? Dim2
                                                                         : Dim3; }
        void GetDims(int *dims) const
        {
            dims[0] = Dim1;
            dims[1] = Dim2;
            dims[2] = Dim3;
        ...
        void LinearIndexToXYZ(int i, int &x, int &y, int &z) const
        {
            x = LinearIndexToX(i);
            y = LinearIndexToY(i);
            z = LinearIndexToZ(i);
        }
        void LinearIndexToXYZ(int i, TPointFloat &p) const
        {
            p.X = LinearIndexToX(i);
            p.Y = LinearIndexToY(i);
            p.Z = LinearIndexToZ(i);
        }
        void LinearIndexToXYZ(int i, TPointDouble &p) const
        {
            p.X = LinearIndexToX(i);
            p.Y = LinearIndexToY(i);
            p.Z = LinearIndexToZ(i);
...
#define UpdateApplication                                             \
    ...
    void TArray3<TypeD>::Insert(TArray3<TypeD> &fromarray, int *origin, double intensityrescale, int downsampling)
    {
        ...
        { // Shifting, no downsampling, optional rescaling
        ...
        { // Shifting, downsampling & rescaling
    ...
    void TArray3<TypeD>::GetMinMaxValues(double &minvalue, double &maxvalue,
                                         TPointFloat &minpos, TPointFloat &maxpos) const
    {
        minvalue = Highest(minvalue);
        maxvalue = Lowest(maxvalue);
