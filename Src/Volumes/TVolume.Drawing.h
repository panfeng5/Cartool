...
namespace crtl
{
        ...
        for (int x = bounding.XMin(); x <= bounding.XMax(); x++)
            for (int y = bounding.YMin(); y <= bounding.YMax(); y++)
                for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
                {
                    ...
                    for (xdi = 0, p.X = x - 0.5 + substep / 2 - O.X; xdi < numsubsampling; xdi++, p.X += substep)
                        for (ydi = 0, p.Y = y - 0.5 + substep / 2 - O.Y; ydi < numsubsampling; ydi++, p.Y += substep)
                            for (zdi = 0, p.Z = z - 0.5 + substep / 2 - O.Z; zdi < numsubsampling; zdi++, p.Z += substep)
                            {
                                ...
                                // is inside cylinder?
                                if (p1 >= 0 && p1 <= length && r <= radius + feather)
                    ...
                    if (typeoperation == OperationMax)
                        Maxed(Array[IndexesToLinearIndex(x, y, z)], value);
                    else if (typeoperation == OperationSet)
                        Array[IndexesToLinearIndex(x, y, z)] = value;
                                     ...
                                     double radius)
    {
            ...
            for (int x = bounding.XMin(); x <= bounding.XMax(); x++) for (int y = bounding.YMin(); y <= bounding.YMax(); y++) for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
        {
            ...
            double value = ratio * blurred[IndexesToLinearIndex(x, y, z)] + (1 - ratio) * Array[IndexesToLinearIndex(x, y, z)];
        ...
        for (int x = bounding.XMin(); x <= bounding.XMax(); x++)
            for (int y = bounding.YMin(); y <= bounding.YMax(); y++)
                for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
                {
                    ...
                    if (ps > feather)
                        Array[IndexesToLinearIndex(x, y, z)] = (TypeD)0;
                    else if (ps >= -feather)
                        Array[IndexesToLinearIndex(x, y, z)] = (TypeD)((feather - ps) / (2 * feather) * Array[IndexesToLinearIndex(x, y, z)]);
                ...
                for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
                {
                    ...
                    for (xdi = 0, p.X = x - 0.5 + substep / 2 - center.X; xdi < numsubsampling; xdi++, p.X += substep)
                        for (ydi = 0, p.Y = y - 0.5 + substep / 2 - center.Y; ydi < numsubsampling; ydi++, p.Y += substep)
                            ...
                            {
                                // no rounding here!
                                if (p.X >= -radius && p.X <= radius && p.Y >= -radius && p.Y <= radius && p.Z >= -radius && p.Z <= radius)
                                    count++;
                            } // subsampled block
                    ...
                    if (filtertype == FilterTypeMedian)
                        value = (TypeD)(count >= bucketsize / 2 ? v : 0);
                    else if (filtertype == FilterTypeMax)
                        value = (TypeD)(count ? v : 0);
                    else if (filtertype == FilterTypeMean)
                        value = (TypeD)((double)count / bucketsize * v);

                    // merge value to existing data
                    if (filtertype == FilterTypeMedian)
                        Array[IndexesToLinearIndex(x, y, z)] = value ? value : Array[IndexesToLinearIndex(x, y, z)];
                    else if (filtertype == FilterTypeMax)
                        Array[IndexesToLinearIndex(x, y, z)] = value ? value : Array[IndexesToLinearIndex(x, y, z)];
                    else if (filtertype == FilterTypeMean)
                        Array[IndexesToLinearIndex(x, y, z)] = Array[IndexesToLinearIndex(x, y, z)] + value;
