...
namespace crtl
{
    ...
    OptimizeOff
        ...
        void
        PreprocessMris(const TGoF &gofin,
                       ...
                       TSuperGauge *gauge, bool showsubprocess)
    {
        ...
        for (int i = 0; i < (int)gofin; i++)
        {
            ...
            if (doresizing)
            {
                ...
                if (anyresizing && RelativeDifference(scalingfactor, 1) > 1e-3 || !anyresizing && scalingfactor < 1.0 /*&& refsize > resizingdim*/)
                {

                    transform.Scale(scalingfactor, scalingfactor, scalingfactor, MultiplyLeft);

                    if (resizing == ResizingDimensions)
                        StringAppend(infixfile, ".Dim", IntegerToString(buff, resizingvalue));
                    else if (resizing == ResizingVoxels)
                        StringAppend(infixfile, ".Vox", FloatToString(buff, resizingvalue, IsInteger(resizingvalue) ? 0 : 3));
                    else if (resizing == ResizingRatios)
                        if (resizingvalue >= 1)
                            StringAppend(infixfile, ".Down", FloatToString(buff, resizingvalue, IsInteger(resizingvalue) ? 0 : 3));
                        else
                            StringAppend(infixfile, ".Up", FloatToString(buff, scalingfactor, IsInteger(1 / resizingvalue) ? 0 : 3));
                }
            ...
            // setting origin?
            else if (IsOriginSetAlways(origin) || IsOriginSetNoDefault(origin) && !mrioriginok)
            {

                if (IsOriginArbitrary(origin))
                    sourceorigin = arbitraryorigin;
                else /*if ( IsOriginCenter    ( origin ) )*/
                    sourceorigin = mridoc->GetDefaultCenter();
            }
            ...
            verbose.NextTopic("Post-Processing:");
            {
                ...
                if (dosps)
                {
                    ...
                    if (sppreset == SPPresetCompute)
                    {
                        ...
                        if (numsps > 0)
                            verbose.Put("Number of Solution Points targetted:", numsps);
                        else if (ressps > 0)
                            verbose.Put("Solution Points resolution:", ressps, 2, "[mm]");
                    }
            ...
            if (sagittalplane)
            {
                ...
                if (sourceorigin.IsNull() /*! mridoc->IsValidOrigin ( sourceorigin )*/)
                    if (mrioriginok)
                        ...
                        sourceorigin = mridoc->GetDefaultCenter(); // in last resort!
            ...
            if (IsOriginReset(origin) || sourceorigin.IsNull()) // assume a null source origin, at that point, means it is useless -> reset the target origin, too, as to avoid a weird corner origin
            ...
            else if (targetsize == BoundingSizeGiven)
            {
                ...
                if (sizeuser.IsNull())
                { // used to specify Transformed Size
            ...
            if (userinfix)
                StringAppend(preprocfile, ".", infixfilename);
            else if (StringIsNotEmpty(infixfile))
                StringAppend(preprocfile, infixfile);
            else
                StringAppend(preprocfile, ".Preproc");
            ...
            // apply transform and save to file
            mridoc->GetData()->TransformAndSave(transform,
                                                TransformAndSaveFlags(TransformToTarget | TransformSourceRelative | (IsOriginReset(origin) ? TransformTargetReset : TransformTargetRelative)),
                                                filtertype, interpolate, numsubsampling,
                                                &sourceorigin, &targetorigin,
                                                mridoc->GetVoxelSize(), 0,
            ...
            if (doskullstripping && isfullhead)
            {
                ...
                if (StringContains((const char *)brainfile, "." InfixHead))
                    StringReplace(brainfile, "." InfixHead, "." InfixBrain);
                else
                    PostfixFilename(brainfile, "." InfixBrain);
            } // Brain
            ...
            if (brain.IsAllocated() && bfc)
            {
                ...
                */
                ...
                if (!userinfix)
                    PostfixFilename(brainfile, "." InfixBiasFieldCorrection);
            } // BFC
            ...
            if (dogrey && brain.IsAllocated())
            {
                ...
                if (StringContains((const char *)greyfile, "." InfixBrain))
                    StringReplace(greyfile, "." InfixBrain, "." InfixGrey);
                else
            ...
            if (dosps && grey.IsAllocated())
            {
                ...
                if (StringContains((const char *)spfile, "." InfixGrey))
                    StringDelete(spfile, "." InfixGrey);
