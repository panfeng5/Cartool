...
namespace crtl
{
    ...
    void TVolumeRegion::Compact()
    {
        ...
        // same size?
        if (newbounding.GetXExtent() == Dim1 && newbounding.GetYExtent() == Dim2 && newbounding.GetZExtent() == Dim3)
            // already compacted!
            return;
    ...
    */
    ...
    {
        if (how == SortRegionsCount)
            Sort(0, NumRegions() - 1, GreaterCount);
        else if (how == SortRegionsCompact)
            Sort(0, NumRegions() - 1, GreaterCompact);
        else if (how == SortRegionsCompactCount)
            Sort(0, NumRegions() - 1, GreaterCompactCount);
    ...
    void TVolumeRegions::Sort(int l, int r, bool (*isgreater)(const TVolumeRegion *va, const TVolumeRegion *vb))
    {
        ...
        do
        {
            while ((*isgreater)(Group[i], v))
                i++;
            while ((*isgreater)(v, Group[j]))
                j--;
