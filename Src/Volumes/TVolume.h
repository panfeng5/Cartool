...
namespace crtl
{
    ...
    {
        if ((it & InterpolateForcePositive) && v < 0 || (it & InterpolateForceNegative) && v > 0)
    ...
    public:
        ...
        void WriteFile(const char *file,                     // bin nii hdr
                       ...
                       AtomFormatType atomformattype = UnknownAtomFormat) const;
...
#define UpdateApplication                                             \
    {                                                                 \
        if (IsMainThread())                                           \
            CartoolObjects.CartoolApplication->PumpWaitingMessages(); \
    }
    ...
    inline void SetOvershootingOption(InterpolationType &it, const TypeD *data, int numdata, bool fast)
    {
        ...
        if (hasnegative && haspositive)
            ; // signed data, overshooting can not really be controlled
        else if (hasnegative && !haspositive)
            SetFlags(it, InterpolateForceNegative); // negative data, will prevent returning positive results
        else if (!hasnegative && haspositive)
            SetFlags(it, InterpolateForcePositive); // positive data, will prevent returning negative results
        ...
        for (int x = 0; x < Dim1; x++)
            for (int y = 0; y < Dim2; y++)
                for (int z = 0; z < Dim3; z++)
                {
                    ...
                    if (filtertype == FilterTypeMedian)
                        Array[IndexesToLinearIndex(x, y, z)] = (TypeD)stat.Median();
                    else if (filtertype == FilterTypeMax)
                        Array[IndexesToLinearIndex(x, y, z)] = (TypeD)v;
                    else if (filtertype == FilterTypeMean)
                        Array[IndexesToLinearIndex(x, y, z)] = (TypeD)(count ? v / count : 0);
                } // for z
    }
...
#define MaxGreyWhiteHistoSize 30000
        ...
        if (bestgauss(1, 0) >= 0 && bestgauss(2, 0) >= 0
            //&& bestnumgauss       >  3
        ...
        int gaussblacki = bestnumgauss == 3       ? bestgauss(2, 0)
                          : bestgauss(3, 0) != -1 ? bestgauss(3, 0)
                                                  : bestgauss(2, 0); // first or second gaussians

        // check we have at least 2 valid gaussians!
        if (gaussgreylowi == -1 || gaussgreyhighi == -1 || gausswhitei == -1)
        {
        ...
        // average grey center
        double greycenter = (greylowcenter * greylowsdr * greylowheight + greyhighcenter * greyhighsdl * greyhighheight) / (greylowsdr * greylowheight + greyhighsdl * greyhighheight);
        ...
        double blackgreysummits = (blackcenter * 1.0 * gaussians(bestnumgaussi, gaussgreylowi, gaussheight) + greylowcenter * 5.0 * gaussians(bestnumgaussi, gaussblacki, gaussheight)) / (1.0 * gaussians(bestnumgaussi, gaussgreylowi, gaussheight) + 5.0 * gaussians(bestnumgaussi, gaussblacki, gaussheight));
    ...
    MriContentType TVolume<TypeD>::GuessTypeFromContent(double backgroundvalue, TBoundingBox<double> *bounding) const
    {
        ...
        if (!somenegative && maxvalue <= 32 && isdiscrete)
        {
            ...
            SetFlags(extracontenttype, MriContentTypeSegmentedMask, maxvalue == 1 ? MriContentTypeBinaryMask : MriContentTypeRoiMask);
        ...
        while (backhighbin > 0 && Hlow[backhighbin] < 0.10 * HlowMaxValue)
            backhighbin--;
        // continue backward, until we pass the max
        while (backhighbin > 1 && Hlow[backhighbin] < Hlow[backhighbin - 1])
            backhighbin--;
        // move forward, to get passed the max
        backhighbin += backhighbin == 0 ? 1 : 2;

        // stretch forward the regular background
        while (backlowbin < lastpos / 2 && Hlow[backlowbin] > Hlow[backlowbin + 1])
            backlowbin++;
...
#define numvoxelscanned 6133
        ...
        if (bounding)
            ...
            Bounding.Set(*this, false, backgroundvalue); // boundary of non-null data
        ...
        if (Bounding.MinSize() >= 4)
        {
            ...
            if (binarymask)
                ...
                SetFlags(extracontenttype, MriContentTypeMaskMask, MriContentTypeRoiMask);
            else
        ...
        */
        ...
        if (IsMask(extracontenttype))
            ResetFlags(extracontenttype, MriContentTypeBlob); // mask -> not a blob
            ...
            for (int xd0 = 0, xd = xi + xd0 - Lanczos2Size2 + 1; xd0 < Lanczos2Size2; xd0++, xd++)
                for (int yd0 = 0, yd = yi + yd0 - Lanczos2Size2 + 1; yd0 < Lanczos2Size2; yd0++, yd++)
                    for (int zd0 = 0, zd = zi + zd0 - Lanczos2Size2 + 1; zd0 < Lanczos2Size2; zd0++, zd++)
                    {
                        ...
                        v += GetValueChecked(xd, yd, zd) * lx[xd0] * ly[yd0] * lz[zd0];
