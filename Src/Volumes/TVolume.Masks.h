                ...
                for (int x = 0; x < Dim1; x++)
            {
                ...
                for (int y = 0; y < Dim2; y++)
                    for (int z = 0; z < Dim3; z++)
                    {
                        ...
                        if (GetValue(x, y, z))
                        {
                            if (numneighbors <= Knumset2)
                                Array[IndexesToLinearIndex(x, y, z)] = 0;
                        }
                        else if (numneighbors >= Knumset2)
            ...
            for (int x = 0; x < Dim1; x++)
        {
            ...
            for (int y = 0; y < Dim2; y++)
                for (int z = 0; z < Dim3; z++)
                {
                    ...
                    if (filtertype == FilterTypePercentFullness)
                        v = (double)numpoints / Knumset * 100;
                    else if (filtertype == FilterTypeCutBelowFullness)
                        v = numpoints < belowcut ? 0 : Array[IndexesToLinearIndex(x, y, z)];

                    else if (filtertype == FilterTypeSAV)
                        v = numpoints ? Power(surface / numpoints, 1.5) * 100 : 0; // relative
                    ...
                    else if (filtertype == FilterTypeCompactness)
                        v = numpoints ? (1 - Power(surface / numpoints, 1.5)) * 100 : 0; // simpler to invert the SAV
    ...
    void TVolume<TypeD>::FilterThinning(FctParams &params, bool showprogress)
    {
        ...
        for (int r = 0; r < repeat; r++)
        {
            ...
            OmpParallelFor
                ...
                for (int x = 1; x < Dim1; x++) for (int y = 1; y < Dim2; y++) for (int z = 1; z < Dim3; z++)
            {
                ...
                if (!temp.GetValue(x, y, z + 1) && !temp.GetValue(x, y, z - 1) || !temp.GetValue(x, y + 1, z) && !temp.GetValue(x, y - 1, z) || !temp.GetValue(x + 1, y, z) && !temp.GetValue(x - 1, y, z)

                    || !temp.GetValue(x + 1, y + 1, z) && !temp.GetValue(x - 1, y - 1, z) || !temp.GetValue(x + 1, y - 1, z) && !temp.GetValue(x - 1, y + 1, z) || !temp.GetValue(x + 1, y, z + 1) && !temp.GetValue(x - 1, y, z - 1) || !temp.GetValue(x + 1, y, z - 1) && !temp.GetValue(x - 1, y, z + 1) || !temp.GetValue(x, y + 1, z + 1) && !temp.GetValue(x, y - 1, z - 1) || !temp.GetValue(x, y + 1, z - 1) && !temp.GetValue(x, y - 1, z + 1)

                    || !temp.GetValue(x + 1, y + 1, z + 1) && !temp.GetValue(x - 1, y - 1, z - 1) || !temp.GetValue(x + 1, y + 1, z - 1) && !temp.GetValue(x - 1, y - 1, z + 1) || !temp.GetValue(x + 1, y - 1, z + 1) && !temp.GetValue(x - 1, y + 1, z - 1) || !temp.GetValue(x + 1, y - 1, z - 1) && !temp.GetValue(x - 1, y + 1, z + 1))
                ...
                case 0:
                    ...
                    if (c1 == 5 && c2 == 0 || c1 == 0 && c2 == 5)
                        Array[IndexesToLinearIndex(x - 1, y - 1, z - 1)] = 0;
                    break;
            ...
            if (showprogress)
                if (converge)
                    Gauge.SetValue(SuperGaugeDefaultPart, Percentage((maxnumset - numset) * 1.15, maxnumset));
                else
                    Gauge.Next();
        } // for repeat
    }
        ...
        for (TypeD l = firstl; IsInsideLimits(l, minlevel, maxlevel); l += incl)
        {
            ...
            do
            {
                    ...
                    for (int x = bounding.XMin(); x <= bounding.XMax(); x++) for (int y = bounding.YMin(); y <= bounding.YMax(); y++) for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
                {
                    ...
                    auto IsBestNeighbor = [&](int X, int Y, int Z)
                    { if ( height ( X, Y, Z ) && height ( X, Y, Z ) < newh )  { newh = height ( X, Y, Z );  region ( x, y, z ) = region ( X, Y, Z ); } };
            ...
            OmpParallelFor
                ...
                for (int x = AtLeast(1, bounding.XMin()); x <= NoMore(Dim1 - 2, bounding.XMax()); x++) for (int y = AtLeast(1, bounding.YMin()); y <= NoMore(Dim2 - 2, bounding.YMax()); y++) for (int z = AtLeast(1, bounding.ZMin()); z <= NoMore(Dim3 - 2, bounding.ZMax()); z++)
            {
...
#define FusionOrEdge(X, Y, Z)                                                                        \
    ...
    {                                                                                                \
        if ((RelativeDifference(w, height(X, Y, Z)) < reldiff) && height(x, y, z) < height(X, Y, Z)) \
        {                                                                                            \
                                  ...
                                  ArrayOperationType typeoperation)
    {
        ...
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] && operand2[i]);
            else if (typeoperand1 == OperandMask && typeoperand2 == OperandData)
                Array[i] = (TypeD)(Array[i] ? operand2[i] : 0);
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(operand2[i] ? Array[i] : 0);
            else if (typeoperand1 == OperandData && typeoperand2 == OperandData)
                Array[i] = (TypeD)(min(Array[i], operand2[i])); // not clear what to do?
        }
        ...
        else if (typeoperation == OperationOr)
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                ...
                Array[i] = (TypeD)(operand2[i] ? operand2[i] : Array[i]); // priority to data value (operand 2)
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] ? Array[i] : operand2[i]); // priority to data value (operand 1)
            else if (typeoperand1 == OperandData && typeoperand2 == OperandData)
                Array[i] = (TypeD)(max(Array[i], operand2[i])); // max of value
        }

        else if (typeoperation == OperationXor)
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                Array[i] = (TypeD)((bool)Array[i] ^ (bool)operand2[i]);
            else if (typeoperand1 == OperandMask && typeoperand2 == OperandData)
                Array[i] = (TypeD)(Array[i] && !operand2[i] ? true : !Array[i] && operand2[i] ? operand2[i]
                                                                                              : 0);
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] && !operand2[i] ? Array[i] : !Array[i] && operand2[i] ? true
                                                                                                  : 0);
            else if (typeoperand1 == OperandData && typeoperand2 == OperandData)
                Array[i] = (TypeD)(Array[i] && !operand2[i] ? Array[i] : !Array[i] && operand2[i] ? operand2[i]
                                                                                                  : 0);
        }
        ...
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] || operand2[i]); // taken from OR
            else if (typeoperand1 == OperandMask && typeoperand2 == OperandData)
                Array[i] = (TypeD)(operand2[i] ? operand2[i] : Array[i]); // taken from OR
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] ? Array[i] : operand2[i]); // taken from OR
            else if (typeoperand1 == OperandData && typeoperand2 == OperandData)
                Array[i] = Clip((TypeD)(Array[i] + operand2[i]), minv, maxv); // arithmetic +
        }
        // Subtraction or Clearing
        else if (typeoperation == OperationSub)
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] && !operand2[i]); // AND NOT
            else if (typeoperand1 == OperandMask && typeoperand2 == OperandData)
                Array[i] = (TypeD)(Array[i] && !operand2[i]); // AND NOT
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(!operand2[i] ? Array[i] : 0); // AND NOT
            else if (typeoperand1 == OperandData && typeoperand2 == OperandData)
                Array[i] = Clip((TypeD)(Array[i] - operand2[i]), minv, maxv); // arithmetic -
        }
        ...
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(Array[i] && operand2[i]); // AND
            else if (typeoperand1 == OperandMask && typeoperand2 == OperandData)
                Array[i] = (TypeD)(Array[i] ? operand2[i] : 0); // AND
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                Array[i] = (TypeD)(operand2[i] ? Array[i] : 0); // AND
            else if (typeoperand1 == OperandData && typeoperand2 == OperandData)
                Array[i] = Clip((TypeD)(Array[i] * operand2[i]), minv, maxv); // arithmetic *
        }
        // Division or XOR
        ...
        {
            if (typeoperand1 == OperandMask && typeoperand2 == OperandMask)
                Array[i] = (TypeD)((bool)Array[i] ^ (bool)operand2[i]); // XOR
            else if (typeoperand1 == OperandMask && typeoperand2 == OperandData)
                Array[i] = (TypeD)(Array[i] && !operand2[i] ? true : !Array[i] && operand2[i] ? operand2[i]
                                                                                              : 0);
            else if (typeoperand1 == OperandData && typeoperand2 == OperandMask)
                ...
                Array[i] = Clip((TypeD)(operand2[i] ? Array[i] / operand2[i] : 0), minv, maxv); // arithmetic /
        }
    }
    ...
    */
    ...
    void TVolume<TypeD>::FilterToMask(FctParams &params, bool showprogress)
    {
        ...
        OmpFor
            ...
            for (int x = 0; x < Dim1; x++) for (int y = 0; y < Dim2; y++)
        {
        ...
        OmpFor
            ...
            for (int x = 0; x < Dim1; x++) for (int z = 0; z < Dim3; z++)
        {
        ...
        OmpFor
            ...
            for (int y = 0; y < Dim2; y++) for (int z = 0; z < Dim3; z++)
        {
    ...
    void TVolume<TypeD>::FilterSymmetrize(FctParams &params)
    {
        ...
        if (axis == 0)
            ...
            OmpParallelFor
                ...
                for (int y = 0; y < Dim2; y++) for (int z = 0; z < Dim3; z++) for (int d = 0, x1 = center - 1, x2 = center + 1; d < delta; d++, x1--, x2++)
            ...
            OmpParallelFor
                ...
                for (int x = 0; x < Dim1; x++) for (int z = 0; z < Dim3; z++) for (int d = 0, y1 = center - 1, y2 = center + 1; d < delta; d++, y1--, y2++)
            ...
            OmpParallelFor
                ...
                for (int x = 0; x < Dim1; x++) for (int y = 0; y < Dim2; y++) for (int d = 0, z1 = center - 1, z2 = center + 1; d < delta; d++, z1--, z2++)
