...
namespace crtl
{
                                  ...
                                  bool verbose)
    {
        ...
        if (fabs(originalcenter[gomri.LeftRightIndex] - center[gomri.LeftRightIndex]) < 0.10 && fabs(gomri.GetValue(RotationY)) < 0.10 && fabs(gomri.GetValue(RotationZ)) < 0.10)
        {
                             ...
                             const char *title)
    {
        ...
        for (int combo = 0; combo < 3; combo++)
        {
            ...
            if (combo == 0)
            {
                ...
                how = SagittalPlaneSymmetric;
            } // T1 Head, T1 Brain, T1 Weird Head, T1Gad Weird Head, T2 Head, T2 Brain
            else if (combo == 1)
            {
                gomethod = CyclicalCrossHairScan;
        ...
        { // either close to no transform at all, or way too big
                                    ...
                                    bool verbose)
    {
        ...
        // parameters energy: more transform -> more energy
        double mintransf = sqrt(Square(suspicioustrz) + Square(suspicioustry) + Square(suspiciousrotx)
        ...
        // check for a null transform (less than 0.10 voxel shift & less than 0.10?rotations)
        if (fabs(originalcenter[gomri.UpDownIndex] - center[gomri.UpDownIndex]) < 0.10 && fabs(originalcenter[gomri.FrontBackIndex] - center[gomri.FrontBackIndex]) < 0.10 && fabs(gomri.GetValue(RotationX)) < 0.10
                               ...
                               const char *title)
    {
        ...
        for (int combo = 0; combo < 4; combo++)
        {
            ...
            if (combo == 0)
            {
                gomethod = GlobalNelderMead;
                how = TransversePlaneMNIT1;
            } // T1 slice
            else if (combo == 1)
            {
                gomethod = GlobalNelderMead;
                how = TransversePlaneMNIT2;
            } // T2 slice
            else if (combo == 2)
            {
                gomethod = GlobalNelderMead;
                how = TransversePlaneBiggestBox;
            } // backup search 1 - Biggest Box works well on weirdly shaped brains
            else
            {
                gomethod = GlobalNelderMead;
                how = TransversePlaneBiggestSurface;
            } // backup search 2 - Biggest Surface works results are very similar to the AC-PC transverse slice
                                 ...
                                 bool verbose)
    {
        ...
        bool upispositive = mrihead->GetAxis(ToUp) == ZMaxSide || mrihead->GetAxis(ToUp) == YMaxSide || mrihead->GetAxis(ToUp) == XMaxSide;
        ...
        // parameters energy: more transform -> more energy
        double mintransf = sqrt(Square(suspicioustrz) + Square(suspiciousrotx) + Square(suspiciousroty));
                            ...
                            const char *title)
    {
        ...
        for (int combo = 0; combo < 2; combo++)
        {
            ...
            if (combo == 0)
                ...
                gomethod = WeakestDimensionCrossHairScan;
