...
namespace crtl
{
    ...
    {
        // Mandatory volumes
        if (StringIsEmpty(filehead) || StringIsEmpty(fileskull) || StringIsEmpty(filecsf) || StringIsEmpty(filegrey) || StringIsEmpty(filewhite))
        ...
        {
            if (StringIsNotEmpty(filehead))
                verbose.Put("Full head mask:", filehead);
            if (StringIsNotEmpty(fileskull))
                verbose.Put("Skull (compact+spongy) mask:", fileskull);
            if (StringIsNotEmpty(fileskullsp))
                verbose.Put("Spongy skull sub-mask:", fileskullsp);
            if (StringIsNotEmpty(filecsf))
                verbose.Put("CSF probabilities:", filecsf);
            if (StringIsNotEmpty(filegrey))
                verbose.Put("Grey Matter probabilities:", filegrey);
            if (StringIsNotEmpty(filewhite))
                verbose.Put("White Matter probabilities:", filewhite);
            if (StringIsNotEmpty(fileblood))
                verbose.Put("Venous sinus mask:", fileblood);
            if (StringIsNotEmpty(fileair))
                verbose.Put("Air sinus mask:", fileair);
            if (StringIsNotEmpty(fileeyes))
                verbose.Put("Eyes mask:", fileeyes);
        }
        ...
        // Mandatory volumes
        Gauge.Next();
        MriHead.Open(filehead, OpenDocHidden);
        Gauge.Next();
        MriSkull.Open(fileskull, OpenDocHidden);
        Gauge.Next();
        MriCsf.Open(filecsf, OpenDocHidden);
        Gauge.Next();
        MriGrey.Open(filegrey, OpenDocHidden);
        Gauge.Next();
        MriWhite.Open(filewhite, OpenDocHidden);

        // Optional volumes
        Gauge.Next();
        if (StringIsNotEmpty(fileskullsp))
            MriSkullSpongy.Open(fileskullsp, OpenDocHidden);
        Gauge.Next();
        if (StringIsNotEmpty(fileblood))
            MriBlood.Open(fileblood, OpenDocHidden);
        Gauge.Next();
        if (StringIsNotEmpty(fileair))
            MriAir.Open(fileair, OpenDocHidden);
        Gauge.Next();
        if (StringIsNotEmpty(fileeyes))
            MriEyes.Open(fileeyes, OpenDocHidden);
            ...
            for (int i = 0; i < dim; i++)
        {
            ...
            tissues(ScalpIndex) = (bool)head(i) && !(bool)insideskull(i); // !not considered as "scalp" if inside the skull!
            ...
            if (tissues(AirIndex))
                vol(i) = AirIndex; // !overwrites bone!
            else if (tissues(BloodIndex))
                vol(i) = BloodIndex; // !overwrites bone!
            else if (tissues(SkullSpongyIndex))
                vol(i) = SkullSpongyIndex;
            else if (tissues(SkullIndex))
                vol(i) = skullindex;
            else if (tissues(EyeIndex))
                vol(i) = EyeIndex;
            // default OUTSIDE the skull is Scalp tissue (i.e. skin / fat / muscle)
            else if (tissues(ScalpIndex))
                vol(i) = ScalpIndex;
            ...
            if (insideskull(i) && !(vol(i) == AirIndex // don't overwrite these labels
                                    || vol(i) == EyeIndex || vol(i) == BloodIndex))
            {

                if (tissues(GreyIndex) > tissues(WhiteIndex) && tissues(GreyIndex) > tissues(CsfIndex))
                    vol(i) = GreyIndex;

                else if (tissues(WhiteIndex) > tissues(GreyIndex) && tissues(WhiteIndex) > tissues(CsfIndex))
                    vol(i) = WhiteIndex;

                else if (tissues(CsfIndex) > tissues(GreyIndex) && tissues(CsfIndex) > tissues(WhiteIndex))
                    vol(i) = CsfIndex;
            }
        }
        ...
        OmpParallelFor
            ...
            for (int i = 0; i < dim; i++)
                ...
                morecsf(i) = IsInsideLimits((TissuesIndex)(int)vol(i), BrainMinIndex, BrainMaxIndex) || vol(i) == CsfIndex || vol(i) == BloodIndex;
        ...
        if (createspongy)
        {
            ...
            if (air)
                autospongy.BinaryOp(*air, OperandMask, OperandMask, OperationSub);
            ...
            for (int i = 0; i < dim; i++)
        {
            ...
            if (createspongy)
            {
                ...
                if (autospongy(i))
                    vol(i) = SkullSpongyIndex; // force replace with our auto spongy
                else if (vol(i) == SkullIndex || vol(i) == SkullSpongyIndex)
                    vol(i) = SkullCompactIndex; // otherwise, forcibly converts the other skull indexes into compact skull
            }
            ...
            if (!morecsf(i) || !insideskull(i))
            {
                ...
                if (vol(i) == CsfIndex || vol(i) == GreyIndex || vol(i) == WhiteIndex)
