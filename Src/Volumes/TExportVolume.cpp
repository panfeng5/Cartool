...
namespace crtl
{
    ...
    void TExportVolume::Begin()
    {
        ...
        { // no Type override? get it from the filename
        ...
        RescalingToInteger = VolumeFormat == AtomFormatByte    ? (IsInteger(MaxValue) // integer test would better be ran on the whole data to be 100% correct
                                                                       && IsInsideLimits((int)MaxValue, 1, (int)Highest<UCHAR>())
                                                                      ? 1 // data is integer and fits in 1 byte already
    ...
    void TExportVolume::WriteHeader()
    {
        ...
        if (StringIs(Type, FILEEXT_MRIAVW_HDR))
        {
            ...
            if (StringLength(Orientation) == 3 && StringLength(hout.hist.descrip) + StringLength("; " ExportOrientation) + 4 < 80)
        ...
        else if (StringIs(Type, FILEEXT_MRINII))
        {
            ...
            headernii.srow_x[0] = VoxelSize.X;
            headernii.srow_x[1] = 0;
            headernii.srow_x[2] = 0;
            headernii.srow_x[3] = -Origin.X * VoxelSize.X;
            headernii.srow_y[0] = 0;
            headernii.srow_y[1] = VoxelSize.Y;
            headernii.srow_y[2] = 0;
            headernii.srow_y[3] = -Origin.Y * VoxelSize.Y;
            headernii.srow_z[0] = 0;
            headernii.srow_z[1] = 0;
            headernii.srow_z[2] = VoxelSize.Z;
            headernii.srow_z[3] = -Origin.Z * VoxelSize.Z;
