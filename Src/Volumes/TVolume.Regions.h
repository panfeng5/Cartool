...
namespace crtl
{
    ...
    void TVolume<TypeD>::LevelsToRegions(TVolumeRegions &gor)
    {
        ...
        OmpParallelFor
            ...
            for (int i = 0; i < LinearDim; i++) if (Array[i] == 0)
                voxelstodo[i] = false;
    ...
    void TVolume<TypeD>::LevelsClustersToRegions(TVolumeRegions &gor, int minvoxels, int maxvoxels, NeighborhoodType neighborhood, bool showprogress)
    {
        ...
        do
        {
            ...
            do
            { // progress the edge, if the same level, connected, and not already done
        ...
        seedexists:
            ...
            do
            {
                ...
                OmpParallelFor
                    ...
                    for (int x = bseed.XMin(); x <= bseed.XMax(); x++) for (int y = bseed.YMin(); y <= bseed.YMax(); y++) for (int z = bseed.ZMin(); z <= bseed.ZMax(); z++)
                {
...
#define regiongrowinginnerloop(J)       \
    if (voxelcode[J] == VoxelToDo)      \
    {                                   \
        voxelcode[J] = VoxelSeed;       \
        expanded = true;                \
    }                                   \
    else if (voxelcode[J] == VoxelDone) \
        break;
            ...
            OmpParallelFor
                ...
                for (int x = bremain.XMin(); x <= bremain.XMax(); x++) for (int y = bremain.YMin(); y <= bremain.YMax(); y++) for (int z = bremain.ZMin(); z <= bremain.ZMax(); z++)
            {
...
#define IsVoxelRing(V) ((V) == iteration - 1)
            ...
            if (globalstats)
            {
                ...
                for (int i = 0; i < LinearDim; i++)
                {
                    ...
                    if (statsmask && IsVoxelMask(regionvol[i]) || statsring && IsVoxelRing(regionvol[i]))

                        globalregionstat.Add(GetValue(i));

                    else if (!IsVoxelMask(regionvol[i]))
                    { // not inside global mask
                    ...
                    for (int xk = xmin; xk <= xmax; xk++)
                        for (int yk = ymin; yk <= ymax; yk++)
                            for (int zk = zmin; zk <= zmax; zk++)
                            {
                                ...
                                if (statsmask && IsVoxelMask(regionvol[in]) || statsring && IsVoxelRing(regionvol[in]))
...
#define UpdateNeighbor(X, Y, Z)                                                     \
    {                                                                               \
        ...
        {                                                                           \
            ...
            if (median)
            {
                ...
                */
                ...
                for (int i = 0; i < LinearDim; i++)
                {
                    ...
                    else if (moveregion) // if test is false, reset only
            ...
            if (iteration > 2 && !(smartneighborhood && bigneighborhood) && convergence > convergenceold)
            {
