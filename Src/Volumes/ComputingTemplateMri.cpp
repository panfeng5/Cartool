...
namespace crtl
{
                   ...
                   TSuperGauge &Gauge)
    {
        ...
        if ((int)mrifiles < 1 || (howtemplate == BuildTemplateMNI && StringIsEmpty(mnifile)) || allnorms == 0)
...
#define InfixGeneration "Gen"
        ...
        else
        {
            ...
            avgorigin = TPointDouble(avgdim1, avgdim2, avgdim3) * TPointDouble((double)96 / 192, (double)132 / 228, (double)78 / 192);
        ...
        Gauge.SetRange(-1, (numiterations + 1) * (4 + nummrifiles * 3 + maxslicex) + nummrifiles);
        ...
        for (int li = 0; li <= numiterations; li++)
        {
            ...
            for (int mi = 0; mi < nummrifiles; mi++)
            {
                ...
                else
                {
                    ...
                    */
                    ...
                    if (savingcoregmris && islast)
                    {
                        ...
                        if (govtov.HasValue(Scale))
                            scalestat.Add(govtov.GetValue(Scale));
                        if (govtov.HasValue(ScaleX))
                            scalestat.Add(govtov.GetValue(ScaleX));
                        if (govtov.HasValue(ScaleY))
                            scalestat.Add(govtov.GetValue(ScaleY));
                        if (govtov.HasValue(ScaleZ))
                            scalestat.Add(govtov.GetValue(ScaleZ));
                ...
                if (!(openall || howtemplate == BuildTemplateSelfRef && isbooting && mi == initref)) // let the first MRI open in case for the booting part
    ...
    void ComputingTemplateMri(const TGoF &mrifiles,
                              ...
                              bool savingcoregmris)
    {
        if ((int)mrifiles < 1 || (howtemplate == BuildTemplateMNI && StringIsEmpty(mnifile)) || (howsp == TemplateLoadSPFromFile && StringIsEmpty(mnispfile)) || (howsp == TemplateExtractSPFromTemplate && numsolpoints <= 0 && ressolpoints <= 0))
        ...
        verbose.NextTopic("Output Files:");
        {
            ...
            if (greyfile.IsNotEmpty())
                verbose.Put("Template grey matter file:", greyfile);
            if (spfile.IsNotEmpty())
                verbose.Put("Template solution points file:", spfile);
