...
namespace crtl
{
    ...
    void TTalairachOracle::PointsToVolume(const char *path)
    {
        ...
        for (int x = scanxmin; x <= scanxmax; x++)
        {
            ...
            do
            {
                ...
                // origin offset
                tal(x - xmin, y - ymin, z - zmin) = TalairachEncodeLabel1(l1) | TalairachEncodeLabel2(l2) | TalairachEncodeLabel3(l3) | TalairachEncodeLabel4(l4) | TalairachEncodeLabel5(l5);
            } while (true);
    ...
    bool TTalairachOracle::PositionHasCode(const TPointFloat &pos, UINT talcode) const
    {
        ...
        if (TalairachDecodeLabel1(t) == talcode)
            return true;
        if (TalairachDecodeLabel2(t) == talcode)
            return true;
        if (TalairachDecodeLabel3(t) == talcode)
            return true;
        if (TalairachDecodeLabel4(t) == talcode)
            return true;
        if (TalairachDecodeLabel5(t) == talcode)
            return true;
    ...
    const char *TTalairachOracle::PositionToString(const TPointFloat &pos, char *name, bool groupresult) const
    {
        ...
        if (groupresult)
        {
            ...
            if (talcodes[0])
                StringAppend(name, TalairachLabels[talcodes[0]]);
            if (talcodes[3])
                StringAppend(name, talcodes[0] ? ", " : "", TalairachLabels[talcodes[3]]);
            if (talcodes[0] || talcodes[3])
                StringAppend(name, NewLine);

            if (talcodes[1])
                StringAppend(name, TalairachLabels[talcodes[1]]);
            if (talcodes[2])
                StringAppend(name, talcodes[1] ? ", " : "", TalairachLabels[talcodes[2]]);
            if (talcodes[1] || talcodes[2])
                StringAppend(name, NewLine);

            if (talcodes[4])
                StringAppend(name, TalairachLabels[talcodes[4]]);
            if (talcodes[4])
                StringAppend(name, NewLine);
        }
        else
