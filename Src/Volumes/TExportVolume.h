    ...
    )
    {
        ...
        if (datain)
        {
            ...
            else
            {
                ...
                // masks & (most of) ROIs
                if (IsInsideLimits(minvalue, maxvalue, (MriType)0, (MriType)Highest<UCHAR>()) && isint)
        ...
        if (interpolate != InterpolateUnknown)
        {
            ...
            bool transformint = (filtertype == FilterTypeMedian                                                                                                   // only these filters preserve integers
                                 || filtertype == FilterTypeMin || filtertype == FilterTypeMax || filtertype == FilterTypeMinMax || filtertype == FilterTypeNone) // no sub-sampling case, value are being copied
        ...
        if (StringIsEmpty(ext) && StringIs(DefaultMriExt, FILEEXT_MRINII) || StringIs(ext, FILEEXT_MRINII))
        {
        ...
        else if (StringIs(ext, FILEEXT_MRIVMR) || StringIs(ext, FILEEXT_MRIAVS))
        {
            // only byte type for these files
            atomtype = AtomFormatByte;
        }
        else
        { // if ( StringIs ( ext, FILEEXT_MRIAVW_HDR ) ) {
