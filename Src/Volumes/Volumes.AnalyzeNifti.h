...
#define NiftiDescripLabels "Labels:"
    ...
    inline bool IsAnalyzeTypeInteger(int dt) { return dt == DT_UNSIGNED_CHAR || dt == DT_UNSIGNED_SHORT || dt == DT_SIGNED_SHORT || dt == DT_SIGNED_SHORT_POS || dt == DT_SIGNED_INT; }
    inline bool IsAnalyzeTypeFloat(int dt) { return dt == DT_FLOAT || dt == DT_DOUBLE; }
...
#define NIFTI_TYPE_BINARY DT_BINARY
    ...
    inline bool IsNiftiTypeInteger(int dt) { return dt == NIFTI_TYPE_UINT8 || dt == NIFTI_TYPE_INT8 || dt == NIFTI_TYPE_UINT16 || dt == NIFTI_TYPE_INT16 || dt == NIFTI_TYPE_UINT32 || dt == NIFTI_TYPE_INT32 || dt == NIFTI_TYPE_UINT64 || dt == NIFTI_TYPE_INT64; }
    inline bool IsNiftiTypeFloat(int dt) { return dt == NIFTI_TYPE_FLOAT32 || dt == NIFTI_TYPE_FLOAT64 || dt == NIFTI_TYPE_FLOAT128; }
    inline bool IsNiftiTypeComplex(int dt) { return dt == NIFTI_TYPE_COMPLEX64 || dt == NIFTI_TYPE_COMPLEX128 || dt == NIFTI_TYPE_COMPLEX256; }
    inline bool IsNiftiTypeRGB(int dt) { return dt == NIFTI_TYPE_RGB24 || dt == NIFTI_TYPE_RGBA32; }
    ...
    const char *AnalyzeDataTypeToString(int dt)
    {
        ...
        case DT_UNKNOWN:
            return "Unknown Analyze Data Type";
        ...
        case DT_COMPLEX:
            return "Single Float Complex";
        default:
            return "Unrecognized Analyze Data Type";
        }
    }
    ...
    const char *NiftiDataTypeToString(int dt)
    {
        ...
        {
            // not ordered by values, but by family types
        case NIFTI_TYPE_UNKNOWN:
            return "Unknown Nifti Data Type";

        case NIFTI_TYPE_BINARY:
            return "Binary 1 bit";
        ...
        case NIFTI_TYPE_FLOAT128:
            return "Long Double Float";

        case NIFTI_TYPE_COMPLEX64:
            return "Single Float Complex";
        case NIFTI_TYPE_COMPLEX128:
            return "Double Float Complex";
        case NIFTI_TYPE_COMPLEX256:
            return "Long Double Float Complex";
        ...
        case NIFTI_TYPE_RGBA32:
            return "RGBA Bytes";
        default:
            return "Unrecognized Nifti Data Type";
        }
    }
    ...
    const char *NiftiSpatialUnitsToString(int u)
    {
        ...
        case NIFTI_UNITS_MICRON:
            return "[micrometer]";
        default:
            return "Unknown Nifti Spatial Unit";
        }
    }
    ...
    const char *NiftiTemporalUnitsToString(int u)
    {
        ...
        default:
            return "Unknown Nifti Temporal Unit";
        }
    }
    ...
    const char *NiftiTransformToString(int qs)
    {
        ...
        case NIFTI_XFORM_UNKNOWN:
            return "Arbitrary Transform (0)";
        case NIFTI_XFORM_SCANNER_ANAT:
            return "Scanner-based coordinates (1)";
        case NIFTI_XFORM_ALIGNED_ANAT:
            return "Re-aligned coordinates (2)";
        case NIFTI_XFORM_TALAIRACH:
            return "Talairach coordinates (3)";
        case NIFTI_XFORM_MNI_152:
            return "MNI coordinates (4)";
        case NIFTI_XFORM_TEMPLATE_OTHER:
            return "Normalized coordinates (5)";
        default:
            return "Unknown Nifti Transform";
        }
    }
    ...
    const char *NiftiTransformMethodString(NiftiTransformMethod niftimethod)
    {
        ...
        case UnknwonNiftiMethod:
            return "Unknown transform method";
            break;
        case NiftiMethod1:
            return "Nifti Method 1: Voxel transform (scaling)";
            break;
        case NiftiMethod2:
            return "Nifti Method 2: Quaternion transform (scaling + rotation + translation)";
            break;
        case NiftiMethod3:
            return "Nifti Method 3: Standardization transform (3x4 transform matrix)";
            break;
        default:
            return "Unknown transform method";
            break;
    ...
    const char *NiftiTransformMethodShortString(NiftiTransformMethod niftimethod)
    {
        ...
        case UnknwonNiftiMethod:
            ...
            break;
        case NiftiMethod1:
            return "Nifti Method 1";
            break;
        case NiftiMethod2:
            return "Nifti Method 2";
            break;
        case NiftiMethod3:
            return "Nifti Method 3";
            break;
        default:
            return "Unknown method";
            break;
                                                 ...
                                                 bool askuser)
    {
        if (askuser)
        {
            ...
            if (answer == '0' || answer == 'N')
                return UnknwonNiftiMethod;
            else if (answer == '1' || answer == 'V')
                return NiftiMethod1;
            else if (answer == '2' || answer == 'Q')
                return (qform_code != NIFTI_XFORM_UNKNOWN ? NiftiMethod2 : sform_code != NIFTI_XFORM_UNKNOWN ? NiftiMethod3
                                                                                                             : NiftiMethod1);
            else if (answer == '3' || answer == 'S')
                return (sform_code != NIFTI_XFORM_UNKNOWN ? NiftiMethod3 : qform_code != NIFTI_XFORM_UNKNOWN ? NiftiMethod2
                                                                                                             : NiftiMethod1);
            // else automatic choice below
        }

        if (qform_code == NIFTI_XFORM_UNKNOWN && sform_code == NIFTI_XFORM_UNKNOWN)
    ...
    {
        if (StringContains(filename, "." InfixP "."))
            return NIFTI_INTENT_PVAL;
        else if (StringContains(filename, "." Infix1MinusP "."))
            return NIFTI_INTENT_PVAL; // 1-p doesn't seem to exist
        else if (StringContains(filename, "." InfixLogP "."))
            return NIFTI_INTENT_LOG10PVAL;
        else if (StringContains(filename, "." InfixT "."))
        ...
        else if (StringContains(filename, "." PostfixStandardizationZScore))
            return NIFTI_INTENT_ZSCORE; // statistical tests are done AFTER Z-Scoring
        else if (IsExtension(filename, FILEEXT_RIS))
            return NIFTI_INTENT_ESTIMATE; // default for ris is a sort of estimate
        else
    ...
    )
    {
        ...
        else if (!(fileinhdr.IsExtension(FILEEXT_MRIAVW_HDR) || fileinhdr.IsExtension(FILEEXT_MRINII)))
        ...
        bool swap = !(sizeof_hdr == ANALYZE_HEADER_SIZE || sizeof_hdr == NIFTI1_HEADER_SIZE || sizeof_hdr == NIFTI2_HEADER_SIZE);
        ...
        if (swap)
        {
            ...
            if (!(sizeof_hdr == ANALYZE_HEADER_SIZE || sizeof_hdr == NIFTI1_HEADER_SIZE || sizeof_hdr == NIFTI2_HEADER_SIZE))
        ...
        if (filehdr)
            StringCopy(filehdr, fileinhdr);
        if (fileimg)
            StringCopy(fileimg, fileinimg); // will be empty anyway if no dual files
