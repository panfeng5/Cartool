            ...
            for (int x = bounding.XMin(); x <= bounding.XMax(); x++)
        {
            ...
            for (int y = bounding.YMin(); y <= bounding.YMax(); y++)
                for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
                {
                    ...
                    if (IsFlag(flags, GreyMatterRegular) || IsFlag(flags, GreyMatterFat))
                    {
                    ...
                    if (IsFlag(flags, GreyMatterSlim))
                    {
                        ...
                        Grey(x, y, z) = fabs(v - values[GreyMode]) < fabs(v - values[WhiteMode]) + (values[WhiteMode] - values[WhiteMin]) && fabs(v - values[GreyMode]) < fabs(v - values[BlackMode]) + (values[BlackMax] - values[BlackMode]) ? 1 : 0;
                    ...
                    else if (IsFlag(flags, GreyMatterRegular))
                    {
                        ...
                        Grey(x, y, z) = (pgreynum > pblacknum || pgreygauss > pblackgauss) && (pgreynum >= pwhitenum || pgreygauss >= pwhitegauss) ? 1 : 0;
                    ...
                    else if (IsFlag(flags, GreyMatterFat))
                    {
                        ...
                        // testing each probabilities independently
                        Grey(x, y, z) = (pgreynum > pblacknum || pgreygauss > pblackgauss * 0.50) && (pgreynum >= pwhitenum * 0.50 || pgreygauss >= pwhitegauss) ? 1 : 0;
