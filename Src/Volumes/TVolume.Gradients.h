...
namespace crtl
{
    ...
    void TVolume<TypeD>::GetPrincipalGradient(int x, int y, int z, TPointFloat &g, int d, double &C) const
    {
        ...
        if (D[0] / D[2] < SingleFloatEpsilon)
            D[0] = 0;
        if (D[1] / D[2] < SingleFloatEpsilon)
            D[1] = 0;
        ...
        C = Square(D[0] - D[1]) + Square(D[0] - D[2]) + Square(D[1] - D[2]);
    ...
    void TVolume<TypeD>::GetSurfacePoints(TPoints *surfp[NumCubeSides],
                                          ...
                                          TPointFloat center,
                                          double depth,
                                          SurfacePointsFlags flags) const
    {
            ...
            for (p.Y = boundmin.Y; p.Y <= boundmax.Y; p.Y += step.Y)
                for (p.Z = boundmin.Z; p.Z <= boundmax.Z; p.Z += step.Z)
                {
                    ...
                    for (p.X = boundmin.X; p.X <= limitmin.X; p.X++) // note this loop does NOT use step, we don't want to miss the real surface!
                        if (safe && (GetValue(p) >= threshold || GetValue(p.X, p.Y + 1, p.Z) >= threshold || GetValue(p.X, p.Y - 1, p.Z) >= threshold || GetValue(p.X, p.Y, p.Z + 1) >= threshold || GetValue(p.X, p.Y, p.Z - 1) >= threshold) || !safe && GetValue(p) >= threshold)
                        {
            ...
            for (p.X = boundmin.X; p.X <= boundmax.X; p.X += step.X)
                for (p.Z = boundmin.Z; p.Z <= boundmax.Z; p.Z += step.Z)
                {
                    ...
                    for (p.Y = boundmin.Y; p.Y <= limitmin.Y; p.Y++)
                        if (safe && (GetValue(p) >= threshold || GetValue(p.X + 1, p.Y, p.Z) >= threshold || GetValue(p.X - 1, p.Y, p.Z) >= threshold || GetValue(p.X, p.Y, p.Z + 1) >= threshold || GetValue(p.X, p.Y, p.Z - 1) >= threshold) || !safe && GetValue(p) >= threshold)
                        {
            ...
            for (p.X = boundmin.X; p.X <= boundmax.X; p.X += step.X)
                for (p.Y = boundmin.Y; p.Y <= boundmax.Y; p.Y += step.Y)
                {
                    ...
                    for (p.Z = boundmin.Z; p.Z <= limitmin.Z; p.Z++)
                        if (safe && (GetValue(p) >= threshold || GetValue(p.X + 1, p.Y, p.Z) >= threshold || GetValue(p.X - 1, p.Y, p.Z) >= threshold || GetValue(p.X, p.Y + 1, p.Z) >= threshold || GetValue(p.X, p.Y - 1, p.Z) >= threshold) || !safe && GetValue(p) >= threshold)
                        {
