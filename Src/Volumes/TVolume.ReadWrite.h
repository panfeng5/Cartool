...
namespace crtl
{
                                          ...
                                          const char *title) const
    {
        ...
        if (!(filtertype == FilterTypeAntialiasing || filtertype == FilterTypeMedian || filtertype == FilterTypeMax || filtertype == FilterTypeMean || filtertype == FilterTypeGaussian || numsubsampling == 1 // filter is to be ignored when there is no sub-sampling
              ))
        {
...
#define boundingmarginmin 5.0
        ...
        if ((targetsize == BoundingSizeGivenNoShift || targetsize == BoundingSizeGiven) && giventargetsize // size should of course be valid - otherwise case will devolve into BoundingSizeOptimal
        ...
        { // BoundingSizeOptimal and any other fallback cases
        ...
        for (int z = 0; z < expvol.Dimension.Z; z++)
        {
            ...
            for (int y = 0; y < expvol.Dimension.Y; y++)
                for (int x = 0; x < expvol.Dimension.X; x++)
                {
                            ...
                            for (int yd = 0; yd < numsubsampling; yd++)
                                for (int xd = 0; xd < numsubsampling; xd++)
                                {
                                    ...
                                    if (filtertype == FilterTypeMedian)
                                        stat.Add(getvalue, ThreadSafetyIgnore);
                                    else if (filtertype == FilterTypeMax)
                                        stat.Add(getvalue, ThreadSafetyIgnore);
                                    else if (filtertype == FilterTypeMean)
                                        stat.Add(getvalue, ThreadSafetyIgnore);
                                    else if (filtertype == FilterTypeGaussian)
                                        v += K(xd, yd, zd) * getvalue; // K holds the exact weights, we can sum up directly
                                } // subsampled block
                    ...
                    if (savetofile)
                        expvol.Write(v); // give it a double, write will convert according to its internal type
                    if (savetomemory)
                        dataout->GetValue(x, y, z) = (TypeD)v;
                } // for y, x
        } // for z
    ...
    TypeD TVolume<TypeD>::GetAntialiasedValue(TMatrix44 &transformi,
                                              ...
                                              double size,
                                              InterpolationType interpolate) const
    {
        ...
        p.Set(pcenter.X - d, pcenter.Y - d, pcenter.Z - d);
        transformi.Apply(p);
        v[0] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X - d, pcenter.Y - d, pcenter.Z + d);
        transformi.Apply(p);
        v[1] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X - d, pcenter.Y + d, pcenter.Z - d);
        transformi.Apply(p);
        v[2] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X - d, pcenter.Y + d, pcenter.Z + d);
        transformi.Apply(p);
        v[3] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X + d, pcenter.Y - d, pcenter.Z - d);
        transformi.Apply(p);
        v[4] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X + d, pcenter.Y - d, pcenter.Z + d);
        transformi.Apply(p);
        v[5] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X + d, pcenter.Y + d, pcenter.Z - d);
        transformi.Apply(p);
        v[6] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
        p.Set(pcenter.X + d, pcenter.Y + d, pcenter.Z + d);
        transformi.Apply(p);
        v[7] = GetValueChecked(p.X, p.Y, p.Z, interpolate) * w;
...
#define neighbordisp (35 * w)
            ...
            {
                p.Set(pcenter.X - d, pcenter.Y - d, pcenter.Z - d);
                v[0] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[1])
            {
                p.Set(pcenter.X - d, pcenter.Y - d, pcenter.Z + d);
                v[1] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[2])
            {
                p.Set(pcenter.X - d, pcenter.Y + d, pcenter.Z - d);
                v[2] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[3])
            {
                p.Set(pcenter.X - d, pcenter.Y + d, pcenter.Z + d);
                v[3] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[4])
            {
                p.Set(pcenter.X + d, pcenter.Y - d, pcenter.Z - d);
                v[4] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[5])
            {
                p.Set(pcenter.X + d, pcenter.Y - d, pcenter.Z + d);
                v[5] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[6])
            {
                p.Set(pcenter.X + d, pcenter.Y + d, pcenter.Z - d);
                v[6] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
            if (resample[7])
            {
                p.Set(pcenter.X + d, pcenter.Y + d, pcenter.Z + d);
                v[7] = GetAntialiasedValue(transformi, p, d, interpolate);
            }
        }
    ...
    void TVolume<TypeD>::ReadFile(char *file, TStrings *tracknames)
    {
        ...
        else if (freqformat)
        {
            ...
            if (!IsMagicNumber(freqheader.Version, FREQBIN_MAGICNUMBER1) && !IsMagicNumber(freqheader.Version, FREQBIN_MAGICNUMBER2))
                return;
            ...
            LONGLONG DataOrg = sizeof(TFreqHeader) + freqheader.NumChannels * sizeof(TFreqChannel) + freqheader.NumFrequencies * (IsMagicNumber(freqheader.Version, FREQBIN_MAGICNUMBER1) ? sizeof(TFreqFrequency) : sizeof(TFreqFrequencyName));
    ...
    void TVolume<TypeD>::WriteFile(const char *file,
                                   ...
                                   int niftitransform, int niftiintentcode, const char *niftiintentname,
                                   AtomFormatType atomformattype) const
    {
