...
namespace crtl
{
    ...
    {
        if (filtertype == FilterTypeGaussian || filtertype == FilterTypeHighPassLight || filtertype == FilterTypeMean)
            FilterLinear(filtertype, params, showprogress);

        else if (filtertype == FilterTypeFastGaussian)
            FilterFastGaussian(filtertype, params, showprogress);

        else if (filtertype == FilterTypeAnisoGaussian)
            FilterAnisoGaussian(params, showprogress);

        else if (filtertype == FilterTypeMin || filtertype == FilterTypeMax || filtertype == FilterTypeMinMax || filtertype == FilterTypeRange || filtertype == FilterTypeMedian || filtertype == FilterTypeMaxMode || filtertype == FilterTypeMeanSub || filtertype == FilterTypeLogMeanSub || filtertype == FilterTypeMeanDiv || filtertype == FilterTypeLogMeanDiv || filtertype == FilterTypeMAD || filtertype == FilterTypeSD || filtertype == FilterTypeSDInv || filtertype == FilterTypeCoV || filtertype == FilterTypeLogCoV || filtertype == FilterTypeLogSNR || filtertype == FilterTypeMCoV || filtertype == FilterTypeLogMCoV || filtertype == FilterTypeMADSDInv || filtertype == FilterTypeModeQuant || filtertype == FilterTypeEntropy)
            FilterStat(filtertype, params, showprogress);

        else if (filtertype == FilterTypeRank || filtertype == FilterTypeRankRamp)
            FilterRank(filtertype, params);

        else if (filtertype == FilterTypeThreshold || filtertype == FilterTypeThresholdAbove || filtertype == FilterTypeThresholdBelow || filtertype == FilterTypeThresholdBinarize)
            FilterThreshold(filtertype, params);
        else if (filtertype == FilterTypeBinarize)
            FilterBinarize(params);
        else if (filtertype == FilterTypeRevert)
            FilterRevert(params);
        else if (filtertype == FilterTypeIntensityMult || filtertype == FilterTypeIntensityDiv)
            FilterIntensityScale(filtertype, params);
        else if (filtertype == FilterTypeIntensityNorm)
            FilterIntensityNorm(params);
        else if (filtertype == FilterTypeIntensityAdd || filtertype == FilterTypeIntensitySub)
            FilterIntensityAdd(filtertype, params);
        else if (filtertype == FilterTypeIntensityRemap)
            FilterIntensityRemap(params);
        else if (filtertype == FilterTypeKeepValue)
            FilterKeepValue(params);
        else if (filtertype == FilterTypeIntensityOpposite)
            FilterIntensityOpposite(params);
        else if (filtertype == FilterTypeIntensityInvert)
            FilterIntensityInvert(params);
        else if (filtertype == FilterTypeIntensityAbs)
            FilterIntensityAbsolute(params);
        else if (filtertype == FilterTypeToMask)
            FilterToMask(params, showprogress);
        else if (filtertype == FilterTypeSymmetrize)
            FilterSymmetrize(params);

        else if (filtertype == FilterTypeHistoEqual)
            FilterHistoEqual(params, showprogress);
        else if (filtertype == FilterTypeHistoEqualBrain)
            FilterHistoEquBrain(params, showprogress);
        else if (filtertype == FilterTypeHistoCompact)
            FilterHistoCompact(params, showprogress);

        else if (filtertype == FilterTypeErode)
            FilterErode(params, showprogress);
        else if (filtertype == FilterTypeDilate)
            FilterDilate(params, showprogress);
        else if (filtertype == FilterTypeOpen)
            FilterOpen(params, showprogress);
        else if (filtertype == FilterTypeClose)
            FilterClose(params, showprogress);
        else if (filtertype == FilterTypeMorphGradient)
            FilterMorphGradient(filtertype, params, showprogress);
        else if (filtertype == FilterTypeMorphGradientInt)
            FilterMorphGradient(filtertype, params, showprogress);
        else if (filtertype == FilterTypeMorphGradientExt)
            FilterMorphGradient(filtertype, params, showprogress);
        else if (filtertype == FilterTypeThinning)
            FilterThinning(params, showprogress);
        else if (filtertype == FilterTypeWaterfallRidges || filtertype == FilterTypeWaterfallValleys)
            FilterWaterfall(filtertype, params, showprogress);

        else if (filtertype == FilterTypeLessNeighbors || filtertype == FilterTypeMoreNeighbors)
            FilterNeighbors(filtertype, params, showprogress);
        else if (filtertype == FilterTypeRelax)
            FilterRelax(params, showprogress);
        else if (filtertype == FilterTypeKeepBiggestRegion)
            KeepRegion(SortRegionsCount, Truncate(GetLinearDim() * 0.001), Highest<int>(), Neighbors26, 0, showprogress);
        else if (filtertype == FilterTypeKeepCompactRegion)
            KeepRegion(SortRegionsCompactCount, Truncate(GetLinearDim() * 0.001), Highest<int>(), Neighbors26, 0, showprogress);
        else if (filtertype == FilterTypeKeepBiggest2Regions)
            KeepRegion(SortRegionsCount, Truncate(GetLinearDim() * 0.001), Highest<int>(), Neighbors26, 0.85, showprogress);
        else if (filtertype == FilterTypeKeepCompact2Regions)
            KeepRegion(SortRegionsCompactCount, Truncate(GetLinearDim() * 0.001), Highest<int>(), Neighbors26, 0.85, showprogress);
        else if (filtertype == FilterTypeClustersToRegions)
            ClustersToRegions(params, Neighbors26, showprogress);

        else if (filtertype == FilterTypePercentFullness || filtertype == FilterTypeCutBelowFullness || filtertype == FilterTypeSAV || filtertype == FilterTypeCompactness)
            FilterShapeFeature(filtertype, params, showprogress);
        ...
        */
        ...
        for (int n = 0; n < numrepeat; n++)
        {
            ...
            OmpFor
                ...
                for (int y = 1; y < tdim2; y++) for (int z = 1; z < tdim3; z++)
            {
            ...
            OmpFor
                ...
                for (int x = 1; x < tdim1; x++) for (int y = 1; y < tdim2; y++)
            {
            ...
            OmpFor
                ...
                for (int x = 1; x < tdim1; x++) for (int z = 1; z < tdim3; z++)
            {
        ...
        OmpParallelEnd
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // copy & clip results
            for (int x = 0; x < Dim1; x++) for (int y = 0; y < Dim2; y++) for (int z = 0; z < Dim3; z++)
            ...
            for (int yk = 0; yk < Kf.GetDim2(); yk++)
                for (int zk = 0; zk < Kf.GetDim3(); zk++)
                {
                    ...
                    if (filtertype == FilterTypeGaussian)
                        v = gauss(kp.Norm());
                    else if (filtertype == FilterTypeHighPassLight)
                        v = (kp.X || kp.Y || kp.Z ? -1 : 4) * gauss(kp.Norm()); // very rough, only for 3x3x3
    ...
    void TVolume<TypeD>::FilterStat(FilterTypes filtertype, FctParams &params, bool showprogress)
    {
        ...
        bool histostat = filtertype == FilterTypeModeQuant || filtertype == FilterTypeEntropy;

        bool robuststat = histostat || filtertype == FilterTypeMedian || filtertype == FilterTypeMaxMode || filtertype == FilterTypeMAD || filtertype == FilterTypeMCoV || filtertype == FilterTypeLogMCoV || filtertype == FilterTypeMADSDInv;
        ...
        bool logfilters = filtertype == FilterTypeLogCoV || filtertype == FilterTypeLogMCoV || filtertype == FilterTypeLogMeanSub || filtertype == FilterTypeLogMeanDiv || filtertype == FilterTypeLogSNR;
            ...
            for (int x = 0; x < Dim1; x++)
        {
            ...
            for (int y = 0; y < Dim2; y++)
                for (int z = 0; z < Dim3; z++)
                {
                    ...
                    if (filtertype == FilterTypeMin)
                        v = stat.Min();
                    else if (filtertype == FilterTypeMax)
                        v = stat.Max();
                    else if (filtertype == FilterTypeRange)
                        v = stat.Range();
                    else if (filtertype == FilterTypeMedian)
                        v = stat.Median();
                    else if (filtertype == FilterTypeMaxMode)
                        v = stat.MaxModeHSM();
                    else if (filtertype == FilterTypeSD)
                        v = stat.SD();
                    else if (filtertype == FilterTypeSDInv)
                        v = (v = stat.SD()) != 0 ? NoMore(1e10, 1 / v) : 0; // capping the max(?)
                    else if (filtertype == FilterTypeCoV)
                        v = stat.CoV();
                    else if (filtertype == FilterTypeLogCoV)
                        v = Log10((logoffset + stat.SD()) / (logoffset + stat.Average()));
                    //      else if ( filtertype == FilterTypeSNR               )   v   = stat.SNR ();
                    else if (filtertype == FilterTypeLogSNR)
                        v = Log10(logoffset + stat.SNR()); // much better with a log

                    // Sharp Window: take the one closest, the min or the max
                    else if (filtertype == FilterTypeMinMax)
                        v = stat.MinMax(GetValue(x, y, z));

                    // Mean Subtraction Window
                    else if (filtertype == FilterTypeMeanSub)
                        v = GetValue(x, y, z) - stat.Average();
                    else if (filtertype == FilterTypeLogMeanSub)
                        v = Log10(logoffset + fabs(GetValue(x, y, z) - stat.Average()));
                    else if (filtertype == FilterTypeMeanDiv)
                        v = GetValue(x, y, z) / NonNull(stat.Average()); // "real" formula
                    ...
                    else if (filtertype == FilterTypeLogMeanDiv)
                        v = Log10((logoffset + GetValue(x, y, z)) / (logoffset + stat.Average()));
                    else if (filtertype == FilterTypeMAD)
                        v = stat.MAD(CanAlterData); // Median Absolute Deviation (MAD)
                    else if (filtertype == FilterTypeMCoV)
                        v = stat.RobustCoV(); // Same as Coefficient of Variation, but with Median
                    else if (filtertype == FilterTypeLogMCoV)
                        v = Log10((logoffset + stat.InterQuartileRange()) / (logoffset + stat.Median()));
                    ...
                    if (filterresult == FilterResultNegative)
                        v = AtLeast(0.0, -v);
                    else if (filterresult == FilterResultPositive)
                        v = AtLeast(0.0, v);
                    else if (filterresult == FilterResultAbsolute)
                        v = fabs(v);
    ...
    void TVolume<TypeD>::FilterHistoEquBrain(FctParams & /*params*/, bool showprogress)
    {
        ...
        */
            ...
            for (int i = 0; i < LinearDim; i++)
        {
            ...
            if (Array[i] == 0)
                v = 0; // always force remapping 0 to 0, otherwise the background will look weird
            else if (Array[i] <= values[BlackMin])
                v = LinearRescaleNoise(Array[i], 0, values[BlackMin], 0, normblackmin);
            else if (Array[i] <= values[GreyMin])
                v = LinearRescaleNoise(Array[i], values[BlackMin], values[GreyMin], normblackmin, normgreymin);
            else if (Array[i] <= values[GreyMode])
                v = LinearRescaleNoise(Array[i], values[GreyMin], values[GreyMode], normgreymin, normgreymode);
            else if (Array[i] <= values[GreyMax])
                v = LinearRescaleNoise(Array[i], values[GreyMode], values[GreyMax], normgreymode, normgreymax);
            else if (Array[i] <= values[WhiteMode])
                v = LinearRescaleNoise(Array[i], values[GreyMax], values[WhiteMode], normgreymax, normwhitemode);
            else if (Array[i] <= whitemax)
                v = LinearRescaleNoise(Array[i], values[WhiteMode], whitemax, normwhitemode, normwhitemax);
            else
                v = LinearRescaleNoise(Array[i], whitemax, maxvalue, normwhitemax, normmaxvalue);
...
#define numdodecahedron 20
        ...
        */
            ...
            for (int j = i + 1; j < numicosahedron; j++)
                ...
                if ((dir[j] - dir[i]).Norm() <= edgelength)
                    ...
                    for (int k = j + 1; k < numicosahedron; k++)
                        ...
                        if ((dir[k] - dir[i]).Norm() <= edgelength && (dir[k] - dir[j]).Norm() <= edgelength && v < maxaxis)
        ...
        if (usehalfdir)
        {
            ...
            for (int i = 0; i < maxaxis; i++)
                if ((dir[i].X > 0 || fabs(dir[i].X) < 1e-3 && dir[i].Y > 0 || fabs(dir[i].X) < 1e-3 && fabs(dir[i].Y) < 1e-3 && dir[i].Z > 0) && v < numaxis)
        ...
        for (int c = 0, axis; c < numcorrections * numaxis; c++)
        {
            ...
            for (int si = boundary(axis, BoundaryInferior /*BoundaryInferiorMargin*/); si <= boundary(axis, BoundarySuperior /*BoundarySuperiorMargin*/); si++)
            {
                ...
                OmpParallelFor
                    ...
                    for (int x = bounding.XMin(); x <= bounding.XMax(); x += downsampling) for (int y = bounding.YMin(); y <= bounding.YMax(); y += downsampling) for (int z = bounding.ZMin(); z <= bounding.ZMax(); z += downsampling)
                {
            ...
            bool badstats = stat.SD() > (hperpmax / 17) * 1.5 || stat.SD() == 0;
            ...
            OmpParallelFor
                ...
                for (int x = bounding.XMin(); x <= bounding.XMax(); x++) for (int y = bounding.YMin(); y <= bounding.YMax(); y++) for (int z = bounding.ZMin(); z <= bounding.ZMax(); z++)
            {
            ...
            OmpParallelFor
                ...
                for (int i = 0; i < LinearDim; i++) if (mask(i)) correction(i) /= avgrescale;
            else correction(i) = 1;
        ...
        bool transfok = globalquality.Average() < (hperpmax / 64) * 1.5 && globalquality.SD() < (hperpmax / 51) * 1.5;
            ...
            for (int x = 0; x < Dim1; x++)
        {
            ...
            for (int y = 0; y < Dim2; y++)
                for (int z = 0; z < Dim3; z++)
                {
                    ...
                    // skip background
                    if (mask.IsAllocated() && temp(x, y, z) < backvalue && !mask(x, y, z))
                    {
                    ...
                    //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    else if (filtertype == FilterTypeHessianEigenMax || filtertype == FilterTypeHessianEigenMin)
                    {
                        ...
                        if (d[DiffDxx] || d[DiffDyy] || d[DiffDzz])
                        {
                            ...
                            if (filtertype == FilterTypeHessianEigenMax)
                                v = max(D[0], D[1], D[2]); // / maxvalue;
                            else if (filtertype == FilterTypeHessianEigenMin)
                                v = min(D[0], D[1], D[2]); // / maxvalue;
    ...
    {
        if (filtertype == FilterTypeThreshold || filtertype == FilterTypeThresholdBinarize)
        {
