...
namespace crtl
{
    ...
    void TSecondaryView::EvKeyDown(uint key, uint repeatCount, uint flags)
    {
        ...
        default:
            if (LinkedViewId // forward only relevant messages (navigation)
                && (key == VK_LEFT || key == VK_RIGHT || key == VK_NEXT || key == VK_PRIOR || key == VK_HOME || key == VK_END || key == VK_TAB || key == VK_SPACE || key == VK_F3 /*|| key == 'F' && ControlKey*/))
    ...
    void TSecondaryView::CmSyncViews()
    {
        ...
        else if (CaptureMode == CaptureTFSync)
