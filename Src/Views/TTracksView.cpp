...
namespace crtl
{
        ...
        : TBaseView(doc, parent, group), EEGDoc(&doc)
    {
        ...
        if (EEGDoc->GetNumElectrodes() == 0 || EEGDoc->GetNumTimeFrames() == 0)
        {
        ...
        else if (EEGDoc->GetNumMarkers(CombineFlags(MarkerTypeTrigger, MarkerTypeUserCoded)) <= NumUserMarkers)
            markertype = CombineFlags(MarkerTypeTrigger, MarkerTypeUserCoded);
        else
            markertype = MarkerTypeUserCoded;
        ...
        if (!isderived)
        { // i.e. tracks display, not frequency display
        ...
        if (BuffSize > EegMaxPointsDisplay / 2 || (EEGDoc->GetSamplingFrequency() && TimeFrameToSeconds(EEGDoc->GetNumTimeFrames(), EEGDoc->GetSamplingFrequency()) > 50) || EEGDoc->IsSpontaneous())
        {
            // Wider size
        ...
        // .t .p .sd  etc...
        if (EEGDoc->IsSpecialInfix() || EEGDoc->IsP())
        {
        ...
        else if (EEGDoc->IsContentType(ContentTypeSeg))
        {
            ...
            for (int e = 0; e < EEGDoc->GetTotalElectrodes(); e++)
                if (StringStartsWith(GetElectrodeName(e), TrackNameGFP) || StringStartsWith(GetElectrodeName(e), TrackNameRMS))
                    SelTracks.Set(e);

            if (SelTracks.NumSet() == 0)
                for (int e = 0; e < EEGDoc->GetTotalElectrodes(); e++)
                    if (StringStartsWith(GetElectrodeName(e), TrackNameDIS) || StringStartsWith(GetElectrodeName(e), TrackNameDISPlus))
                        SelTracks.Set(e);
            ...
            for (int e = 0; e < EEGDoc->GetTotalElectrodes(); e++)
            {
                ...
                if (StringStartsWith(GetElectrodeName(e), TrackNameGFP) || StringStartsWith(GetElectrodeName(e), TrackNameRMS) || StringStartsWith(GetElectrodeName(e), TrackNameDIS) || StringStartsWith(GetElectrodeName(e), TrackNameDISPlus) || StringStartsWith(GetElectrodeName(e), TrackNamePolarity) || StringStartsWith(GetElectrodeName(e), TrackNameSeg) // set a default coloring, maybe overriden later
                    || StringStartsWith(GetElectrodeName(e), TrackNameGEV) || StringStartsWith(GetElectrodeName(e), TrackNameCorr))
                {
                    ...
                    for (int f = e; f < EEGDoc->GetTotalElectrodes();)
                    {
                        ...
                        f += StringStartsWith(GetElectrodeName(e), TrackNameCorr) || StringStartsWith(GetElectrodeName(e), TrackNameGEV) ? -1 : 1;
                    }
                }
        ...
        else if (EEGDoc->IsContentType(ContentTypeData) || EEGDoc->IsContentType(ContentTypeErrorData))
        {
        ...
        else
        {
            ...
            if (IsExtensionAmong(EEGDoc->GetTitle(), FILEEXT_EEGEP " " FILEEXT_EEGEPH " " FILEEXT_EEGSEF " " FILEEXT_RIS) && EEGDoc->GetNumRegularElectrodes() < 32)
    ...
    void TTracksView::ResetScalingLevel()
    {
        ...
        if (EEGDoc->IsSpecialInfix() || EEGDoc->IsP())
        {
            ...
            SetScaling(EEGGLVIEW_STVINIT * Attr.H / (double)NonNull(125 * sqrt((int)SelTracks)));
        else if (EEGDoc->IsContentType(ContentTypeErrorData))
            SetScaling(EEGGLVIEW_STVINIT * Attr.H / (double)NonNull(66 * (int)SelTracks));
        else if (EEGDoc->IsContentType(ContentTypeHistogram))
            SetScaling(EEGGLVIEW_STVINIT * Attr.H / (double)NonNull(33 * NoMore(50, (int)SelTracks)));
    }
    ...
    void TTracksView::ResetScalingContrast()
    {
        ...
        if (EEGDoc->IsSpecialInfix() || EEGDoc->IsP())
        {
            ...
            if (EEGDoc->IsP() || IsStringAmong(ext2, InfixP))
                SetScalingContrast(ColorTableToScalingContrast(10.0));
    ...
    void TTracksView::SetupWindow()
    {
        ...
        if (!(EEGDoc->IsContentType(ContentTypeSeg) || EEGDoc->IsContentType(ContentTypeData) || EEGDoc->IsContentType(ContentTypeErrorData)))
    ...
    void TTracksView::TFToString(double dtf,
                                 char *stf,
                                 char *stime, TimeDisplayEnum horizscale) const
    {
    ...
    void TTracksView::CursorToTitle(TTFCursor *tfc, char *title) const
    {
        ...
        if (tfc->IsSplitted())
        {
            ...
            if (*stimemin)
            {
                StringAppend(title, stfmin, "..", stfmax, " (", stfdelta, ") / ");
                StringAppend(title, stimemin, "..", stimemax, " (", stimedelta, ")  ", sdate);
            }
            // "Title   X=tf..tf (dtf)  date"
            else if (*sdate)
                StringAppend(title, stfmin, "..", stfmax, " (", stfdelta, ")  ", sdate);
            // "Title   X=tf..tf (dtf)"
            else
                StringAppend(title, stfmin, "..", stfmax, " (", stfdelta, ")");
        }
        else
        {
            ...
            // "Title   X=tf / ms  date"
            if (*stimemin)
                StringAppend(title, stfmin, " / ", stimemin, "  ", sdate);
            // "Title   X=tf  date"
            else if (*sdate)
                StringAppend(title, stfmin, "  ", sdate);
            // "Title   X=tf"
            else
                StringAppend(title, stfmin);
        }
    }
    ...
    void TTracksView::UpdateCaptionTracksValues(char *buff)
    {
        ...
        if ((!nhr && nhp) || (!nsr && nsp))
        { // only pseudos, highlighted or selected
            ...
            { // scan all TFs
                ...
                for (TIteratorSelectedForward eli(*tosel, EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex()); (bool)eli; ++eli)
                {
                    ...
                    {
                        if (EegBuff[eli()][tfi] < minv)
                            minv = EegBuff[eli()][tfi];
                        if (EegBuff[eli()][tfi] > maxv)
                            maxv = EegBuff[eli()][tfi];
                    }
        ...
        { // some regular tracks, ignoring pseudos if any
          // select highlighted or display
    ...
    void TTracksView::UpdateCaption()
    {
        ...
        if (EEGDoc->IsContentType(ContentTypeSeg) && SelFilling && SelFilling->NumSet() > 0 && SelFilling->NumSet() <= SelFilling->MaxValue() - 1)
        {
    ...
    void TTracksView::SetPartWindows(TSelection &sel, bool refresh)
    {
        ...
        if (renderingmode == LayoutFourPages && numtracks < 4)
            renderingmode = LayoutTwoPages;
        if (renderingmode == LayoutTwoPages && numtracks < 2)
            renderingmode = LayoutOnePage;
        ...
        { // tracks
            ...
            for (s = 0; s < numregslots; s++)
            {
                ...
                if (renderingmode == LayoutOneTrackOneBox || renderingmode == LayoutOneTrackOneBoxXyzFlat || renderingmode == LayoutOneTrackOneBoxXyz3D)
        ...
        if (renderingmode == LayoutOneTrackOneBox || renderingmode == LayoutOneTrackOneBoxXyzFlat || renderingmode == LayoutOneTrackOneBoxXyz3D)
        {
            ...
            else if (renderingmode == LayoutOneTrackOneBoxXyzFlat || renderingmode == LayoutOneTrackOneBoxXyz3D)
            {
                ...
                for (TIteratorSelectedForward si(sel); (bool)si && pw < numslots; ++si, ++pw)
                {
                    ...
                    { // pseudo
                        ...
                        if (CurrentDisplaySpace == DisplaySpace3D)
                            ...
                            //                                     * w * 1.5 );
                            el = center - (toup * b->Radius() * 1.05) + toright * (((double)psindex - (numaftermin - 1) / 2.0) * w * 1.5 / regrescale);
                        else
                            //                    el = center - ( toupproj * ( b->XMin() - 2.0 * h ) )
                            //                       + toright * ( ( (double) psindex - numaftermin / 2 + 0.5  )
                            //                                     * w * 1.5 );
                            el = center - (toupproj * (b->XMin() - 2.0 * h / regrescale)) + toright * (((double)psindex - (numaftermin - 1) / 2.0) * w * 1.5 / regrescale);
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // layout "?la page"
        else if (renderingmode == LayoutFourPages || renderingmode == LayoutTwoPages || renderingmode == LayoutOnePage)
        {
            ...
            int numcolumns = renderingmode == LayoutFourPages ? 4 : renderingmode == LayoutTwoPages ? 2
                                                                : renderingmode == LayoutOnePage    ? 1
                                                                                                    : 1;
    ...
    */
    ...
    void TTracksView::SetupHorizontalGrid(double &hgridorg, double &hgridstep)
    {
        if (EEGDoc->IsContentType(ContentTypeErrorData))
        {
            ...
            hgridstep = AtLeast(1, Round(2.5 * (EEGDoc->GetNumTimeFrames() * NumIntegerDigits(EEGDoc->GetNumTimeFrames()) * SFont->GetAvgWidth()) / WindowSlots[0].ToRight.Norm()));
        ...
        hgridstep = (5 + (EEGDoc->GetSamplingFrequency() && !EEGDoc->IsContentType(ContentTypeHistogram) ? 7 : 0) + (EEGDoc->DateTime.MicrosecondPrecision ? 5 : 0) + (EEGDoc->DateTime.RelativeTime ? 5 : 9) // relative time is quite compact, while full time is not
                     ) * 1.5 *
                        SFont->GetAvgWidth() * CDPt.GetLength() / WindowSlots[0].ToRight.Norm() * tr +
                    1;
    ...
    void TTracksView::GLPaint(int how, int renderingmode, TGLClipPlane *otherclipplane)
    {
        ...
        if (((bool)otherclipplane[0] || (bool)otherclipplane[1] || (bool)otherclipplane[2]) && ((bool)ClipPlane[0] || (bool)ClipPlane[1] || (bool)ClipPlane[2]) || (how & GLPaintForceClipping))
...
#define MarginVariables                                                                                                                       \
    ...
    double margin = drawmargin ? TextMargin : 0;
        ...
        if (HasWindowSlots() && (how & GLPaintOpaque))
        {
            ...
            bool draworigin = CurrentDisplaySpace == DisplaySpaceNone && ShowBaseline
                              /*&& allnames*/
                              && EEGDoc->GetSamplingFrequency() && !(EEGDoc->DateTime.RelativeTime && EEGDoc->DateTime.RelativeTimeOffset == 0);
            ...
            for (int pass = CurrentDisplaySpace == DisplaySpace3D ? 0 : 1; pass < 2; pass++)
                ...
                for (int s = 0; s < GetNumWindowSlots(); s++)
                {
                    ...
                    trackssuper = !IsIntensityModes() && ((TracksSuper && firstsel <= EEGDoc->GetLastRegularIndex()) || !enoughh);

                    if (CurrentDisplaySpace != DisplaySpaceNone)
                    { // all shifts are already included
                    ...
                    else if (trackssuper || avgrois)
                    {
                        ...
                        if (ispositive && !IsIntensityModes())
                        { // slightly shift downward
                    ...
                    bool canvertgrid = numTracks && CurrentDisplaySpace == DisplaySpaceNone && (trackssuper || enoughh || -trdelta.Y > stepy / 4.0);

                    bool drawvertscale = ShowVertScale && canvertgrid;

                    bool drawhoriscale = ShowHorizScale && numTracks && CurrentDisplaySpace == DisplaySpaceNone
                                         // && enoughw
                    ...
                    */
                    ...
                    //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    // triggers
                    if (ShowTags && (how & GLPaintOwner) && (CurrentDisplaySpace != DisplaySpace3D || trorg.Z <= ModelRadius))
                    { // no triggers in back part of 3D
                    ...
                    if (CurrentDisplaySpace == DisplaySpaceNone && drawhoriscale && IsTracksMode() && (how & GLPaintOwner))
                    {
                    ...
                    else if (BadTracks[st])
                        glColor4f(BadColor);
                    else if (AuxTracks[st])
                        glColor4f(AuxColor);
                    else
                        toslot->Color.GLize();
                    ...
                    for (TIteratorSelectedForward sti(*sel); (bool)sti; ++sti, ++st0, firsttrack = false)
                    {
                        ...
                        if (CurrentDisplaySpace == DisplaySpaceNone && drawvertscale && !IsIntensityModes() && (!trackssuper || st == firstsel) && (how & GLPaintOwner))
                        {
                            ...
                            if (pol * dv / descaley < 20)
                                dv *= 2; // too close -> increase space
                            else if (pol * dv / descaley > 200)
                                dv /= 10; // too far   -> reduce space
                        ...
                        */
                        ...
                        if (Highlighted[st])
                            LineColor.Set(0, HighlightColor);
                        else if (BadTracks[st])
                            LineColor.Set(0, BadColor);
                        else if ((bool)Using)
                        { // override when linking multiple Eegs
                            ...
                            LineColor.Set(0, AuxColor);
                        else
                            LineColor[0] = toslot->Color;

                        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        // Filling with computed colors (indexed or analog)
                        if (IsFilling && IsTracksMode() && Filling[st])
                        {
                            ...
                            // handy lambdas
                            auto IsSegmentTransition = [&toref](int p)
                            { return p == 0 /*minp*/ || *toref != *(toref - 1); };

                            auto IsSharpTransition = [&tobuff](int p)
                            { return p == 1                                                                                                                               // transition on first TF
                                     || p > 0 && *tobuff * *(tobuff - 1) <= 0                                                                                             // sign inversion
                                     || p > 1 && *(tobuff - 1) == *(tobuff - 2); }; // blocks of constant values

                            // Segments' coloring
                            if (how & GLPaintOwner)
                            { // overlapping hides everything... maybe transparent?

                                if (IsFilling == FillingVoid || (bool)Using || (discrete && SelFilling && SelFilling->IsNotSelected(*toref)) || !Filling[st]->GetColor(*toref, col1))
                            ...
                            if (drawtext)
                            {
                                ...
                                for (tobuff = EegBuff[st] + minp, toref = EegBuff[ref] + minp, p = minp; p <= maxp; p++, tobuff++, toref++)
                                    ...
                                    if (IsSegmentTransition(p))
                                    {
                                        ...
                                        if (IsFilling == FillingColor && !Filling[st]->GetColor(*toref, col1) && !(bool)Using || !nextseg)
                                            col1 = TextColor[0];
                                        ...
                                        if (nextseg // draw beginning of segment's separator - !needed if tracks have sharp sign transitions!
                                            && p > 0)
                                        { // don't draw a line if starting point of segment is out of sight
                        ...
                        if (!IsIntensityModes() && (!regel || CurrentDisplaySpace != DisplaySpaceNone))
                        {
                                    ...
                                    if ((bool)Montage[st]) // draw the graphical subtraction
                                        ...
                                        if (Montage[st].TwoTracks())
                                            for (tobuff1 = Montage[st].ToData1 + minp,
                                                tobuff2 = Montage[st].ToData2 + minp, p = minp;
                                                 p <= maxp; p++, tobuff1++, tobuff2++)
                        ...
                        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        // draw baseline axis
                        if (ShowBaseline && !IsIntensityModes() && (firsttrack || !trackssuper)
                            /*&& ( how & GLPaintOwner )*/)
                        {
                        ...
                        if (ShowSD && IsTracksMode() && EEGDoc->HasStandardDeviation() && regel && !(bool)Montage)
                        {
                            ...
                            // fill anti-aliased
                            if (ShowSD == SDFilledTransparent || ShowSD == SDFilledOpaque)
                            {
                            ...
                            // draw 2 borders
                            if (ShowSD == SDDashed)
                            { // either dashed or on top of the filled to emulate anti-aliasing
                        ...
                        if (drawmargin && !roiname && allnames && !avgrois && !trackssuper && (how & GLPaintOwner))
                        ...
                        if (IsIntensityModes())
                        {
                            ...
                            if (ScalingAuto != ScalingAutoOff && !(RButtonDown && IntensityOverride && MouseAxis == MouseAxisHorizontal))
                            {
                                ...
                                if (regel)
                                    ...
                                    for (tobuff = EegBuff[st] + minp, p = minp; p <= maxp; p++, tobuff++)
                                    {
                                        ...
                                        if (IsFilling && Filling[st]
                                            ...
                                            && (p > 0 && *tobuff * *(tobuff - 1) <= 0 || p > 1 && *(tobuff - 1) == *(tobuff - 2))) // does not detect 1 TF constant before...
                        ...
                        if (ShowSD && IsBarsMode() && EEGDoc->HasStandardDeviation() && regel && !(bool)Montage)
                        {
                    ...
                    if (CurrentDisplaySpace == DisplaySpaceNone && ShowBaseline && !IsIntensityModes() && trackssuper && (how & GLPaintOwner))
                    {
                    ...
                    if (drawmargin && !roiname && (!allnames || trackssuper) && (how & GLPaintOwner))
                    {
        ...
        */
        ...
        if (!Outputing() && !NestedPaint && (how & GLPaintTransparent))
        ...
        for (int i = indexmin; i <= indexmax; i++)
        {
            ...
            fromx = trorg.X + (tomarker->From < CDPt.GetMin()   ? 0
                               : tomarker->From > CDPt.GetMax() ? CDPt.GetLength()
                                                                : tomarker->From - CDPt.GetMin()) *
                                  scaleh;
            tox = trorg.X + (tomarker->To < CDPt.GetMin()   ? 0
                             : tomarker->To > CDPt.GetMax() ? CDPt.GetLength() - 1
                                                            : tomarker->To - CDPt.GetMin() + (OneMoreHoriz() ? 1 : 0)) *
                                scaleh; // + ( CurrentDisplaySpace == DisplaySpaceNone ? 1 : 0 );
...
#define numoverlap 32
            ...
            for (int i = indexmin; i <= indexmax; i++)
            {
                ...
                fromx = trorg.X + (tomarker->From < CDPt.GetMin() ? 0 : tomarker->From > CDPt.GetMax() ? CDPt.GetLength()
                                                                                                       : tomarker->From - CDPt.GetMin() + (OneMoreHoriz() && tomarker->IsNotExtended() ? 1.05 : 0.05)) *
                                      scaleh +
                        3;
                ...
                for (imo = 0; imo < numoverlap; imo++)
                {
                    ...
                    if (tox < toright && maxoverlap[imo] <= fromx // regular case
                        || tox >= toright && maxoverlap[imo] < toright - stringw)
                    { // special case when going further than border
    ...
    void TTracksView::DrawTFCursor(bool undrawold, bool localhdc)
    {
                              ...
                              bool thin)
        {
            ...
            if (posmin == posmax && !OneMoreHoriz())
            {
                ...
                { // thick
                    glBegin(GL_QUADS);
                    glVertex3f(fromx + 1, fromy, fromz);
                    glVertex3f(fromx + 1, toy, fromz);
                    glVertex3f(fromx - 1, toy, fromz);
                    glVertex3f(fromx - 1, fromy, fromz);
                    glEnd();
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    { // thick
                        ...
                        glVertex3f(fromx, fromy, fromz);
                        glVertex3f(fromx, toy, fromz);
                        glVertex3f(fromx - 2, toy, fromz);
                        glVertex3f(fromx - 2, fromy, fromz);
                        glVertex3f(tox, toy, fromz);
                        glVertex3f(tox, fromy, fromz);
                        glVertex3f(tox + 2, fromy, fromz);
                        glVertex3f(tox + 2, toy, fromz);
                        glEnd();
                    }

                    // filling box?
                    if (CurrentDisplaySpace == DisplaySpaceNone && !IsIntensityModes() && v2[0] - v1[0] > 1 /*&& IsFilling != FillingColor*/)
                    {
        ...
        for (int s = 0; s < GetNumWindowSlots(); s++)
        {
            ...
            else
            {
                ...
                if (CurrentDisplaySpace == DisplaySpace3D && fromz >= ModelRadius)
                    continue;
            }
    ...
    void TTracksView::EvKeyDown(uint key, uint repeatCount, uint flags)
    {
        ...
        case 'D':
            if (IsIntensityModes())
                CmSetScalingAdapt();
            else if (EEGDoc->HasStandardDeviation())
                CmShowSD();
            break;
        ...
        case VK_UP:
            ...
            else if (ShiftKey && IsIntensityModes())
    ...
    void TTracksView::EvLButtonDown(uint i, const TPoint &p)
    {
        ...
        if (CaptureMode == CaptureGLLink)
        {
            ...
            if (view->GetViewId() != GetViewId())
            {
                ...
                else
                {
                    if (!UsedBy.IsInside(view))
                    {
                        ...
                        if (EEG2 && EEG2->GetTotalElectrodes() == EEGDoc->GetTotalElectrodes() && EEG2->GetNumTimeFrames() == EEGDoc->GetNumTimeFrames())
                        {
            ...
            else
            {
                ...
                foreachin(Using, iteratorview)
                    iteratorview()
                        ->UsedBy.Remove(this, DontDeallocate);
        ...
        else if (CurrentDisplaySpace == DisplaySpaceNone && ControlKey)
        { // make an alias Ctrl-Left = Middle
        ...
        else
        {
            ...
            if (CurrentDisplaySpace == DisplaySpaceNone && CaptureMode == CaptureNone)
            {
                ...
                if (toslot)
                {
                    ...
                    if (tf < 0)
                        tf = CDPt.GetMin();
                    else if (tf > (long)CDPt.GetLength())
                        tf = CDPt.GetMax();
                    else
                        tf += CDPt.GetMin();
    ...
    void TTracksView::EvLButtonUp(uint i, const TPoint &p)
    {
        ...
        else if (CaptureMode == CaptureGLSelect)
        { // make an alias Ctrl-Left = Middle
    ...
    void TTracksView::EvMouseMove(uint i, const TPoint &p)
    {
        ...
        if (RButtonDown)
        {
            ...
            if (MouseAxis == MouseAxisHorizontal || MouseAxis == MouseAxisDiagonal)
            {
                ...
                if (IsIntensityModes() && IntensityOverride)
                {
                    ...
                    if (dx > 0)
                        ScalingLevel *= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 1.0 : 0.2);
                    else
                        ScalingLevel /= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 1.0 : 0.2);
                ...
                else if (ShiftKey)
                {
                    ...
                    if (dx > 0)
                        STH *= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 0.6 : 0.2);
                    else
                        STH /= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 0.6 : 0.2);
            ...
            /*else*/ if (MouseAxis == MouseAxisVertical || MouseAxis == MouseAxisDiagonal)
            {
                ...
                else if (ShiftKey)
                {
                    ...
                    if (ControlKey && (bool)Highlighted)
                    {
                        ...
                        for (int e = 0; e < EEGDoc->GetTotalElectrodes(); e++)
                        {
                            ...
                            if (dy > 0)
                                ScaleTracks[e] /= 1 + min(ady, MouseMoveScale) / (double)MouseMoveScale * 0.3;
                            else
                                ScaleTracks[e] *= 1 + min(ady, MouseMoveScale) / (double)MouseMoveScale * 0.3;
                        }
                    ...
                    { // ! ControlKey
                      // adjust global scaling
                        ...
                        if (dy > 0)
                            ScalingLevel /= 1 + min(ady, MouseMoveScale) / (double)MouseMoveScale * (ady > MouseMoveScaleFast ? 1.0 : 0.3);
                        else
                            ScalingLevel *= 1 + min(ady, MouseMoveScale) / (double)MouseMoveScale * (ady > MouseMoveScaleFast ? 1.0 : 0.3);
                ...
                { // && ! ShiftKey
                    ...
                    if (dy > 0)
                        OffsetTracks += min(ady, MouseMoveScale) / (double)MouseMoveScale * (ady > MouseMoveScaleFast ? 75 : 25) / scalev * (TracksFlipVert ? -1 : 1);
                    else
                        OffsetTracks -= min(ady, MouseMoveScale) / (double)MouseMoveScale * (ady > MouseMoveScaleFast ? 75 : 25) / scalev * (TracksFlipVert ? -1 : 1);
        ...
        else if (CurrentDisplaySpace == DisplaySpaceNone && MButtonDown || LButtonDown && CurrentDisplaySpace == DisplaySpaceNone && ControlKey)
        { // track selection
    ...
    void TTracksView::EvMouseWheel(uint modKeys, int zDelta, const TPoint &)
    {
        ...
        if (ControlKey && ShiftKey)
        {
            ...
            // recheck limits
            if (newlength > CDPt.GetLimitLength())
                newlength = CDPt.GetLimitLength();
            else if (newlength < mintf)
                newlength = mintf | oddtf;
    ...
    */
    ...
    void TTracksView::BestXYZBox(double &w, double &h)
    {
        ...
        for (int e = 0; e < numpoints; e++)
        {
            ...
            for (int f = e + 1; f < numpoints; f++)
            {
                ...
                if (dx < dy)
                { // take the infinite norm, that is the max
                    if (dy < dminy)
                        dminy = dy; // test separately the min distances
                }
                else if (dy < dx)
                {
                    if (dx < dminx)
    ...
    void TTracksView::CheckCDPtAndTFCursor(bool fromtfcursor)
    {
        ...
        // or nothing to do?
        if (TFCursor.GetPosMin() == CDPt.GetMin() && TFCursor.GetPosMax() == CDPt.GetMax())
            return;

        if (fromtfcursor)
        { // which direction of transfer?
            long oldtfmin = CDPt.GetMin();
            long oldtfmax = CDPt.GetMax();
            ...
            else if (TFCursor.GetPosMin() != CDPt.GetMin() || TFCursor.GetPosMax() != CDPt.GetMax())
            {
    ...
    void TTracksView::UpdateBuffers(long oldtfmin, long oldtfmax, long newtfmin, long newtfmax)
    {
        ...
        auto bigblock = [](const TArray2<float> &buff) -> size_t
        { return buff.GetDim1() * buff.GetDim2(); };
        ...
        if (newtfmin > oldtfmax || newtfmax < oldtfmin || EEGDoc->AreFiltersActivated() && EEGDoc->GetFilters()->HasTemporalFilter())
        { // only temporal filters need to care for margins and full reload
        ...
        { // case 3: deltamin < 0 && deltamax > 0 (expand)
    ...
    void TTracksView::ResetScaleTracks(const TSelection *sel)
    {
        ...
        if (EEGDoc->IsContentType(ContentTypeSeg))
        {
            ...
            for (int e = 0; e < EEGDoc->GetTotalElectrodes(); e++)
            {
                ...
                if (StringStartsWith(GetElectrodeName(e), TrackNameGFP))
                    si = scalegfp;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameRMS))
                    si = scalegfp;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameDIS))
                    si = scaledis;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameDISPlus))
                    si = scaledis;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameGEV))
                    si = scalegev;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameCorr))
                    si = scalecorr;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameEV))
                    si = scaleev;
                else if (StringStartsWith(GetElectrodeName(e), TrackNamePolarity))
                    si = scalepol;
                else if (StringStartsWith(GetElectrodeName(e), TrackNameSeg))
                    si = scaleseg;
                else
                    si = scaleothers;
        ...
        { // regular file
            ...
            for (int e = 0; e < EEGDoc->GetTotalElectrodes(); e++)
            {
                ...
                if (sel && sel->IsNotSelected(e))
                    continue;

                if (e <= EEGDoc->GetLastRegularIndex() && !(BadTracks[e] || AuxTracks[e]))
                {
                ...
                { // auxs and bads are scaled individually
                    ...
                    if (e == EEGDoc->GetGfpIndex())
                        ScaleTracks[e] *= 2.0;
                    else if (e == EEGDoc->GetDisIndex())
                        ScaleTracks[e] *= 1.5;
                    else if (e == EEGDoc->GetAvgIndex() && maxtrack / NonNull(EEGDoc->GetAbsMaxValue()) < 1e-6) // nearly 0?
    ...
    void TTracksView::CmZoomVert(owlwparam w)
    {
        ...
        if (w == IDB_COMPTRACKSV || w == IDB_ISDECBRIGHT)
            ScalingLevel /= ShiftKey ? ScalingLevelBigStep : ScalingLevelSmallStep;
        else if (w == IDB_EXTTRACKSV || w == IDB_ISINCBRIGHT)
            ScalingLevel *= ShiftKey ? ScalingLevelBigStep : ScalingLevelSmallStep;
    ...
    void TTracksView::CmZoomHorz(owlwparam w)
    {
        ...
        do
        { // update STH until it produces a new length
            if (w == IDB_EXTTRACKSH)
                STH *= ShiftKey ? EEGGLVIEW_STHBIGSTEP : EEGGLVIEW_STHSMALLSTEP;
            else if (w == IDB_COMPTRACKSH)
                STH /= ShiftKey ? EEGGLVIEW_STHBIGSTEP : EEGGLVIEW_STHSMALLSTEP;
    ...
    void TTracksView::ScrollDisplay(int numpoints)
    {
        ...
        if (TFCursor.GetPosMax() > CDPt.GetMax() || TFCursor.GetPosMin() < CDPt.GetMin())
        {
            if (TFCursor.IsExtending())
    ...
    void TTracksView::ScrollCursor(int desiredPoints, bool forward)
    {
        ...
        if (forward)
        {
            ...
            if ((CDPt.GetMax() + readpoints) < TFCursor.GetPosMax())
            { // reading is not enough, so jump
        ...
        else
        {
            ...
            if ((CDPt.GetMin() - readpoints) > TFCursor.GetPosMin())
            { // reading is not enough, so jump
                long tf1 = CDPt.GetMin();
                long tf2 = CDPt.GetMax();
    ...
    void TTracksView::CmPageForward(owlwparam w)
    {
        ...
        // need to move the cursor ?
        if (TFCursor.GetPosMin() > CDPt.GetMax() || TFCursor.GetPosMax() < CDPt.GetMin())
        {
            if (TFCursor.IsExtending())
                TFCursor.SetExtendingPos(TFCursor.GetExtendingPos() + CDPt.GetMin() - oldTFMin);
            else
        ...
        if (IsRoiMode())
        {
            ...
            else if (w == IDB_MORETRACKS)
            {
                ...
                else
                {
                    ...
                    if (ShiftKey)
                        Rois->Set();
                    else if ((r = roissel->FirstNotSelected(roissel->FirstSelected())) != -1)
                        Rois->Set(r);
                    else
    ...
    void TTracksView::CmChangeNumPseudoTracks(owlwparam w)
    {
        ...
        if (w == IDB_LESSPSEUDOTRACKS)
        {
            ...
            if (numPseudoTracks <= 0)
                return;

            if (ShiftKey)
                EEGDoc->ClearPseudo(SelTracks);
            else
                SelTracks.Reset(SelTracks.LastSelected(EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex()));

            SetPartWindows(SelTracks);
        }
        else
        { // IDB_MOREPSEUDOTRACKS
          // if tracks selected, keep them
            if ((bool)Highlighted)
            {
            ...
            if (numPseudoTracks == NumPseudoTracks)
                return;

            if (ShiftKey)
                EEGDoc->SetPseudo(SelTracks);
            else
                SelTracks.Set(SelTracks.FirstNotSelected(EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex()));
    ...
    void TTracksView::ShiftTracks(owlwparam w, int num)
    {
        ...
        if (IsRoiMode())
        {
            ...
            if (w == IDB_UP)
            {
                if (n1 && n1 < Rois->GetNumRois())
                    Rois->ShiftUp(ShiftKey ? n1 : num);
                else
                    SelTracks.ShiftUp(EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex(), ShiftKey ? n2 : num);
            }
            else if (w == IDB_DOWN)
            {
                if (n1 && n1 < Rois->GetNumRois())
                    Rois->ShiftDown(ShiftKey ? n1 : num);
                else
                    SelTracks.ShiftDown(EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex(), ShiftKey ? n2 : num);
            }
        ...
        { // not IsRoiMode ()
            ...
            if (w == IDB_UP)
            {
                if (n1 && n1 < GetMaxTracks())
                    SelTracks.ShiftUp(EEGDoc->GetFirstRegularIndex(), EEGDoc->GetLastRegularIndex(), ShiftKey ? n1 : num);
                else
                    SelTracks.ShiftUp(EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex(), ShiftKey ? n2 : num);
            }
            else if (w == IDB_DOWN)
            {
                if (n1 && n1 < GetMaxTracks())
                    SelTracks.ShiftDown(EEGDoc->GetFirstRegularIndex(), EEGDoc->GetLastRegularIndex(), ShiftKey ? n1 : num);
                else
                    SelTracks.ShiftDown(EEGDoc->GetFirstPseudoIndex(), EEGDoc->GetLastPseudoIndex(), ShiftKey ? n2 : num);
            }
        }
    ...
    void TTracksView::CmSetReference(owlwparam w)
    {
        ...
        case CM_EEGREFMONTAGE:
            ...
            do
            {
                ...
                // update auxs
                if (IsElectrodeNameAux(GetElectrodeName(Montage[mi].El1, false)) || IsElectrodeNameAux(GetElectrodeName(Montage[mi].El2, false)) || StringIsNotEmpty(mname) && IsElectrodeNameAux(mname))
    ...
    void TTracksView::CmSetScaling(owlwparam w)
    {
        ...
        else if (w == CM_EEGRESETSCALING)
        { // reset relative tracks scaling
    ...
    void TTracksView::GoToMarker(const TMarker *marker, bool centerextended)
    {
        ...
        if (marker->From < CDPt.GetMin() || marker->From > CDPt.GetMax() || marker->To < CDPt.GetMin() || marker->To > CDPt.GetMax())
        {

            if (centerextended && marker->IsExtended() || !centerextended && marker->Length() >= CDPt.GetLength()) // only markers that don't fit entirely
    ...
    void TTracksView::CmPreviousNextMarker(owlwparam w)
    {
        ...
        if (!(tolastmarker != 0 && IsFlag(tolastmarker->Type, DisplayMarkerType) && tolastmarker->From == TFCursor.GetPosMin() && tolastmarker->To == TFCursor.GetPosMax()))
        {
            ...
            if (w == IDB_NEXTMARKER)
            {
                ...
                for (int i = 0; i < markers.Num(); i++)
                {
                    ...
                    if (!(markers[i]->From > TFCursor.GetPosMin() || markers[i]->From == TFCursor.GetPosMin() && markers[i]->To > TFCursor.GetPosMax()))
                        continue;
            ...
            else
            {
                ...
                for (int i = markers.Num() - 1; i >= 0; i--)
                {
                    ...
                    if (!(markers[i]->To < TFCursor.GetPosMax() || markers[i]->To >= TFCursor.GetPosMax() && markers[i]->From < TFCursor.GetPosMin()))
                        continue;
        ...
        do
        {
            ...
            notposition = tomarker->From == TFCursor.GetPosMin() && tomarker->To == TFCursor.GetPosMax();
            if (notposition)
                continue;
    ...
    void TTracksView::CmDuplicateTriggersToMarkers()
    {
        ...
        for (int sessioni = fromsession; sessioni <= tosession; sessioni++)
        {
            ...
            { // does not check for MarkersInsertErrors errors
        ...
        for (int sessioni = fromsession; sessioni <= tosession; sessioni++)
        {
            ...
            for (int i = 0; i < markers.Num(); i++)
            {
                ...
                do
                {
                    ...
                    int db = toc - name;
                    if (db)
                        StringCopy(before, name, db);
                    else
                        ClearString(before);

                    int da = StringLength(name) - ((toc - name) + StringLength(from));
                    if (da)
                        StringCopy(after, toc + StringLength(from), da);
                    else
                        ClearString(after);
        ...
        for (int sessioni = fromsession; sessioni <= tosession; sessioni++)
        {
            ...
            for (int firsti = 0; firsti < oldmarkers.Num(); firsti++)
            {
                ...
                for (int nexti = firsti + 1; nexti < oldmarkers.Num(); nexti++)
                {
                    ...
                    if (nextmarker->From == firstmarker->From && nextmarker->To == firstmarker->To && nextmarker->Type == firstmarker->Type)
                    {
    ...
    void TTracksView::CmSearchMarker(owlwparam w)
    {
        ...
        if (w == CM_EEGMRKSEARCH || w == CM_EEGMRKSEARCHNEXT)
        {
            ...
            for (int i = 0; i < markers.Num(); i++)
            {
                ...
                if (!(markers[i]->From > tfmin || markers[i]->From == tfmin && markers[i]->To > tfmax))
                    continue;

                // restrict search to current display filtering
                if ((bool)GrepMarkerFilter && !GrepMarkerFilter.Matched(markers[i]->Name) || (bool)GrepMarkerSearch && !GrepMarkerSearch.Matched(markers[i]->Name))
                    continue;
        ...
        else if (w == CM_EEGMRKSEARCHPREVIOUS)
        {
            ...
            for (int i = markers.Num() - 1; i >= 0; i--)
            {
                ...
                if (!(markers[i]->To < tfmax || markers[i]->To >= tfmax && markers[i]->From < tfmin))
                    continue;
    ...
    void TTracksView::CmSetMarkerDisplay(owlwparam w)
    {
        ...
        case CM_EEGMRKNONE:
            DisplayMarkerType = MarkerTypeUnknown;
            break;
        case CM_EEGMRKTRIGGER:
            XorFlags(DisplayMarkerType, MarkerTypeTrigger);
            break;
        case CM_EEGMRKMARKER:
            XorFlags(DisplayMarkerType, MarkerTypeMarker);
            break;
        case CM_EEGMRKEVENT:
            XorFlags(DisplayMarkerType, MarkerTypeEvent);
            break;
        case CM_EEGMRKALL:
        default:
            DisplayMarkerType = AllMarkerTypes;
            break;
        }
    ...
    void TTracksView::CmScanForBadEpochs()
    {
        ...
        if (!GetOptionsFromUser("How precise would you like the search to be:" NewLine
                                    NewLine Tab "- (L)iberal " Tab Tab Tab Tab "(more flagged as bad)" NewLine Tab "- (R)egular " Tab Tab Tab Tab "(best compromise)" NewLine Tab "- (S)trict " Tab Tab Tab Tab "(less flagged as bad)" NewLine Tab "- Specific tolerance, in [0..100]" Tab "(0: super-liberal; 100: super-strict)",
        ...
        if (*answer == 'L')
            tolerance = BadEpochsToleranceLiberal;
        else if (*answer == 'R')
            tolerance = BadEpochsToleranceRegular;
        else if (*answer == 'S')
            tolerance = BadEpochsToleranceStrict;
        else
            tolerance = AtLeast(0.0, StringToDouble(answer) / 100); // allow user to manually bypass the tolerance factor
    ...
    void TTracksView::CmFilter()
    {
        ...
        if (IsCommandSender())
        {
            ...
            if (senderris && GODoc && GODoc->GetNumSpDoc())
                dialogok = EEGDoc->SetFilters(0, GODoc->GetSpDoc(CurrSp)->GetDocPath());
            else if (!senderris && GODoc && GODoc->GetNumXyzDoc())
                dialogok = EEGDoc->SetFilters(0, GODoc->GetXyzDoc(CurrXyz)->GetDocPath());
            else
                dialogok = EEGDoc->SetFilters(0);
        }

        else
        { // IsCommandReceiver ()          // commands cloning AND a receiving cloned view -> forward the parameters set by caller view
    ...
    {
        // choose 3D set here
        if (RenderingMode == LayoutOneTrackOneBoxXyzFlat)
            CurrentDisplaySpace = DisplaySpaceProjected;
        else if (RenderingMode == LayoutOneTrackOneBoxXyz3D)
    ...
    void TTracksView::CmSetRenderingMode()
    {
        ...
        int m = numtracks > MaxWindowSlots ? NumLayoutsTooMuchTracks : XYZDoc ? NumLayouts
                                                                              : NumLayoutsNoXyz;
    ...
    {
        // choose 3D set here
        if (current3dset == DisplaySpaceProjected || current3dset == DisplaySpaceHeight)
            RenderingMode = LayoutOneTrackOneBoxXyzFlat - (ShiftKey ? -1 : 1);
        else if (current3dset == DisplaySpace3D)
            RenderingMode = LayoutOneTrackOneBoxXyz3D - (ShiftKey ? -1 : 1);
    ...
    */
    ...
    void TTracksView::CmSelection(owlwparam w)
    {
        ...
        case CM_BADEXCLUDE:
            ...
            { // a bit tricky: no displayed tracks -> restore all non-bads
                Highlighted = ~BadTracks;
                EEGDoc->ClearPseudo(BadTracks);
                CmSelection(CM_KEEPSEL);
            }
        ...
        case CM_BADAUXSELECTNON:
            ...
            { // a bit tricky: no displayed tracks -> use all bads/auxs
                Highlighted = ~BadTracks;
                Highlighted &= ~AuxTracks;
            }
    ...
    void TTracksView::CmFreqAnalysis()
    {
        ...
        if (XYZDoc)
        { // provide the coordinates for very first init
            StringCopy(transfer.XyzDocFile, XYZDoc->GetDocPath());
            transfer.UseXyzDoc = BoolToCheck(true);
            transfer.XyzLinked = true; // to prevent switching to another XYZ
        }
    ...
    void TTracksView::CmInterpolateTracks()
    {
        ...
        if ((bool)Highlighted)
        { // take this whole selection as is
        ...
        else if ((bool)BadTracks)
        { // perform some checks on this selection, though

            transfer.BadElectrodesSelection = BadTracks;
            // if current selection mixes regular and pseudo, clear pseudos
            if (EEGDoc->GetNumSelectedRegular(transfer.BadElectrodesSelection) && EEGDoc->GetNumSelectedPseudo(transfer.BadElectrodesSelection))
...
#define DefaultNumberOfFiles 10
    ...
    void TTracksView::RisToCloudVectorsUI()
    {
        ...
        if (sel.NumSet() == risdoc->GetNumElectrodes() && !GetAnswerFromUser("Are you sure you want to output all solution points?", RisToPointsTitle, this))
            return;
    ...
    void TTracksView::CmTimeRef(owlwparam w)
    {
        ...
        if (w == CM_EEGTIMERELATIVETO)
        {
            ...
            StringCopy(message,
                       "New time origin, with any of these syntaxes (strings in bracket are optional):" NewLine Tab "XXXX [ TF ]" NewLine Tab "[ HH: ]MM:SS" NewLine Tab "[ HH hour ] [ MM min ] [ SS sec ] [ mm msec ]");
    ...
    void TTracksView::CmGotoTime()
    {
        ...
        if (IsCommandSender())
        {
            ...
            StringAppend(message,
                         NewLine Tab "XXXX [TF]" NewLine Tab "[HH:]MM:SS" NewLine Tab "[HH hour] [MM min] [SS sec] [mm msec]" NewLine Tab "marker name");
        ...
        { // commands cloning AND a receiving cloned view -> forward the parameters set by caller view
    ...
    void TTracksView::CmSetTimeWindow()
    {
        ...
        if (IsCommandSender())
        {
            ...
            StringCopy(message,
                       "New time range per page, with any of these syntaxes (strings in bracket are optional):" NewLine Tab "XXXX [ TF ]" NewLine Tab "[ HH: ]MM:SS" NewLine Tab "[ HH hour ] [ MM min ] [ SS sec ] [ mm msec ]");
        ...
        long reqlen = Clip(tr, mintf, (long)CDPt.GetLimitLength()) | oddtf;
        ...
        if (CDPt.GetLength() != (tr | oddtf) && IsCommandSender())
        {
            char buff[256];
    ...
    void TTracksView::CmNextSession()
    {
        ...
        if (IsInsideLimits(newsession, 1, EEGDoc->GetNumSessions()) && newsession != currentsession)
        {
    ...
    void TTracksView::CmOpenSeg(owlwparam w)
    {
        ...
        if ((w == CM_OPENFITTINGTEMPLATES1 || w == CM_OPENFITTINGTEMPLATES2) && StringIsNotEmpty(filetempl))
        {
            ...
            if (FitTransfer.LastDialogId == IDD_FITTING1)
                TMicroStatesFitFilesDialog(this, IDD_FITTING1).Execute();
            else
                TMicroStatesFitParamsDialog(this, IDD_FITTING2).Execute();
        }

        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        else if (w == CM_OPENSTATSFITTING && (bool)filescsv)
        {
    ...
    void TTracksView::CmNextSegment(owlwparam w)
    {
        ...
        if (SelFilling->NumSet() > 0 && SelFilling->NumSet() <= SelFilling->MaxValue() - 1)
        {
            ...
            if (w == CM_NEXTSEGMENT && SelFilling->IsSelected(SelFilling->MaxValue()) || w == CM_PREVSEGMENT && SelFilling->IsSelected(1))
            {
                SelFilling->Set();
                SelFilling->Reset(0);
            }
    ...
    void TTracksView::CmEasyPublish()
    {
        ...
        {
        case 0:
            StringCopy(buff, "Please install Service Pack 23 and reboot.");
            break;
        case 1:
            StringCopy(buff, "This will be ready just in time for Windows 57.");
            break;
        case 2:
            StringCopy(buff, "So... can you remind me what you are paid for, exactly?");
            break;
        case 3:
            StringCopy(buff, "Ha ha, that was a good one.");
            break;
        case 4:
            StringCopy(buff, "Unimplemented feature, but working on it...");
            break;
        case 5:
            StringCopy(buff, "The 'easy publish' wizard is drunk, come back later please.");
            break;
        case 6:
            StringCopy(buff, "Follow these steps:\n\n 1) Psdkfj wd lk alknn qsd\n 2) Rrlkn kj ioiq nk\n 3) Otrl lkn s jnkja");
            break;
        case 7:
            StringCopy(buff, "I thought this menu was well hidden, so I will bury it even deeper!");
            break;
        case 8:
            StringCopy(buff, "Hey, YOU got the brain, not ME, I'm a stupid computer after all!");
            break;
        case 9:
            StringCopy(buff, "Nice try...");
            break;
        }
    ...
    void TTracksView::CmHistogram(owlwparam w)
    {
        ...
        bool cdf = w == CM_HISTOTIMECDF || w == CM_HISTOTRACKSCDF || w == CM_HISTOALLCDF;
        bool pdf = !cdf;

        bool logoutput = pdf && (w == CM_HISTOTIMELOG || w == CM_HISTOTRACKSLOG || w == CM_HISTOALLLOG);

        // spikey or smoothey?
        bool curvesmooth = w == CM_HISTOTIMESMOOTH || w == CM_HISTOTRACKSSMOOTH || w == CM_HISTOALLSMOOTH || cdf || logoutput;

        // makes max = 1, easier on the eyes but not mathematically correct
        bool normalizemax = false;

        // cooking the whole option flags
        HistogramOptions histooptions = (HistogramOptions)((cdf ? HistogramCDF : HistogramPDF) | (curvesmooth ? HistogramSmooth : HistogramRaw) | (normalizemax ? HistogramNormMax : HistogramNormArea) | (logoutput ? HistogramLog : HistogramLinear));
        ...
        if ((pertf || alldata) && !(bool)Highlighted)
        {
        ...
        if (pertrack)
            stats.Resize(numel, downtf.NumDownData);
        else if (pertf)
            stats.Resize(downtf.NumDownData, numel);
        else if (alldata)
            stats.Resize(1, numel * downtf.NumDownData);

        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        TSuperGauge Gauge;

        Gauge.Set(pertrack ? "Per Tracks Histogram" : pertf ? "Per Time Histogram"
                                                            : "All Data Histogram");
        ...
        if (pdf)
            StringCopy(infix, ".", InfixHistogramShort);
        else if (cdf)
            StringCopy(infix, ".", InfixCDF);
        else
            ClearString(infix);

        if (logoutput)
            StringAppend(infix, "Log");
        ...
        if (allvarstogether)
        {
            ...
            if (pertrack || alldata)
            {
                ...
                if (elselnames.Length() > 32)
                    StringAppend(expfile.Filename, " ", "Tracks"); // fallback
                else
                    StringAppend(expfile.Filename, " ", elselnames);
            }
            ...
            if (pertf || alldata)
            {
                ...
                if (downtf.NumDownData == 1)
                    StringAppend(expfile.Filename, " TF ", IntegerToString(buff, readfromtf, numtfdigits));
                else
                    StringAppend(expfile.Filename, " TF ", IntegerToString(buff, readfromtf, numtfdigits), "-", IntegerToString(buff2, readtotf, numtfdigits));
            }
        ...
        { // ! allvarstogether - alldata NOT a valid case here
            ...
            for (int si = 0; si < stats.GetNumStats(); si++)
            {
                ...
                if (pertrack)
                    StringAppend(expfile.Filename, " ", SkewnessToString(skewness(si)), GetElectrodeName(elsel.GetValue(si)));
                else
                    StringAppend(expfile.Filename, " TF ", IntegerToString(buff, readfromtf + si, numtfdigits));
    ...
    void TTracksView::CmAnalyzeTracks(owlwparam w)
    {
        ...
        if (!(bool)Highlighted)
        {
            ...
            if (w == CM_ANALYZESELFLAT)
                StringCopy(buff, "No flat tracks were found...");
            else if (w == CM_ANALYZESELHIGH)
                StringCopy(buff, "No broken / jerky tracks were found...");
            else if (w == CM_ANALYZESELGFP)
                StringCopy(buff, "No outlier / noisy tracks were found...");
            else if (w == CM_ANALYZESELNOISY)
                StringCopy(buff, "No noisy tracks were found...");
            else if (w == CM_ANALYZESELARTEFACTS)
                StringCopy(buff, "No artefacted tracks were found...");
        ...
        do
        {
            ...
            if (compound > 0.30)
            {
                ...
                for (int e = 0; e < numel; e++)
                    ...
                    if (!(BadTracks[e] || AuxTracks[e] || Highlighted[e]) && measure[e] > maxavg)
                    {
    ...
    */
    ...
    void TTracksView::CmSetIntensityMode()
    {
        ...
        {
            if (IsTracksMode())
                SetIntensitySmoothMode();
            else if (IsBarsMode())
                SetTracksMode();
            else if (IsIntensitySquareMode())
                SetBarsMode();
    ...
    void TTracksView::CmSetContrast(owlwparam w)
    {
        ...
        else if (w == IDB_ISINCCONTRAST)
            ScalingContrast = AtLeast(0.01, ScalingContrast) * 1.50;
        else if (w == IDB_ISDECCONTRAST)
            ScalingContrast /= 1.50;
    ...
    void TTracksView::CmNextRois()
    {
        ...
        if (AverageRois)
        { // cancel average rois when leaving rois
            UpdateBuffers(-1, -1, CDPt.GetMin(), CDPt.GetMax());
    ...
    {
        if (!(GODoc && GODoc->GetNumRoiDoc() && IsRoiMode() && !(GODoc->GetNumMriDoc() && GODoc->GetNumSpDoc() && GODoc->GetNumIsDoc())))
            return;
    ...
    {
        tce.Enable(GODoc && GODoc->GetNumRoiDoc() && IsRoiMode() && !(GODoc->GetNumMriDoc() && GODoc->GetNumSpDoc() && GODoc->GetNumIsDoc()));
    }
