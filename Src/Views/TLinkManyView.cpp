...
namespace crtl
{
    ...
    {
        return GODoc->GetNumEegDoc() + GODoc->GetNumRoiDoc() + GODoc->GetNumXyzDoc() + GODoc->GetNumIsDoc() + GODoc->GetNumRisDoc() + GODoc->GetNumSpDoc() + GODoc->GetNumMriDoc();
    }
    ...
    void TLinkManyView::EvKeyDown(uint key, uint repeatCount, uint flags)
    {
        ...
        case 'A':
            ...
            if (ControlKey && ShiftKey)
                CmSyncUtility(CM_DESYNCBETWEENEEG);
            else if (ControlKey && !ShiftKey)
                CmSyncUtility(CM_SYNCBETWEENEEG);
            else if (!ControlKey && ShiftKey)
                CmSyncUtility(CM_DESYNCALL);
            else if (!ControlKey && !ShiftKey)
                CmSyncUtility(CM_SYNCALL);
    ...
    void TLinkManyView::EvLButtonDblClk(uint, const TPoint &p)
    {
        ...
        for (view = doc->GetViewList(); view != 0; view = doc->NextView(view))
        {
            ...
            if (view->GODoc && view->GODoc != GODoc)
                continue;
    ...
    void TLinkManyView::CmGTV(owlwparam w)
    {
        if (w == IDB_GTVKEEPSIZE)
            GODoc->GroupTileViews(CombineFlags(GroupTilingViews_Move, GroupTilingViews_Insert));
        else if (w == IDB_GTVSTANDSIZE)
            GODoc->GroupTileViews(CombineFlags(GroupTilingViews_StandSize, GroupTilingViews_Insert));
        else if (w == IDB_GTVFIT)
            GODoc->GroupTileViews(CombineFlags(GroupTilingViews_BestFitSize, GroupTilingViews_Insert));
    }
    ...
    void TLinkManyView::CmAddToGroup()
    {
        ...
        else if (CaptureMode == CaptureAddToGroup)
        { // cancel the sync manoeuvre
