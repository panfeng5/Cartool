    ...
    void TSolutionPointsView::GLPaint(int how, int renderingmode, TGLClipPlane *otherclipplane)
    {
        ...
        if (renderingmode != PointsShowNone && (how & GLPaintOpaque))
        {
            ...
            for (int sp = 0; sp < numsp; sp++, tosp++)
            {
                ...
                if (ncutplanes && !((bool)clipplane[0] && fabs(tosp->X - cx) <= mind || (bool)clipplane[1] && fabs(tosp->Y - cy) <= mind || (bool)clipplane[2] && fabs(tosp->Z - cz) <= mind))
                ...
                { // simpler coloring for linked windows
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if ((how & GLPaintOwner))
                                MaterialSp.GetDiffuse().GLize();
                            else
                                MaterialSp2.GetDiffuse().GLize();
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // draw names
        if (ShowLabels && (how & GLPaintTransparent) && (numsp < 1000 || ncutplanes))
        {
            ...
            for (int sp = 0; sp < numsp; sp++, tosp++)
            {
                ...
                if ((bool)clipplane[0] && fabs(tosp->X - cx) > mind)
                    continue;
                if ((bool)clipplane[1] && fabs(tosp->Y - cy) > mind)
                    continue;
                if ((bool)clipplane[2] && fabs(tosp->Z - cz) > mind)
                    continue;
                ...
                */
                if ((bool)clipplane[0])
                    alternate = (((int)(y / meddistance)) & 1) ^ (((int)(z / meddistance)) & 1);
                else if ((bool)clipplane[1])
                    alternate = (((int)(x / meddistance)) & 1) ^ (((int)(y / meddistance)) & 1);
                else if ((bool)clipplane[2])
                    alternate = (((int)(x / meddistance)) & 1) ^ (((int)(y / meddistance)) & 1);

                if (ShowLabels == ShowLabelText)
                    StringCopy(label, SPDoc->GetSPName(sp));
                else if (ShowLabels == ShowLabelIndex)
                    StringCopy(label, "#", IntegerToString(buff, sp + 1));

                //      SFont->Print ( x, y, z, label, TA_CENTER | TA_CENTERY | TA_BOX );

                SFont->Print(x, y, z, label,
                             TA_CENTER | (alternate == -1 ? TA_CENTERY : alternate ? TA_TOP
                                                                                   : TA_BOTTOM) |
                                 TA_BOX,
                             (GLfloat)0, (GLfloat)0, (GLfloat)0.025);
        ...
        if ((how & GLPaintOwner) && (how & GLPaintOpaque) && ncutplanes)
        {
