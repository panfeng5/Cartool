...
#define SDTransparency (GLfloat)0.30
    ...
    public:
        ...
        {
            DisplayMarkerType = KeepFlags(markertype, AllMarkerTypes);
            CmSetMarkerDisplay(0);
        }
        DisplayTracksEnum GetDisplayMode() const { return DisplayMode; }
        void SetDisplayMode(DisplayTracksEnum displaymode)
        {
            DisplayMode = displaymode;
            Invalidate(false);
        }
        void SetSelTracks(TSelection &seltracks)
        {
            SelTracks.Copy(EEGDoc->GetFirstRegularIndex(), EEGDoc->GetLastPseudoIndex(), &seltracks);
