...
namespace crtl
{
    ...
    TDib *TButtonGadgetDpi::GetGlyphDib()
    {
        ...
        if (ResId)
        {
            ...
            glyph->MapUIColors(TDib::MapFace | TDib::MapText | TDib::MapShadow | TDib::MapHighlight);
    ...
    void TBaseView::CaptureMouse(CaptureMouseEnum how)
    {
        ...
        if (how == Capture && !LockedCapture || how == ForceCapture)
        {
        ...
        // release if captured
        else if (how == Release && LockedCapture || how == ForceRelease)
        {
    ...
    {
        if (group)
            GODoc = group; // given at creation
        else
            GODoc = BaseDoc->LastGroupDoc; // or get it from the doc, whatever 0 or real doc
    ...
    TBaseView::~TBaseView()
    {
        ...
        if (!CartoolApplication->Closing)
        { // maybe the windows pointed to are already destroyed?
    ...
    bool TBaseView::CancelFriendship() // (so sad)
    {
        ...
        { // modify friend views and replace their friendship ID
            uint friendid = 0;
    ...
    {
        if (GetCommandsCloning() && CaptureMode != CaptureGLLink && CaptureMode != CaptureTFSync)
        {
            ...
            else if (id != IDB_2OBJECT && id != IDB_SYNCVIEWS && id != CM_EDITCOPYBM && id != CM_EDITCOPY && id != CM_INTERPOLATE && id != CM_AVERAGING && id != CM_SEGMENTEEG && id != CM_FITTING && id != CM_EXPORTTRACKS && id != CM_FREQANALYSIS && id != CM_GEOMETRYTRANSFORM && id != CM_DOWNSAMPLEMRI && id != CM_NORMALIZEMRI && id != CM_CREATEROIS && id != CM_COREGISTRATION && id != CM_DOWNSAMPLINGELECTRODES && id != CM_AVERAGEXYZ && id != CM_SKULLSTRIPPING && id != CM_CUTBRAINSTEM && id != CM_HEADCLEANUP && id != CM_STATISTICSTRACKS && id != CM_STATISTICSRIS && id != CM_STATISTICSFREQ && id != CM_HELPCONTENTS && id != CM_HELPABOUT && id != CM_EEGTIMERELATIVETO && id != CM_EEGTIMERELATIVE && id != CM_EEGTIMEABSOLUTE
                     //         && id != CM_EEGUSERSCALE
                     && id != CM_GOTOTIME && id < MaxUserCommandId)
            { // above are general commands
                ...
                for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc; doc = CartoolDocManager->DocListNext(doc))
                    for (TBaseView *view = doc->GetViewList(GODoc); view; view = doc->NextView(view, GODoc))
                        ...
                        if (view != this && typeid(*view) == typeid(*this))
        ...
        if (GetCommandsCloning() && CaptureMode != CaptureGLLink && CaptureMode != CaptureTFSync)
        {
            ...
            else
            {
                ...
                if ((msg == WM_KEYDOWN && p1 != VK_TAB                                                                                        // don't allow anything dealing with time
                     ...
                     && (p1 != VK_LEFT || VkShift()) && p1 != VK_PRIOR && p1 != VK_NEXT && p1 != VK_HOME && p1 != VK_END && p1 != VK_MULTIPLY // window resizing
                     && p1 != VK_DIVIDE && p1 != VK_RETURN && p1 != 'L')                                                                      // don't forward linking
                    ...
                    // mouse stuff
                    || ((msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP) && (leftok || CaptureMode == CaptureGLMagnify) || msg == WM_RBUTTONDOWN || msg == WM_RBUTTONUP || msg == WM_MOUSEWHEEL || msg == WM_MOUSEMOVE && (LButtonDown && leftok || RButtonDown || CaptureMode == CaptureGLMagnify)))
                {
    ...
    void TBaseView::FitItemsInWindow(int numitems, TSize itemsize, int &byx, int &byy, TRect *winrect)
    {
        ...
        double ratio = NonNull(fabs((wx / NonNull(wy)) / NonNull(ox / NonNull(oy))));
        ...
        if (GODoc)
        {
            ...
            if (typeid(*this) == typeid(TTracksView))
            {
                ...
                for (view = BaseDoc->GetViewList(); view != 0; view = BaseDoc->NextView(view))
                    ...
                    if (view->GODoc == GODoc && typeid(*view) == typeid(TTracksView))
    ...
    void TBaseView::PrePaint(TDC &dc, TRect &, double objectradiusx, double objectradiusy, double objectradiusz)
    {
        ...
        // OpenGL inits
        if (GLrc.IsOpen())
            GLrc.GLize(dc); // use context
        else
            GLrc.Set(dc, GLpfd); // (re)create window context if needed
        ...
        if (CaptureMode == CaptureGLMagnify)
        {
            ...
            if ((bool)AnimFx)
            {
                ...
                if (AnimFx.GetTimerId() == TimerMagnifierIn)
                    m = AnimFx.GetPercentageCompletion();
                else if (AnimFx.GetTimerId() == TimerMagnifierOut)
                    m = AnimFx.GetPercentageRemaining();
        ...
        { // 3D mode?
            ...
            double viewratio = (double)PaintRect.Height() / NonNull(PaintRect.Width()) / NonNull(objectradiusy) * objectradiusx;
    ...
    void TBaseView::PostPaint(TDC &dc)
    {
        ...
        if ((bool)UsedBy && !CartoolApplication->Closing)
            ...
            foreachin(UsedBy, iterator)
            {
                ...
                if (!iterator()->GetCommandsCloning())
                    iterator()->Invalidate(); // help cumulate Paint coming from cloned windows
                else if (!IsFriendView(iterator()))
                    iterator()->ShowNow(); // for faster interaction
            }
    }
    ...
    void TBaseView::HintsPaint()
    {
        ...
        if (LButtonDown || MButtonDown || RButtonDown)
        {
            ...
            if (LButtonDown)
            {
                ...
                {
                    if (ModelRotAngle[2])
                    { // && !ModelRotAngle[0] && !ModelRotAngle[1] ) {
            ...
            else if (MButtonDown)
            {
                ...
                if (Picking.IsNotNull())
                {
                    ...
                    { // output (optionally modified) coordinates
    ...
    void TBaseView::DrawAxis()
    {
        ...
        if (OriginRadius > 0)
        {
            ...
            TGLBillboardSphere *tobbsphere = &BbLowSphere;
            ; // Outputing() ? &BbHighSphere : &BbLowSphere;
                                 ...
                                 TGLCoordinates<GLfloat> &border2, int &textattr2)
    {
        ...
        if (dx)
        {
            ...
            {
                border1.X = 0;
                border1.Y = (c - dy * border1.X) / dx;
                border1.Z = 0;
                textattr1 = TA_LEFT | TA_BOTTOM /*TA_CENTERY*/;
                border2.X = PaintRect.Width() - 1;
                border2.Y = (c - dy * border2.X) / dx;
                border2.Z = 0;
                textattr2 = TA_RIGHT | TA_BOTTOM /*TA_CENTERY*/;
            }
            else
            {
                border2.X = 0;
                border2.Y = (c - dy * border2.X) / dx;
                border1.Z = 0;
                textattr2 = TA_LEFT | TA_BOTTOM /*TA_CENTERY*/;
                border1.X = PaintRect.Width() - 1;
                border1.Y = (c - dy * border1.X) / dx;
                border2.Z = 0;
                textattr1 = TA_RIGHT | TA_BOTTOM /*TA_CENTERY*/;
            }
        }
            ...
            (!dx || dx && (border1.Y < 0 || border2.Y < 0 || border1.Y > PaintRect.Height() || border2.Y > PaintRect.Height())))
        {
            ...
            {
                border1.Y = 0;
                border1.X = (c - dx * border1.Y) / dy;
                border1.Z = 0;
                textattr1 = TA_CENTER | TA_BOTTOM;
                border2.Y = PaintRect.Height() - 1;
                border2.X = (c - dx * border2.Y) / dy;
                border2.Z = 0;
                textattr2 = TA_CENTER | TA_TOP;
            }
            else
            {
                border2.Y = 0;
                border2.X = (c - dx * border2.Y) / dy;
                border2.Z = 0;
                textattr2 = TA_CENTER | TA_BOTTOM;
                border1.Y = PaintRect.Height() - 1;
                border1.X = (c - dx * border1.Y) / dy;
                border1.Z = 0;
                textattr1 = TA_CENTER | TA_TOP;
            }
        }
    ...
    void TBaseView::DrawOrientation(const TOrientationType *boxsides)
    {
        ...
        if (fabs(dir.Z) < SinMaxAngleProj)
        {
            ...
            if (!border2.X)
                border2.X += 1;
            if (!border1.Y)
                border1.Y += SFont->GetHeight();
            if (!border2.Y)
                border2.Y += SFont->GetHeight();
    ...
    void TBaseView::DrawCross(const TPointFloat &p, double r, bool colormin)
    {
        ...
        DepthRange.GLize();
        if (colormin)
            glColor3f(GLBASE_MINCOLOR);
        else
            glColor3f(GLBASE_MAXCOLOR);
    ...
    void TBaseView::GLEditCopyBitmap()
    {
        ...
        for (int y = 0; y < size.Y(); y++)
            ...
            if (bytesperpixel == 3)
                glReadPixels(0, size.Y() - y - 1, size.X(), 1, GL_BGR_EXT, GL_UNSIGNED_BYTE, pPixelData + y * bytesperline);
            else
                glReadPixels(0, size.Y() - y - 1, size.X(), 1, GL_BGRA_EXT, GL_UNSIGNED_BYTE, pPixelData + y * bytesperline);
    ...
    void TBaseView::EvKeyDown(uint key, uint /*repeatCount*/, uint /*flags*/)
    {
        ...
        case VK_MULTIPLY:
            ...
            if (LeftKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_INCL);
            else if (RightKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_INCR);
            else if (UpKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_INCT);
            else if (DownKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_INCB);
            else
                CartoolMdiClient->CmWinAction(CM_WINSIZE_INC);
            break;

        case VK_DIVIDE:

            if (LeftKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_DECL);
            else if (RightKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_DECR);
            else if (UpKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_DECT);
            else if (DownKey)
                CartoolMdiClient->CmWinAction(CM_WINSIZE_DECB);
            else
                CartoolMdiClient->CmWinAction(CM_WINSIZE_DEC);
            break;
        }
    }
    ...
    void TBaseView::GLMouseMove(uint, const TPoint &p)
    {
        if (CaptureMode == CaptureGLMagnify)
        {
            ...
            if (Magnifier[0] < -0.1)
                Magnifier[0] = -0.1;
            else if (Magnifier[0] > 1.1)
                Magnifier[0] = 1.1;
            if (Magnifier[1] < -0.1)
                Magnifier[1] = -0.1;
            else if (Magnifier[1] > 1.1)
                Magnifier[1] = 1.1;
        ...
        else if (LButtonDown)
        {
            ...
            else
            {
                ...
                if (nc < 0.33 * winsize || fabs(spocd) < fabs(spcd))
                    //          || dx * dx + dy * dy < 2 && dx && dy )
    ...
    void TBaseView::Cm2Object()
    {
        ...
        else if (CaptureMode == CaptureGLLink)
        { // cancel the link manoeuvre
    ...
    {
        DepthRange.UserShift = (double)(w - CM_SHIFTDEPTHRANGE0) / (CM_SHIFTDEPTHRANGE4 - CM_SHIFTDEPTHRANGE0) * MaxShiftDepthRange;
