...
namespace crtl
{
        ...
        : TSecondaryView(doc, parent, group)
    {
        ...
        if (OnlyRis)
        { // not allowed to GetIs, cause EEG is RIS here !
...
#define ScalingContrastInit (ScalingMinContrast + 0.070 * ScalingMaxContrast)
    ...
    {
        return GODoc && GODoc->GetNumSpDoc() && ((GODoc->GetNumEegDoc() && GODoc->GetNumIsDoc()) || GODoc->GetNumRisDoc()) && GODoc->GetNumMriDoc() && EEGDoc->GetNumTimeFrames();
    }
    ...
    bool TInverseView::ModifyPickingInfo(TPointFloat &Picking, char *buff)
    {
        ...
        if (/*SliceMode ||*/
            !(Show2DIs || ShowVectorsIs || ShowSp || Show3DIs == InverseRendering3DSolPoints || Show3DIs == InverseRendering3DIsoOnion))
    ...
    void TInverseView::GLPaint(int how, int renderingmode, TGLClipPlane *otherclipplane)
    {
        ...
        for (int itf = 0; tf < MRCNumTF; tf += MRCStepTF, itf++)
        {
            ...
            if (Rois)
            { // get a mean position from the right Rois
            ...
            for (int i = 0; i < (int)spvol; i++, toi4++)
                ...
                if (toi4->IsAllocated())
                    // real value
                    spvol(i) = (toi4->w1 * InvBuffS[toi4->i1] + toi4->w2 * InvBuffS[toi4->i2] + toi4->w3 * InvBuffS[toi4->i3] + toi4->w4 * InvBuffS[toi4->i4]) / TWeightedPoints4SumWeights;
            ...
            for (int nv = 0; nv < nviews; nv++)
            {
                ...
                //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // print the label
                if (ShowSequenceLabels && (ManageRangeCursor == MRCSequence || ManageRangeCursor == MRCAnimation) && NotSmallWindow() && timepos * SlicesPerY * msy > 35 && nv % SlicesPerX == 0 && nv / SlicesPerX == SlicesPerY - 1 // left bottom corner of slices
                ...
                if (SliceMode && (how & GLPaintOpaque))
                {
                        ...
                        if (IsSliceModeX())
                            ...
                            for (TIteratorSelectedForward si(*slch->GetSelection()); (bool)si; ++si)
                            {
                                ...
                                else
                                {
                                    ...
                                    Prim.Draw3DWireFrame(si() + shift.X, si() + shift.X, mrimin[1], mrimax[1], mrimin[2], mrimax[2], 1);
                                }
                            }
                            ...
                            for (TIteratorSelectedForward si(*slch->GetSelection()); (bool)si; ++si)
                            {
                                ...
                                {
                                    glColor4f(SliceColor);
                                    Prim.Draw3DWireFrame(mrimin[0], mrimax[0], si() + shift.Y, si() + shift.Y, mrimin[2], mrimax[2], 1);
                                }
                            }
                        else
                            ...
                            for (TIteratorSelectedForward si(*slch->GetSelection()); (bool)si; ++si)
                            {
                                ...
                                {
                                    glColor4f(SliceColor);
                                    Prim.Draw3DWireFrame(mrimin[0], mrimax[0], mrimin[1], mrimax[1], si() + shift.Z, si() + shift.Z, 1);
                                }
                            }
                    ...
                    { // regular slices
                        ...
                        if (IsSliceModeX())
                        {
                            ...
                            if (Show3DMri && volumemri)
                                volumemri->DrawPlaneX(s, qualitymri);
                        ...
                        else if (IsSliceModeY())
                        {
                            ...
                            if (Show3DMri && volumemri)
                                volumemri->DrawPlaneY(s, qualitymri);
                        ...
                        { // if ( IsSliceModeZ () )
                            ...
                            if (Show3DMri && volumemri)
                                volumemri->DrawPlaneZ(s, qualitymri);
...
#define IsRatio3DPotato 0.22
                ...
                if (ShowVectorsIs && (how & GLPaintOpaque))
                {
                    ...
                    for (TIteratorSelectedForward spi(SelSP); (bool)spi; ++spi)
                    {
                        ...
                        if (ShowVectorsIs == InverseRenderingVectors2D && !((bool)clipplane[0] && fabs(tosp->X - clip.X) <= md || (bool)clipplane[1] && fabs(tosp->Y - clip.Y) <= md || (bool)clipplane[2] && fabs(tosp->Z - clip.Z) <= md))
                ...
                if (Show3DIs == InverseRendering3DIsosurface && (how & GLPaintOpaque))
                {
                    ...
                    if (RedoSurfaceIS || ManageRangeCursor == MRCSequence)
                    { // || true ) { //|| (bool) AnimTF ) {
                ...
                if (Show3DMri && MRIView)
                {
                    ...
                    MRIView->GLPaint(GLPaintLinked | (qualitymri != QuadBestQuality || LeftKey || RightKey ? GLPaintFast : 0) // not used anymore for volumes, textures are doing the job
                                         | GLPaintOpaqueOrTransparent | GLPaintForceClipping,
                                     Show3DMri, clipplane);
                ...
                if (!SliceMode && ncutplanes && ((Show2DIs == InverseRendering2DOpaque && (how & GLPaintOpaque)) || (Show2DIs == InverseRendering2DOvercast && (how & GLPaintTransparent)) || (Show2DIs == InverseRendering2DTransparent && (how & GLPaintTransparent))))
                {
                    ...
                    {
                        if (cl1)
                            clipplane[1].GLize(InvertedClipPlane);
                        if (cl2)
                            clipplane[2].GLize(InvertedClipPlane);
                        // draw the IS on top
                        if (Show2DIs)
                            DrawPlaneIsX(clip.X);
                    ...
                    {
                        if (cl0)
                            clipplane[0].GLize(InvertedClipPlane);
                        if (cl2)
                            clipplane[2].GLize(InvertedClipPlane);
                        // draw the IS on top
                        if (Show2DIs)
                ...
                if (ShowSp != PointsShowNone && SPView && (how & GLPaintOpaque) && SPView->IsRenderingMode(RenderingOpaque))
                {

                    SPView->GLPaint(GLPaintLinked | (AnimFx || AnimTF ? GLPaintFast : 0) | GLPaintOpaqueOrTransparent /*GLPaintOpaque*/
    ...
    void TInverseView::EvKeyDown(uint key, uint repeatCount, uint flags)
    {
        ...
        case 'U':
            if (ControlKey && ShiftKey)
                ...
                CmReg(ControlKey ? IDB_RESETREG : ShiftKey ? IDB_PREVREG
                                                           : IDB_NEXTREG);
            break;
        ...
        if (RButtonDown)
        {
            ...
            { // change color scaling
                ...
                if (MouseAxis == MouseAxisHorizontal)
                {
                    ...
                    if (dx > 0)
                        ScalingLevel /= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 1.0 : 0.3);
                    else
                        ScalingLevel *= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 1.0 : 0.3);
    ...
    {
        tce.Enable(GODoc && GODoc->GetNumIsDoc() != 0 && ISDoc->GetNumRegularizations() > 1);
    }

    void TInverseView::CmNextIsEnable(TCommandEnabler &tce)
    {
        tce.Enable(GODoc && (GODoc->GetNumIsDoc() + GODoc->GetNumRisDoc()) > 1 && (Show3DIs || ShowVectorsIs || ((bool)ClipPlane[0] || (bool)ClipPlane[1] || (bool)ClipPlane[2]) && Show2DIs));
    }
    ...
    {
        tce.Enable((Show3DIs || ShowVectorsIs || ((bool)ClipPlane[0] || (bool)ClipPlane[1] || (bool)ClipPlane[2]) && Show2DIs) && ScalingAuto == ScalingAutoOff);
    }
    ...
    void TInverseView::CmReg(owlwparam w)
    {
        ...
        if (w == IDB_PREVREG)
            CurrReg = max(0, CurrReg - 1); //  ( CurrReg + ISDoc->GetMaxRegularization () - 1 ) % ISDoc->GetMaxRegularization ();
        else if (w == IDB_NEXTREG)
            CurrReg = min(ISDoc->GetMaxRegularization() - 1, CurrReg + 1); // ( CurrReg + 1 ) % ISDoc->GetMaxRegularization ();
        else if (w == IDB_RESETREG)
            CurrReg = 0;
        else
            return;
    ...
    void TInverseView::CmNextIs()
    {
        ...
        { // currently in ris?
            ...
            if (!CurrRis && GODoc->GetNumIsDoc() && !OnlyRis)
            { // switch to is?
                CurrRis = -1;
                CurrIs = 0;
                ISDoc = GODoc->GetIsDoc(CurrIs);
        ...
        if (CurrIs >= 0)
        {
            ...
            { // first call, inherit from current display
    ...
    void TInverseView::CmNextRois()
    {
        ...
        if (ControlBarGadgets)
        { // test window has been initialized
...
#undef ClipTra
    ...
    long TInverseView::SearchAndSetIntensity(bool precise)
    {
        ...
        { // just take the max gfp position as a reasonable estimate
