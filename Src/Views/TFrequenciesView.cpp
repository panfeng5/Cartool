    ...
    double margin = 50.0 * drawmargin;
        ...
        else
        {
            ...
            if (IsModeFE())
            {
                ...
                if (renderingmode == LayoutFourPages && numfreqs < 4)
                    renderingmode = LayoutTwoPages;
                if (renderingmode == LayoutTwoPages && numfreqs < 2)
                    renderingmode = LayoutOnePage;
    ...
    */
    ...
    void TFrequenciesView::SetupHorizontalGrid(double &hgridorg, double &hgridstep)
    {
        ...
        else if (IsFreqHorizontal() && FreqDoc->GetNumFrequencies() >= 3)
        {
            ...
            double stepfreq = StringToDouble(FreqDoc->GetFrequencyName(1)) - StringToDouble(FreqDoc->GetFrequencyName(0));
            double stepfreqend = StringToDouble(FreqDoc->GetFrequencyName(FreqDoc->GetNumFrequencies() - 1)) - StringToDouble(FreqDoc->GetFrequencyName(FreqDoc->GetNumFrequencies() - 2));
            ...
            else
            {
                ...
                // resetting?
                if (hgridorg == FreqDoc->GetNumFrequencies() - 1)
                {
                    hgridorg = FreqDoc->GetNumFrequencies();
                    hgridstep = 1;
                } // hgridorg   = 0;
            }
    ...
    void TFrequenciesView::GLPaint(int how, int renderingmode, TGLClipPlane *otherclipplane)
    {
        ...
        if (how & GLPaintOpaque)
        {
            ...
            bool draworigin = IsTimeHorizontal() && CurrentDisplaySpace == DisplaySpaceNone && ShowBaseline
                              /*&& allnames*/
                              && FreqDoc->GetSamplingFrequency() && !(FreqDoc->DateTime.RelativeTime && FreqDoc->DateTime.RelativeTimeOffset == 0);
            ...
            if (AverageMode)
            {
                ...
                if (IsModeSpectrum())
                    scalev *= PowerRoot(numtf, 4);
                else if (IsModeEF())
                    scalev *= sqrt((double)numfreq);
            ...
            auto Scaling = [this, &notdiss](int f)
            { return FreqNormalize && notdiss ? ScaleFreqs[f] : 1; };
            ...
            for (int pass = CurrentDisplaySpace == DisplaySpace3D ? 0 : 1; pass < 2; pass++)
                ...
                for (int slot = 0; slot < GetNumWindowSlots(); slot++)
                {
                    ...
                    scaleh = CurrentDisplaySpace == DisplaySpaceNone && !(IsBarsMode() || IsIntensitySquareMode()) ? (double)(toslot->ToRight.Norm() - margin - 1) / (nump > 1 ? nump - 1 : 1)
                                                                                                                   : (double)(toslot->ToRight.Norm() - margin) / nump;

                    // smart superimpose: asked / not enough space / not pseudo tracks
                    trackssuper = (!IsIntensityModes() || IsModeSpectrum()) && ((TracksSuper && (firstsel < numel || AverageMode /*IsModeFEAvg*/)) || !enoughh);
                    ...
                    drawvertscale = ShowVertScale && (!IsModeFE() || IsModeFE() && (AverageMode && !IsIntensityModes() && (trackssuper || numTracks == 1) || IsIntensityModes())) && numTracks && CurrentDisplaySpace == DisplaySpaceNone && (enoughh || trackssuper) && enoughtrsp && enoughwdeco1 && (how & GLPaintOwner);

                    drawhoriscale = ShowHorizScale && numTracks && CurrentDisplaySpace == DisplaySpaceNone && (toslot->ToRight.Norm() >= EEGGLVIEW_ENOUGHWHORIZGRID) && enoughwdeco1
                                    // && enoughh && enoughw
                                    && (how & GLPaintOwner);
                    ...
                    if (!LineWidth)
                    {
                        ...
                        else
                        {
                            ...
                            linewidth *= NoMore(2.0,
                                                sqrt(WindowSlots[0].ToUp.Norm() / (IsModeFEAvg() ? numfreq : WindowSlots[0].SelTracks.NumSet()) / (IsEnumerateVertical() ? numfreq : 1)) * 0.15);
                    ...
                    */
                    ...
                    //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    // triggers
                    if (ShowTags && IsTimeHorizontal() && (how & GLPaintOwner) && (CurrentDisplaySpace != DisplaySpace3D || trorg.Z > ModelRadius))
                    ...
                    if (drawhoriscale && IsTracksMode() && (how & GLPaintOwner))
                    {
...
#define stime (buff + 128)
                    ...
                    for (TIteratorSelectedForward sti(*sel); (bool)sti; ++sti, firsttrack = false)
                    {
                        ...
                        if (drawvertscale && (IsModeSpectrum() || !IsIntensityModes()) && (!trackssuper || firsttrack) && (!IsModeEFEnum() || numfreq == 1))
                        {
                        ...
                        if (NestedPaint || (bool)Using)
                        { // account for multiple Eegs
                        ...
                        { // this Eeg alone
                            if (st >= numel)
                                LineColor.Set(0, PseudotrackColor);
                            else if (AuxTracks[st])
                                LineColor.Set(0, AuxColor);
                            else
                                LineColor.Set(0, TrackColor);
                        }

                        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        // draw baseline axis - pre
                        if ((ShowBaseline || Show3DText) && (!trackssuper || firsttrack) && (how & GLPaintOwner) && (!IsIntensityModes() || IsModeSpectrum()) && CurrentDisplaySpace == DisplaySpaceNone)
                        {
...
#define MinBaselineH 1.5
                            ...
                            else if (IsModeFE() && (CurrentDisplaySpace == DisplaySpaceNone || numfreq == 1))
                            { // other FE
                            ...
                            else if (IsModeEFAvg())
                            {
                                ...
                                if (text)
                                {
                                    ...
                                    if (numfreq == 1)
                                        StringCopy(buff, FreqDoc->GetFrequencyName(firstfreq));
                                    else
                            ...
                            else if (IsModeEF() || IsModeSpectrum())
                            { // other EF
...
#define stimemax (buff + 96)
                                        ...
                                        if (*stimemin)
                                            StringCopy(buff, stimemin);
                                        else
                                            StringCopy(buff, stfmin);
                                    }
                                    else
                                    {
                                        ...
                                        if (*stimemin)
                                            StringCopy(buff, stimemin, " - ", stimemax);
                                        else
                                            StringCopy(buff, stfmin, " - ", stfmax);
                                    }
                        ...
                        // draw name
                        if (drawmargin && textenum && !trackssuper && !IsModeFE() && (how & GLPaintOwner))
                        ...
                        if (IsModeSpectrum())
                        { // frequencies by electrodes

                            if (AverageMode)
                            { // either 1 TF or an average of n TFs
                        ...
                        { // electrodes by frequencies / time
                            ...
                            else if (IsIntensityModes())
                            {
                                ...
                                if (numfreq <= 1 && IsIntensitySquareMode())
                                    dtrh /= 2; // or  trh *= 2  for bigger stripes
                                               // offset
                        ...
                        if (IsModeFE())
                        { // frequencies by electrodes / time
                            ...
                            { // 1 el or averaged n el
                                ...
                                for (int f = lastfreq; f >= firstfreq; f--)
                                {
                                    ...
                                    {
                                        if (Highlighted[firstsel])
                                            glColor4f(HighlightColor);
                                        else if (BadTracks[firstsel])
                                            glColor4f(BadColor);
                                        else if (AuxTracks[firstsel])
                                            glColor4f(AuxColor);
                                    }
                                    else
                                        ColorFreqs.GLize((double)f);
                            ...
                            else if (IsModeFEEnum())
                            {
                                ...
                                for (TIteratorSelectedForward fi(toslot->SelFreqs); (bool)fi; ++fi)
                                {
                                    ...
                                    for (TIteratorSelectedForward sti(*sel, firstsel); (bool)sti; ++sti)
                                    {
                                        ...
                                        if ((bool)Using)
                                            ;
                                        else if (Highlighted[sti()])
                                            glColor4f(HighlightColor);
                                        else if (BadTracks[sti()])
                                            glColor4f(BadColor);
                                        else if (AuxTracks[sti()])
                                            glColor4f(AuxColor);
                                        else
                                            ColorFreqs.GLize((double)fi());
                        ...
                        if (Show3DText && (!trackssuper || firsttrack) && (how & GLPaintOwner) && IsIntensityModes())
                        {
                            ...
                            if (IsModeEF() && IsIntensityModes())
                            {
                                ...
                                */
                                ...
                                baseline = Show3DText && numfreq > 1 && dtrh / descaley > 2 * MinBaselineH && dtrh / descaley > SFont->GetHeight();

                                text = Show3DText && enoughw && dtrh / descaley > SFont->GetHeight();

                                alltext = Show3DText && enoughw && !text && firsttrack;
                        ...
                        if (!trackssuper && !IsModeFEAvg()) // translation accounts for the scaling
                    ...
                    if (drawhoriscale && (IsModeSpectrum() && firsttrack || !IsTracksMode()) && (how & GLPaintOwner))
                    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (CurrentDisplaySpace == DisplaySpaceNone && !IsIntensityModes() && v2[0] - v1[0] > 1 /*&& IsFilling != FillingColor*/)
                    {
                        ...
                        if (IsFreqHorizontal())
                            glColor4f(AntiFreqCursorFillColor);
                        else
    ...
    void TFrequenciesView::EvLButtonDown(uint i, const TPoint &p)
    {
            ...
            if (view->GetViewId() != GetViewId())
            {
                ...
                else
                {
                    if (!UsedBy.IsInside(view))
                    {
                        ...
                        if (freq2 && freq2->GetTotalElectrodes() == EEGDoc->GetTotalElectrodes() && freq2->GetNumTimeFrames() == EEGDoc->GetNumTimeFrames() && FreqDoc->GetNumFrequencies() == freq2->GetNumFrequencies())
                        {
        ...
        else if (CurrentDisplaySpace == DisplaySpaceNone && CaptureMode == CaptureNone && !ControlKey && IsFreqHorizontal())
        {
    ...
    */
    ...
    void TFrequenciesView::UpdateBuffers(long oldtfmin, long oldtfmax, long newtfmin, long newtfmax)
    {
        ...
        int offset;
        auto bigblock = [&]()
        { return (EegBuff.GetMaxPlanes() * EegBuff.GetDim1() * EegBuff.GetDim2()); };
    ...
    void TFrequenciesView::ResetScaleTracks(const TSelection *sel)
    {
        ...
        for (int e = 0; e < FreqDoc->GetTotalElectrodes(); e++)
        {
            ...
            { // auxs and bads are scaled individually, as to be "neutralized"
    ...
    {
        if (w == IDB_PREVIOUSFREQ && FCursor.GetPosMin() == CDPf.GetLimitMin() && !TFCursor.IsExtending() || w == IDB_NEXTFREQ && FCursor.GetPosMax() == CDPf.GetLimitMax() && !TFCursor.IsExtending())
            return;
    ...
    { // will modify FCursor, NOT CDPf
        ...
        if (w == IDB_MOREFREQS && oldlen == FCursor.GetLimitLength() || w == IDB_LESSFREQS && oldlen == 1)
            return;
        ...
        if (ShiftKey)
        {
            ...
            if (w == IDB_MOREFREQS)
                FCursor.SetPos(FCursor.GetLimitMin(), FCursor.GetLimitMax());
            else if (w == IDB_LESSFREQS)
                FCursor.SetPos(FCursor.GetPosMin()); // FCursor.SetPos    ( FCursor.GetPosMiddle () );
        }
        ...
        else
        {
            ...
            if (w == IDB_MOREFREQS)
            {
                ...
                if (FCursor.GetPosMin() == FCursor.GetLimitMin())
                    FCursor.SetPos(FCursor.GetPosMin(), FCursor.GetPosMax() + stepf);
                else if (FCursor.GetPosMax() == FCursor.GetLimitMax())
                    FCursor.SetPos(FCursor.GetPosMin() - stepf, FCursor.GetPosMax());
                else
    ...
    void TFrequenciesView::CmSaveScaleFreqs()
    {
        ...
        file.ReplaceExtension("Normalization"
                              "." InfixSpectrum "." FILEEXT_EEGSEF);
