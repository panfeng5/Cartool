...
#define ToScalingContrast(C) sqrt((C - 0.10) / 9.90)
        ...
        : TBaseView(doc, parent, group), MRIDoc(&doc)
    {
        ...
        if (MRIDoc->GetTessSurfacePos()->IsEmpty() && MRIDoc->GetTessSurfaceNeg()->IsEmpty())
        ...
        ScalingContrast = ToScalingContrast(MRIDoc->IsMask() ? 1 : MRIDoc->IsRis() ? 10
                                                                                   : 0.5);
        ...
        if (MRIDoc->IsFullHead())
        { // make the slices a little closer to the center
...
#define MaterialTrDiffuse 0.55, 0.55, 0.55, MRIALPHA
    ...
    void TVolumeView::SetColorTable(AtomType datatype)
    {
        ...
        if (IsAbsolute(datatype))
        {
            ...
            // special cases
            if (MRIDoc->IsRoiMask() && MRIDoc->GetMaxValue() > 1 || MRIDoc->IsBlob() || MRIDoc->IsRis())
        ...
        { // if ( IsScalar ( datatype ) )  // all other cases
    ...
    {
        SurfaceMode = MRIDoc->IsRoiMask() || MRIDoc->IsBlob() || MRIDoc->IsRis() || MRIDoc->IsScalar(AtomTypeUseCurrent);
    }
    ...
    void TVolumeView::SearchAndSetIntensity()
    {
        ...
        if (MRIDoc->IsMask() || MRIDoc->IsBlob()
            /*|| MRIDoc->IsSegmented ()*/
        )
            // exact scaling
    ...
    bool TVolumeView::ModifyPickingInfo(TPointFloat &Picking, char *buff)
    {
        ...
        */
        ...
        if (IsInteger(value))
            IntegerToString(StringEnd(buff), value);
        else
            FloatToString(StringEnd(buff), value, 2);
    ...
    bool TVolumeView::IsRenderingMode(int renderingmode)
    {
        if (renderingmode == RenderingOpaque)
            return RenderingMode == MriSliceOpaque || RenderingMode == MriOpaque;
        else if (renderingmode == RenderingTransparent)
            return RenderingMode == MriSliceOvercast || RenderingMode == MriTransparent;
        else
            return false;
    }
    ...
    void TVolumeView::DrawMinMax(TPointFloat &pos, bool colormin, bool showminmaxcircle, double scale)
    {
        ...
        if (showminmaxcircle)
        {
            ...
            if (colormin)
                glColor4f(GLBASE_MINCOLOR, 0.5);
            else
                glColor4f(GLBASE_MAXCOLOR, 0.5);
    ...
    */
    ...
    void TVolumeView::GLPaint(int how, int renderingmode, TGLClipPlane *otherclipplane)
    {
        ...
        if (FindMinMax && (how & GLPaintOwner) && !(RButtonDown && ControlKey))
        {
            ...
            {
                if ((bool)clipplane[0])
                    clipplane[0].SetAbsPosition(clip.X);
                if ((bool)clipplane[1])
                    clipplane[1].SetAbsPosition(clip.Y);
                if ((bool)clipplane[2])
                    clipplane[2].SetAbsPosition(clip.Z);
            }
        }
        ...
        if (ScalingAuto != ScalingAutoOff && !(RButtonDown && !ControlKey && MouseAxis == MouseAxisHorizontal))
        {
            ...
            if (MRIDoc->IsMask() || MRIDoc->IsBlob() || MRIDoc->IsRis() || MRIDoc->IsSegmented()) // exact scaling
        ...
        for (int nv = 0; nv < nviews; nv++)
        {
            ...
            */
            ...
            if (slicemode && (how & GLPaintOpaque))
            {
                ...
                if (slch->IsSummarySlice(slicei))
                {
                    ...
                    if (IsSliceModeX())
                        ...
                        for (TIteratorSelectedForward si(*slch->GetSelection()); (bool)si; ++si)
                        {
                            ...
                            if (slch->IsSummarySlice(si()))
                                break;
                            else if (slch->IsMinSlice(si()))
                            {
                                glColor4f(0.00, 0.00, 1.00, 1.00);
                                Prim.Draw3DWireFrame(minPos.X, minPos.X, mrimin[1] - 1, mrimax[1] + 1, mrimin[2] - 1, mrimax[2] + 1, 3);
                            }
                            else if (slch->IsMaxSlice(si()))
                            {
                                glColor4f(1.00, 0.00, 0.00, 1.00);
                                Prim.Draw3DWireFrame(maxPos.X, maxPos.X, mrimin[1] - 1, mrimax[1] + 1, mrimin[2] - 1, mrimax[2] + 1, 3);
                            }
                            else
                            {
                                glColor4f(SliceColor);
                                Prim.Draw3DWireFrame(si() + xshift, si() + xshift, mrimin[1], mrimax[1], mrimin[2], mrimax[2], 1);
                            }
                        }
                        ...
                        for (TIteratorSelectedForward si(*slch->GetSelection()); (bool)si; ++si)
                        {
                            ...
                            {
                                glColor4f(0.00, 0.00, 1.00, 1.00);
                                Prim.Draw3DWireFrame(mrimin[0] - 1, mrimax[0] + 1, minPos.Y, minPos.Y, mrimin[2] - 1, mrimax[2] + 1, 3);
                            }
                            else if (slch->IsMaxSlice(si()))
                            {
                                glColor4f(1.00, 0.00, 0.00, 1.00);
                                Prim.Draw3DWireFrame(mrimin[0] - 1, mrimax[0] + 1, maxPos.Y, maxPos.Y, mrimin[2] - 1, mrimax[2] + 1, 3);
                            }
                            else
                            {
                                glColor4f(SliceColor);
                                Prim.Draw3DWireFrame(mrimin[0], mrimax[0], si() + yshift, si() + yshift, mrimin[2], mrimax[2], 1);
                            }
                        }
                    else
                        ...
                        for (TIteratorSelectedForward si(*slch->GetSelection()); (bool)si; ++si)
                        {
                            ...
                            {
                                glColor4f(0.00, 0.00, 1.00, 1.00);
                                Prim.Draw3DWireFrame(mrimin[0] - 1, mrimax[0] + 1, mrimin[1] - 1, mrimax[1] + 1, minPos.Z, minPos.Z, 3);
                            }
                            else if (slch->IsMaxSlice(si()))
                            {
                                glColor4f(1.00, 0.00, 0.00, 1.00);
                                Prim.Draw3DWireFrame(mrimin[0] - 1, mrimax[0] + 1, mrimin[1] - 1, mrimax[1] + 1, maxPos.Z, maxPos.Z, 3);
                            }
                            else
                            {
                                glColor4f(SliceColor);
                                Prim.Draw3DWireFrame(mrimin[0], mrimax[0], mrimin[1], mrimax[1], si() + zshift, si() + zshift, 1);
                            }
                        }
                ...
                else
                {
                    ...
                    if (IsSliceModeX())
                    {
                        ...
                        if (ShowMin && (int)minPos.X == s)
                            DrawMinMax(minPos, true, showminmaxcircle, mrisize);
                        if (ShowMax && (int)maxPos.X == s)
                            DrawMinMax(maxPos, false, showminmaxcircle, mrisize);
                    }
                    ...
                    else if (IsSliceModeY())
                    {
                        ...
                        if (ShowMin && (int)minPos.Y == s)
                            DrawMinMax(minPos, true, showminmaxcircle, mrisize);
                        if (ShowMax && (int)maxPos.Y == s)
                            DrawMinMax(maxPos, false, showminmaxcircle, mrisize);
                    }
                    ...
                    { // if ( IsSliceModeZ () )
                        ...
                        if (ShowMin && (int)minPos.Z == s)
                            DrawMinMax(minPos, true, showminmaxcircle, mrisize);
                        if (ShowMax && (int)maxPos.Z == s)
                            DrawMinMax(maxPos, false, showminmaxcircle, mrisize);
                    }
            ...
            if (ncutplanes && renderingmode == MriSliceOpaque && (how & GLPaintOpaque))
            {
                ...
                if ((bool)clipplane[0])
                    volume->DrawPlaneX(cx, quality);
                if ((bool)clipplane[1])
                    volume->DrawPlaneY(cy, quality);
                if ((bool)clipplane[2])
                    volume->DrawPlaneZ(cz, quality);
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // draw the isosurface
            if (renderingmode == MriTransparent && (how & GLPaintTransparent) || renderingmode == MriOpaque && (how & GLPaintOpaque))
            {
                ...
                for (int n = 0; n < ndraws; n++)
                {
                    ...
                    if (ncutplanes)
                    {
                        ...
                        if (n == 0)
                            clipplane[drawi[0]].GLize(clipmode);
                        else if (n == 1)
                            clipplane[drawi[0]].GLize(clipmode | InvertedClipPlane);

                        if (n == 1)
                            clipplane[drawi[1]].GLize(clipmode);
                        else if (n == 2)
                            clipplane[drawi[1]].GLize(clipmode | InvertedClipPlane);

                        if (n == 2)
                            clipplane[drawi[2]].GLize(clipmode);
                    }
                    ...
                    if (maxtrihullpos)
                    {
                        ...
                        if (SurfaceMode && !MRIDoc->IsRoiMask())
                        {
                            ...
                            if (MRIDoc->IsBinaryMask())
                                ColorTable.GetColorIndex(MRIDoc->GetMaxValue(), glcol);
                            else
                                ColorTable.GetColorIndex(MRIDoc->GetIsoSurfaceCut(), glcol);
                    ...
                    if (maxtrihullneg)
                    {
                        ...
                        if (SurfaceMode && !MRIDoc->IsRoiMask())
                        {
                            // override with color of value cut
                            if (MRIDoc->IsBinaryMask())
                                ColorTable.GetColorIndex(MRIDoc->GetMinValue(), glcol);
                            else
                                ColorTable.GetColorIndex(-MRIDoc->GetIsoSurfaceCut(), glcol);
            ...
            if (CappingMode != MriNoCapping && ncutplanes
                ...
                && (renderingmode == MriTransparent && (how & GLPaintTransparent) || renderingmode == MriOpaque && (how & GLPaintOpaque)))
            {
                ...
                if ((bool)clipplane[0])
                {
                    ...
                    if ((bool)clipplane[1])
                        clipplane[1].GLize(clipmode | InvertedClipPlane);
                    if ((bool)clipplane[2])
                        clipplane[2].GLize(clipmode | InvertedClipPlane);
                ...
                if ((bool)clipplane[1])
                {
                    ...
                    if ((bool)clipplane[0])
                        clipplane[0].GLize(clipmode | InvertedClipPlane);
                    if ((bool)clipplane[2])
                        clipplane[2].GLize(clipmode | InvertedClipPlane);
            ...
            if (Editing && Picking.IsNotNull() && (how & GLPaintTransparent))
            {
                ...
                if (Editing == EditingToolSphereCenter || Editing == EditingToolSphereCenterBlurr)
                {
...
// bonus display
    ...
    void TVolumeView::EvLButtonDown(uint i, const TPoint &p)
    {
        ...
        else if (/*ControlKey ||*/ VkKey(MouseMiddeClicksurrogate))
        ...
        else if (CaptureMode == CaptureOperation)
        {
            ...
            if (CurrentOperation == CM_OPERATIONAND)
            {
                MRIDoc->GetData()->ApplyMaskToData(*tomri->GetData());
                StringCopy(opext, "AND");
            }
            else if (CurrentOperation == CM_OPERATIONOR)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandData, OperandData, OperationOr); // OR will be Max of 2 competing values
                StringCopy(opext, "OR");
            }
            else if (CurrentOperation == CM_OPERATIONXOR)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandData, OperandData, OperationXor);
                StringCopy(opext, "XOR");
            }
            else if (CurrentOperation == CM_OPERATIONADD)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandData, OperandData, OperationAdd);
                StringCopy(opext, "Add");
            }
            else if (CurrentOperation == CM_OPERATIONSUB)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandData, OperandData, OperationSub);
                StringCopy(opext, "Sub");
            }
            else if (CurrentOperation == CM_OPERATIONMULT)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandData, OperandData, OperationMultiply);
                StringCopy(opext, "Mult");
            }
            else if (CurrentOperation == CM_OPERATIONDIV)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandData, OperandData, OperationDivide);
                StringCopy(opext, "Div");
            }
            else if (CurrentOperation == CM_OPERATIONCLEAR)
            {
                MRIDoc->GetData()->ClearMaskToData(*tomri->GetData());
                StringCopy(opext, "Remove");
            }
            else if (CurrentOperation == CM_OPERATIONFILL)
            {
                MRIDoc->GetData()->ApplyDataToMask(*tomri->GetData());
                StringCopy(opext, "Fill");
            }
            else if (CurrentOperation == CM_OPERATIONREPLACE)
            {
                MRIDoc->GetData()->BinaryOp(*tomri->GetData(), OperandMask, OperandData, OperationOr); // OR but with priority to new volume
                StringCopy(opext, "Insert");
            }
            else
            {
                ShowMessage("Unsupported Operation!", "Volume Operation", ShowMessageWarning);
                StringCopy(opext, "Op");
            }
    ...
    void TVolumeView::EvMouseMove(uint i, const TPoint &p)
    {
        ...
        if (RButtonDown)
        {
            ...
            else if (Editing)
            {
                ...
                if (Editing == EditingToolSphereCenter || Editing == EditingToolSphereCenterBlurr || Editing == EditingToolSphereSurface || Editing == EditingToolCylinder && adx > ady)
                {

                    if (dx > 0)
                        EditingRadius *= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 0.5 : 0.1);
                    else
                        EditingRadius /= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 0.5 : 0.1);
                ...
                else if (Editing == EditingToolCylinder && adx < ady)
                {
                    ...
                    if (dy > 0)
                        EditingDepth /= 1 + 2 * min(ady, MouseMoveScale) / (double)MouseMoveScale * (ady > MouseMoveScaleFast ? 0.5 : 0.1);
                    else
                        EditingDepth *= 1 + 2 * min(ady, MouseMoveScale) / (double)MouseMoveScale * (ady > MouseMoveScaleFast ? 0.5 : 0.1);
            ...
            { // change color scaling
                ...
                if (MouseAxis == MouseAxisVertical)
                {
                    ...
                    if (IsoMode == MriIsoManual)
                    {
                        ...
                        double bt = fabs(MRIDoc->GetIsoSurfaceCut()) - (double)dy * (ady < MouseMoveScale ? 0.5 : 1.5) * MRIDoc->GetAbsMaxValue() / 1000;
                ...
                else if (MouseAxis == MouseAxisHorizontal)
                {
                    // brightness & contrast
                    if (dx > 0)
                        ScalingLevel /= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 0.5 : 0.1);
                    else
                        ScalingLevel *= 1 + min(adx, MouseMoveScale) / (double)MouseMoveScale * (adx > MouseMoveScaleFast ? 0.5 : 0.1);
                                       ...
                                       double feather)
    {
        ...
        bool invert = how == EditingCutOutside && !(tool == EditingToolSphereCenterBlurr // not implemented
        ...
        if (MRIDoc->IsMask())
            feather = 0; // masks shouldn't have any feathering, as it produces intermediate values
        else
            feather /= 2.0; // feather is applied half on each side

        Volume &vol = *MRIDoc->GetData();

        Volume savedvol;
        if (invert)
            savedvol = *MRIDoc->GetData();
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Do it!
        if (tool == EditingToolSphereCenter || tool == EditingToolSphereSurface)
            vol.DrawSphere(pos1, radius, feather, fillingvalue, OperationSet);
        else if (tool == EditingToolSphereCenterBlurr)
            vol.BlurrSphere(pos1, radius);
        else if (tool == EditingToolCylinder)
            vol.DrawCylinder(pos1, pos2, radius * EditingCylinderRadiusRatio, feather, fillingvalue, OperationSet);
        else if (tool == EditingToolPlane)
            vol.ErasePlane(pos1, EditingGradient, feather);
    ...
    void TVolumeView::CmSetRenderingMode()
    {
        ...
        if (RenderingMode == MriSliceOpaque && orm != MriSliceOvercast || RenderingMode == MriSliceOvercast && orm != MriSliceOpaque || orm == MriSliceOpaque && RenderingMode != MriSliceOvercast || orm == MriSliceOvercast && RenderingMode != MriSliceOpaque)
        {
            ...
            { // re-entering oriented clipping? restore directions if apropriate

                if (SavedState.Clip[3] || SavedState.Clip[4] || SavedState.Clip[5])
                {

                    if ((bool)ClipPlane[0] && SavedState.Clip[3])
                        ClipPlane[0].Set(SavedState.Clip[3]);
                    if ((bool)ClipPlane[1] && SavedState.Clip[4])
                        ClipPlane[1].Set(SavedState.Clip[4]);
                    if ((bool)ClipPlane[2] && SavedState.Clip[5])
                        ClipPlane[2].Set(SavedState.Clip[5]);
                }
                else
                {
                    ClipPlane[0].SetNone();
...
#define ClipTra ClipPlane[2]
        ...
        if (sag)
        {
            ...
            if (tra && !cor)
                ModelRotMatrix.RotateY(10 * TrueToPlus(sagforward), MultiplyLeft);
            // main rotation
            if (cor)
                ModelRotMatrix.RotateY(45 * TrueToPlus(sagforward ^ corforward), MultiplyLeft);
            // main rotation
            if (tra)
                ModelRotMatrix.RotateX(45 * TrueToPlus(traforward), MultiplyLeft);
            // cosmetic rotation - not mandatory, just disambiguate a bit the perspective
            if (cor && !tra)
                ModelRotMatrix.RotateX(10, MultiplyLeft);
        }
        ...
        else if (cor)
        {
            ...
            // cosmetic rotation - not mandatory, just disambiguate a bit the perspective
            if (tra)
                ModelRotMatrix.RotateY(-10, MultiplyLeft);
        }
        ...
        if (noclipping)
        {
            ...
            if (sagold)
                Orientation = OrientSagittalLeft;
            else if (corold)
                Orientation = OrientCoronalFront;
            else if (traold)
                Orientation = OrientTransverseTop;
...
#undef ClipTra
    ...
    */
    ...
    void TVolumeView::CmCoregistration()
    {
        ...
        if (MRIDoc->GetGeometryTransform() && MRIDoc->GetGeometryTransform()->Type != GeometryUnknown)

            StringCopy(inclusionhint, "T");

        else if (mri2 && mri2->GetGeometryTransform() && mri2->GetGeometryTransform()->Type != GeometryUnknown)
        ...
        if (answer == 'T')
            CoregistrationMrisUI(mri2, MRIDoc);
        else if (answer == 'S')
            CoregistrationMrisUI(MRIDoc, mri2);
        else
            CoregistrationMrisUI(0, 0);
    }
    ...
    {
        if (w == CM_FILTERMEAN)
            MRIDoc->Filter(FilterTypeMean);
        else if (w == CM_FILTERGAUSSIAN)
            MRIDoc->Filter(FilterTypeGaussian);
        else if (w == CM_FILTERANISOGAUSSIAN)
            MRIDoc->Filter(FilterTypeAnisoGaussian);

        else if (w == CM_FILTERINTENSITYADD)
            MRIDoc->Filter(FilterTypeIntensityAdd);
        else if (w == CM_FILTERINTENSITYSUB)
            MRIDoc->Filter(FilterTypeIntensitySub);
        else if (w == CM_FILTERINTENSITYMULT)
            MRIDoc->Filter(FilterTypeIntensityMult);
        else if (w == CM_FILTERINTENSITYDIV)
            MRIDoc->Filter(FilterTypeIntensityDiv);
        else if (w == CM_FILTERINTENSITYOPPOSITE)
            MRIDoc->Filter(FilterTypeIntensityOpposite);
        else if (w == CM_FILTERINTENSITYINVERT)
            MRIDoc->Filter(FilterTypeIntensityInvert);
        else if (w == CM_FILTERINTENSITYABS)
            MRIDoc->Filter(FilterTypeIntensityAbs);
        else if (w == CM_FILTERINTENSITYREMAP)
            MRIDoc->Filter(FilterTypeIntensityRemap);
        else if (w == CM_FILTERINTENSITYNORM)
            MRIDoc->Filter(FilterTypeIntensityNorm);
        else if (w == CM_FILTERKEEPVALUE)
            MRIDoc->Filter(FilterTypeKeepValue);

        else if (w == CM_FILTERMIN)
            MRIDoc->Filter(FilterTypeMin);
        else if (w == CM_FILTERMAX)
            MRIDoc->Filter(FilterTypeMax);
        else if (w == CM_FILTERMINMAX)
            MRIDoc->Filter(FilterTypeMinMax);
        else if (w == CM_FILTERRANGE)
            MRIDoc->Filter(FilterTypeRange);
        else if (w == CM_FILTERMEDIAN)
            MRIDoc->Filter(FilterTypeMedian);
        else if (w == CM_FILTERMAXMODE)
            MRIDoc->Filter(FilterTypeMaxMode);
        else if (w == CM_FILTERMEANSUB)
            MRIDoc->Filter(FilterTypeMeanSub);
        else if (w == CM_FILTERLOGMEANSUB)
            MRIDoc->Filter(FilterTypeLogMeanSub);
        else if (w == CM_FILTERMEANDIV)
            MRIDoc->Filter(FilterTypeMeanDiv);
        else if (w == CM_FILTERLOGMEANDIV)
            MRIDoc->Filter(FilterTypeLogMeanDiv);
        else if (w == CM_FILTERMAD)
            MRIDoc->Filter(FilterTypeMAD);
        else if (w == CM_FILTERMADSD)
            MRIDoc->Filter(FilterTypeMADSDInv);
        else if (w == CM_FILTERSD)
            MRIDoc->Filter(FilterTypeSD);
        else if (w == CM_FILTERSDINV)
            MRIDoc->Filter(FilterTypeSDInv);
        else if (w == CM_FILTERCOV)
            MRIDoc->Filter(FilterTypeCoV);
        else if (w == CM_FILTERLOGCOV)
            MRIDoc->Filter(FilterTypeLogCoV);
        else if (w == CM_FILTERSNR)
            MRIDoc->Filter(FilterTypeLogSNR);
        else if (w == CM_FILTERMCOV)
            MRIDoc->Filter(FilterTypeMCoV);
        else if (w == CM_FILTERLOGMCOV)
            MRIDoc->Filter(FilterTypeLogMCoV);
        else if (w == CM_FILTERMODE)
            MRIDoc->Filter(FilterTypeModeQuant);
        else if (w == CM_FILTERENTROPY)
            MRIDoc->Filter(FilterTypeEntropy);
        else if (w == CM_FILTERPERCENTFULL)
            MRIDoc->Filter(FilterTypePercentFullness);
        else if (w == CM_FILTERCUTBELOWFULL)
            MRIDoc->Filter(FilterTypeCutBelowFullness);
        else if (w == CM_FILTERSAV)
            MRIDoc->Filter(FilterTypeSAV);
        else if (w == CM_FILTERCOMPACTNESS)
            MRIDoc->Filter(FilterTypeCompactness);
        else if (w == CM_FILTERRANK)
            MRIDoc->Filter(FilterTypeRank);
        else if (w == CM_FILTERRANKRAMP)
            MRIDoc->Filter(FilterTypeRankRamp);

        else if (w == CM_FILTERTHRESHOLD)
            MRIDoc->Filter(FilterTypeThreshold);
        else if (w == CM_FILTERTHRESHOLDABOVE)
            MRIDoc->Filter(FilterTypeThresholdAbove);
        else if (w == CM_FILTERTHRESHOLDBELOW)
            MRIDoc->Filter(FilterTypeThresholdBelow);
        else if (w == CM_FILTERSYMMETRIZE)
            MRIDoc->Filter(FilterTypeSymmetrize);
        else if (w == CM_FILTERBINARIZE)
            MRIDoc->Filter(FilterTypeBinarize);
        else if (w == CM_FILTERTOMASK)
            MRIDoc->Filter(FilterTypeToMask);
        else if (w == CM_FILTERIREVERT)
            MRIDoc->Filter(FilterTypeRevert);
        else if (w == CM_FILTERLESSNEIGH)
            MRIDoc->Filter(FilterTypeLessNeighbors);
        else if (w == CM_FILTERMORENEIGH)
            MRIDoc->Filter(FilterTypeMoreNeighbors);
        else if (w == CM_FILTERRELAX)
            MRIDoc->Filter(FilterTypeRelax);
        else if (w == CM_FILTERTHINNING)
            MRIDoc->Filter(FilterTypeThinning);
        else if (w == CM_FILTERWATERFALLRIDGES)
            MRIDoc->Filter(FilterTypeWaterfallRidges);
        else if (w == CM_FILTERWATERFALLVALLEYS)
            MRIDoc->Filter(FilterTypeWaterfallValleys);
        else if (w == CM_FILTERKEEPBIGGEST)
            MRIDoc->Filter(FilterTypeKeepBiggestRegion);
        else if (w == CM_FILTERKEEPCOMPACT)
            MRIDoc->Filter(FilterTypeKeepCompactRegion);
        else if (w == CM_FILTERKEEPBIGGEST2)
            MRIDoc->Filter(FilterTypeKeepBiggest2Regions);
        else if (w == CM_FILTERKEEPCOMPACT2)
            MRIDoc->Filter(FilterTypeKeepCompact2Regions);
        else if (w == CM_FILTERCLUSTERSTOREGIONS)
            MRIDoc->Filter(FilterTypeClustersToRegions);

        else if (w == CM_FILTERHISTOEQUALIZE)
            MRIDoc->Filter(FilterTypeHistoEqual);
        else if (w == CM_FILTERHISTOEQUALIZEBRAIN)
            MRIDoc->Filter(FilterTypeHistoEqualBrain);
        else if (w == CM_FILTERHISTOCOMPACT)
            MRIDoc->Filter(FilterTypeHistoCompact);

        else if (w == CM_FILTERGRADIENT)
            MRIDoc->Filter(FilterTypeGradient);
        else if (w == CM_FILTERCANNY)
            MRIDoc->Filter(FilterTypeCanny);
        else if (w == CM_FILTERLAPLACIAN)
            MRIDoc->Filter(FilterTypeLaplacian);
        else if (w == CM_FILTERHESSIANEIGENMAX)
            MRIDoc->Filter(FilterTypeHessianEigenMax);
        else if (w == CM_FILTERHESSIANEIGENMIN)
            MRIDoc->Filter(FilterTypeHessianEigenMin);
        else if (w == CM_FILTERKCURVATURE)
            MRIDoc->Filter(FilterTypeKCurvature);
        else if (w == CM_FILTERKCCURVATURE)
            MRIDoc->Filter(FilterTypeKCCurvature);

        else if (w == CM_FILTERERODE)
            MRIDoc->Filter(FilterTypeErode);
        else if (w == CM_FILTERDILATE)
            MRIDoc->Filter(FilterTypeDilate);
        else if (w == CM_FILTEROPEN)
            MRIDoc->Filter(FilterTypeOpen);
        else if (w == CM_FILTERCLOSE)
            MRIDoc->Filter(FilterTypeClose);
        else if (w == CM_FILTERMORPHGRADIENT)
            MRIDoc->Filter(FilterTypeMorphGradient);
        else if (w == CM_FILTERMORPHGRADIENTINT)
            MRIDoc->Filter(FilterTypeMorphGradientInt);
        else if (w == CM_FILTERMORPHGRADIENTEXT)
            MRIDoc->Filter(FilterTypeMorphGradientExt);

        else if (w == CM_FILTERBIASFIELD)
            MRIDoc->Filter(FilterTypeBiasField);
        else if (w == CM_FILTERSEGMENTCSF)
            MRIDoc->Filter(FilterTypeSegmentCSF);
        else if (w == CM_FILTERSEGMENTGREY)
            MRIDoc->Filter(FilterTypeSegmentGrey);
        else if (w == CM_FILTERSEGMENTWHITE)
            MRIDoc->Filter(FilterTypeSegmentWhite);
        else if (w == CM_FILTERSEGMENTTISSUES)
            MRIDoc->Filter(FilterTypeSegmentTissues);
        else if (w == CM_FILTERMASKTOSP)
            MRIDoc->Filter(FilterTypeMaskToSolutionPoints);
        else if (w == CM_SKULLSTRIPPING)
            MRIDoc->Filter(FilterTypeSkullStripping);
        else if (w == CM_CUTBRAINSTEM)
            MRIDoc->Filter(FilterTypeBrainstemRemoval);
        else if (w == CM_HEADCLEANUP)
            MRIDoc->Filter(FilterTypeHeadCleanup);
    ...
    void TVolumeView::CmShowMinMax(owlwparam w)
    {
        ...
        if (w == IDB_SHOWMIN)
        {
            ...
            if (IsSignedData())
            {
                ...
                if (ShowMin && ShowMax)
                    ShowMax = ShowMin; // last one pressed propagates
            }
        ...
        else if (w == IDB_SHOWMAX)
        {
            ...
            if (ShowMin && ShowMax)
                ShowMin = ShowMax; // last one pressed propagates
    ...
    void TVolumeView::CmOutputHistogram(owlwparam w)
    {
        ...
        else if (w == CM_FILTERHISTOCOMPUTELOG)
        {
            ...
            ReplaceExtension(file, InfixHistogramLong "Log"
                                                      "." FILEEXT_EEGSEF);
    ...
    void TVolumeView::CmSetContrast(owlwparam wparam)
    {
        ...
        if (wparam == IDB_ISINCCONTRAST)
            ScalingContrast += 0.05;
        else if (wparam == IDB_ISDECCONTRAST)
            ScalingContrast -= 0.05;
    ...
    void TVolumeView::CmSetSliceMode()
    {
        ...
        if (SliceMode)
        {
            ...
            if (!osm)
            { // first time we switch to slices?
              // save 3D state
    ...
    {
        if (SliceMode == SliceModeCor)
            ModelRotMatrix = CTSMatrix[0]; // not SliceX but SliceCoronal
        else if (SliceMode == SliceModeTra)
            ModelRotMatrix = CTSMatrix[1];
        else if (SliceMode == SliceModeSag)
            ModelRotMatrix = CTSMatrix[2];
        else
        {
            TBaseView::CmOrient();
        }
