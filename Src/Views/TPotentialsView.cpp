...
namespace crtl
{
    ...
    bool TPotentialsView::IsRenderingMode(int renderingmode)
    {
        if (renderingmode == RenderingOpaque)
            return RenderingMode == PotentialsRenderingSpheres || RenderingMode == PotentialsRenderingSolid;
        else if (renderingmode == RenderingTransparent)
            return RenderingMode == PotentialsRenderingTransparent || RenderingMode == PotentialsRenderingOvercast;
        else
            return false;
    }
        ...
        */
        ...
        auto TriangleLoop = [&]()
        {
            ...
            for (int t = 0; t < numtri; t++)
            {
                ...
                if (!(BadTracks[*toi1] || BadTracks[*toi2] || BadTracks[*toi3]))
                {
                    ...
                    if (CurrentDisplaySpace == DisplaySpaceHeight)
                    {
                        ...
                        v = EegBuff(*toi1, tf);
                        if (v >= 0)
                            tot1[2] = v / ScalingPMax * ms2 / 3;
                        else
                            tot1[2] = -v / ScalingNMax * ms2 / 3;

                        v = EegBuff(*toi2, tf);
                        if (v >= 0)
                            tot2[2] = v / ScalingPMax * ms2 / 3;
                        else
                            tot2[2] = -v / ScalingNMax * ms2 / 3;

                        v = EegBuff(*toi3, tf);
                        if (v >= 0)
                            tot3[2] = v / ScalingPMax * ms2 / 3;
                        else
                            tot3[2] = -v / ScalingNMax * ms2 / 3;
                    } // DisplaySpaceHeight
                    ...
                    if ((bool)AnimFx && (AnimFx.GetTimerId() == TimerToFlat || AnimFx.GetTimerId() == TimerTo3D))
                    {
                        ...
                        long dr = ((char *)XYZDoc->GetSurfaceRendering(DisplaySpaceProjected)->GetListTriangles()) - ((char *)XYZDoc->GetSurfaceRendering(DisplaySpace3D)->GetListTriangles());
...
#define SPSaturate 0.83
            ...
            ColorTable.SetParameters(ScalingPMax, ScalingNMax, contrast, RenderingMode == PotentialsRenderingOvercast ? AlphaSquare : RenderingMode == PotentialsRenderingTransparent ? 0.5
                                                                                                                                                                                      : 1);
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // print the label
            if (ShowSequenceLabels && (ManageRangeCursor == MRCSequence || ManageRangeCursor == MRCAnimation) && NotSmallWindow() && timepos > 35 && (how & GLPaintOwner) && (how & GLPaintOpaque))
            {
            ...
            if (!trilimit)
            {
                ...
                trilimit = 75 / Zoom / Zoom * XYZDoc->GetMedianDistance(CurrentDisplaySpace)                                  // account for electrode resolution: small interval -> increase division
            ...
            */
            ...
            if (RenderingMode == PotentialsRenderingSpheres && (how & GLPaintOpaque))
            {
                ...
                for (int e = 0; e < XYZDoc->GetNumElectrodes(); e++)
                {
                    ...
                    if (CurrentDisplaySpace == DisplaySpaceHeight)
                    {
                        ...
                        v = EegBuff(e, tf);
                        if (v >= 0)
                            z = v / ScalingPMax * ms2 / 3;
                        else
                            z = -v / ScalingNMax * ms2 / 3;
                    }
                    ...
                    {
                        if (Highlighted[e])
                            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, MaterialElSel.GetDiffuse());
                        else if (BadTracks[e])
                            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, MaterialElBad.GetDiffuse());
                    }
            ...
            if (ShowLabels && (how & GLPaintTransparent))
            {
                ...
                for (int e = 0; e < XYZDoc->GetNumElectrodes(); e++)
                {
                    ...
                    */
                    ...
                    if (ShowLabels == ShowLabelText)
                        StringCopy(label, XYZDoc->GetElectrodeName(e));
                    else if (ShowLabels == ShowLabelIndex)
                        StringCopy(label, "#", IntegerToString(buff, e + 1));
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // draw the surface
            if ((RenderingMode == PotentialsRenderingTransparent || RenderingMode == PotentialsRenderingOvercast) && (how & GLPaintTransparent))
            {
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // equator circle & axis
            if (ShowAxis && (CurrentDisplaySpace != DisplaySpace3D && XYZDoc->IsHead() && ManageRangeCursor != MRCSequenceOverlay) && (how & GLPaintOpaque) && (how & GLPaintOwner))
            {
    ...
    void TPotentialsView::Paint(TDC &dc, bool /*erase*/, TRect &rect)
    {
        ...
        if (ManageRangeCursor == MRCSequence)
        { // switch back to regular resizing if only 1 TF in sequence
        ...
        if (MButtonDown)
        {
            ...
            if (hit >= 0)
            {
                ...
                if (ShowElectrodes || RenderingMode == PotentialsRenderingSpheres)
                    ...
                    { // if ( ControlKey ){      // control + middle = select
    ...
    void TPotentialsView::CmSetContrast(owlwparam wparam)
    {
        ...
        if (wparam == IDB_SPINCCONTRAST)
            ScalingContrast += 0.10;
        else if (wparam == IDB_SPDECCONTRAST)
            ScalingContrast -= 0.10;
    ...
    {
        if (ShiftKey)
            ShowMinMax = (ShowMinMax + PotentialsNumMinMax - 1) % PotentialsNumMinMax;
        else
            ShowMinMax = (ShowMinMax + 1) % PotentialsNumMinMax;
    ...
    {
        if (w == CM_INTERPOL_N)
            RenderingMode = PotentialsRenderingSpheres;
        else if (w == CM_INTERPOL_Y)
            RenderingMode = PotentialsRenderingSolid;
    ...
    void TPotentialsView::CmSetManageRangeCursor(owlwparam wparam)
    {
        ...
        case IDB_RANGESEQ:
            ...
            if (ManageRangeCursor == MRCSequence)
                ManageRangeCursor = MRCSequenceOverlay;
            else if (ManageRangeCursor == MRCSequenceOverlay)
