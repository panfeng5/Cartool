...
namespace crtl
{
    ...
    bool TBaseVolumeView::SetNumSlices(owlwparam w, bool ShiftKey)
    {
        ...
        else
        {
            ...
            if (ShiftKey)
                slch->SetNumSlices(min(Slices[0].GetRange(), Slices[1].GetRange(), Slices[2].GetRange()));
            else if (ons <= 14)
                slch->AddNumSlices(1);
            else
                slch->AddNumSlices(5);
    ...
    void TBaseVolumeView::MoveSlice(TBaseDoc *BaseDoc, owlwparam w, bool ShiftKey)
    {
        ...
        int whichside = IsSliceModeX() ? XMinSide : IsSliceModeY() ? YMinSide
                                                                   : ZMinSide;

        // then test if it's something "left" "top" or "fron" -> the left side
        bool movefirst = boxsides[whichside] == ToFront || boxsides[whichside] == ToUp || boxsides[whichside] == ToLeft || boxsides[whichside] == ToXMin || boxsides[whichside] == ToYMin || boxsides[whichside] == ToZMin;
    ...
    void TBaseVolumeView::InitClipPlanes(TGLClipPlane ClipPlane[3])
    {
        ...
        if (SliceMode == SliceModeCor || SliceMode == SliceModeTra)
        {
