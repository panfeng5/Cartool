...
namespace crtl
{
                           ...
                           TGLColorTable &colormap, double sizelimit, int level)
    {
        ...
        bool verticesveryclose = fabs(v1[0] - v2[0]) + fabs(v1[1] - v2[1]) + fabs(v1[2] - v2[2]) + fabs(v2[0] - v3[0]) + fabs(v2[1] - v3[1]) + fabs(v2[2] - v3[2]) + fabs(v1[0] - v3[0]) + fabs(v1[1] - v3[1]) + fabs(v1[2] - v3[2]) < sizelimit;
    ...
    void TGLQuadStrip::NextVertices(GLfloat v1[3], GLfloat v2[3], float val1, float val2)
    {
        ...
        else if (Quality == QuadRegularQuality)
        {
            ...
            if (NotFirstCoordinates)
            {
                ...
                ColorTable->GLize(valbuff[MidAll]);
                glVertex3fv(vecbuff[MidAll]);
                ColorTable->GLize(Value3);
                glVertex3fv(Vertex3);
                ColorTable->GLize(valbuff[MidLeft]);
                glVertex3fv(vecbuff[MidLeft]);
                ColorTable->GLize(val1);
                glVertex3fv(v1);
                ColorTable->GLize(valbuff[MidAbove]);
                glVertex3fv(vecbuff[MidAbove]);
                ColorTable->GLize(val2);
                glVertex3fv(v2);
                ColorTable->GLize(valbuff[MidAboveAboveLeft]);
                glVertex3fv(vecbuff[MidAboveAboveLeft]);
                ColorTable->GLize(Value4);
                glVertex3fv(Vertex4);
                ColorTable->GLize(valbuff[MidLeftAboveLeft]);
                glVertex3fv(vecbuff[MidLeftAboveLeft]);
                ColorTable->GLize(Value3);
                glVertex3fv(Vertex3);
                glEnd();
            }
...
#define valueleft Value3
    ...
    void TGLQuadMesh::NextVertex(GLfloat v[3], float val)
    {
        ...
        if (FirstRow)
        { // process the first row, just by storing in a buffer
