...
namespace crtl
{
                              ...
                              GLdouble screenoffsetx, GLdouble screenoffsety, GLdouble screenoffsetz)
    {
        ...
        if (textalign & (TA_TOFRONT))
            p.Z = 1;
        else if (textalign & (TA_TOBACK))
            p.Z = (GLfloat)(-1 + SingleFloatEpsilon); // should be -1, but some drivers don't get up to there...
        else
            p.Z = 1 - 2 * p.Z;
        ...
        for (int i = 0; i < (int)tokens; i++)
        {
            ...
            case TA_BASELINE:
                break;
            case TA_BOTTOM:
                yorig -= BaseLineHeight;
                break;
            case TA_TOP:
                yorig += FontHeight - BaseLineHeight;
                break;
            case TA_CENTERY:
                yorig += FontHeight / 2 - BaseLineHeight;
                break;
            }
            ...
            if (textalign & (TA_BOTTOM | TA_BASELINE))
                p.Y += FontHeight;
            else
                p.Y -= FontHeight;
