...
namespace crtl
{
                           ...
                           TGLBitmapFont *font)
  {
    ...
    for (int ci = 0; ci <= height; ci++)
    {
      ...
      if (ci < ci0)
        GLize(NMax * (ci - ci0) / -ci0);
      else
        GLize(PMax * (ci - ci0) / (height - ci0));
  ...
  {
    return TableType == AbsColorTable_BlackWhite || TableType == AbsColorTable_WhiteBlack || TableType == SignedColorTable_WhiteBlackWhite || TableType == SignedColorTable_BlackWhiteBlack || TableType == SignedColorTable_BlackGrayWhite;
  }
  ...
  void TGLColorTable::SetParameters(double pmax, double nmax, double contrast, double alpha, double maxvalue)
  {
    ...
    if (Alpha == AlphaValueLinear && MaxValue == 0 || Alpha == AlphaBool && MaxValue == 0)
      MaxValue = 1;
  }
  ...
  int TGLColorTable::GetIndex(double value) const
  {
    if (value >= 0)
      if (value <= PMax)
        return ZeroIndex + powl(value / PMax, Contrast) * DeltaIndex;

      else
        return MaxIndex;
    else if (value >= NMax && ZeroIndex)
      return ZeroIndex - powl(value / NMax, Contrast) * DeltaIndex;
  ...
  */
  ...
  int TGLColorTable::GetColorIndex(double value, TGLColor<GLfloat> &glcol) const
  {
    ...
    if (Alpha > AlphaTable)
      glcol.Alpha = Alpha; // slightly negative threshold allows full slices with black background
                           // variable alphas
    else if (Alpha == AlphaTable)
      ;

    else if (Alpha == AlphaLinear)
      glcol.Alpha = palindex == ZeroIndex ? 0 : fabs((double)(palindex - ZeroIndex)) / DeltaIndex;

    else if (Alpha == AlphaLinearSaturated)
      glcol.Alpha = palindex == ZeroIndex ? 0 : NoMore(1.0, 2 * fabs((double)(palindex - ZeroIndex)) / DeltaIndex);

    else if (Alpha == AlphaSquare)
      glcol.Alpha = Square((palindex - ZeroIndex) / (double)DeltaIndex); // the square will remove the sign

    else if (Alpha == AlphaValueLinear)
      glcol.Alpha = fabs(value) / MaxValue;

    else if (Alpha == AlphaBool)
      glcol.Alpha = (bool)value ? MaxValue : 0;

    else if (Alpha == AlphaGreater)
      glcol.Alpha = fabs(value) >= MaxValue ? 0.45 : 0; // alpha value could be given somehow...
                                        ...
                                        InterpolateColorsEnum how, double logstrength)
  {
    ...
    for (i = Index1, toc = Table + i; i <= index2; i++, toc++)
    {
      ...
      if (how == InterpolateColorsLogForward)
      {
        ...
        w = (log(i - Index1 + logstrength) - log(logstrength)) / (log(deltai + logstrength) - log(logstrength));
      ...
      else if (how == InterpolateColorsLogBackward)
      {
        ...
        w = (log(index2 - i + logstrength) - log(logstrength)) / (log(deltai + logstrength) - log(logstrength));
      ...
      { // if ( how == InterpolateColorsLinear ) {
