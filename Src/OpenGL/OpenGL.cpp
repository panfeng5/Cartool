...
namespace crtl
{
    ...
    FormatWish ValueToFormatWish(int v)
    {
        ...
        {
        case -1:
            return FormatUseBest;
        case 0:
            return FormatDontUse;
        case 1:
            return FormatUse;
                                              ...
                                              FormatWish &ask_accumbits, FormatWish &ask_doublebuffer, FormatWish &ask_accel)
    {
        ...
        for (int i = 1; i <= numpf; i++)
        {
            ...
            if (isopengl && iswindow)
                q |= 0x80000000; // without these, that would be a bad starting point...

            if (ask_accel == FormatDontUse && software)
                q |= 0x40000000;

            if ((ask_accel == FormatUseBest || ask_accel >= FormatUse) && accel)
                q |= accelicd ? 0x40000000 // more weight to real (ICD) acceleration
                              : 0x20000000;

            if ((ask_colorbits == FormatDontUse && colorbits == FormatDontUse) || ask_colorbits == FormatUseBest || ask_colorbits >= colorbits)
                q |= 0x00080000;

            if ((ask_depthbits == FormatDontUse && depthbits == FormatDontUse) || ask_depthbits == FormatUseBest || ask_depthbits >= depthbits)
                q |= 0x00040000;

            if ((ask_accumbits == FormatDontUse && accumbits == FormatDontUse) || ask_accumbits == FormatUseBest || ask_accumbits >= accumbits)
                q |= 0x00020000; // more priority to accumulation buffer than stencil

            if ((ask_stencilbits == FormatDontUse && stencilbits == FormatDontUse) || ask_stencilbits == FormatUseBest || ask_stencilbits >= stencilbits)
                q |= 0x00010000;

            if ((ask_doublebuffer == FormatDontUse && doublebuff == FormatDontUse) || ((ask_doublebuffer == FormatUseBest || ask_doublebuffer >= FormatUse) && doublebuff == FormatUse))
                q |= 0x00004000;
            ...
            if (!ispalette)
                q |= 0x00000080;
            if (accelicd)
                q |= 0x00000040; // hardware acceleration
            if (accelmcd)
                q |= 0x00000020; // some acceleration
            if (colorbits >= 16)
                q |= 0x00000010;
            if (depthbits >= 16)
                q |= 0x00000008;
            ...
            if (ask_stencilbits >= FormatUse && stencilbits == FormatDontUse)
                q = 0;
            if (ask_accel >= FormatUse && !accel)
                q = 0;
            if (ask_doublebuffer >= FormatUse && doublebuff == FormatDontUse)
                q = 0;
            if (ask_accumbits >= FormatUse && accumbits == FormatDontUse)
                q = 0;
            ...
            if (forceformat > 0 && i == forceformat

                || forceformat <= 0 && q && q >= best_qual && colorbits >= ask_colorbits // q does not include the # of bits within it (not enough bits!)
                       && depthbits >= ask_depthbits                                     // so we test here the actual various resolutions
                       && stencilbits >= ask_stencilbits && accumbits >= ask_accumbits)
            {
    ...
    void GLDisplayVersion()
    {
        ...
        sprintf(buff, "Version:" Tab Tab "%s" NewLine "Vendor:" Tab Tab "%s" NewLine "Renderer:" Tab "%s" NewLine "Extensions:" Tab "%s",
    ...
    bool GLGetScreenCoord(GLfloat v[3], GLint *viewp, GLdouble *proj, GLdouble *model)
    {
        ...
        if (viewp)
            CopyVirtualMemory(Viewport, viewp, 4 * sizeof(*viewp));
        else
            glGetIntegerv(GL_VIEWPORT, Viewport);

        if (proj)
            CopyVirtualMemory(ProjMatrix, proj, 16 * sizeof(*proj));
        else
            glGetDoublev(GL_PROJECTION_MATRIX, ProjMatrix);

        if (model)
            CopyVirtualMemory(ModelMatrix, model, 16 * sizeof(*model));
        else
