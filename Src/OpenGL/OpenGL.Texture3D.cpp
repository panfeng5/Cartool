    ...
    */
    ...
    void TGLVolume::GLize(const Volume *data, TGLColorTable *colortable, bool interpolate, const double *origin)
    {
        ...
        */
        ...
        if (!tex.IsTextureLoaded() || (newhash != tex.ColorTableHash && !tex.IsPalettedTextureEnable()))
        {
            // create empty texture - it will be deleted when context is deleted
            ...
            */
            ...
            auto InterpolateRGBA = [tex, rescalevaluepos, rescalevalueneg, ColorInterpolPrecBits, ColorInterpolPrec](const MriType &v)
            {
                ...
                return (UINT)((((totex[ci1][0] * w1 + totex[ci2][0] * w2) >> ColorInterpolPrecBits)) | (((totex[ci1][1] * w1 + totex[ci2][1] * w2) >> ColorInterpolPrecBits) << 8) | (((totex[ci1][2] * w1 + totex[ci2][2] * w2) >> ColorInterpolPrecBits) << 16) | (((totex[ci1][3] * w1 + totex[ci2][3] * w2) >> ColorInterpolPrecBits) << 24));
            };
            // expand index values to RGBA
            // same dimensions -> fastest way
            if (datacache->GetLinearDim() == Data->GetLinearDim() && texdownsampling == 1)
            {
            ...
            { // different dimensions -> slower way
    ...
    void TGLVolume::DrawPlaneX(GLfloat x, int quality, bool moreborder)
    {
        ...
        { // use Quads
            ...
            if (Texture[texi].MaxFilter == GL_LINEAR)
