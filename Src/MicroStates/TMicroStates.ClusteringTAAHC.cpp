...
namespace crtl
{
                                        ...
                                        bool ranking)
    {
        ...
        do
        {
            ...
            if (index1 == UndefinedLabel || index2 == UndefinedLabel)
                break;
            ...
            OmpParallelBegin
                ...
                OmpFor for (int aci = lastcorri; aci >= 0; aci--)
            {

                if (allcorr(aci, 0) == index1 || allcorr(aci, 1) == index1 || allcorr(aci, 0) == index2 || allcorr(aci, 1) == index2)

                    allcorr(aci, 0) =
                        allcorr(aci, 1) = UndefinedLabel;

                if (allcorr(aci, 0) > index2)
                    allcorr(aci, 0)--;
                if (allcorr(aci, 1) > index2)
                    allcorr(aci, 1)--;
            }

            // update labeling
            OmpFor for (long tf = 0; tf < NumTimeFrames; tf++)
            {

                if (labels[tf] == index2)
                    labels[tf] = index1; // merge index2 into index1 (the smallest index value)
                else if (labels[tf] > index2)
