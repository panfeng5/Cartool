...
namespace crtl
{
                              ...
                              bool linesassamples, bool variablesshortnames)
    {
        ...
        if (subji == 0)
        {
            ...
            { // R default - do we really want short names in that case?
                ofv << FitSubjectNameLong;
                ofv << LocaleListSeparator;
                                   ...
                                   const char *basefilename)
    {
        ...
        if (writesegfiles)
            CreatePath(BaseDirSegmentation, false);
        if (writeclustersfiles)
            CreatePath(BaseDirDataCluster, false);
        if (writecorrelationfiles)
            CreatePath(BaseDirCorrelation, false);
        if (writestatdurationsfiles)
            CreatePath(BaseDirStatDurations, false);
        if (writesegfrequencyfiles)
            CreatePath(BaseDirSegFrequency, false);
        if (markov)
            CreatePath(BaseDirMarkov, false);
        ...
        Gauge.AddPart(gaugefitinit,
                      gogof.NumFiles(0, gogof.NumGroups() - 1) + gogof.NumFiles(0, numwithinsubjects - 1), 15);
        ...
        if (epochfrom.IsEmpty() || epochto.IsEmpty() || epochmaps.IsEmpty())
        {
        ...
        verbose.NextTopic("Fitting Method:");
        {
            ...
            verbose.Put("Data polarity:", IsAbsolute(datatype) ? "Not relevant" : polarity == PolarityEvaluate ? "Ignore"
                                                                                                               : "Account");
        }
        ...
        verbose.NextTopic("Templates:");
        {
            ...
            verbose.Put("Templates polarity:", IsAbsolute(datatype) ? "Not relevant" : polarity == PolarityEvaluate ? "Ignore"
                                                                                                                    : "Account");
        }
        ...
        verbose.NextTopic("Variables Computed:");
        {
            ...
            if (variablesshortnames)
                StringCopy(buff, FitGroupNameShort "*_" FitMapNameShort "*_");
            else
                StringCopy(buff, FitGroupNameLong "*_" FitMapNameLong "*_");

            for (TIteratorSelectedForward vi(varout); (bool)vi; ++vi)
            {

                if (vi() == fitfonset)
                    verbose.Put("First TF of onset:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitloffset)
                    verbose.Put("Last TF of offset:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitnumtf)
                    verbose.Put("Number of TFs:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fittfcentroid)
                    verbose.Put("TF of centroid:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitmeancorr)
                    verbose.Put("Mean Correlation:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitgev)
                    verbose.Put("Global Explained Variance:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitbcorr)
                    verbose.Put("Best Correlation value:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fittfbcorr)
                    verbose.Put("TF of the Best Correlation:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitgfptfbcorr)
                    verbose.Put("GFP of the TF of Best Correlation:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitmaxgfp)
                    verbose.Put("Max GFP:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fittfmaxgfp)
                    verbose.Put("TF of Max GFP:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitmeangfp)
                    verbose.Put("Mean GFP:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitmeanduration)
                    verbose.Put("Mean Segment Duration:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fittimecoverage)
                    verbose.Put("Time Coverage:", buff, FitVarNames[variablesshortnames][vi()]);
                else if (vi() == fitsegdensity)
                    verbose.Put("Segment Count Density:", buff, FitVarNames[variablesshortnames][vi()]);
            }
        }
        ...
        for (int gofi1 = 0, gofi2 = gofi1 + numwithinsubjects - 1; gofi1 < numgroups && gofi2 < numgroups; gofi1 += numwithinsubjects, gofi2 += numwithinsubjects)
        {
            ...
            if (numwithinsubjects > 1)
                StringCopy(GroupString, InfixGroup "s", IntegerToString(gofi1 + 1), "-", IntegerToString(gofi2 + 1));
            else
                StringCopy(GroupString, InfixGroup, IntegerToString(gofi1 + 1));
                ...
                for (int relg = 0, absg = gofi1 + relg; relg < numwithinsubjects; relg++, absg++)
                {
                    ...
                    for (int frommap = 0, mmi = 0; frommap < numtracks; frommap++)
                        for (int destmap = 0; destmap < numtracks; destmap++, mmi++)
                        {
                            ...
                            if (frommap < nclusters)
                                StringCopy(ton, "M", IntegerToString(frommap + 1));
                            else
                                StringCopy(ton, "M"
                                                "U");
                            ...
                            if (destmap < nclusters)
                                StringAppend(ton, "M", IntegerToString(destmap + 1));
                            else
                                StringAppend(ton, "M"
                                                  "U");
                        }
                    ...
                    for (int relg = 0; relg < NumFiles; relg++)
                    {
                        ...
                        for (long tf = tfmin; tf <= tfmax; tf++)
                            ...
                            if (labels.IsUndefined(tf))
                                SubjTimeFrames(relg, subji, EpochsDurationUnlabeled)++;
                            else
                                SubjTimeFrames(relg, subji, EpochsDurationLabeled)++;
                    }
                }
...
#define UndefinedLabelPositive nclusters
                ...
                auto SegToIndex = [&UndefinedLabelPositive](const LabelType &L)
                { return L == UndefinedLabel ? UndefinedLabelPositive : L; };

                // convert back UndefinedLabelPositive to UndefinedLabel + test for map selection
                auto IsSegOk = [&nclusters, &mapsel](const LabelType &L)
                { return L == UndefinedLabelPositive || (IsInsideLimits(L, 0, nclusters - 1) && mapsel[L]); };
                ...
                for (int relg = 0, f = relg, absg = gofi1 + relg; relg < NumFiles; relg++, f++, absg++)
                {
                    ...
                    if (competitive)
                    {
                        ...
                        for (long tf = tfmin; tf <= tfmax; tf++)
                        {
                            ...
                            if (!(tf == tfmax || labels[tf + 1] != labels[tf]))
                                continue;
                    ...
                    { // competitive - best fit on all segments
                    ...
                    { // competitive - GEV on all segments
                ...
                if (writecorrelationfiles)
                {
                    ...
                    for (int relg = 0, absg = gofi1 + relg; relg < NumFiles; relg++, absg++)
                    {
                        ...
                        StringCopy(corrfile.Filename, basefilename, "."
                                                                    "CorrDataTempl");
                    ...
                    for (TIteratorSelectedForward absgi(groupsel); (bool)absgi; ++absgi, relg++)
                    {
                        ...
                        if (saveonefilepervariable)
                        {
                            ...
                            for (TIteratorSelectedForward vi(varout); (bool)vi; ++vi)
                            {
                                ...
                                if (writeexcelfiles)
                                    WriteVariablesToFile(ExcelFile, var, groupsel2, mapsel, onevar, subji, relg, SubjectName[subji], true, variablesshortnames);
                                if (writeRfiles)
                                    WriteVariablesToFile(RFile, var, groupsel2, mapsel, onevar, subji, relg, SubjectName[subji], false, variablesshortnames);
                            }
                        ...
                        { // ! saveonefilepervariable
                            ...
                            if (writeexcelfiles)
                                WriteVariablesToFile(ExcelFile, var, groupsel2, mapsel, varout, subji, relg, SubjectName[subji], true, variablesshortnames);
                            if (writeRfiles)
                                WriteVariablesToFile(RFile, var, groupsel2, mapsel, varout, subji, relg, SubjectName[subji], false, variablesshortnames);
                        } // ! saveonefilepervariable
                ...
                if (writeclustersfiles)
                {
                    ...
                    for (int nc = 0; nc < nclusters; nc++)
                    {
                        ...
                        StringAppend(expclusters.Filename, "."
                                                           "Cluster");
                    ...
                    for (int relg = 0; relg < numwithinsubjects; relg++)
                        for (int subji = 0; subji < maxfilespergroup; subji++)
                        {
                            ...
                            for (int vari = 0; vari < NumSegVariables; vari++)
                                ...
                                if (subji < gogof[gofi1 + relg].NumFiles() && tf < SubjTimeFrames(relg, subji, FileDuration))
                ...
                for (int relg = 0, absg = gofi1 + relg; relg < numwithinsubjects; relg++, absg++)
                    for (int subji = 0; subji < maxfilespergroup; subji++)
                    {
                        ...
                        for (int tfi = 0; tfi < freqmapfile.NumTime; tfi++)
                        {
                            ...
                            if (!(tfi >= SubjTimeFrames(relg, subji, FileDuration) - 1 || bigsegdata[subji](relg, SegVarSegment, tfi + 1) != bigsegdata[subji](relg, SegVarSegment, tfi)))
