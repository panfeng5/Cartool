...
namespace crtl
{
                                    ...
                                    bool complimentaryopen)
    {
        ...
        if (analysis == UnknownAnalysis || modality == UnknownModality
            /*|| samplingtime == UnknownSamplingTime*/) // we can go easy on this one, as it is for info only...
            return false;

        if (clusteringmethod == UnknownClustering || reqminclusters > reqmaxclusters)
            return false;
        ...
        bool isrestingstates = analysis == AnalysisRestingStatesIndiv || analysis == AnalysisRestingStatesGroup;
        ...
        if (isesipreset)
        {
            ...
            if (StringIsEmpty(SPFile) || StringIsEmpty(GreyFile))
            {
                ...
                while (!(SPDoc.IsOpen() && MRIDocBackg.IsOpen()) && ToLastDirectory(path))
                {
                    ...
                    if (!gofspi.GrepFiles(path, grepspi, GrepOptionDefaultFiles))
                        continue;

                    if (!gofvol.GrepFiles(path, grepvol, GrepOptionDefaultFiles))
                        continue;
            ...
            HasEsiFiles = MRIDocBackg.IsOpen() && SPDoc.IsOpen();
        } // isesipreset
        ...
        Gauge.SetRange(gaugesegcluster, clusteringmethod == ClusteringKMeans ? numclusters * numrandomtrials
                                            ...
                                            (NumTimeFrames * (NumTimeFrames - 1)) / 2 // size of triangular matrix
                                                + (NumTimeFrames - minclusters + 1) + (numclusters - 1) * (numclusters - 2));
...
#define verbose mainverb
        ...
        verbose.NextTopic("Clustering Method:");
        {
            ...
            verbose.NextLine();
            if (analysis == AnalysisERP)
                verbose.Put("Files Preset:", "ERPs Segmentation");
            else if (analysis == AnalysisRestingStatesIndiv && samplingtime == UnknownSamplingTime)
                verbose.Put("Files Preset:", "Resting States, First Stage on Individuals");
            else if (analysis == AnalysisRestingStatesIndiv && samplingtime == SamplingTimeWhole)
                verbose.Put("Files Preset:", "Resting States, First Stage on Individuals, using whole dataset");
            else if (analysis == AnalysisRestingStatesIndiv && samplingtime == SamplingTimeResampling)
                verbose.Put("Files Preset:", "Resting States, First Stage on Individuals, with resampling");
            else if (analysis == AnalysisRestingStatesGroup && samplingtime == UnknownSamplingTime)
                verbose.Put("Files Preset:", "Resting States, Second Stage on Group(s)");
            else if (analysis == AnalysisRestingStatesGroup && samplingtime == SamplingTimeWhole)
                verbose.Put("Files Preset:", "Resting States, Second Stage on Group(s), using whole dataset");
            else if (analysis == AnalysisRestingStatesGroup && samplingtime == SamplingTimeResampling)
                verbose.Put("Files Preset:", "Resting States, Second Stage on Group(s), with resampling");
            ...
            if (IsScalar(datatype))
                ...
                verbose.Put("Data type:", "3D Vectorial Data");
            ...
            verbose.Put("Data polarity:", IsPositive(datatype) ? "Not relevant" : polarity == PolarityEvaluate ? "Ignore"
                                                                                                               : "Account");
        }
        ...
        verbose.NextTopic("Templates:");
        {
            ...
            verbose.NextLine();
            if (IsScalar(datatype))
                verbose.Put("Templates type:", "Signed Data");
            else if (IsPositive(datatype))
                verbose.Put("Templates type:", allrisv ? "Positive Data (Norm of Vectors)" : "Positive Data");
            else if (IsVector(datatype))
                verbose.Put("Templates type:", "3D Vectorial Data");
            ...
            verbose.Put("Templates polarity:", IsPositive(datatype) ? "Not relevant" : polarity == PolarityEvaluate ? "Ignore"
                                                                                                                    : "Account");
        }
        ...
        verbose.NextTopic("Alternative Templates:");
        {
            ...
            if (dualdata)
            {
                ...
                if (IsScalar(dualdatatype))
                    verbose.Put("Alternative dataset type:", "Signed Data");
                else if (IsPositive(dualdatatype))
                    verbose.Put("Alternative dataset type:", "Positive Data");
                else if (IsVector(dualdatatype))
                    verbose.Put("Alternative dataset type:", "3D Vectorial Data");
                ...
                verbose.Put("Alternative dataset polarity:", IsPositive(dualdatatype) ? "Not relevant" : dualpolarity == PolarityEvaluate ? "Ignore"
                                                                                                                                          : "Account");
            }
        }
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // initialize number of clusters, even �f the minclusters is > 1
        ...
        for (nclusters = minclusters; nclusters <= maxclusters; nclusters++)
        {
            ...
            */
            ...
            if (computecriteria && nummaps > 0)
            {
                ...
                var(segCalinskiHarabasz, nclusters) = (critsel[segCalinskiHarabasz] || critsel[segCalinskiHarabaszDeriv] || critsel[segCalinskiHarabaszDerivRobust]) && nclusters > 1 ? ComputeCalinskiHarabasz(nummaps)
                                                                                                                                                                                      : 0;

                var(segCIndex, nclusters) = (critsel[segCIndex] || critsel[segCIndexDeriv] || critsel[segCIndexDerivRobust]) && nclusters > 1 ? ComputeCIndex(nummaps)
                                                                                                                                              : 0;

                var(segCrossValidation, nclusters) = (critsel[segCrossValidation] || critsel[segCrossValidationDeriv] || critsel[segCrossValidationDerivRobust]) && nclusters > 1 ? ComputeCrossValidation(nummaps, NumElectrodes, maps, labels, polarity, 0, NumTimeFrames - 1)
                                                                                                                                                                                  : 0;

                var(segDaviesBouldin, nclusters) = (critsel[segDaviesBouldin] || critsel[segDaviesBouldinDeriv] || critsel[segDaviesBouldinDerivRobust]) && nclusters > 1 ? ComputeDaviesBouldin(nummaps, maps, labels, polarity)
                                                                                                                                                                          : 0;

                var(segDunn, nclusters) = (critsel[segDunn] || critsel[segDunnDeriv] || critsel[segDunnDerivRobust]) && nclusters > 1 ? ComputeDunn(nummaps)
                                                                                                                                      : 0;

                var(segDunnRobust, nclusters) = (critsel[segDunnRobust] || critsel[segDunnRobustDeriv] || critsel[segDunnRobustDerivRobust]) && nclusters > 1 ? ComputeDunnRobust(nummaps)
                                                                                                                                                              : 0;

                var(segSumWPD2, nclusters) = critsel[segKrzanowskiLai] || critsel[segKrzanowskiLaiC] || critsel[segKrzanowskiLaiRobust] || critsel[segKrzanowskiLaiCRobust] ? ComputeKrzanowskiLaiW(nummaps, maps, labels, polarity)
                                                                                                                                                                            : 0;
                ...
                var(segMcClain, nclusters) = (critsel[segMcClain] || critsel[segMcClainDeriv] || critsel[segMcClainDerivRobust]) && nclusters > 1 ? ComputeMcClain(nummaps)
                                                                                                                                                  : 0;

                var(segPointBiserial, nclusters) = (critsel[segPointBiserial] || critsel[segPointBiserialDeriv] || critsel[segPointBiserialDerivRobust]) && nclusters > 1 ? ComputePointBiserial(nummaps, labels)
                                                                                                                                                                          : 0;

                var(segRatkowski, nclusters) = (critsel[segRatkowski] || critsel[segRatkowskiDeriv] || critsel[segRatkowskiDerivRobust]) && nclusters > 2 ? ComputeRatkowski(nummaps, maps, labels, polarity)
                                                                                                                                                          : 0;

                var(segSilhouettes, nclusters) = (critsel[segSilhouettes] || critsel[segSilhouettesDeriv] || critsel[segSilhouettesDerivRobust]) && nclusters > 1 ? ComputeSilhouettes(nummaps, labels)
                                                                                                                                                                  : 0;

                var(segTraceW, nclusters) = critsel[segTraceW] && nclusters > 1 ? ComputeTraceW(nummaps)
                                                                                : 0;
                ...
                if (critsel[segGamma] || critsel[segGammaDeriv] || critsel[segGammaDerivRobust] || critsel[segGPlus] || critsel[segTau])

                    Gamma = nclusters > 1 ? ComputeGamma(nummaps, GPlusCriterion, Tau)
                                          : 0;

                var(segGamma, nclusters) = (critsel[segGamma] || critsel[segGammaDeriv] || critsel[segGammaDerivRobust]) && nclusters > 1 ? Gamma
                                                                                                                                          : 0;
                var(segGPlus, nclusters) = critsel[segGPlus] && nclusters > 1 ? GPlusCriterion
                                                                              : 0;
                var(segTau, nclusters) = critsel[segTau] && nclusters > 1 ? Tau
                                                                          : 0;
...
#define verbose detailverb
            ...
            verbose.NextTopic("Output Files:");
            {
                ...
                StringCopy(buff, BaseFileNameMoreCluster, "."
                                                          "AllTemplates",
                           "." DefaultMriExt);
                if (writevolumes)
            ...
            */
            ...
            if (writeclustersfiles)
            {
                ...
                for (int nc = 0; nc < tfpercluster.GetDim1() - 1; nc++)
                {
                    ...
                    if (nc == 0)
                        StringAppend(expclusters.Filename, ".Unlabeled");
                    else
                        StringAppend(expclusters.Filename, ".Cluster", IntegerToString(nc, 2));
                    // we're saving only the processing data
                    //          if ( dualversions )
                    //              if ( dualgofopen )  StringAppend ( expclusters.Filename, ZScoreInfix );
                    if (dataallzscore)
                        StringAppend(expclusters.Filename, ZScoreInfix);
                    ...
                    if (tfpercluster[nc] == 0)
                    {
                        ...
                        if (writeemptyclusters && (nc > 0 || dolimitcorr))
                        { // accounting for the non-labeled case - dolimitcorr is curretly the only way to have unlabeled time points
                    ...
                    for (long tf = 0; tf < NumTimeFrames; tf++)
                    {
                        ...
                        if (!(nc == 0 && labels.IsUndefined(tf) || nc > 0 && labels[tf] == nc - 1))
                            continue;
            ...
            if (writevolumes && HasEsiFiles)
            {
                ...
                StringCopy(expvol.Filename, BaseFileNameMoreCluster, "."
                                                                     "AllTemplates",
                           "." DefaultMriExt);
            ...
            if (HasEsiFiles && MRIDocBackg->GetGeometryTransform() && StringIs(MRIDocBackg->GetGeometryTransform()->Name, "Talairach") && (bool)SPDoc->GetDisplaySpaces())
            {
                ...
                for (int nc = 0; nc < nummaps; nc++)
                {
                    ...
                    for (int e = 0; e < NumElectrodes; e++)
                    {
                        ...
                        for (int tgi = 0; tgi < NumTalairachGroups; tgi++)
                            ...
                            if (talcodes[tgi])
                                talbucket(talcodes[tgi], tgi) += mapt[e];
                    }
