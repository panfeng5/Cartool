...
namespace crtl
{
                               ...
                               double scale)
    {
        ...
        for (int ncl = minclusters; ncl <= maxclusters; ncl++)
        {
            ...
            if (IsFlag(flag, CriterionDerivativePrime))
                datao[ncl] = (vp1 - vm1) / 2;
            else if (IsFlag(flag, CriterionDerivativeSecond))
                datao[ncl] = vm1 + vp1 - 2 * v0;
            else
                datao[ncl] = 0;
                            ...
                            NormalizeCurveFlag minflag, NormalizeCurveFlag maxflag, bool ignorenull)
    {
        ...
        else
        {
            ...
            else if (maxflag == NormalizeTo0 || maxflag == NormalizeToE)
            {
...

    {
        ...
        for (int nc = 0; nc < numcl; nc++)
        {
            ...
            if (nc == 0 || nc != 0 && sortdata(nc, SortDataValue) != sortdata(nc - 1, SortDataValue))
        ...
        for (int ri = 0; ri < numdata; ri++)
            ...
            if (flag == RankCriterionLinear)
                datao[(int)sortdata(ri, SortDataIndex)] = (numranks + 1 - sortdata(ri, SortDataRank)) / (double)numranks; // weight by Reciprocal Rank (with non-null values)  3/3   2/3   1/3
            else if (flag == RankCriterionSquare)
                datao[(int)sortdata(ri, SortDataIndex)] = Square((numranks + 1 - sortdata(ri, SortDataRank)) / (double)numranks); // weight by relative rank (non-null values)  3/3   2/3   1/3
            else if (flag == RankCriterionInverse)
                datao[(int)sortdata(ri, SortDataIndex)] = 1 / (double)(sortdata(ri, SortDataRank)); // absolute weight for winners
    }
                                ...
                                TEasyStats *indexall)
    {
        ...
        bool scannoncluster = statnoncluster != 0 || statall != 0 || indexall != 0;
        ...
        for (long tf = downmaps.From, Wj = -1; tf <= downmaps.To; tf += downmaps.Step)
        {
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            if (IsFlag(flags, WFromCentroid))
            { // compute all distance from centroid (template)
                ...
                if (statcluster && labels[tf] == nc)
                    statcluster->Add(d, ThreadSafetyIgnore);
                if (statnoncluster && labels[tf] != nc)
                    statnoncluster->Add(d, ThreadSafetyIgnore);
                if (statall)
                    statall->Add(d, ThreadSafetyIgnore);
                if (indexall)
                    indexall->Add(tf, ThreadSafetyIgnore);
            } // WFromCentroid
            ...
            else if (IsFlag(flags, WFlag(WPooled | WPooledDeterminant)))
            {
                ...
                for (long tf2 = tf + downmaps.Step, Wi = Wj; tf2 <= downmaps.To; tf2 += downmaps.Step)
                {
                    ...
                    if (IsFlag(flags, WPooled))
                    {
                        ...
                        if (statcluster && labels[tf] == nc && labels[tf2] == nc)
                            statcluster->Add(d, ThreadSafetyIgnore);
                        if (statnoncluster && labels[tf] != labels[tf2])
                            statnoncluster->Add(d, ThreadSafetyIgnore);
                        if (statall)
                            statall->Add(d, ThreadSafetyIgnore);
                        if (indexall)
                        {
                            indexall->Add(tf, ThreadSafetyIgnore);
                            indexall->Add(tf2, ThreadSafetyIgnore);
                        }
                    }
        ...
        if (IsFlag(flags, WPooledDeterminant))
        {
            ...
            if (statcluster)
                statcluster->Add(logdet /*det*/, ThreadSafetyIgnore);
        } // WPooledDeterminant
    }
        ...
        OmpSectionEnd
            ...
            OmpParallelSectionsEnd
                ...
                OmpParallelSectionsBegin
                    ...
                    OmpSectionBegin var(segWCD, nclusters) = StatWCentroidDistance.Median();
        OmpSectionEnd
            OmpSectionBegin var(segWCD2, nclusters) = StatWCentroidSquareDistance.Median();
        OmpSectionEnd
            OmpSectionBegin var(segBCD2, nclusters) = StatBCentroidSquareDistance.Median();
        OmpSectionEnd
            OmpSectionBegin var(segWPD, nclusters) = TEasyStats(StatWPooledDistance).Median();
        OmpSectionEnd // !work with a copy to keep the original ordering!
            OmpSectionBegin var(segBPD, nclusters) = TEasyStats(StatBPooledDistance).Median();
        OmpSectionEnd // !work with a copy to keep the original ordering!
            OmpSectionBegin var(segAPD, nclusters) = TEasyStats(StatAPooledDistance).Median();
        OmpSectionEnd // !work with a copy to keep the original ordering!
            OmpSectionBegin var(segWPD2, nclusters) = StatWPooledSquareDistance.Median();
        OmpSectionEnd
            OmpSectionBegin var(segWPdD2, nclusters) = 0; /*StatWPooledDetSquareDistance  .Median ();*/
        OmpSectionEnd
    ...
    */
    ...
    double TMicroStates::ComputePointBiserial(int nclusters, TLabeling &labels)
    {
        ...
        // same labels -> false
        auto AreDifferentLabels = [](const auto &L1, const auto &L2)
        { return L1 == L2 ? 0 : 1; };
                                      ...
                                      double &GPlusCriterion, double &Tau)
    {
        ...
        */
        ...
        //                                          * ( numpairsa - numpairsb ) );
        //  Rohlf
        double tauden = sqrt((nn1 / 2.0 - numpairsa) * (nn1 / 2.0));
