...
namespace crtl
{
        ...
        try
        {
            ...
            do
            {
                ...
                for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc != 0; doc = CartoolDocManager->DocListNext(doc))
                    ...
                    if (StringIs(doc->GetTemplate()->GetDefaultExt(), FILEEXT_EEGEPH) || StringIs(doc->GetTemplate()->GetDefaultExt(), FILEEXT_EEGEP))
                    {
    ...
    void TCartoolMdiClient::CmWinAction(owlwparam w)
    {
        ...
        double scale = w == CM_WINSIZE_DEC || w == CM_WINSIZE_DECL || w == CM_WINSIZE_DECR || w == CM_WINSIZE_DECT || w == CM_WINSIZE_DECB ? PowerRoot(2, -4) // done 4 times, factor will halve..
        ...
        int width = w == CM_WINSIZE_INCT || w == CM_WINSIZE_INCB || w == CM_WINSIZE_DECT || w == CM_WINSIZE_DECB ? r.Width()
                                                                                                                 : Round(r.Width() * scale);
        int height = w == CM_WINSIZE_INCL || w == CM_WINSIZE_INCR || w == CM_WINSIZE_DECL || w == CM_WINSIZE_DECR ? r.Height()
        ...
        int top = w == CM_WINSIZE_INCT || w == CM_WINSIZE_DECT ? r.Top() - (height - r.Height())
                                                               : r.Top();
        int left = w == CM_WINSIZE_INCL || w == CM_WINSIZE_DECL ? r.Left() - (width - r.Width())
                                                                : r.Left();
    ...
    void TCartoolMdiClient::CmGroupWinAction(owlwparam w)
    {
        ...
        if (w == IDB_GTVKEEPSIZE)
            currentview->GODoc->GroupTileViews(CombineFlags(GroupTilingViews_Move, GroupTilingViews_Insert));
        else if (w == IDB_GTVSTANDSIZE)
            currentview->GODoc->GroupTileViews(CombineFlags(GroupTilingViews_StandSize, GroupTilingViews_Insert));
        else if (w == IDB_GTVFIT)
            currentview->GODoc->GroupTileViews(CombineFlags(GroupTilingViews_BestFitSize, GroupTilingViews_Insert));
    ...
    void TCartoolMdiClient::CmNewLMFromExisting()
    {
        ...
        for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc != 0; doc = CartoolDocManager->DocListNext(doc))
        {
            ...
            if (mridoc && mridoc->IsBinaryMask() && lmdoc->AddToGroup(mridoc, false))
            {
                // hide these guys
        ...
        for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc != 0; doc = CartoolDocManager->DocListNext(doc))
        {
            ...
            if (mridoc && mridoc->IsSegmented() && !mridoc->IsMask() && lmdoc->AddToGroup(mridoc, false))
            {
                // hide these guys
                mridoc->MinimizeViews();
                lmdoc->RefreshWindows(false);
        ...
        for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc != 0; doc = CartoolDocManager->DocListNext(doc))
        {
            ...
            if (lmdoc->AddToGroup(doc, false) && IsExtensionAmong(doc->GetDocPath(), AllCoordinatesFilesExt
    ...
    void TCartoolMdiClient::CmInteractiveAveraging()
    {
        ...
        if (TAvgTransfer.LastDialogId == IDD_AVERAGE1)
            TTracksAveragingFilesDialog(this, IDD_AVERAGE1).Execute();
        else if (TAvgTransfer.LastDialogId == IDD_AVERAGE2)
            TTracksAveragingParamsDialog(this, IDD_AVERAGE2).Execute();
        else
            TTracksAveragingOutputsDialog(this, IDD_AVERAGE3).Execute();
    }
    ...
    void TCartoolMdiClient::CmStatistics(owlwparam w)
    {
        ...
        if (StatTransfer.LastDialogId == IDD_STATISTICS1)
            TStatisticsFilesDialog(this, IDD_STATISTICS1, w).Execute();
        else
            TStatisticsParamsDialog(this, IDD_STATISTICS2).Execute();
    }
    ...
    void TCartoolMdiClient::CmSegmentEeg()
    {
        ...
        if (SegTransfer.LastDialogId == IDD_SEGMENT1)
            TMicroStatesSegFilesDialog(this, IDD_SEGMENT1).Execute();
        else
            TMicroStatesSegParamsDialog(this, IDD_SEGMENT2).Execute();
    }
    ...
    void TCartoolMdiClient::CmFitting()
    {
        ...
        else if (FitTransfer.LastDialogId == IDD_FITTING2)
            TMicroStatesFitParamsDialog(this, IDD_FITTING2).Execute();
        else
            TMicroStatesFitResultsDialog(this, IDD_FITTING3).Execute();
    }
