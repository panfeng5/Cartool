...
namespace crtl
{
    ...
    FileRegistrationInfo ExtReg[NumExtensionRegistrations] =
        {
            ...
            {FILEEXT_LM, " "
                         "Cartool Link Many",
             IDI_LM, OpenCartool | OpenNotePad},
    ...
    void TCartoolApp::SetScreen(const char *optmonitorname)
    {
        ...
        if (StringIsEmpty(optmonitorname))
            GetCurrentMonitorName(CartoolMainWindow->GetHandle(), monitorname);
        else
            StringCopy(monitorname, optmonitorname, CCHDEVICENAME - 1);
    ...
    void TCartoolApp::InitInstance()
    {
        ...
        DefineCLIOptionEnum(toapp, "", __mainwindow, "Main window initial state")
            ->DefaultString(DefaultWindowState == SW_SHOWMAXIMIZED ? __maximized : DefaultWindowState == SW_SHOWMINIMIZED ? __minimized
                                                                                                                          : __normal)
        ...
        catch (const CLI::ParseError &e)
        {
            ...
            PrintConsole(string("Error in command-line parameters: ") + /*CmdLine*/ e.what() + NewLine + NewLine + "See the correct command-line syntax by calling either:" NewLine + NewLine + ToFileName(ApplicationFullPath) + " " + __help + NewLine + ToFileName(ApplicationFullPath) + " <subcommand> " + __help + NewLine);
        ...
        if (HasCLIOption(toapp, __help) // redefined NOT as a flag
            || HasCLIFlag(regsub, __help) || HasCLIFlag(reprocsub, __help) || HasCLIFlag(computingrissub, __help) || HasCLIFlag(ristovolsub, __help))
        {

            string showhelp = GetCLIOptionString(toapp, __help);
            string helpmessage;

            if (HasCLIFlag(regsub, __help))
                helpmessage = regsub->help(); // <subcommand>  --help
            else if (HasCLIFlag(reprocsub, __help))
                helpmessage = reprocsub->help();
            else if (HasCLIFlag(computingrissub, __help))
                helpmessage = computingrissub->help();
            else if (HasCLIFlag(ristovolsub, __help))
                helpmessage = ristovolsub->help();
            else if (showhelp.empty())
                helpmessage = app.help(); // <application> --help

            else
                try
                { // try some specialized help message
        ...
        else if (IsSubCommandUsed(regsub))
        {
            ...
            if (HasCLIFlag(regsub, __yes)) /* PrintConsole    ( GetCLIOptionDescription ( regsub, "--yes"   ) ); */
                RegisterInfo();
            else if (HasCLIFlag(regsub, __no)) /* PrintConsole    ( GetCLIOptionDescription ( regsub, "--no"    ) ); */
                UnRegisterInfo();
            else if (HasCLIFlag(regsub, __reset)) /* PrintConsole    ( GetCLIOptionDescription ( regsub, "--reset" ) ); */
                ResetRegisterInfo();
        ...
        if ((nCmdShow == SW_SHOWMAXIMIZED || nCmdShow == SW_SHOWNORMAL || nCmdShow == SW_SHOWDEFAULT) && IsInteractive() && !HasCLIFlag(toapp, __nosplash))

            CreateSplashScreen();

        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // BEFORE window creation
        if (HasCLIOption(toapp, __mainwindow) || HasCLIOption(toapp, __mainwindowsize) || HasCLIOption(toapp, __mainwindowpos) || HasCLIOption(toapp, __monitor))
        {
            ...
            if (mw == __maximized)
                nCmdShow = SW_SHOWMAXIMIZED; // overriding main window state before creation
            else if (mw == __minimized)
                nCmdShow = SW_SHOWMINIMIZED;
            else if (mw == __normal || hasmwsize // also switching to normal window if any of these were specified
                     || hasmwpos)
                nCmdShow = SW_SHOWNORMAL;
            else
                nCmdShow = DefaultWindowState; // fall back to Cartool default
        }
        ...
        if (HasCLIOption(toapp, __mainwindowsize) || HasCLIOption(toapp, __mainwindowpos) || HasCLIOption(toapp, __monitor))
        {
        ...
        if (HasCLIOption(toapp, __files))
        {
            ...
            for (int filei = 0; filei < (int)gof; filei++)
            {
                ...
                if (haschw)
                {
                    ...
                    if (chw[i] == __maximized)
                        view->SetWindowState(WindowStateMaximized);
                    else if (chw[i] == __minimized)
                        view->SetWindowState(WindowStateMinimized);
                }
    ...
    void TCartoolApp::CmPrefsRegistry(owlwparam w)
    {
        ...
        if (w == CM_RESETREGISTRY)
        {
            ...
            if (!GetAnswerFromUser("Do you want to associate all known files (.xyz .hdr etc...) with Cartool?" NewLine NewLine
        ...
        else if (w == CM_CLEARREGISTRY)
        {
            ...
            if (!GetAnswerFromUser("Do you want to remove all known files associations of Cartool?" NewLine NewLine
    ...
    {
        if (w == CM_HELPCONTENTS)
            ShellExecute(NULL, "open", TFileName(WebPageCartoolHelp) + "/reference-guide.html", NULL, NULL, SW_SHOWNORMAL);
        ...
        else if (w == CM_HELPCARTOOLGITHUB)
            ShellExecute(NULL, "open", WebPageCartoolGitHub, NULL, NULL, SW_SHOWNORMAL);
        else if (w == CM_HELPCARTOOLCOMMUNITY)
            ShellExecute(NULL, "open", WebPageCartool, NULL, NULL, SW_SHOWNORMAL);
        else if (w == CM_HELPWEBRELEASES)
            ShellExecute(NULL, "open", TFileName(WebPageCartoolGitHub) + "/releases/latest", NULL, NULL, SW_SHOWNORMAL);
        else if (w == CM_HELPWEBCIBM)
            ShellExecute(NULL, "open", "https://cibm.ch/", NULL, NULL, SW_SHOWNORMAL);
    }
    ...
    void TCartoolApp::EvDisplayChange(uint, uint /*resx*/, uint /*resy*/)
    {
        ...
        { // Not-so-easy case: non-maximized window has to be manually resized
    ...
    {
        if ((childratiox == 0 || childratiox == 1) && (childratioy == 0 || childratioy == 1))
            return;
