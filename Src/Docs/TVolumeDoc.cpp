...
namespace crtl
{
    ...
    char *GetMriContentTypeName(int ect, char *s)
    {
        ...
        // Main type
        if (IsUnknownMriType(ect))
            StringCopy(s, MriContentTypeNames[0]);
        else if (IsFullHead(ect))
            StringCopy(s, MriContentTypeNames[1]);
        else if (IsSegmented(ect))
            StringCopy(s, MriContentTypeNames[2]);

        // Additional sub-type
        if (IsBinaryMask(ect))
            StringAppend(s, ", ", MriContentTypeNames[3]);
        else if (IsRoiMask(ect))
            StringAppend(s, ", ", MriContentTypeNames[4]);
        else if (IsGrey(ect))
            StringAppend(s, ", ", MriContentTypeNames[5]);
        else if (IsWhite(ect))
            StringAppend(s, ", ", MriContentTypeNames[6]);
        else if (IsBrain(ect))
            StringAppend(s, ", ", MriContentTypeNames[7]);
        else if (IsBlob(ect))
            StringAppend(s, ", ", MriContentTypeNames[8]);
        else if (IsRis(ect))
            StringAppend(s, ", ", MriContentTypeNames[9]);
    ...
    bool TVolumeDoc::IsFullHead() const
    {
        ...
        bool issegmentname = crtl::IsGrey(guessmriname) // if we get lucky, file contains something like "Brain" or "Grey"
                             || crtl::IsWhite(guessmriname) || crtl::IsRoiMask(guessmriname) || crtl::IsBrain(guessmriname);
    ...
    const char *TVolumeDoc::GetContentTypeName(char *s) const
    {
        ...
        if (IsUnknownMriType())
            StringCopy(s, ContentNames[ContentType]); // just to show something
        else
            GetMriContentTypeName(ExtraContentType, s);
    ...
    void TVolumeDoc::SetMri(MriInitFlags flags, double backgroundvalue)
    {
        ...
        if (TessSurfacePos.IsNotEmpty() || TessSurfaceNeg.IsNotEmpty())
    ...
    void TVolumeDoc::InitLimits(InitType /*how*/)
    {
        ...
        else
        {
            ...
            if (MinValue >= 0)
                ...
                SetAtomType(AtomTypeScalar);
            // by safety
            else if (IsUnknownType(AtomTypeUseCurrent))
                SetAtomType(OriginalAtomType);
        } // ! IsUnknownType
    ...
    {
        if (IsBlob())
            IsoSurfaceCut = BackgroundValue + (MaxValue - BackgroundValue) * 0.333; // not used since long
        else if (IsRis())
            IsoSurfaceCut = RISBackgroundValue; // epsilon
        else if (IsMask())
            IsoSurfaceCut = max(MaskBackgroundValue, MinValue, -MaxValue); // at least 1 - also accounting for negative data
        else
            IsoSurfaceCut = BackgroundValue; // all other cases
    }
    ...
    void TVolumeDoc::SetContentType()
    {
        ...
        if (StringContains(GetTitle(), "." FILEEXT_RIS "."))
            ExtraContentType = MriContentTypeSegmented | MriContentTypeRis;

        // Guessing from Nifti header
        else if (StringIsNot(NiftiIntentName, NiftiIntentNameDefault))
        {

            if (StringIs(NiftiIntentName, NiftiIntentNameRis))
                ExtraContentType = MriContentTypeSegmented | MriContentTypeRis;
            else if (StringIs(NiftiIntentName, NiftiIntentNameBrain))
                ExtraContentType = MriContentTypeSegmented | MriContentTypeBrain;
            else if (StringIs(NiftiIntentName, NiftiIntentNameGreyMask))
                ExtraContentType = MriContentTypeSegmented | MriContentTypeGrey;
            else if (StringIs(NiftiIntentName, NiftiIntentNameLabels))
                ExtraContentType = MriContentTypeSegmented | (MaxValue == 1 ? MriContentTypeBinaryMask
                                                                            : MriContentTypeRoiMask);
            else if (StringIs(NiftiIntentName, NiftiIntentNameSkull))
                ExtraContentType = MriContentTypeSegmented; // | MriContentTypeSkull;
        }

        // Guessing from Nifti header
        else if (NiftiIntentCode == NIFTI_INTENT_LABEL)
            ExtraContentType = MriContentTypeSegmented | MriContentTypeRoiMask;

        // In last resort, guessing by content, which is a tiny bit more adventurous...
        else
            ExtraContentType = Data.GuessTypeFromContent(BackgroundValue, Bounding);
    }
    ...
    void TVolumeDoc::SetIsoSurface()
    {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // any negative data?
        if (GetMinValue() < 0 && isoparam.How != IsosurfaceBox)
        { // !do not compute twice the big box!
          // 0 is OK
            isoparam.IsoValue = -fabs(IsoSurfaceCut);
    ...
    void TVolumeDoc::SetGeometryTransform()
    {
        ...
        if (StringIs(extension, FILEEXT_MRINII))
        {
            ...
            // Otherwise, do it the old way by file name guessing
                     ...
                     || StringContains((const char *)filename, InfixToCoregistered "mni_icbm152_") || StringContains((const char *)filename, InfixToCoregistered "MNI152_"))
        ...
        else if (StringIs(extension, FILEEXT_MRIAVW_HDR))
        {
            ...
            else if (StringStartsWith((const char *)filename, "MNI152.Nlin") || StringStartsWith((const char *)filename, "NIHPD324.NlinAsym") || StringContains((const char *)filename, InfixToCoregistered "MNI152.Nlin"))

                GeometryTransform = new TGeometryTransform(TalairachMNI152_2009);

            else if (StringStartsWith((const char *)filename, "MNI152T1") || StringContains((const char *)filename, InfixToCoregistered "MNI152T1"))

                GeometryTransform = new TGeometryTransform(TalairachMNI152_2001);

            else if (StringStartsWith((const char *)filename, "NIHPD.Asym.Infant") || StringContains((const char *)filename, InfixToCoregistered "NIHPD.Asym.Infant"))
                                 ...
                                 const char *title)
    {
        ...
        if ((ma & HasTranslation) && transform.GetTranslationX() < 0 // restrict to these cases
            && transform.GetTranslationY() < 0 && transform.GetTranslationZ() < 0)
        {
    ...
    void TVolumeDoc::NewIsoSurfaceCut(bool automatic, double bt)
    {
        ...
        if (automatic)
        {
            ...
            if (*answer == 'A')
                isosurfacecut = 0; // !keep it automatic here!
            else
            {
                isosurfacecut = StringToDouble(answer);
                automatic = false;
            }
        }
        else
            ...
            for (int z = 0; z < Size.GetZExtent(); z++)
                for (int y = 0; y < Size.GetYExtent(); y++)
                {
                    ...
                    for (int x = 0; x < Size.GetXExtent(); x++)
                        d[x] = Data(x, y, z);
                    for (int x = 0; x < Size.GetXExtent(); x++)
                        Data(x, y, z) = d[Size.GetXExtent() - 1 - x];
                }
            break;
            ...
            for (int z = 0; z < Size.GetZExtent(); z++)
                for (int x = 0; x < Size.GetXExtent(); x++)
                {
                    ...
                    for (int y = 0; y < Size.GetYExtent(); y++)
                        d[y] = Data(x, y, z);
                    for (int y = 0; y < Size.GetYExtent(); y++)
                        Data(x, y, z) = d[Size.GetYExtent() - 1 - y];
                }
            break;
            ...
            for (int y = 0; y < Size.GetYExtent(); y++)
                for (int x = 0; x < Size.GetXExtent(); x++)
                {
                    ...
                    for (int z = 0; z < Size.GetZExtent(); z++)
                        d[z] = Data(x, y, z);
                    for (int z = 0; z < Size.GetZExtent(); z++)
                        Data(x, y, z) = d[Size.GetZExtent() - 1 - z];
                }
            break;
    ...
    bool TVolumeDoc::IsValidOrigin(const TPointDouble &origin) const
    {
        ...
        if ((origin.X <= Size.XMin() || origin.X >= Size.XMax()) || (origin.Y <= Size.YMin() || origin.Y >= Size.YMax()) || (origin.Z <= Size.ZMin() || origin.Z >= Size.ZMax()))
            return false;
        ...
        double exc = sqrt(Square((2 * origin.X - Size.XMax()) / Size.XMax()) + Square((2 * origin.Y - Size.YMax()) / Size.YMax()) + Square((2 * origin.Z - Size.ZMax()) / Size.ZMax()));
                                               ...
                                               TStrings &tentennames) const
    {
        ...
        for (int alphai = 0; alphai < numstepsalpha - 1; alphai++)
        {
            ...
            if (frontbackdist[alphai] <= czcl && frontbackdist[alphai + 1] > czcl)
                cz = frontbackline[alphai];
            if (frontbackdist[alphai] <= fpzcl && frontbackdist[alphai + 1] > fpzcl)
                fpz = frontbackline[alphai];
            if (frontbackdist[alphai] <= ozcl && frontbackdist[alphai + 1] > ozcl)
                oz = frontbackline[alphai];
        }

        // results in absolute original coordinates
        tentenpositions.Add(cz);
        tentennames.Add("Cz");
        tentenpositions.Add(fpz);
        tentennames.Add("Fpz");
        tentenpositions.Add(oz);
        tentennames.Add("Oz");
        ...
        for (int alphai = 0; alphai < numstepsalpha - 1; alphai++)
        {
            ...
            if (rightleftdist[alphai] <= t8cl && rightleftdist[alphai + 1] > t8cl)
                t8 = rightleftline[alphai];
            if (rightleftdist[alphai] <= t7cl && rightleftdist[alphai + 1] > t7cl)
                t7 = rightleftline[alphai];
        }
        ...
        tentenpositions.Add(t8);
        tentennames.Add("T8");
        tentenpositions.Add(t7);
        tentennames.Add("T7");
    }
    ...
    void TVolumeDoc::Filter(FilterTypes filtertype)
    {
        ...
        if (filtertype == FilterTypeMean || filtertype == FilterTypeGaussian || filtertype == FilterTypeFastGaussian || filtertype == FilterTypeAnisoGaussian)
        {
            // Trick: Fast Gaussian can have negative values, used as the number of repetitions
            if (!GetValueFromUser("Diameter of the filter (any value >= 2.0):", FilterPresets[filtertype].Text, p(FilterParamDiameter), "3.47", CartoolMainWindow) || !(filtertype == FilterTypeFastGaussian || p(FilterParamDiameter) > 0))
                return;

            // here, we bypass user's will to boost the processing, or make sure we filter something...
            if (filtertype == FilterTypeGaussian && p(FilterParamDiameter) >= 4)
                filtertype = FilterTypeFastGaussian;
            else if (filtertype == FilterTypeFastGaussian && p(FilterParamDiameter) < 4)
                filtertype = FilterTypeGaussian;

            StringCopy(NiftiIntentName, NiftiIntentNameDefault);
        }

        else if (filtertype == FilterTypeIntensityMult || filtertype == FilterTypeIntensityDiv)
        {

            if (!GetValueFromUser(filtertype == FilterTypeIntensityMult ? "Intensity multiplying factor:" : "Intensity dividing factor:", FilterPresets[filtertype].Text, p(FilterParamOperand), "", CartoolMainWindow) || p(FilterParamOperand) == 1.0)
                return;
        }

        else if (filtertype == FilterTypeIntensityAdd || filtertype == FilterTypeIntensitySub)
        {

            if (!GetValueFromUser(filtertype == FilterTypeIntensityAdd ? "Intensity adding value:" : "Intensity subtracting value:", FilterPresets[filtertype].Text, p(FilterParamOperand), "", CartoolMainWindow) || p(FilterParamOperand) == 0.0)
                return;
        }
        ...
        else if (filtertype == FilterTypeMin || filtertype == FilterTypeMax || filtertype == FilterTypeMinMax || filtertype == FilterTypeRange || filtertype == FilterTypeMedian || filtertype == FilterTypeInterquartileMean || filtertype == FilterTypeMaxMode || filtertype == FilterTypeMeanSub || filtertype == FilterTypeLogMeanSub || filtertype == FilterTypeMeanDiv || filtertype == FilterTypeLogMeanDiv || filtertype == FilterTypeMAD || filtertype == FilterTypeSD || filtertype == FilterTypeSDInv || filtertype == FilterTypeMADSDInv || filtertype == FilterTypeCoV || filtertype == FilterTypeLogCoV || filtertype == FilterTypeLogSNR || filtertype == FilterTypeMCoV || filtertype == FilterTypeLogMCoV || filtertype == FilterTypeModeQuant || filtertype == FilterTypeEntropy || filtertype == FilterTypePercentFullness || filtertype == FilterTypeCutBelowFullness || filtertype == FilterTypeSAV || filtertype == FilterTypeCompactness || filtertype == FilterTypeRelax)
        {
            ...
            else if (filtertype == FilterTypeMeanSub || filtertype == FilterTypeLogMeanSub
                     //         || filtertype == FilterTypeMeanDiv
                     || filtertype == FilterTypeLogMeanDiv || filtertype == FilterTypeLogCoV || filtertype == FilterTypeLogSNR || filtertype == FilterTypeLogMCoV)
            {
        ...
        else if (filtertype == FilterTypeThresholdAbove || filtertype == FilterTypeThresholdBelow)
        {
        ...
        else if (filtertype == FilterTypeLessNeighbors || filtertype == FilterTypeMoreNeighbors)
        {
            ...
            if (!GetValueFromUser(filtertype == FilterTypeLessNeighbors ? "Reset voxels having less neighbors than (1..26):" : "Set voxels having more neighbors than (0..26):",
                                  FilterPresets[filtertype].Text, p(FilterParamNumNeighbors), "", CartoolMainWindow) ||
                p(FilterParamNumNeighbors) < 1)
                return;
        ...
        else if (filtertype == FilterTypeKeepBiggestRegion || filtertype == FilterTypeKeepCompactRegion || filtertype == FilterTypeKeepBiggest2Regions || filtertype == FilterTypeKeepCompact2Regions)
        {
            // no parameters
        ...
        else if (filtertype == FilterTypeErode || filtertype == FilterTypeDilate || filtertype == FilterTypeOpen || filtertype == FilterTypeClose || filtertype == FilterTypeMorphGradient || filtertype == FilterTypeMorphGradientInt || filtertype == FilterTypeMorphGradientExt)
        {
        ...
        else if (filtertype == FilterTypeWaterfallRidges || filtertype == FilterTypeWaterfallValleys)
        {

            StringCopy(NiftiIntentName, NiftiIntentNameLabels);
        }

        else if (filtertype == FilterTypeGradient || filtertype == FilterTypeCanny || filtertype == FilterTypeLaplacian || filtertype == FilterTypeHessianEigenMax || filtertype == FilterTypeHessianEigenMin || filtertype == FilterTypeKCurvature || filtertype == FilterTypeKCCurvature)
        {
            ...
            if (filtertype == FilterTypeGradient || filtertype == FilterTypeCanny || filtertype == FilterTypeHessianEigenMax || filtertype == FilterTypeHessianEigenMin)
        ...
        else if (filtertype == FilterTypePeeling || filtertype == FilterTypeRank || filtertype == FilterTypeRankRamp)
        {
        ...
        else if (filtertype == FilterTypeSegmentWhite || filtertype == FilterTypeSegmentGrey || filtertype == FilterTypeSegmentCSF || filtertype == FilterTypeSegmentTissues || filtertype == FilterTypeSkullStripping || filtertype == FilterTypeBrainstemRemoval)
        {
            ...
            if (filtertype == FilterTypeSegmentWhite)
            {
                ...
                if (p(FilterParamWhiteDetails) < 0)
                    return;
        ...
        */
        ...
        bool lowisocut = filtertype == FilterTypeRange || filtertype == FilterTypeMinMax || filtertype == FilterTypeMeanSub || filtertype == FilterTypeLogMeanSub || filtertype == FilterTypeMeanDiv || filtertype == FilterTypeLogMeanDiv || filtertype == FilterTypeMAD || filtertype == FilterTypeSD || filtertype == FilterTypeSDInv || filtertype == FilterTypeMADSDInv || filtertype == FilterTypeCoV || filtertype == FilterTypeLogCoV || filtertype == FilterTypeLogSNR || filtertype == FilterTypeMCoV || filtertype == FilterTypeLogMCoV || filtertype == FilterTypeModeQuant || filtertype == FilterTypeEntropy || filtertype == FilterTypePercentFullness || filtertype == FilterTypeSAV || filtertype == FilterTypeCompactness || filtertype == FilterTypeWaterfallRidges || filtertype == FilterTypeWaterfallValleys || filtertype == FilterTypeGradient || filtertype == FilterTypeCanny || filtertype == FilterTypeLaplacian || filtertype == FilterTypeHessianEigenMax || filtertype == FilterTypeHessianEigenMin || filtertype == FilterTypeKCurvature || filtertype == FilterTypeKCCurvature || filtertype == FilterTypeSegmentCSF || filtertype == FilterTypeSegmentGrey || filtertype == FilterTypeSegmentWhite || filtertype == FilterTypeSegmentTissues || filtertype == FilterTypePeeling;
        ...
        if (filtertype == FilterTypeSegmentCSF || filtertype == FilterTypeSegmentGrey || filtertype == FilterTypeSegmentWhite || filtertype == FilterTypeSegmentTissues)
    ...
    void TVolumeDoc::FindOrientation()
    {
        ...
        if (IsFullHead())
            FindOrientationFullHead();
        else
            FindOrientationSegmented();
    }
    ...
    void TVolumeDoc::FindOrientationSegmented()
    {
        ...
        for (p1.Y = boundmin.Y; p1.Y <= boundmax.Y; p1.Y++)
            for (p1.Z = boundmin.Z; p1.Z <= boundmax.Z; p1.Z++)
                for (p1.X = center.X + 1, p2.X = center.X - 1; p1.X < boundmax.X && p2.X > boundmin.X; p1.X++, p2.X--)
                    ...
                    stat1.Add(fabs((double)2 * (vol(p2.X, p1.Y, p1.Z) >= back) + (vol(p2.X - 1, p1.Y, p1.Z) >= back) + (vol(p2.X + 1, p1.Y, p1.Z) >= back) - 2 * (vol(p1.X, p1.Y, p1.Z) >= back) - (vol(p1.X - 1, p1.Y, p1.Z) >= back) - (vol(p1.X + 1, p1.Y, p1.Z) >= back)));

        for (p1.X = boundmin.X; p1.X <= boundmax.X; p1.X++)
            for (p1.Z = boundmin.Z; p1.Z <= boundmax.Z; p1.Z++)
                for (p1.Y = center.Y + 1, p2.Y = center.Y - 1; p1.Y < boundmax.Y && p2.Y > boundmin.Y; p1.Y++, p2.Y--)

                    stat2.Add(fabs((double)2 * (vol(p1.X, p2.Y, p1.Z) >= back) + (vol(p1.X, p2.Y - 1, p1.Z) >= back) + (vol(p1.X, p2.Y + 1, p1.Z) >= back) - 2 * (vol(p1.X, p1.Y, p1.Z) >= back) - (vol(p1.X, p1.Y - 1, p1.Z) >= back) - (vol(p1.X, p1.Y + 1, p1.Z) >= back)));

        for (p1.X = boundmin.X; p1.X <= boundmax.X; p1.X++)
            for (p1.Y = boundmin.Y; p1.Y <= boundmax.Y; p1.Y++)
                for (p1.Z = center.Z + 1, p2.Z = center.Z - 1; p1.Z < boundmax.Z && p2.Z > boundmin.Z; p1.Z++, p2.Z--)

                    stat3.Add(fabs((double)2 * (vol(p1.X, p1.Y, p2.Z) >= back) + (vol(p1.X, p1.Y, p2.Z - 1) >= back) + (vol(p1.X, p1.Y, p2.Z + 1) >= back) - 2 * (vol(p1.X, p1.Y, p1.Z) >= back) - (vol(p1.X, p1.Y, p1.Z - 1) >= back) - (vol(p1.X, p1.Y, p1.Z + 1) >= back)));
        ...
        if (symetry(0) >= symetry(1) && symetry(0) >= symetry(2))
        {
            XAxis = LeftRight;
            quality.Add(symetry(0) / (symetry(0) + max(symetry(1), symetry(2))));
        }
        else if (symetry(1) >= symetry(0) && symetry(1) >= symetry(2))
        {
            YAxis = LeftRight;
            quality.Add(symetry(1) / (symetry(1) + max(symetry(0), symetry(2))));
        }
        else if (symetry(2) >= symetry(1) && symetry(2) >= symetry(0))
        {
            ZAxis = LeftRight;
            quality.Add(symetry(2) / (symetry(2) + max(symetry(0), symetry(1))));
        }
        ...
        */
        ...
        for (int s = 0; s < NumCubeSides; s++)
        {
            ...
            if (s == 0)
                bettercenter.X = bounding.XMax();
            else if (s == 1)
                bettercenter.X = bounding.XMin();
            else if (s == 2)
                bettercenter.Y = bounding.YMax();
            else if (s == 3)
                bettercenter.Y = bounding.YMin();
            else if (s == 4)
                bettercenter.Z = bounding.ZMax();
            else if (s == 5)
                bettercenter.Z = bounding.ZMin();
            ...
            diameterhalf[s] = sqrt(Square(bounding.GetRadius(s / 2 == 0 ? 1 : 0)) + Square(bounding.GetRadius(s / 2 == 2 ? 1 : 2)));
        ...
        if (XAxis)
            if (updownratio[1] > updownratio[2])
            {
                YAxis = UpDown;
                quality.Add(updownratio[1] / (updownratio[1] + updownratio[2]));
            }
            else
            {
                ZAxis = UpDown;
                quality.Add(updownratio[2] / (updownratio[1] + updownratio[2]));
            }
        else if (YAxis)
            if (updownratio[0] > updownratio[2])
            {
                XAxis = UpDown;
                quality.Add(updownratio[0] / (updownratio[0] + updownratio[2]));
            }
            else
            {
                ZAxis = UpDown;
                quality.Add(updownratio[2] / (updownratio[0] + updownratio[2]));
            }
        else if (ZAxis)
            if (updownratio[0] > updownratio[1])
            {
                XAxis = UpDown;
                quality.Add(updownratio[0] / (updownratio[0] + updownratio[1]));
            }
            else
            {
                YAxis = UpDown;
                quality.Add(updownratio[1] / (updownratio[0] + updownratio[1]));
            }
        ...
        int updowni1 = XAxis == UpDown ? 0 : YAxis == UpDown ? 2
                                                             : 4;
        ...
        if (XAxis == UpDown)
            XAxis = updown1 < updown2 ? ToDown : ToUp;
        else if (YAxis == UpDown)
            YAxis = updown1 < updown2 ? ToDown : ToUp;
        else if (ZAxis == UpDown)
            ZAxis = updown1 < updown2 ? ToDown : ToUp;
        ...
        int frontback1 = XAxis == FrontBack ? 0 : YAxis == FrontBack ? 2
                                                                     : 4;
        ...
        int frontbacki = XAxis == FrontBack ? 0 : YAxis == FrontBack ? 1
                                                                     : 2;
        int leftrighti = updowni + frontbacki == 1 ? 2 : updowni + frontbacki == 3 ? 0
                                                                                   : 1;
        ...
        */
        ...
        if (XAxis == FrontBack)
            XAxis = back1 < back2 ? ToBack : ToFront;
        else if (YAxis == FrontBack)
            YAxis = back1 < back2 ? ToBack : ToFront;
        else if (ZAxis == FrontBack)
            ZAxis = back1 < back2 ? ToBack : ToFront;
    ...
    void TVolumeDoc::FindOrientationFullHead()
    {
        ...
        for (p1.Y = boundmin.Y; p1.Y <= boundmax.Y; p1.Y++)
            for (p1.Z = boundmin.Z; p1.Z <= boundmax.Z; p1.Z++)
                for (p1.X = center.X + 1, p2.X = center.X - 1; p1.X < boundmax.X && p2.X > boundmin.X; p1.X++, p2.X--)
                    ...
                    stat1.Add(fabs(2 * vol(p2.X, p1.Y, p1.Z) + vol(p2.X - 1, p1.Y, p1.Z) + vol(p2.X + 1, p1.Y, p1.Z) - 2 * vol(p1.X, p1.Y, p1.Z) - vol(p1.X - 1, p1.Y, p1.Z) - vol(p1.X + 1, p1.Y, p1.Z)));

        for (p1.X = boundmin.X; p1.X <= boundmax.X; p1.X++)
            for (p1.Z = boundmin.Z; p1.Z <= boundmax.Z; p1.Z++)
                for (p1.Y = center.Y + 1, p2.Y = center.Y - 1; p1.Y < boundmax.Y && p2.Y > boundmin.Y; p1.Y++, p2.Y--)

                    stat2.Add(fabs(2 * vol(p1.X, p2.Y, p1.Z) + vol(p1.X, p2.Y - 1, p1.Z) + vol(p1.X, p2.Y + 1, p1.Z) - 2 * vol(p1.X, p1.Y, p1.Z) - vol(p1.X, p1.Y - 1, p1.Z) - vol(p1.X, p1.Y + 1, p1.Z)));

        for (p1.X = boundmin.X; p1.X <= boundmax.X; p1.X++)
            for (p1.Y = boundmin.Y; p1.Y <= boundmax.Y; p1.Y++)
                for (p1.Z = center.Z + 1, p2.Z = center.Z - 1; p1.Z < boundmax.Z && p2.Z > boundmin.Z; p1.Z++, p2.Z--)

                    stat3.Add(fabs(2 * vol(p1.X, p1.Y, p2.Z) + vol(p1.X, p1.Y, p2.Z - 1) + vol(p1.X, p1.Y, p2.Z + 1) - 2 * vol(p1.X, p1.Y, p1.Z) - vol(p1.X, p1.Y, p1.Z - 1) - vol(p1.X, p1.Y, p1.Z + 1)));
        ...
        if (symetry(1) > symetry(0) && symetry(1) > symetry(2))
        {
            YAxis = LeftRight;
            quality.Add(symetry(1) / (symetry(1) + max(symetry(0), symetry(2))));
        }
        if (symetry(2) > symetry(1) && symetry(2) > symetry(0))
        {
            ZAxis = LeftRight;
            quality.Add(symetry(2) / (symetry(2) + max(symetry(0), symetry(1))));
        }
        ...
        if (XAxis)
            if (histodistratio[1] > histodistratio[2])
            {
                YAxis = UpDown;
                quality.Add(histodistratio[1] / (histodistratio[1] + histodistratio[2]));
            }
            else
            {
                ZAxis = UpDown;
                quality.Add(histodistratio[2] / (histodistratio[1] + histodistratio[2]));
            }
        else if (YAxis)
            if (histodistratio[0] > histodistratio[2])
            {
                XAxis = UpDown;
                quality.Add(histodistratio[0] / (histodistratio[0] + histodistratio[2]));
            }
            else
            {
                ZAxis = UpDown;
                quality.Add(histodistratio[2] / (histodistratio[0] + histodistratio[2]));
            }
        else if (ZAxis)
            if (histodistratio[0] > histodistratio[1])
            {
                XAxis = UpDown;
                quality.Add(histodistratio[0] / (histodistratio[0] + histodistratio[1]));
            }
            else
            {
                YAxis = UpDown;
                quality.Add(histodistratio[1] / (histodistratio[0] + histodistratio[1]));
            }
        ...
        int updowni = XAxis == UpDown ? 0 : YAxis == UpDown ? 1
                                                            : 2;
        ...
        if (XAxis == UpDown)
            XAxis = histodistratio[updowni] > 0 ? ToUp : ToDown;
        else if (YAxis == UpDown)
            YAxis = histodistratio[updowni] > 0 ? ToUp : ToDown;
        else if (ZAxis == UpDown)
            ZAxis = histodistratio[updowni] > 0 ? ToUp : ToDown;
        ...
        */
        ...
        if (XAxis == FrontBack)
            XAxis = front1 > front2 ? ToBack : ToFront;
        else if (YAxis == FrontBack)
            YAxis = front1 > front2 ? ToBack : ToFront;
        else if (ZAxis == FrontBack)
            ZAxis = front1 > front2 ? ToBack : ToFront;
