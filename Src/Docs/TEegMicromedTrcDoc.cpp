...
namespace crtl
{
    ...
    bool TEegMicromedTrcDoc::ReadFromHeader(const char *file, ReadFromHeaderType what, void *answer)
    {
        ...
        if (!(header.Header_Type == 4 && header.Compression == 0))
            return false;
        ...
        case ReadNumAuxElectrodes:
            ...
            for (el = 0; el < NumElectrodes; el++)
            {
                ...
                if ((toel->Type & 0x1E) || IsElectrodeNameAux(toel->Positive_Input_Label))
        ...
        if (GetDocPath())
        {
            ...
            if (NumSequences)
            {
                ...
                for (int i = 0; i < NumSequences; i++)
                {
                    ...
                    if (i == NumSequences - 1)
                        Sequences[i].NumTimeFrames = NumTimeFrames - Segment[i].Sample;
                    else
                        Sequences[i].NumTimeFrames = Segment[i + 1].Sample - Segment[i].Sample;
            ...
            { // create a default sequence for internal reasons
