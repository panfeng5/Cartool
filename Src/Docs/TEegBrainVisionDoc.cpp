...
namespace crtl
{
    ...
    void TEegBrainVisionDoc::ReadNativeMarkers()
    {
        ...
        do
        {
            ...
            // comment
            if (StringStartsWith(buff, ";"))
                continue;

            // beginning of a block?
            if (StringIs(buff, "[Marker Infos]"))
            {
                part = MarkerInfos;
                continue;
            }
            else if (StringIsEmpty(buff))
            {
                part = NoInfos;
                continue;
            } // empty line ends current part
            ...
            if (part == MarkerInfos)
            {
                ...
                for (char *toc = StringContains(buff, '=') + 1, toci = 1; toci <= 6; toci++, toc += StringSize(toc))
                {
                    ...
                    if (toci == 1)
                        StringCopy(bvmtype, toc);
                    else if (toci == 2)
                    {
                        StringCopy(bvmdesc, toc);
                        ReplaceChars(bvmdesc, "\"", "");
                    }
                    else if (toci == 3)
                        fromtf = StringToInteger(toc) - 1;
                    else if (toci == 4)
                        totf = fromtf + StringToInteger(toc) - 1;
                ...
                if (StringIs(bvmtype, "New Segment"))
                {
                    ...
                    AppendMarker(TMarker(fromtf, totf, (MarkerCode)StringToInteger(bvmdesc),
                                         StringIsNotEmpty(bvmdesc) ? bvmdesc : StringIsNotEmpty(bvmtype) ? bvmtype
                                                                                                         : "New Segment",
                                         MarkerTypeEvent));
                    continue;
                }
                else if (StringIs(bvmtype, "Trigger") || StringIs(bvmtype, "Stimulus"))
                {
                ...
                { // Response, Comment, SyncStatus, Time_0 (?) or "Time 0" (??), "DC Correction" (?)
    ...
    bool TEegBrainVisionDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            do
            {
                ...
                // beginning of a block?
                if (StringIs(buff, "[Common Infos]"))
                {
                    part = CommonInfos;
                    continue;
                }
                else if (StringIs(buff, "[Binary Infos]"))
                {
                    part = BinaryInfos;
                    continue;
                }
                else if (StringIs(buff, "[ASCII Infos]"))
                {
                    part = AsciiInfos;
                    continue;
                }
                else if (StringIs(buff, "[Channel Infos]"))
                {
                    part = ChannelInfos;
                    continue;
                }
                else if (StringIs(buff, "[Coordinates]"))
                {
                    part = Coordinates;
                    continue;
                }
                else if (StringIs(buff, "[Comment]"))
                {
                    part = Comment;
                    continue;
                }
                else if (StringStartsWith(buff, "["))
                {
                    part = NoInfos;
                    continue;
                } // unknown case
                ...
                else if (part == AsciiInfos)
                {
                    ...
                    if (StringStartsWith(buff, "DecimalSymbol="))
                    {
                        ...
                        {
                            ShowMessage("Unrecognized file format!" NewLine "Decimal symbol is neither "
                                        ","
                                        " nor "
                                        "."
                                        "",
                                        "Open File", ShowMessageWarning);
                            numfieldsneeded = -100;
                            break;
                        }
                        continue;
