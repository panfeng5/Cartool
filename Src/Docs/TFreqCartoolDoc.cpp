...
namespace crtl
{
    ...
    bool TFreqCartoolDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            DataOrg = sizeof(TFreqHeader) + NumElectrodes * sizeof(TFreqChannel) + NumFrequencies * (IsMagicNumber(freqheader.Version, FREQBIN_MAGICNUMBER1) ? sizeof(TFreqFrequency) : sizeof(TFreqFrequencyName));
