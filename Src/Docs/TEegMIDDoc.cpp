    ...
    bool TEegMIDDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            if (AuxTracks.NumSet() != NumAux)
            {
                ...
                if (NumAux)
                    AuxTracks.Set(NumElectrodes - NumAux, NumElectrodes - 1);
                else
                    AuxTracks.Reset();
            }
        }
        ...
        if (DataCalibrated)
        {
            ...
            if (ci && XHCalibration)
            {
                ...
                for (t = 0, toT = Tracks, toFB = FileBuff,
                    tociZ = ci->zero, toZ = XHCI.zero, tociR = ci->range, toR = XHCI.range;
                     t < NumElectrodes; t++, toT++, toFB++, toZ++, toR++, tociZ++, tociR++)

                    *toT = (((double)*toFB - Zero) / (double)(Unit * XHCI.ampl) * *toR + *toZ - *tociZ) / (double)*tociR * ci->ampl;
            }
            ...
            for (t = 0, toT = Tracks, toFB = FileBuff,
                toZ = ci ? ci->zero : XHCI.zero, toR = ci ? ci->range : XHCI.range, ciampl = ci ? ci->ampl : XHCI.ampl;
                 t < NumElectrodes; t++, toT++, toFB++, toZ++, toR++)
