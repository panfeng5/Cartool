...
#define ComputedTracksNames TrackNameGFP " " TrackNameDIS " " TrackNameAVG " " TrackNameDISPlus " " TrackNameRMS
    ...
    {
        if (AllNonAuxNames.Contains(elname))
            return false; // 'white list' of non-aux names has overriding priority
        else if (AllAuxNames.PartiallyContains(elname))
            return true; // 'black list' of PARTIAL aux names. F.ex. "EOG1" or "LEOG" electrodes will both match to aux list "EOG"
        else
            return false; // not in either list, default is 'not an aux'
    ...
    public:
        ...
        virtual void SetSamplingFrequency(double sf)
        {
            Maxed(sf, 0.0);
            if (sf != SamplingFrequency)
            {
                SamplingFrequency = sf;
                DirtySamplingFrequency = true;
            }
        } // !caller is accountable for correct check & call!
        ...
        void SetPseudo(TSelection &sel) const
        {
            if (HasPseudoElectrodes())
                sel.Set(GetFirstPseudoIndex(), GetLastPseudoIndex());
        }
        void ClearPseudo(TSelection &sel) const
        {
            if (HasPseudoElectrodes())
                sel.Reset(GetFirstPseudoIndex(), GetLastPseudoIndex());
        }
