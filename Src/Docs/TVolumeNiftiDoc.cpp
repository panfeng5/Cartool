...
namespace crtl
{
    ...
    bool TVolumeNiftiDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            if (!IsNifti(type))
            { // this should be resolved before, but check it again
            ...
            if (datatype == NIFTI_TYPE_UNKNOWN || !(datatype == NIFTI_TYPE_UINT8 || datatype == NIFTI_TYPE_INT8 || datatype == NIFTI_TYPE_UINT16 || datatype == NIFTI_TYPE_INT16 || datatype == NIFTI_TYPE_INT32 || datatype == NIFTI_TYPE_FLOAT32 || datatype == NIFTI_TYPE_FLOAT64))
            {
            ...
            else if (method == NiftiMethod2 || method == NiftiMethod3)
            ...
            if (scl_slope != 0)
            {
                ...
                if (IsNiftiTypeInteger(datatype) && scl_slope != 1.0)
                {
                    ...
                    if (minval < 0 && maxval > 0)
                    {
                        ...
                        {
                            minval = 0;
                            Data.UnaryOp(OperandData, OperationThresholdAbove, &minval);
                        }
                        else if (maxval / minval < precision)
                        {
                            maxval = 0;
                            Data.UnaryOp(OperandData, OperationThresholdBelow, &maxval);
                        }
                    }
                }
