...
namespace crtl
{
    ...
    int TInverseMatrixDoc::GetGlobalRegularization(const TMaps *maps,
                                                   TTracksView *eegview,
                                                   long fromtf, long totf, long steptf) const
    {
        if (!(GetNumRegularizations() >= 3 && (maps || eegview)))
            return 0;
        ...
        for (long tf = fromtf; tf <= totf; tf += steptf)
            ...
            if (maps)
                stat.Add(GetBestRegularization(&(*maps)[tf], 0, tf));
            else
                stat.Add(GetBestRegularization(0, eegview, tf));
    ...
    int TInverseMatrixDoc::GetBestRegularization(const TMap *map,
                                                 TTracksView *eegview, long tf) const
    {
        ...
        for (regi = 0; regi < GetNumRegularizations(); regi++)
        {
            ...
            if ((dv1 + dv2) / 2 / RegSearch[regi - 1] >= /*-0.045*/ -0.06)
                break; // lower value -> earlier stop -> lower reg
        }
    ...
    const char *TInverseMatrixDoc::GetInverseName(char *isname) const
    {
        ...
        if (StringIsEmpty(filename))
            StringCopy(isname, InfixDefaultEsi);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseMN))
            StringCopy(isname, InfixInverseMN);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseWMN))
            StringCopy(isname, InfixInverseWMN);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseLoreta))
            StringCopy(isname, InfixInverseLoreta);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseLaura))
            StringCopy(isname, InfixInverseLaura);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseEpifocus))
            StringCopy(isname, InfixInverseEpifocus);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseSLoreta))
            StringCopy(isname, InfixInverseSLoreta);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseELoreta))
            StringCopy(isname, InfixInverseELoreta);
        else if (StringContains((const char *)filename, (const char *)"." InfixInverseDale))
            StringCopy(isname, InfixInverseDale);
        else
        {
            StringShrink(filename, isname, 20);
            StringCamelCase(isname);
        }
...
#if defined(_DEBUG)
        if (reg == RegularizationAutoLocal)
        {
            ShowMessage("Not implemented", "TInverseMatrixDoc::MultiplyMatrix auto local", ShowMessageWarning);
            exit(1);
        }
#endif
