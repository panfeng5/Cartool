    ...
    void TEegEgiRawDoc::ReadNativeMarkers()
    {
        ...
        for (tfi = 1; tfi < NumTimeFrames; tfi++)
        {
            ...
            for (e = 0; e < NumEvents; e++)
            {
                ...
                {
                    // cumulate din? values
                    if (StringIs(eventnames[e], "din1"))
                        dincode |= 0x1;
                    else if (StringIs(eventnames[e], "din2"))
                        dincode |= 0x2;
                    else if (StringIs(eventnames[e], "din3"))
                        dincode |= 0x4;
                    else if (StringIs(eventnames[e], "din4"))
                        dincode |= 0x8;

                    else
                    { // not known code, put it straight away
