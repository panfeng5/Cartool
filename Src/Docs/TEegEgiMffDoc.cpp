...
namespace crtl
{
        ...
        for (int i = 0; i < (int)eventsfiles; i++)
        {
            ...
            do
            {
                ...
                deltatime = deltaday * 24 * 60 * 60 * 1000 * 1000.0 // difference of days in micro-seconds
                            + (((((hh - DateTime.GetHour()) * 60.0) + (mm - DateTime.GetMinute())) * 60.0 + (ss - DateTime.GetSecond())) * 1000.0 - DateTime.GetMillisecond()) * 1000.0 - DateTime.GetMicrosecond();
                ...
                if (tf1 >= 0 && tf2 < NumTimeFrames)
                {
                    ...
                    if (!dinfile && (filemarkertype == MarkerTypeEvent || StringContains((const char *)EgiEventCodes, (const char *)code)))
                    ...
                    { // Multi-Port ECI
                      // build final marker name
                        ...
                        if (StringIsNotEmpty(description))
                            StringAppend(markername, description);
                        else if (StringIsNotEmpty(label))
                            StringAppend(markername, label);
                        // crop final marker!
                        ...
                        do
                        {
                            ...
                            if (StringIsNotEmpty(keydata))
                                StringAppend(keycode, "=", keydata);
                            // crop final marker
    ...
    bool TEegEgiMffDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            FileStream.SeekCurrent(NumElectrodes * sizeof(TEegEgi_Mff_Bin_HeaderVariable1) + NumElectrodes * sizeof(TEegEgi_Mff_Bin_HeaderVariable2));
            ...
            for (int b = 0; b < NumBlocks; b++)
            {
                ...
                for (int el = 0; el < NumElectrodes; el++)
                {
                    ...
                    if (EqualTracks && (el && Blocks[b].ChannelsSpec[el].ChannelSize != Blocks[b].ChannelsSpec[el - 1].ChannelSize || b && Blocks[b].ChannelsSpec[el].ChannelSize != Blocks[b - 1].ChannelsSpec[el].ChannelSize))
            ...
            if (XMLGotoElement(fi, "fileInfo", 0, 0, true) && XMLGetElement(fi, "mffVersion", "fileInfo", element))
            {
            ...
            // get date & time?
            if (XMLGotoElement(fi, "fileInfo", 0, 0, true) && XMLGetElement(fi, "recordTime", "fileInfo", element))
            {
            ...
            // get calibrations?
            if (XMLGotoElement(fix, "dataInfo", 0, 0, true) && XMLGotoElement(fix, "calibrations", "dataInfo", 0))
            {
                ...
                do
                {
                    ...
                    if (!XMLGetElement(fix, "type", "calibration", element) || !IsStringAmong(element, "GCAL ZCAL ICAL"))
            ...
            if (XMLGotoElement(fix, "dataInfo", 0, 0, true) && XMLGotoElement(fix, "generalInformation", "dataInfo", 0) && XMLGetElement(fix, "fileDataType", "generalInformation", element))
            {
            ...
            if (ispns && CanOpenFile(filepns, CanOpenFileRead))
            {
                ...
                if (XMLGotoElement(fipns, "PNSSet", 0, 0, true) && XMLGotoElement(fipns, "sensors", "PNSSet", 0))
                {
                    ...
                    do
                    {
                        ...
                        if (!XMLGotoElement(fipns, "sensor", "sensors", 0) || el >= NumElectrodes)
            ...
            */
            ...
            if (XMLGotoElement(fie, "epochs", 0, 0, true) && (NumSequences = XMLCountElement(fie, "epoch", "epochs")) != 0)
            {
                ...
                bool headertotaldurationOK = headertotalduration != 0 && NumSequences == 1;

                // we can compare the file size estimated duration only for 1 sequence
                bool filesizedurationOK = headertotaldurationOK && (headertotalduration - filesizeduration) > Blocks[0].BlockDuration // difference between header duration and file size duration should be significant
                                          && NumSequences == 1;
                ...
                for (int i = 0; i < NumSequences; i++)
                {
                    ...
                    Sequences[i].DateTime = TDateTime(DateTime.GetYear(), DateTime.GetMonth(), DateTime.GetDay(),
                                                      ...
                                                      DateTime.GetMicrosecond() + microsec + Truncate(nanosec / 1000.0));
                    ...
                    INT32 epochsduration = MicrosecondsToTimeFrame(DurationStringToMicroseconds(endtime) - DurationStringToMicroseconds(begintime),
                    ...
                    if (headertotalduration != sumblocksduration && headertotaldurationOK || headertotalduration != epochsduration && headertotaldurationOK || headertotalduration != filesizeduration && headertotaldurationOK && filesizedurationOK || filesizeduration != sumblocksduration && filesizedurationOK || filesizeduration != epochsduration && filesizedurationOK || sumblocksduration != epochsduration)
                    {
                        ...
                        StringAppend(buff, NewLine " - "
                                                   "Sequence #",
                                     IntegerToString(i + 1), " sum of blocks     : ", TimeFrameToString(sumblocksduration), " [TF]");
                        StringAppend(buff, NewLine " - "
                                                   "Sequence #",
                                     IntegerToString(i + 1), " epochs duration : ", TimeFrameToString(epochsduration), " [TF]");

                        if (headertotaldurationOK)
                            StringAppend(buff, NewLine " - "
                                                       "Total header duration              : ",
                                         TimeFrameToString(headertotalduration), " [TF]");

                        if (filesizedurationOK)
                            StringAppend(buff, NewLine " - "
                                                       "File size duration                     : ",
                                         TimeFrameToString(filesizeduration), " [TF]");
            ...
            if (HasMultipleSessions() && Sequences[0].NumTimeFrames < Sequences[1].NumTimeFrames / 2)
                CurrSequence = 1;
