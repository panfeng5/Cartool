...
namespace crtl
{
    ...
    bool TVolumeAnalyzeDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            if (datatype == DT_UNKNOWN || !(datatype == DT_UNSIGNED_CHAR || datatype == DT_SIGNED_SHORT || datatype == DT_FLOAT || datatype == DT_DOUBLE))
            {
                char buff[256];
            ...
            else if (datatype == DT_SIGNED_SHORT || datatype == DT_SIGNED_SHORT_POS)
            {
