...
namespace crtl
{
    ...
    void TEegNeuroscanCntDoc::ReadNativeMarkers()
    {
        ...
        if (setup.ContinousType == ContinousTypeEventTable1 || setup.ContinousType == ContinousTypeEventTable2)
        {
            // read events
            TNsTEeg teeg;
            ...
            if (event1)
            {
                ...
                for (int e = 0; e < NumEvents; e++)
                {
                    ...
                    else if (ev1.KeyPad_Accept)
                    {
                        ...
                        if (kp == 0xD)
                            sprintf(name, "KPA%0d", code);
                        else if (kp == 0xC)
                            sprintf(name, "KPR%0d", code);
                        else if (kp == 0xE)
                            sprintf(name, "KP%0d", code);
                        else
                            sprintf(name, "KP%0d", code);
                    }
                    else
