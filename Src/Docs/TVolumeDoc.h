...
namespace crtl
{
      ...
      enum MriContentType
      {
            ...
            MriContentTypeMaskMask = MriContentTypeBinaryMask | MriContentTypeRoiMask | MriContentTypeGrey | MriContentTypeWhite,

            MriContentTypeBrain = 0x0400,
            MriContentTypeBlob = 0x1000,
            MriContentTypeRis = 0x2000,
            MriContentTypeSegmentedMask = MriContentTypeMaskMask | MriContentTypeBrain | MriContentTypeBlob | MriContentTypeRis,
      ...
      public:
            ...
            void GetSize(TVector3Int &s) const
            {
                  s.X = Size.GetXExtent();
                  s.Y = Size.GetYExtent();
                  s.Z = Size.GetZExtent();
            ...
            void ToAbs(GLfloat *p) const
            {
                  p[0] = (GLfloat)(p[0] - Origin.X);
                  p[1] = (GLfloat)(p[1] - Origin.Y);
                  p[2] = (GLfloat)(p[2] - Origin.Z);
            }
            void ToAbs(double *p) const
            {
                  p[0] -= Origin.X;
                  p[1] -= Origin.Y;
                  p[2] -= Origin.Z;
            ...
            void ToRel(GLfloat *p) const
            {
                  p[0] = (GLfloat)(p[0] + Origin.X);
                  p[1] = (GLfloat)(p[1] + Origin.Y);
                  p[2] = (GLfloat)(p[2] + Origin.Z);
            }
            void ToRel(double *p) const
            {
                  p[0] += Origin.X;
                  p[1] += Origin.Y;
                  p[2] += Origin.Z;
