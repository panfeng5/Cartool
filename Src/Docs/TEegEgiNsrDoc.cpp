...
namespace crtl
{
    ...
    int TEegEgiNsrMainHeader::GetData(long startaddress, TEegEgiNsrTableCell *list, bool swap, int level)
    {
        ...
        if (datafollow)
        {
            ...
            for (int i = 0; i < n; i++)
            { // here, we read 2 longs each time
                if (swap)
                {
    ...
    bool TEegEgiNsrMainHeader::HasHighpassFilter()
    {
        ...
        return (bool)StringContains((const char *)filtersstring, (const char *)"highpass") || GetVersion() == NsrVersion1 && GetNumFilters() > 0;
    }
    ...
    int TEegEgiNsrMainHeader::FindNumElectrodes()
    {
        ...
        if (*nname)
        { // try to extract the number of electrodes from it
    ...
    void TEegEgiNsrMainHeader::SetAuxElectrodes(TSelection &aux)
    {
        ...
        else if (Version == NsrVersion3 || Version == NsrVersion4)
        { // no auxs
        }
    }
    ...
    int TEegEgiNsrMainHeader::GetNumAuxElectrodes()
    {
        ...
        if (Version == NsrVersion1)
            ...
            return 6;
        else if (Version == NsrVersion3)
            return 0;
        else if (Version == NsrVersion4)
            return 0;
        else
            return 0;
    }
    ...
    bool TEegEgiNsrMainHeader::IsFloatTable()
    {
        ...
        if (IsNotAProperNumber(float1) || IsNotAProperNumber(float2) || IsNotAProperNumber(float3))
            return false;
    ...
    bool TEegEgiNsrMainHeader::GetZeros(double *zeros, int session)
    {
        ...
        { // NsrVersion2 || NsrVersion3 || Version == NsrVersion4
    ...
    bool TEegEgiNsrMainHeader::GetGains(double *gains, int session)
    {
        ...
        { // Version == NsrVersion3  || Version == NsrVersion4
...
#define DINOFFSET 0xFFF0
    ...
    bool TEegEgiNsrDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            if (HasMultipleSessions() && TimeFrames[0] < TimeFrames[1] / 2)
                CurrSequence = 1;
    ...
    bool TEegEgiNsrDoc::SetGainsZeros(TEegEgiNsrMainHeader &mainheader, int session)
    {
        ...
        if (!gz || !gg)
        {
            ...
            if (!(gg && mainheader.IsAtomFloat()))
            {
                ...
                sprintf(buff, "This file does not seem to be calibrated properly:" NewLine Tab "Zeroes: %s" NewLine Tab "Gains:  %s" NewLine "Do you wish to open it anyway?",
            ...
            if (!gg)
                for (int i = 0; i < NumElectrodesInFile; i++)
                    Gains[i] = Version == NsrVersion1 ? 1850.0 : // better than 400 / 0.24
                                   Version == NsrVersion2 ? 16480.0
                                                          : // ad-hoc value, maybe 16384?
                                   Version == NsrVersion3 ? 1850.0
                               : Version == NsrVersion4   ? 1850.0
                                                          : 1850.0;
        }

        double regain = Version == NsrVersion1 ? 400 : Version == NsrVersion2 ? 400
                                                                              : // 1345
