...
namespace crtl
{
    ...
    void TElectrodesDoc::ProjectElectrodes()
    {
        ...
        if ((bool)sel)
        {
            ...
            */
            ...
            iscentered = boxpoints.Min(0) * boxpoints.Max(0) < 0 && boxpoints.Min(1) * boxpoints.Max(1) < 0 && boxpoints.Min(2) * boxpoints.Max(2) < 0;
    ...
    void TElectrodesDoc::Flip(owlwparam wparam)
    {
        ...
        for (int e = 0; e < NumElectrodes; e++)
        {
            ...
            if (wparam == IDB_FLIPX)
                p.X = -p.X;
            else if (wparam == IDB_FLIPY)
                p.Y = -p.Y;
            else if (wparam == IDB_FLIPZ)
                p.Z = -p.Z;
            else if (wparam == IDB_FLIPXY)
                Permutate(p.X, p.Y);
            else if (wparam == IDB_FLIPYZ)
                Permutate(p.Y, p.Z);
            else if (wparam == IDB_FLIPXZ)
                Permutate(p.X, p.Z);
    ...
    void TElectrodesDoc::GetNeighborhoodIndexes(TArray2<int> &neighborhood) const
    {
        ...
        for (int e1 = 0; e1 < NumElectrodes; e1++)
        {
            ...
            for (int e2 = 0; e2 < NumElectrodes; e2++)
                ...
                if (e1 != e2 && allneighbors(e1, e2))
                    allneighbors(e1, NumElectrodes)++;
    ...
    void TElectrodesDoc::FindOrientation()
    {
        ...
        */
        ...
        for (int e = 0; e < NumElectrodes; e++)
        {
            ...
            if (StringStartsWith(ename, "FP"))
                front += p1 * 4;
            else if (StringStartsWith(ename, "NZ"))
                front += p1 * 4;
            else if (StringStartsWith(ename, "AF"))
                front += p1 * 3;
            else if (StringStartsWith(ename, "F"))
                front += p1 * 2;
            else if (StringStartsWith(ename, "FC"))
                front += p1 * 1;

            else if (StringStartsWith(ename, "O"))
                back += p1 * 4;
            else if (StringStartsWith(ename, "IZ"))
                back += p1 * 4;
            else if (StringStartsWith(ename, "PO"))
                back += p1 * 3;
            else if (StringStartsWith(ename, "P"))
                back += p1 * 2;
            else if (StringStartsWith(ename, "CP"))
                back += p1 * 1;
        ...
        if (front.IsNotNull() && back.IsNotNull())
        {
            ...
            if (p1[0] > p1[1] && p1[0] > p1[2])
                XAxis = front[0] > 0 ? ToFront : ToBack;
            else if (p1[1] > p1[0] && p1[1] > p1[2])
                YAxis = front[1] > 0 ? ToFront : ToBack;
            else
                ZAxis = front[2] > 0 ? ToFront : ToBack;
        ...
        if (numdirnames && right.IsNotNull() && left.IsNotNull())
        {
            ...
            // check for greatest axis component
            if (p1[0] > p1[1] && p1[0] > p1[2])
                XAxis = right[0] > 0 ? ToRight : ToLeft;
            else if (p1[1] > p1[0] && p1[1] > p1[2])
                YAxis = right[1] > 0 ? ToRight : ToLeft;
            else
                ZAxis = right[2] > 0 ? ToRight : ToLeft;
...
#define MaxRandomPairs 1000
        ...
        for (int randi = 0; randi < MaxRandomRepeat; randi++)
        {
            ...
            for (int i = 0; i < MaxRandomPairs; i++)
            {
                ...
                do
                    e2 = sel.GetValue(randunif((uint)sel.NumSet()));
                while (e2 == e);
        ...
        if (!XAxis)
            XAxis = updown.X > 0 ? ToUp : ToDown;
        else if (!YAxis)
            YAxis = updown.Y > 0 ? ToUp : ToDown;
        else if (!ZAxis)
            ZAxis = updown.Z > 0 ? ToUp : ToDown;
...
#define udheight 0.20
        ...
        for (TIteratorSelectedForward seli(sel); (bool)seli; ++seli)
        {
            ...
            {
                if (YAxis == ToUp && p1[1] < -udheight || YAxis == ToDown && p1[1] > udheight || ZAxis == ToUp && p1[2] < -udheight || ZAxis == ToDown && p1[2] > udheight)
                    balance++;
            }
            else if (YAxis == FrontBack && p1[1] < -fbdepth)
            {
                if (ZAxis == ToUp && p1[2] < -udheight || ZAxis == ToDown && p1[2] > udheight || XAxis == ToUp && p1[0] < -udheight || XAxis == ToDown && p1[0] > udheight)
                    balance++;
            }
            else if (ZAxis == FrontBack && p1[2] < -fbdepth)
            {
                if (YAxis == ToUp && p1[1] < -udheight || YAxis == ToDown && p1[1] > udheight || XAxis == ToUp && p1[0] < -udheight || XAxis == ToDown && p1[0] > udheight)
                    balance++;
            }
        ...
        if (XAxis == FrontBack)
            XAxis = balance < 0 ? ToBack : ToFront;
        if (YAxis == FrontBack)
            YAxis = balance < 0 ? ToBack : ToFront;
        if (ZAxis == FrontBack)
            ZAxis = balance < 0 ? ToBack : ToFront;
