...
namespace crtl
{
    ...
    public:
        TElectrodesCluster()
        {
            ...
            NumElectrodes = 0;
            ClearString(Name, ElectrodesClusterMaxName);
        }
    ...
    public:
        ...
        void GetCoordinates(int e, float &x, float &y, float &z, int space) const
        {
            const TPoints &sop = GetPoints(space);
            x = sop[e].X;
            y = sop[e].Y;
            z = sop[e].Z;
        }
        void GetCoordinates(int e, double *v, int space) const
        {
            const TPoints &sop = GetPoints(space);
            v[0] = sop[e].X;
            v[1] = sop[e].Y;
