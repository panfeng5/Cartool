...
namespace crtl
{
    ...
    bool TLinkManyDoc::Commit(bool force)
    {
        ...
        for (int i = 0; i < (int)ListXyzDoc; i++)
            *os << ListXyzDoc[i]->GetDocPath() << NewLine;
        for (int i = 0; i < (int)ListSpDoc; i++)
            *os << ListSpDoc[i]->GetDocPath() << NewLine;
        for (int i = 0; i < (int)ListIsDoc; i++)
            *os << ListIsDoc[i]->GetDocPath() << NewLine;
        for (int i = 0; i < (int)ListEegDoc; i++)
            *os << ListEegDoc[i]->GetDocPath() << NewLine;
        for (int i = 0; i < (int)ListRisDoc; i++)
            *os << ListRisDoc[i]->GetDocPath() << NewLine;
        for (int i = 0; i < (int)ListRoisDoc; i++)
            *os << ListRoisDoc[i]->GetDocPath() << NewLine;
        for (int i = 0; i < (int)ListMriDoc; i++)
            *os << ListMriDoc[i]->GetDocPath() << NewLine;
    ...
    bool TLinkManyDoc::Open(int /*mode*/, const char *path)
    {
        ...
        { // create -> ask which files to link through dialogs
            SetDirty(true);
        ...
        if (lm.leeg.IsNotEmpty() && !lm.leeg.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        if (lm.lris.IsNotEmpty() && !lm.lris.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        if (lm.lxyz.IsNotEmpty() && !lm.lxyz.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        if (lm.lsp.IsNotEmpty() && !lm.lsp.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        if (lm.lis.IsNotEmpty() && !lm.lis.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        if (lm.lrois.IsNotEmpty() && !lm.lrois.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        if (lm.lmri.IsNotEmpty() && !lm.lmri.CanOpenFiles(CanOpenFileReadAndAsk))
            goto AbortOpen;
        ...
        for (/*int*/ i = 0; i < (int)ListSpDoc; i++)
            ListSpDoc[i]->AddLink(this);
        for (/*int*/ i = 0; i < (int)ListIsDoc; i++)
            ListIsDoc[i]->AddLink(this);
        for (/*int*/ i = 0; i < (int)ListMriDoc; i++)
            ListMriDoc[i]->AddLink(this);
        for (/*int*/ i = 0; i < (int)ListEegDoc; i++)
            ListEegDoc[i]->AddLink(this);
        for (/*int*/ i = 0; i < (int)ListRisDoc; i++)
            ListRisDoc[i]->AddLink(this);
        for (/*int*/ i = 0; i < (int)ListRoisDoc; i++)
            ListRoisDoc[i]->AddLink(this);
                ...
                for (view = ListEegDoc[i]->GetViewList(this); view != 0; view = ListEegDoc[i]->NextView(view, this))
                    ...
                    if (dynamic_cast<TPotentialsView *>(view))
                        ...
                        for (view2 = ListEegDoc[i]->GetViewList(this); view2 != 0; view2 = ListEegDoc[i]->NextView(view2, this))
                            ...
                            if (dynamic_cast<TInverseView *>(view2) && view->LinkedViewId == view2->LinkedViewId)
    ...
    bool TLinkManyDoc::Close()
    {
        ...
        if (!CartoolApplication->Closing)
        {
            ...
            for (int i = 0; i < (int)ListEegDoc; i++)
            {
                ...
                if (doceeg->CanClose(true))
                    CartoolDocManager->CloseDoc(doceeg, false);
            }
            ...
            for (int i = 0; i < (int)ListRisDoc; i++)
            {
                ...
                if (doc->CanClose(true))
                    CartoolDocManager->CloseDoc(doc, false);
            }
    ...
    {
        return (bool)ListEegDoc || (bool)ListXyzDoc || (bool)ListSpDoc || (bool)ListIsDoc || (bool)ListRisDoc || (bool)ListMriDoc || (bool)ListRoisDoc;
    }

    int TLinkManyDoc::GetNumDocs() const
    {
        return (int)ListEegDoc + (int)ListXyzDoc + (int)ListSpDoc + (int)ListIsDoc + (int)ListRisDoc + (int)ListMriDoc + (int)ListRoisDoc;
    }
    ...
    void TLinkManyDoc::GroupTileViews(GroupTilingViewsFlags flags, UINT viewidabove)
    {
        ...
        if (IsFlag(flags, GroupTilingViews_RightSide))
        {
            ...
            if (maxright == Lowest<int>())
                xinsertion = maxright = maxleft;
            else
                xinsertion = maxright + GroupTilingSpaceBetweenGroups;
        }
                ...
                for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc != 0; doc = CartoolDocManager->DocListNext(doc))
                    for (TBaseView *view = doc->GetViewList(this, false); view != 0; view = doc->NextView(view, this, false))
                        ...
                        if (!view->IsWindowMinimized())
                        {
                            ...
                            if (r.Right() > myleft && r.Left() <= myleft && r.Right() > maxright)
                            {
                                maxright = r.Right();
                                lastgod = view->GODoc;
                            }
                            if (r.Right() <= myleft && r.Right() > maxright)
                            {
                                maxright = r.Right();
                                lastgod = view->GODoc;
                            }
        ...
        if (IsFlag(flags, GroupTilingViews_SizeMask))
        {
            ...
            int wh = mdicr.Height() - GetMinimizedWindowHeight(GetViewList()->GetParentO()); // keep some room at the bottom for the minimized windows to remain visible

            int ww = mdicr.Width() - GetViewList()->GetWindowWidth(); // remove width of our lm view
            ...
            for (TBaseDoc *doc = CartoolDocManager->DocListNext(0); doc != 0; doc = CartoolDocManager->DocListNext(doc))
                for (TBaseView *view = doc->GetViewList(lastgod, false); view != 0; view = doc->NextView(view, lastgod, false))
                    ...
                    if (view->GODoc != this && !view->IsWindowMinimized())
                    {
                        ...
                        if (r.Left() >= myleft && r.Left() < minleft)
                            minleft = r.Left();
                    }

            if (minleft == Highest<int>())
                minleft = myleft;
                                              ...
                                              int *togreyi) const
    {
        ...
        else
        {
            ...
            if (!head)
            {
                head = brain ? brain : grey;
                headi = braini >= 0 ? braini : greyi;
            }
            if (!brain)
            {
                brain = grey ? grey : head;
                braini = greyi >= 0 ? greyi : headi;
            }
            if (!grey)
            {
                grey = brain ? brain : head;
                greyi = braini >= 0 ? braini : headi;
            }
        }
    ...
    bool TLinkManyDoc::AddToGroup(TBaseDoc *doc, bool refresh)
    {
        ...
        if (!((dynamic_cast<TTracksDoc *>(doc) && !dynamic_cast<TSegDoc *>(doc)) // && ! IsExtensionAmong ( doc->GetDocPath (), FILEEXT_SEG" "FILEEXT_EEGEPSD" "FILEEXT_EEGEPSE ) )
              || dynamic_cast<TFreqDoc *>(doc) || dynamic_cast<TElectrodesDoc *>(doc) || dynamic_cast<TSolutionPointsDoc *>(doc) || dynamic_cast<TInverseMatrixDoc *>(doc) || dynamic_cast<TRisDoc *>(doc) || dynamic_cast<TRoisDoc *>(doc) || dynamic_cast<TVolumeDoc *>(doc)))
        {
        ...
        */
        ...
        if (ListEegDoc.IsInside((TTracksDoc *)doc))
            return false;
        if (ListXyzDoc.IsInside((TElectrodesDoc *)doc))
            return false;
        if (ListSpDoc.IsInside((TSolutionPointsDoc *)doc))
            return false;
        if (ListIsDoc.IsInside((TInverseMatrixDoc *)doc))
            return false;
        if (ListRisDoc.IsInside((TRisDoc *)doc))
            return false;
        if (ListMriDoc.IsInside((TVolumeDoc *)doc))
            return false;
        if (ListRoisDoc.IsInside((TRoisDoc *)doc))
            return false;
        ...
        for (int i = 0; i < (int)ListEegDoc; i++)
            leeg.Add(ListEegDoc[i]->GetDocPath());
        for (int i = 0; i < (int)ListXyzDoc; i++)
            lxyz.Add(ListXyzDoc[i]->GetDocPath());
        for (int i = 0; i < (int)ListSpDoc; i++)
            lsp.Add(ListSpDoc[i]->GetDocPath());
        for (int i = 0; i < (int)ListIsDoc; i++)
            lis.Add(ListIsDoc[i]->GetDocPath());
        for (int i = 0; i < (int)ListRisDoc; i++)
            lris.Add(ListRisDoc[i]->GetDocPath());
        for (int i = 0; i < (int)ListMriDoc; i++)
            lmri.Add(ListMriDoc[i]->GetDocPath());
        for (int i = 0; i < (int)ListRoisDoc; i++)
            lrois.Add(ListRoisDoc[i]->GetDocPath());
        ...
        if (adding == addeeg || adding == addfreq)
        {
        ...
        else if (adding == addxyz)
            ListXyzDoc.Append((TElectrodesDoc *)doc);
        else if (adding == addsp)
            ListSpDoc.Append((TSolutionPointsDoc *)doc);
        else if (adding == addis)
            ListIsDoc.Append((TInverseMatrixDoc *)doc);
        else if (adding == addris)
            ListRisDoc.Append((TRisDoc *)doc);
        else if (adding == addroi)
            ListRoisDoc.Append((TRoisDoc *)doc);
        else if (adding == addmri)
            ListMriDoc.Append((TVolumeDoc *)doc);
        ...
        */
        ...
        // add a tracks view
        if (adding == addeeg || adding == addfreq || adding == addris)
        {
            ...
            { // already owned by a group?
                if (adding == addfreq)
                    view = new TFrequenciesView(*((TFreqDoc *)doc), 0, this);
                else
                    view = new TTracksView(*((TTracksDoc *)doc), 0, this);
        ...
        // can create scalp potential ?
        if ((adding == addeeg || adding == addfreq) && (bool)ListXyzDoc)
        {
        ...
        if ((adding == addeeg || adding == addfreq) && numspirr && (bool)ListIsDoc && (bool)ListMriDoc)
        {
        ...
        if (adding == addsp && numspirr == 1 && (bool)ListEegDoc && (bool)ListIsDoc && (bool)ListMriDoc || adding == addis && (int)ListIsDoc == 1 && (bool)ListEegDoc && numspirr && (bool)ListMriDoc || adding == addmri && (int)ListMriDoc == 1 && (bool)ListEegDoc && numspirr && (bool)ListIsDoc)
        {
        ...
        if (adding == addsp && numspirr == 1 && (bool)ListRisDoc && (bool)ListMriDoc || adding == addmri && (int)ListMriDoc == 1 && (bool)ListRisDoc && numspirr)
        {
            TInverseView *viewis;
            TBaseView *view;
            newview = true;
        ...
        if ((bool)ListEegDoc && numspirr && (bool)ListIsDoc && (bool)ListXyzDoc && (bool)ListMriDoc)
        {
            ...
            { // scan pairs of views
                ...
                for (view = ListEegDoc[i]->GetViewList(); view != 0; view = ListEegDoc[i]->NextView(view))
                    ...
                    if (dynamic_cast<TPotentialsView *>(view) && view->GODoc == this)

                        for (view2 = ListEegDoc[i]->GetViewList(); view2 != 0; view2 = ListEegDoc[i]->NextView(view2))

                            if (dynamic_cast<TInverseView *>(view2) && view2->GODoc == this && view->LinkedViewId == view2->LinkedViewId)
        ...
        if (refresh)
        {
            ...
            if (adding == addxyz || adding == addsp || adding == addis || adding == addroi || adding == addmri)
                                     ...
                                     const TGoF &lris) const
    {
        ...
        if (leeg.IsNotEmpty() && lxyz.IsNotEmpty() && CompatEegs.NumTracks != CompatElectrodes.NumElectrodes)
        {
        ...
        if (leeg.IsNotEmpty() && lis.IsNotEmpty() && CompatEegs.NumTracks != CompatInverse.NumElectrodes)
        {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Testing XYZ vs IS
        if (lxyz.IsNotEmpty() && lis.IsNotEmpty() && CompatElectrodes.NumElectrodes != CompatInverse.NumElectrodes)
        {
        ...
        if (lsp.IsNotEmpty() && lis.IsNotEmpty() && CompatSp.NumSolPoints != CompatInverse.NumSolPoints)
        {
        ...
        // also checking vs numelectrodes?
        if (lsp.IsNotEmpty() && lris.IsNotEmpty() && CompatRis.NumSolPoints != CompatSp.NumSolPoints)
        {
    ...
    bool TLinkManyDoc::NotifyDocViews(int event, TParam2 item, TView *vexclude, TDocument *dexclude)
    {
        ...
        foreachin(ListEegDoc, iteratoreeg) if (iteratoreeg() != dexclude)
        {
        ...
        foreachin(ListRisDoc, iteratorris) if (iteratorris() != dexclude)
        {
        ...
        foreachin(ListXyzDoc, iteratorel) if (iteratorel() != dexclude)
        {
        ...
        foreachin(ListSpDoc, iteratorsp) if (iteratorsp() != dexclude)
        {
    ...
    void TLinkManyDoc::Segment()
    {
        ...
        if ((bool)ListEegDoc)
        {
            ...
            foreachin(ListEegDoc, iteratoreeg)
                segdlg.GoGoF.GetLast()
                    .Add(iteratoreeg()->GetDocPath(), MaxPathShort);
        }
        ...
        if ((bool)ListRisDoc)
        {
            ...
            foreachin(ListRisDoc, iteratorris)
                segdlg.GoGoF.GetLast()
                    .Add(iteratorris()->GetDocPath(), MaxPathShort);
        }
