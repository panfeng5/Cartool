...
namespace crtl
{
    ...
    //----------------------------------------------------------------------------
    //----------------------------------------------------------------------------
    TSolutionPointsDoc::TSolutionPointsDoc(TDocument *parent) : TBaseDoc(parent)
    {
    ...
    bool TSolutionPointsDoc::InitDoc()
    {
        ...
        if (!IsOpen())
        {
            ...
            if (StringIs((const char *)filename, "avg152t1v2.segrho"))
                GeometryTransform = new TGeometryTransform(TalairachAvg152T1Fld);
            else if (StringIs((const char *)filename, "avg152T1_smac_pro_grid.3005p"))
                GeometryTransform = new TGeometryTransform(TalairachAvg152T1_smac);
            else if (StringIs((const char *)filename, "ave152_sym_pro_grid.2738p"))
                GeometryTransform = new TGeometryTransform(TalairachAve152_sym);
            else if (StringContains((const char *)filename, "MNI-6239-voxels"))
                GeometryTransform = new TGeometryTransform(TalairachMNI152_2001);

            else if ((StringStartsWith((const char *)filename, "MNI152.Nlin") || StringStartsWith((const char *)filename, "NIHPD324.NlinAsym") || StringStartsWith((const char *)filename, "mni_icbm152_") || StringStartsWith((const char *)filename, "MNI152_")) && !StringContains((const char *)filename, "ToTalairach"))
                GeometryTransform = new TGeometryTransform(TalairachMNI152_2009);
            // wide guess, but prevents what seems to be converted files
            else if (StringStartsWith((const char *)filename, "MNI152") && !StringContains((const char *)filename, "ToTalairach"))
                GeometryTransform = new TGeometryTransform(TalairachMNI152_2001);

            else if (StringStartsWith((const char *)filename, "NIHPD.Asym.Infant") && !StringContains((const char *)filename, "ToTalairach"))
    ...
    void TSolutionPointsDoc::Sort(TPointFloatI *ListSPI, int l, int r)
    {
        ...
        do
        {
            while (ListSPI[i].Z < z ||
                   ...
                   (ListSPI[i].Z == z && ListSPI[i].X == x && ListSPI[i].Y < y))
                i++;

            while (z < ListSPI[j].Z ||
                   (z == ListSPI[j].Z && x < ListSPI[j].X) ||
                   (z == ListSPI[j].Z && x == ListSPI[j].X && y < ListSPI[j].Y))
                j--;
    ...
    void TSolutionPointsDoc::FindOrientation()
    {
        ...
        for (int sp = 0; sp < GetNumSolPoints(); sp += step)
        {
            ...
            if (StringContains(buff, 'L'))
                left += Points[sp];
            else if (StringContains(buff, 'R'))
                right += Points[sp];

            if (StringContains(buff, 'P'))
                post += Points[sp];
            else if (StringContains(buff, 'A'))
                ant += Points[sp];

            if (StringContains(buff, 'I'))
                inf += Points[sp];
            else if (StringContains(buff, 'S'))
                sup += Points[sp];
        }
        ...
        if (stat1.Sum2() < stat2.Sum2() && stat1.Sum2() < stat3.Sum2())
            XAxis = LeftRight;
        if (stat2.Sum2() < stat1.Sum2() && stat2.Sum2() < stat3.Sum2())
            YAxis = LeftRight;
        if (stat3.Sum2() < stat2.Sum2() && stat3.Sum2() < stat1.Sum2())
            ZAxis = LeftRight;
...
#define MaxRandomPairs 1000
        ...
        for (int randi = 0; randi < MaxRandomRepeat; randi++)
        {
            ...
            for (int i = 0; i < MaxRandomPairs; i++)
            {
                ...
                do
                    e2 = randunif((uint)GetNumSolPoints());
                while (e2 == e);
            ...
            if (XAxis)
                avgratio += fabs(globaldir.Y) / (globaldir.Z ? fabs(globaldir.Z) : 1e-6);
            else if (YAxis)
                avgratio += fabs(globaldir.X) / (globaldir.Z ? fabs(globaldir.Z) : 1e-6);
            else if (ZAxis)
                avgratio += fabs(globaldir.X) / (globaldir.Y ? fabs(globaldir.Y) : 1e-6);
            ...
            for (int x = Bounding->XMin(); x < Bounding->XMax(); x += /*step*/ mind)
            {
                ...
                for (int z = Bounding->ZMin(); z < Bounding->ZMax(); z += /*step*/ mind)
                {
                    ...
                    for (int sp = 0; sp < GetNumSolPoints(); sp++)
                    {
                        ...
                        if (fabs(Points[sp].X - x) <= mind && fabs(Points[sp].Z - z) <= mind)
                        {
    ...
    {
        if (!(interpol == SPInterpolationNone || interpol == SPInterpolation1NN || interpol == SPInterpolation4NN))
            return false;
        // already been computed?
        // we could redo the interpolation, if the previous call was not a grey mask and the current one is...
        if (interpol == SPInterpolationNone || interpol == SPInterpolation1NN && HasInterpol1NN() || interpol == SPInterpolation4NN && HasInterpol4NN())
            return true;
        ...
        OmpParallelFor
            ...
            for (int x = 1; x < greyvolfilled.GetDim1() - 1; x++) for (int y = 1; y < greyvolfilled.GetDim2() - 1; y++) for (int z = 1; z < greyvolfilled.GetDim3() - 1; z++)

                if (greyvol(x - 1, y, z) || greyvol(x, y - 1, z) || greyvol(x, y, z - 1) || greyvol(x + 1, y, z) || greyvol(x, y + 1, z) || greyvol(x, y, z + 1)
        ...
        if (interpol == SPInterpolation1NN)
            ComputeInterpol1NN(ListSPI, step, MRIGrey, greyvolfilled);
        else if (interpol == SPInterpolation4NN)
            ComputeInterpol4NN(ListSPI, step, MRIGrey, greyvolfilled);

        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        delete[] ListSPI;

        return interpol == SPInterpolation1NN && HasInterpol1NN() || interpol == SPInterpolation4NN && HasInterpol4NN();
    }
    ...
    void TSolutionPointsDoc::ComputeInterpol4NN(const TPointFloatI *ListSPI, double step, const TVolumeDoc *MRIGrey, const Volume &greyvol)
    {
        ...
        OmpParallelBegin
            ...
            TArray2<float>
                distlist(4, NumDistList); // list of the distances of the current closest 4 points
            ...
            for (pvol.Y = 0; pvol.Y < volsize.Y; pvol.Y++)
                for (pvol.X = 0; pvol.X < volsize.X; pvol.X++)
                {
                    ...
                    { // do a real interpolation
                        ...
                        double sumwi = 1.0 / (1.0 / distlist(0, Distance) + 1.0 / distlist(1, Distance) + 1.0 / distlist(2, Distance) + 1.0 / distlist(3, Distance));
