...
namespace crtl
{
    ...
    void TEegERPSSRdfDoc::ReadNativeMarkers()
    {
        ...
        for (int r = 0; r < NumRecords; r++)
        {
            ...
            for (int e = 0; e < recordheader.rrh_nevents; e++)
            {
                ...
                if (s == PAUSE_MARK)
                    StringCopy(triggername, "PAUSE");
                else if (s == DELETE_MARK)
                    StringCopy(triggername, "DELETE");
                else if (s & 0x8000)
                    StringCopy(triggername, "META-EVENT");
                else
                    sprintf(triggername, "%0hu", s);
        ...
        if (Compressed)
        {
            ...
            for (; reltf >= 0; reltf--)
            {
                ...
                for (int el = 0; el < NumElectrodes; el++)
                {
                    ...
                    if (numnib == 0 || numnib <= 1 && (nib[0] & 0x0c) == 0x08 || numnib <= 2 && (nib[0] & 0x0e) == 0x0c || numnib <= 3 && (nib[0] & 0x0f) == 0x0e)
                    {
                        // read next short
    ...
    void TEegERPSSRdfDoc::ReadRawTracks(long tf1, long tf2, TArray2<float> &buff, int tfoffset)
    {
        ...
        for (tfi = tf1, tf = 0; tfi <= tf2;)
        {
            ...
            {                              // set position to next data block, if not last block
