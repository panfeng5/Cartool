...
namespace crtl
{
    ...
    RegularizationType StringToRegularization(const char *regstr)
    {
        ...
        if (StringStartsWith(regstr, RegularizationNoneStringS))
            return RegularizationNone; // "N" or "None"
        else if (StringStartsWith(regstr, RegularizationAutoGlobalStringS))
            return RegularizationAutoGlobal; // "G" or "Global"
        else if (StringStartsWith(regstr, RegularizationAutoLocalStringS))
            return RegularizationAutoLocal; // "L" or "Local"
        else
        {
    ...
    {
        if (reg == RegularizationNone)
            StringCopy(regstr, shortversion ? RegularizationNoneStringS : "No Regularization");
        else if (reg == RegularizationAutoGlobal)
            StringCopy(regstr, shortversion ? RegularizationAutoGlobalStringS : "Global Optimal Regularization");
        else if (reg == RegularizationAutoLocal)
            StringCopy(regstr, shortversion ? RegularizationAutoLocalStringS : "Local Optimal Regularization");
        else
            IntegerToString(regstr, reg - Regularization0); // Regularization0 to Regularization12
