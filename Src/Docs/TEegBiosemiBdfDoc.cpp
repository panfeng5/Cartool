...
namespace crtl
{
    ...
    void TEegBiosemiBdfDoc::ReadNativeMarkers()
    {
        ...
        for (int tfi = 1, tf0 = 1; tf0 < MaxSamplesPerBlock - 1 && tfi < NumTimeFrames; tfi++, tf0++)
        {
            ...
            //  if ( *toT != 0xFF ) {
            if (*toT != *(toT + cellsize))
            { // if not constant in time, then should contain trigger information
        ...
        if (l != 0)
        { // reach the EOF with an unfinished trigger?
          // close it ourselves
    ...
    bool TEegBiosemiBdfDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            if (yy < 91)
                yy = 2000 + yy; // !format exist since 1991!
            else
                yy = 1900 + yy;
            ...
            for (int el = 0; el < NumElectrodes; el++)
            {
                ...
                double ratio = (physmax[el] - physmin[el]) / NonNull(digitalmax[el] - digitalmin[el]);
            ...
            for (int el = 0; el < NumElectrodesInFile; el++)
            {
                ...
                if (StringStartsWith(buff, "Auxiliar"))
                    AuxTracks.Set(el);
                else if (StringStartsWith(buff, "Computed"))
                    AuxTracks.Set(el);
                else if (StringStartsWith(buff, "Status"))
                    AuxTracks.Set(el);
                else if (StringStartsWith(buff, "Regular"))
                    AuxTracks.Reset(el);
            }
