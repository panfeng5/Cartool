...
namespace crtl
{
    ...
    {
        return FiltersActivated || DirtyReference()
               //  ||  DirtyBadTracks ()
               //  ||  DirtyAuxTracks ()
               || DirtySamplingFrequency || TBaseDoc::IsDirty();
    }
    ...
    bool TTracksDoc::Commit(bool force)
    {
        ...
        if (RISDoc && IsExtension(FILEEXT_RIS) && IsVector(AtomTypeUseOriginal))
            ...
            if (GetOptionFromUser("Saving Vectorial RIS file as:" NewLine
                                      NewLine Tab "(V)ectorial, with the original un-filtered data" NewLine Tab "(S)calar, with the current filters",
    ...
    void TTracksDoc::InitContentType()
    {
        ...
        else if (dynamic_cast<TTracksDoc *>(this))
        {
            ...
            else if (dynamic_cast<TEegCartoolSefDoc *>(this) || dynamic_cast<TEegBrainVisionDoc *>(this) || IsExtension(FILEEXT_EEGEDF))
            {
        ...
        if ((StringIs(ext2, InfixSpectrum) || StringContains((const char *)filename, (const char *)"." InfixSpectrum)) && IsPositive(AtomTypeUseOriginal))
        {
        ...
        if ((StringIs(ext2, InfixP) || StringIs(ext2, Infix1MinusP) || StringIs(ext2, InfixLogP) || StringContains((const char *)filename, (const char *)"." Infix1MinusP ".")) && IsPositive(AtomTypeUseOriginal))
        {
        ...
        if (ExtraContentType == TracksContentERP && NumElectrodes > 1024)
        {
        ...
        if (lookfortemplates && NumTimeFrames > 1)
        {
            ...
            if (stats(PseudoTrackOffsetGfp).SNR() > 1e3                   // EEG Templates around 1e6, others around 4
                ...
                || stats(PseudoTrackOffsetAvg).SNR() > 1e1)
            { // ESI Templates around 40, others around 4
    ...
    {
        SetReferenceType(IsErp() && !(IsAbsolute(AtomTypeUseOriginal) || IsUnknownType(AtomTypeUseOriginal)) ? ReferenceAverage : ReferenceAsInFile);
    }
    ...
    void TTracksDoc::InitFilters()
    {
        ...
        if (IsAbsolute(AtomTypeUseOriginal) || SamplingFrequency == 0 || TimeFrameToSeconds(NumTimeFrames, SamplingFrequency) < 10) // at least 10s of data!
            return;
        ...
        needshighpass = stats.IsNotEmpty() && stats.Average() > 0 && (stats.Average() < 0.06 && stats.SD() < 1.035);
    ...
    void TTracksDoc::InitDateTime()
    {
        ...
        { // either high SF, or a SF that gives non-integer steps (f.ex. 128 Hz)
    ...
    void TTracksDoc::InitSD()
    {
        ...
        for (int sdi = 0; sdi < 6; sdi++)
        {
            ...
            if (sdi == 0)
            { // file.Mean.ext    -> file.SE.ext
                if (grepeegmean.Matched(GetDocPath()))
                    StringReplace(filesd, InfixMean, InfixSE);
                else
                    continue;
            }
            else if (sdi == 1)
            { // file.Mean.ext    -> file.SD.ext
                if (grepeegmean.Matched(GetDocPath()))
                    StringReplace(filesd, InfixMean, InfixSD);
                else
                    continue;
            }
            // file.ext         -> file.SE.ext
            else if (sdi == 2)
                PostfixFilename(filesd, "." InfixSE);

            // file.ext         -> file.SD.ext
            else if (sdi == 3)
                PostfixFilename(filesd, "." InfixSD);

            // file.ext         -> file.EPSE
            else if (sdi == 4)
                ReplaceExtension(filesd, FILEEXT_EEGEPSE);

            // file.ext         -> file.EPSD
            else if (sdi == 5)
                ReplaceExtension(filesd, FILEEXT_EEGEPSD);
            ...
            if (SDEDoc->GetNumElectrodes() == NumElectrodes && SDEDoc->GetNumTimeFrames() == NumTimeFrames && SDEDoc->GetSamplingFrequency() == SamplingFrequency)
            {
    ...
    void TTracksDoc::InitAuxiliaries()
    {
        ...
        if (GetRegistryName(UptoSubversion, regname) && *CartoolApplication->GetPreference(regname, PrefAuxiliaries, auxnames))
        {
                               ...
                               const TRois *rois)
    {
        ...
        if (reference == ReferenceUsingCurrent || reference == ReferenceArbitraryTracks && referencetracks == 0)
        {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Filters & reference
        if (doanyfilter || IsEffectiveReference(reference))
        ...
        if (pseudotracks)
        {
            ...
            auto ComputeDis = [this, &numt, &rescalevector](TArray2<float> &buff1, int tfo1, double avg1, double gfp1, TArray2<float> &buff2, int tfo2, double avg2, double gfp2)
            {
                ...
                for (int el = 0; el < NumElectrodes; el++)
                    if (ValidTracks[el])
                        sumsqr += Square((buff1(el, tfo1) - avg1) / gfp1 - (buff2(el, tfo2) - avg2) / gfp2);
            ...
            for (int tf = 0, tfo = tfoffset; tf < numtf; tf++, tfo++)
            {
                ...
                if (tf == 0)
                { // first occurence in the loop, which means we have to handle some missing data

                    if (tf1 == 0)
                    { // first actual tf: dissimilarity can not be computed, as there is no previous data to compare from

                        buff(OffDis, tfo) = 0;
                    }
                    else
                    { // tf1 > 0 - previous data exist and is to be found in our local 1 TF buffer
                ...
                { // tf > 0 - NOT the first occurence in the loop, and avgbefore and gfpbefore exist
