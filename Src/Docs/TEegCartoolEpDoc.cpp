...
namespace crtl
{
    ...
    bool TEegCartoolEpDoc::Open(int /*mode*/, const char *path)
    {
        ...
        if (GetDocPath())
        {
            ...
            if (IsExtension(FILEEXT_EEGEP))
                StringCopy(ProductName, FILEEXT_EEGEP);
            else if (IsExtension(FILEEXT_EEGEPH))
                StringCopy(ProductName, FILEEXT_EEGEPH);
            else if (IsExtension(FILEEXT_EEGEPSD))
                StringCopy(ProductName, FILEEXT_EEGEPSD);
            else if (IsExtension(FILEEXT_EEGEPSE))
                StringCopy(ProductName, FILEEXT_EEGEPSE);
            Version = 1;
        ...
        { // new file -> choose how to create it
          // stand. dev. not created here!
            if (IsExtension(FILEEXT_EEGEPSD) || IsExtension(FILEEXT_EEGEPSE))
            {
                return false;
            }
    ...
    {
        if (IsExtension(FILEEXT_EEGEPSD) || IsExtension(FILEEXT_EEGEPSE)) // stand. err.: no average reference
