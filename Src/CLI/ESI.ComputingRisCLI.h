...
namespace crtl
{
    ...
    inline void ComputingRisCLI(CLI::App *computingris, const TGoF &gof)
    {
        ...
        if (!IsSubCommandUsed(computingris) || (gof.IsEmpty() /*&& listfiles.IsEmpty ()*/))
        ...
        if (CanOpenFile(xyzfile) && HasCLIOption(computingris, __spatialfilter))
        {
        ...
        bool savingzscorefactors = HasCLIFlag(computingris, __savingzscore) && backnorm != BackgroundNormalizationNone;
