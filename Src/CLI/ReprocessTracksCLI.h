...
namespace crtl
{
    ...
    inline void ReprocessTracksCLIDefine(CLI::App *reprocsub)
    {
        ...
        DefineCLIOptionString(reprocsub, "", __filters, "A whole string of filtering options, in double quote, as coming from a verbose file")
            ...
            ->CheckOption([](const string &str)
                          { return str.find("-", 0) == 0 ? "string must be in double quotes" : ""; });
        ...
        DefineCLIOptionDouble(reprocsub, "", __highpass, "High-Pass Butterworth filter")
            ->CheckOption([](const string &str)
                          { return StringToDouble(str.c_str()) <= 0 ? "frequency cut should be above 0" : ""; });
        ...
        NeedsCLIOption(reprocsub, __envelope, __rectification) // dialog does not enforce this, just in case data is already positive, but we do here
            ->CheckOption([](const string &str)
                          { return StringToDouble(str.c_str()) <= 0 ? "smoothing window, in [ms], should be positive" : ""; });
        ...
        DefineCLIOptionDouble(reprocsub, "", __samplingfrequency, "Default Sampling Frequency, only in case a file has none")
            ->CheckOption([](const string &str)
                          { return StringToDouble(str.c_str()) <= 0 ? "sampling frequency should be above 0" : ""; });
        ...
        DefineCLIOptionInt(reprocsub, "", __downsampling, "Downsampling ratio")
            ->CheckOption([](const string &str)
                          { return StringToInteger(str.c_str()) <= 1 ? "factor should be above 1" : ""; });
    ...
    {
        if (!IsSubCommandUsed(reprocsub) || gof.IsEmpty())
        ...
        if (HasCLIOption(reprocsub, __timemin) || HasCLIOption(reprocsub, __timemax))
        {
        ...
        if (defaultsamplingfrequency == 0)
            ...
            for (int filei = 0; filei < (int)gof; filei++)
                ...
                if (ReadFromHeader(gof[filei], ReadSamplingFrequency, &defaultsamplingfrequency) && defaultsamplingfrequency > 0)
        ...
        if (HasCLIOption(reprocsub, __bandpass) || HasCLIOption(reprocsub, __highpass) && HasCLIOption(reprocsub, __lowpass))
        {
        ...
        if (HasCLIOption(reprocsub, __spatialfilter) && HasCLIOption(reprocsub, __xyzfile))
        {
        ...
        ReferenceType ref = reflist.empty() || reflist == "none" || reflist == "asinfile" ? ReferenceAsInFile
        ...
        int downsampleratio = timeoptions == ExportTimeInterval && sequenceoptions == SequenceProcessing ? GetCLIOptionInt(reprocsub, __downsampling)
                                                                                                         : 0;
...
#undef CLIConsoleOutput
        ...
        if (RoisDoc.IsOpen())
        {
            cout << roisfile << ", " << RoisDoc->ROIs->GetName() << ": " << RoisDoc->ROIs->GetNumRois() << " x " << RoisDoc->ROIs->GetDimension();
        }
        else if (XYZDoc.IsOpen())
        {
            cout << xyzfile << ": " << XYZDoc->GetNumElectrodes();
        }
        else
        {
            cout << "EEG tracks names";
        }
        cout << NewLine;

        cout << "Tracks: " << (tracks.empty() ? "All tracks" : tracks) << NewLine;

        cout << "Time Options: " << (timeoptions == ExportTimeInterval ? "Time Interval" : timeoptions == ExportTimeTriggers ? "Keeping triggers List"
        ...
        cout << "Rescaling: " << (rescalingoptions == NotRescaled ? "No rescaling" : rescalingoptions == ConstantRescaling ? "Constant rescaling"
        ...
        cout << "Sequence Option: " << (sequenceoptions == SequenceProcessing ? "Sequential Data" : sequenceoptions == AverageProcessing ? "Averaging Data in Time"
                                                                                                                                         : "Unknown")
             << NewLine;
