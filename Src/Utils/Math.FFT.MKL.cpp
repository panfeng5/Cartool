...
namespace mkl
{
    ...
    void TMklFft::Set(MklProcessing how, FFTRescalingType rescaling, int numdata)
    {
        ...
        case FromReal:
        case BackToReal:
            Status = DftiCreateDescriptor(&mklh, DFTI_SINGLE, DFTI_REAL, 1, DataSize);
            break;

        case FromComplex:
        case BackToComplex:
            Status = DftiCreateDescriptor(&mklh, DFTI_SINGLE, DFTI_COMPLEX, 1, DataSize);
            break;
        }
        ...
        if (rescaling != FFTRescalingNone)
        {
            ...
            if (How == FromReal || How == FromComplex)

                DftiSetValue(mklh, DFTI_FORWARD_SCALE, rescaling == FFTRescalingSqrtWindowSize ? 1 / sqrt((double)DataSize) : rescaling == FFTRescalingWindowSize ? 1 / (double)DataSize
                                                                                                                                                                  : /*FFTRescalingNone*/ 1);

            else if (How == BackToReal || How == BackToComplex)

                DftiSetValue(mklh, DFTI_BACKWARD_SCALE, rescaling == FFTRescalingSqrtWindowSize ? 1 / sqrt((double)DataSize) : rescaling == FFTRescalingWindowSize ? 1 / (double)DataSize
                                                                                                                                                                   : /*FFTRescalingNone*/ 1);
        } // some rescaling
