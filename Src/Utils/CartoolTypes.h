...
namespace crtl
{
  ...
  inline bool IsUnresolvedType(AtomType at) { return at == UnknownAtomType || at == AtomTypeUseOriginal || at == AtomTypeUseCurrent; }
  ...
  inline bool IsAbsolute(AtomType at) { return at == AtomTypePositive || at == AtomTypeVector; }
  // Asking user
  ...
  inline bool CorrelateTypeSameDuration(CorrelateType c) { return c == CorrelateTypeTimeCorrelation || c == CorrelateTypeCoherence || c == CorrelateTypePhaseIntCoupling; }

  inline bool CorrelateTypeSameFrequencies(CorrelateType c) { return c == CorrelateTypeTimeCorrelation || c == CorrelateTypeCoherence; }

  inline bool CorrelateTypeFrequenciesAllowed(CorrelateType c) { return c == CorrelateTypeTimeCorrelation || c == CorrelateTypeCoherence || c == CorrelateTypePhaseIntCoupling; }
...
#define MinSDToKeepS "3"
  ...
  template <class TypeD>
  inline TypeD ZSignedToZPositive(TypeD &zsigned)
  {
    zsigned = AtLeast(0.0, zsigned / MinSDToKeep + 1.0);
    return zsigned;
  }
  template <class TypeD>
  inline TypeD ZPositiveToZSigned(TypeD &zpos)
  {
    zpos = (zpos - 1.0) * MinSDToKeep;
    return zpos;
  }
  ...
  public:
    ...
    virtual void SetAtomType(AtomType at)
    {
      if (IsUnknownType(OriginalAtomType))
        OriginalAtomType = at;
