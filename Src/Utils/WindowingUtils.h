...
namespace crtl
{
    ...
    inline void WindowMinimize(owl::TWindow *window)
    {
        if (window)
            window->ShowWindow(SW_SHOWMINIMIZED);
    }
    inline void WindowMaximize(owl::TWindow *window)
    {
        if (window)
            window->ShowWindow(SW_SHOWMAXIMIZED);
    }
    inline void WindowRestore(owl::TWindow *window)
    {
        if (window)
            window->ShowWindow(SW_RESTORE);
    }
    inline void WindowHide(owl::TWindow *window)
    {
        if (window)
            window->ShowWindow(SW_HIDE);
    } // Calling WindowRestore before, if we wish to avoid min / max and hidden

    inline void WindowSetOrigin(owl::TWindow *window, int left, int top)
    {
        if (window)
            window->SetWindowPos(0, left, top, 0, 0, SWP_NOCOPYBITS | SWP_SHOWWINDOW | SWP_NOSIZE);
    }
    inline void WindowSetSize(owl::TWindow *window, int width, int height)
    {
        if (window)
            window->SetWindowPos(0, 0, 0, width, height, SWP_NOCOPYBITS | SWP_SHOWWINDOW | SWP_NOMOVE);
    }
    inline void WindowSetPosition(owl::TWindow *window, int left, int top, int width, int height)
    {
        if (window)
            window->SetWindowPos(0, left, top, width, height, SWP_NOCOPYBITS | SWP_SHOWWINDOW);
    }
    inline void WindowSetFrameOrigin(owl::TWindow *window, int left, int top)
    {
        if (window)
            window->SetWindowPos(0, left, top, 0, 0, SWP_FRAMECHANGED | SWP_SHOWWINDOW | SWP_NOSIZE);
    }
    inline void RepositionMinimizedWindow(owl::TWindow *window, int clientheight);
    inline void WindowSetGaugeSize(owl::TWindow *window)
    {
        WindowRestore(window);
        WindowSetSize(window, 600 /*2.5 * GaugeWidth*/, 240 /*7 * GaugeHeight*/);
    }
    ...
    WindowState GetWindowState(const owl::TWindow *window)
    {
        if (window == 0)
            ...
            return WindowStateHidden; // !test early as window could still have the min / max flags on, and be hidden!
        else if (IsWindowMinimized(window))
    ...
    void SetWindowState(owl::TWindow *window, WindowState newstate)
    {
        if (window == 0)
            return;
        else if (newstate == WindowStateMinimized)
            WindowMinimize(window);
        else if (newstate == WindowStateMaximized)
            WindowMaximize(window);
        else if (newstate == WindowStateHidden)
            WindowHide(window);
        else
            WindowRestore(window);
    }
    ...
    void RepositionMinimizedWindow(owl::TWindow *window, int clientheight)
    {
        ...
        if (window == 0 || !GetWindowPlacement(window->GetHandle(), &wndpl) || wndpl.showCmd != SW_SHOWMINIMIZED)
    ...
    */
    ...
    owl::TRect GetMonitorRect(const char *monitorname, DWORD how)
    {
        ...
        if (how == ENUM_CURRENT_SETTINGS || how == ENUM_REGISTRY_SETTINGS)
        {
