...
namespace crtl
{
    ...
    long double Student_TwoTails_t_to_p(long double t, int dof)
    {
        ...
        if (IsOdd(dof))
            p = 1.0 - (theta + sinetheta * cosinetheta * Sum(Square(cosinetheta), 2, dof - 3)) / HalfPi;
        else
            p = 1.0 - sinetheta * Sum(Square(cosinetheta), 1, dof - 3);
                          ...
                          bool clippingpvalue, double pvaluemin)
    {
        ...
        case OutputP:
            return clippingpvalue && p > pvaluemin ? 1.0 : p; // !output 1 to signify not significative!
        case Output1MinusP:
            return clippingpvalue && p > pvaluemin ? 0.0 : 1.0 - p;
        case OutputMinusLogP:
            return clippingpvalue && p > pvaluemin || p <= 0 ? 0.0 : -Log10(p);
        default:
            return 0.0;
        }
    }
                               ...
                               TArray1<int> &sampleindex)
    {
        ...
        for (int s = 0; s < numsamples; s++)
            ...
            if (data1(tf1, v, s) != missingvalue && data2(tf2, v, s) != missingvalue)
                                    ...
                                    TArray3<float> &results)
    {
        ...
        OmpParallelBegin
            ...
            TArray1<int>
                sampleindex(checkmissingvalues ? numsamples : 0);
                    ...
                    TArray3<float> &results1, TArray3<float> &results2, TArray3<float> &jointresults)
    {
        ...
        StringCopy(title, PairedTypeString[paired], " "
                                                    "t-test");
                ...
                for (int tf0 = 0; tf0 < numtf; tf0++)
            {
                ...
                for (int v = 0; v < numvars; v++)
                {
                    ...
                    for (int s = 0; s < numrealsamples; s++)
                    {
                        ...
                        long double temp = data1(tf1, v, checkmissingvalues ? sampleindex[s] : s) - data2(tf2, v, checkmissingvalues ? sampleindex[s] : s);
        ...
        { // unpaired ttest
                ...
                for (int tf0 = 0; tf0 < numtf; tf0++)
            {
                ...
                for (int v = 0; v < numvars; v++)
                {
                    ...
                    if (IsTimeSequential(stattime1) || tf0 == 0)
                        numrealsamples1 = results1(tf1, v, TestNumSamples);
                    if (IsTimeSequential(stattime2) || tf0 == 0)
                        numrealsamples2 = results2(tf2, v, TestNumSamples);
                    ...
                    long double avg = results1(tf1, v, TestMean) - results2(tf2, v, TestMean);
                    // joint var and sd
                    long double var = abs(((numrealsamples1 - 1) * abs(results1(tf1, v, TestVariance)) + (numrealsamples2 - 1) * abs(results2(tf2, v, TestVariance))) / NonNull(dof));
                    // unpaired ttest
                    long double sterr = sqrt(abs(results1(tf1, v, TestVariance) / numrealsamples1 + results2(tf2, v, TestVariance) / numrealsamples2));
                ...
                for (int tf0 = 0; tf0 < numtf; tf0++)
            {
                ...
                for (int v = 0; v < numvars; v++)
                {
                    ...
                    for (int s = 0; s < numrealsamples; s++)
                        ...
                        sumdata += data1(tf1, v, checkmissingvalues ? sampleindex[s] : s) - data2(tf2, v, checkmissingvalues ? sampleindex[s] : s);
                ...
                for (int tf0 = 0; tf0 < numtf; tf0++)
            {
                ...
                for (int v = 0; v < numvars; v++)
                {
                    ...
                    for (int s = 0; s < numrealsamples1; s++)
                        sum1 += data1(tf1, v, checkmissingvalues ? sampleindex1[s] : s);

                    long double sum2 = 0;

                    for (int s = 0; s < numrealsamples2; s++)
                        sum2 += data2(tf2, v, checkmissingvalues ? sampleindex2[s] : s);

                    // !this average retains the sign!
                    long double avg = sum1 / numrealsamples1 - sum2 / numrealsamples2;
                    ...
                    for (int e = 0; e < numrand; e++)
                    {
                        ...
                        do
                            group1pick.Set(randunif((uint)numrealsamples));
                        ...
                        for (int s = 0; s < numrealsamples1; s++)
                            ...
                            if (group1pick[s])
                                ...
                                sum2 += data1(tf1, v, checkmissingvalues ? sampleindex1[s] : s);

                        // assign to sums from group2
                        for (int s = 0; s < numrealsamples2; s++)

                            if (group1pick[s + numrealsamples1])
                                sum1 += data2(tf2, v, checkmissingvalues ? sampleindex2[s] : s);
                            else
                                sum2 += data2(tf2, v, checkmissingvalues ? sampleindex2[s] : s);

                        // 2-tailed, does not care for positive or negative
                        if (abs(sum1 / numrealsamples1 - sum2 / numrealsamples2) >= sumdata)
                            countabove++;
                        ...
                        TRandCoin &randcoin)
    {
        ...
        for (int ci = 0, ri1 = 0, ri2 = 0; ci < numpercentroid; ci++)
        {
            ...
            g1toc1 = ri1 < nummaps1 // check that we are not trying to pick more data than it exists
                         && group1tocentroid1 && (!group2tocentroid1 || group2tocentroid1 && randcoin()) ||
            ...
            { // centroid2: unpaired case

                g1toc2 = ri1 < nummaps1 && group1tocentroid2 && (!group2tocentroid2 || group2tocentroid2 && randcoin()) || ri2 == nummaps2;
