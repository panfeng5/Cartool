...
namespace crtl
{
    ...
    {
        if (OriginDateAvailable)
            StringCopy(s, AsString().c_str());
        else
            ClearString(s);
    ...
    char *TDateTime::GetStringTime(double us, char *s, bool interval) const
    {
        ...
        if (RelativeTime || interval || !OriginTimeAvailable)
        {
            ...
            {
                if (fabs(us) < 1000)
                    sprintf(s, "%s%0d�s", SignToString(us), ToMicrosecond(us));
                else if (fabs(us) / 1e3 < 1000)
                    sprintf(s, "%s%0dms%03d", SignToString(us), ToMillisecond(us), ToMicrosecond(us));
                else if (fabs(us) / 1e6 < 60)
                    sprintf(s, "%s%0ds%03d.%03d", SignToString(us), ToSecond(us), ToMillisecond(us), ToMicrosecond(us));
                else if (fabs(us) / 60e6 < 60)
                    sprintf(s, "%s%0d:%02d.%03d.%03d", SignToString(us), ToMinute(us), ToSecond(us), ToMillisecond(us), ToMicrosecond(us));
                else
            ...
            { // millisecond precision
                if (fabs(us) / 1e3 < 1000)
                    sprintf(s, "%s%0dms", SignToString(us), ToMillisecond(us));
                else if (fabs(us) / 1e6 < 60)
                    sprintf(s, "%s%0ds%03d", SignToString(us), ToSecond(us), ToMillisecond(us));
                else if (fabs(us) / 60e6 < 60)
                    sprintf(s, "%s%0d:%02d.%03d", SignToString(us), ToMinute(us), ToSecond(us), ToMillisecond(us));
                else
    ...
    double TDateTime::RelUsToAbsUs(double us, bool interval) const
    {
        if (interval)
            ...
            return us + RelativeTimeOffset;
        else
            return us + AbsoluteTimeOffset;
    ...
    */
    ...
    if ( OriginTimeAvailable && !( RelativeTime || interval ) ) {
        ...
        if ( MicrosecondPrecision ) {
            if      ( fabs ( us ) <                  1000 ) sprintf ( s, "%s%0d�s",                   us >= 0 ? "" : "-", ToMicrosecond ( us ) );
