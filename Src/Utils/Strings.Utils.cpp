...
namespace crtl
{
    ...
    {
        return StringContains("bcdfghjklmnpqrstvwxzBCDFGHJKLMNPQRSTVWXZ��", c, CaseSensitive);
    }
    ...
    bool StringToBool(char *s)
    {
        ...
        if (StringToTrue(s))
            return true;
        else if (StringToFalse(s))
            return false;
        else
            return false; // don't know? say false
    }
    ...
    */
    ...
    const char *StringContains(const char *searched, const char tosearch, StringFlags flags)
    {
        ...
        if (IsFlag(flags, StringContainsBackward))
            return strrchr(searched, tosearch);
        // default is forward
        else /*( IsFlag ( flags, StringContainsForward  ) )*/
            return strchr(searched, tosearch);
    }
    ...
    char *StringCopy(char *to, const char *from, long totallength)
    {
        ...
        if (to == from)
        { // copying onto itself is useless
    ...
    char *StringAppend(char *to, const char *tail1, const char *tail2, const char *tail3, const char *tail4, const char *tail5, const char *tail6, const char *tail7, const char *tail8)
    {
        ...
        if (StringIsNotEmpty(tail3))
            strcat(to, tail3);
        if (StringIsNotEmpty(tail4))
            strcat(to, tail4);
        if (StringIsNotEmpty(tail5))
            strcat(to, tail5);
        if (StringIsNotEmpty(tail6))
            strcat(to, tail6);
        if (StringIsNotEmpty(tail7))
            strcat(to, tail7);
        if (StringIsNotEmpty(tail8))
            strcat(to, tail8);
    ...
    char *TimeFrameToString(char *s, int tf)
    {
        ...
        if (tf < 1000)
            ...
            sprintf(s, "%0d'%03d", tf / 1000, tf % 1000);
        else if (tf < 1000000000)
            sprintf(s, "%0d'%03d'%03d", tf / 1000000, (tf / 1000) % 1000, tf % 1000);
        else
            sprintf(s, "%0d'%03d'%03d'%03d", tf / 1000000000, (tf / 1000000) % 1000, (tf / 1000) % 1000, tf % 1000);
    ...
    double GetFirstNumber(const char *string)
    {
        ...
        for (int i = 0; i < (int)splits; i++)
        {
            ...
            if (sscanf(splits[i], "%lf", &scandouble) == 1)
                return scandouble;
            if (sscanf(splits[i], "%lf%%", &scandouble) == 1)
                return scandouble;
            if (sscanf(splits[i], "%lf%*s", &scandouble) == 1)
                return scandouble;
        }
    ...
    double StringToTimeFrame(const char *string, double samplingfrequency, bool *answerintf)
    {
        ...
        else if (samplingfrequency == 0 || (int)split == 1)
        {
        ...
        else if ((int)split == 3)
        {
            ...
            if (IsStringAmong(split[1], ": M"))
                ...
                tf = MinutesToTimeFrame(StringToDouble(split[0]), samplingfrequency) + SecondsToTimeFrame(StringToDouble(split[2]), samplingfrequency);

            else if (StringIs(split[1], "H"))

                tf = HoursToTimeFrame(StringToDouble(split[0]), samplingfrequency) + MinutesToTimeFrame(StringToDouble(split[2]), samplingfrequency);

            else if (StringIs(split[1], "S"))

                tf = SecondsToTimeFrame(StringToDouble(split[0]), samplingfrequency) + MillisecondsToTimeFrame(StringToDouble(split[2]), samplingfrequency);
        ...
        else if ((int)split == 4)
        {
            ...
            if (IsStringAmong(split[1], ": H") || IsStringAmong(split[3], ": M"))

                tf = HoursToTimeFrame(StringToDouble(split[0]), samplingfrequency) + MinutesToTimeFrame(StringToDouble(split[2]), samplingfrequency);

            else if (StringIs(split[1], "M") || StringIs(split[3], "S"))

                tf = MinutesToTimeFrame(StringToDouble(split[0]), samplingfrequency) + SecondsToTimeFrame(StringToDouble(split[2]), samplingfrequency);

            else if (StringIs(split[1], "S") || StringIs(split[3], "L"))
        ...
        else if ((int)split == 5)
        {
            ...
            if (IsStringAmong(split[1], ": H") && IsStringAmong(split[3], ": M"))
                ...
                tf = HoursToTimeFrame(StringToDouble(split[0]), samplingfrequency) + MinutesToTimeFrame(StringToDouble(split[2]), samplingfrequency) + SecondsToTimeFrame(StringToDouble(split[4]), samplingfrequency);
        ...
        else if ((int)split == 6)
        {
            ...
            if (StringIs(split[1], "H") && StringIs(split[3], "M") && StringIs(split[5], "S"))

                tf = HoursToTimeFrame(StringToDouble(split[0]), samplingfrequency) + MinutesToTimeFrame(StringToDouble(split[2]), samplingfrequency) + SecondsToTimeFrame(StringToDouble(split[4]), samplingfrequency);

            else if (StringIs(split[1], "M") && StringIs(split[3], "S") && StringIs(split[5], "L"))

                tf = MinutesToTimeFrame(StringToDouble(split[0]), samplingfrequency) + SecondsToTimeFrame(StringToDouble(split[2]), samplingfrequency) + MillisecondsToTimeFrame(StringToDouble(split[4]), samplingfrequency);
        ...
        else if ((int)split == 7)
        {
            ...
            if (StringIs(split[1], "H") && StringIs(split[3], "M") && StringIs(split[5], "S"))
                ...
                tf = HoursToTimeFrame(StringToDouble(split[0]), samplingfrequency) + MinutesToTimeFrame(StringToDouble(split[2]), samplingfrequency) + SecondsToTimeFrame(StringToDouble(split[4]), samplingfrequency) + MillisecondsToTimeFrame(StringToDouble(split[6]), samplingfrequency);
        } // 7 items
        ...
        else if ((int)split == 8)
        {
            ...
            if (StringIs(split[1], "H") && StringIs(split[3], "M") && StringIs(split[5], "S") && StringIs(split[7], "L"))
