...
namespace crtl
{
    ...
    {
        if (p == 0)
            return 1; // force 0^0 = 1, as this is rather used for polynomials
        else
            return pow(v, p);
    }
...
#define HernandezJohnsonLambda6 0.3124
    ...
    */
    ...
    double GeneralizedGaussian(double x, double center, double width, double height, double skew)
    {
        ...
        // test range
        if (skew > 0 && x <= -width / skew || skew < 0 && x >= -width / skew)
            return 0;
    ...
    double Gamma(double x, double center, double width, double height, int shape, double scale)
    {
        ...
        return x <= 0 ? 0                            // official definition
                      : height * Power(x, shape - 1) // k: 1=exponential, 2=log-normal, ++=Gaussian
                            * exp(-x / scale);
    }
    ...
    double CubicHermiteSpline(double xp, double x0, double x1, double x2, double t)
    {
        ...
        return ((-t3 + 2 * t2 - t) * xp + (3 * t3 - 5 * t2 + 2) * x0 + (-3 * t3 + 4 * t2 + t) * x1 + (t3 - t2) * x2) / 2;
    }
    ...
    double UniformCubicBSpline(double xp, double x0, double x1, double x2, double t)
    {
        ...
        // using the 4 basis functions:
        return ((-t3 + 3 * t2 - 3 * t + 1) * xp + (3 * t3 - 6 * t2 + 4) * x0 + (-3 * t3 + 3 * t2 + 3 * t + 1) * x1 + (t3)*x2) / 6;
    }
    ...
    */
                               ...
                               double &x1, double &x2)
    {
        ...
        if (fabs(y2 / h2) < 1e-3)
            numsol--; // ignore second intersection
        ...
        if (fabs(y1 / h2) < 1e-3)
            if (numsol == 2)
            {
                x1 = x2;
                numsol--;
            } // move second intersection to first
            else
                numsol = 0; // that is bad news!
                         ...
                         bool clipborders)
    {
        ...
        return (x - xmin) / (xmax - xmin) * (toymax - toymin) + toymin;
    }
