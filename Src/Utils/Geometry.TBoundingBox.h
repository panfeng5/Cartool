...
namespace crtl
{
    ...
    public:
        ...
        TypeD Min(int axis) const { return axis == 0 ? XMin() : axis == 1 ? YMin()
                                                                          : ZMin(); }
        TypeD Max(int axis) const { return axis == 0 ? XMax() : axis == 1 ? YMax()
                                                                          : ZMax(); }
        void GetMin(float *min) const
        {
            min[0] = xmin;
            min[1] = ymin;
            min[2] = zmin;
        }
        void GetMax(float *max) const
        {
            max[0] = xmax;
            max[1] = ymax;
            max[2] = zmax;
        ...
        TypeD GetExtent(int axis) const { return axis == 0 ? GetXExtent() : axis == 1 ? GetYExtent()
                                                                                      : GetZExtent(); }
        ...
        double *GetMiddle(double *m) const
        {
            m[0] = (double)(xmin + xmax) / 2;
            m[1] = (double)(ymin + ymax) / 2;
            m[2] = (double)(zmin + zmax) / 2;
            return m;
        }
        void GetMiddle(TPointFloat &v) const
        {
            v[0] = (double)(xmin + xmax) / 2;
            v[1] = (double)(ymin + ymax) / 2;
            v[2] = (double)(zmin + zmax) / 2;
        }
        void GetMiddle(TypeD &x, TypeD &y, TypeD &z) const
        {
            x = (double)(xmin + xmax) / 2;
            y = (double)(ymin + ymax) / 2;
            z = (double)(zmin + zmax) / 2;
        }
        void GetMiddle(TypeD &x, TypeD &y) const
        {
            x = (double)(xmin + xmax) / 2;
            y = (double)(ymin + ymax) / 2;
        }
        ...
        TypeD GetRadius(int axis) const { return axis == 0 ? GetXRadius() : axis == 1 ? GetYRadius()
                                                                                      : GetZRadius(); }
        // absolute radius is distance from 0 to center (used for clipping)
        double AbsRadius() const
        {
            double r0 = sqrt(center.X * center.X + center.Y * center.Y + center.Z * center.Z);
            double r = radius;
            return max(r0, r);
        }
        ...
        bool Contains(const TBoundingBox<double> &b) // give a bit of margin when comparing doubles and ints
            const
        {
            return Contains(b.XMin() + 0.5, b.YMin() + 0.5, b.ZMin() + 0.5) && Contains(b.XMax() - 0.5, b.YMax() - 0.5, b.ZMax() - 0.5);
        }
        ...
        TBoundingBox<TypeD> &operator|=(const TBoundingBox<TypeD> &op2)
        {
            Mined(xmin, op2.xmin);
            Mined(ymin, op2.ymin);
            Mined(zmin, op2.zmin);
    ...
    void TBoundingBox<TypeD>::Set(const Volume &volume, bool automatic, double backvalue)
    {
        ...
        */
        ...
        if (fabs((double)(left[0] - left[1])) < 0.05 * volume.GetDim1() && fabs((double)(right[0] - right[1])) < 0.05 * volume.GetDim1() && fabs((double)(top[0] - top[1])) < 0.05 * volume.GetDim2() && fabs((double)(bottom[0] - bottom[1])) < 0.05 * volume.GetDim2() && fabs((double)(front[0] - front[1])) < 0.05 * volume.GetDim3() && fabs((double)(back[0] - back[1])) < 0.05 * volume.GetDim3())
    ...
    bool TBoundingBox<TypeD>::Contains(const TPointDouble &p, double margin) const
    {
        ...
        return IsInsideLimits((double)p.X, xmin - margin * GetXExtent(), xmax + margin * GetXExtent()) && IsInsideLimits((double)p.Y, ymin - margin * GetYExtent(), ymax + margin * GetYExtent()) && IsInsideLimits((double)p.Z, zmin - margin * GetZExtent(), zmax + margin * GetZExtent());
    }
    ...
    void TBoundingBox<TypeD>::Set(int nump, const TPointFloat *listp)
    {
        ...
        OmpParallelSectionsBegin
            ...
            OmpSectionBegin for (int i = 1; i < nump; i++)
        {
            if (listp[i].X < xmin)
                xmin = (TypeD)listp[i].X;
            if (listp[i].X > xmax)
                xmax = (TypeD)listp[i].X;
        }
        OmpSectionEnd
        ...
        {
            if (listp[i].Y < ymin)
                ymin = (TypeD)listp[i].Y;
            if (listp[i].Y > ymax)
                ymax = (TypeD)listp[i].Y;
        }
        OmpSectionEnd
        ...
        {
            if (listp[i].Z < zmin)
                zmin = (TypeD)listp[i].Z;
            if (listp[i].Z > zmax)
                zmax = (TypeD)listp[i].Z;
        }
        OmpSectionEnd
    ...
    {
        // check boundary
        if (v[0] < xmin || v[0] > xmax || v[1] < ymin || v[1] > ymax || v[2] < zmin || v[2] > zmax)
            return;
    ...
    template <>
    inline double TBoundingBox<double>::GetXExtent() const { return xmax - xmin; }
    template <>
    inline double TBoundingBox<double>::GetYExtent() const { return ymax - ymin; }
    template <>
    inline double TBoundingBox<double>::GetZExtent() const { return zmax - zmin; }

    // int version, using integer delta + 1 (f.ex. from 0 to 9 -> 10 bins)
    template <>
    inline int TBoundingBox<int>::GetXExtent() const { return xmax - xmin + 1; }
    template <>
    inline int TBoundingBox<int>::GetYExtent() const { return ymax - ymin + 1; }
    template <>
    inline int TBoundingBox<int>::GetZExtent() const { return zmax - zmin + 1; }

    // double version, stepping is precise
    template <>
    inline double TBoundingBox<double>::XStep(int numsteps) const { return GetXExtent() / (numsteps - 1); }
    template <>
    inline double TBoundingBox<double>::YStep(int numsteps) const { return GetYExtent() / (numsteps - 1); }
    template <>
    inline double TBoundingBox<double>::ZStep(int numsteps) const { return GetZExtent() / (numsteps - 1); }

    // int version, a truncated version of the integer ratio
    template <>
    inline int TBoundingBox<int>::XStep(int numsteps) const { return numsteps >= GetXExtent() ? 1 : Round((double)GetXExtent() / numsteps); }
    template <>
    inline int TBoundingBox<int>::YStep(int numsteps) const { return numsteps >= GetYExtent() ? 1 : Round((double)GetYExtent() / numsteps); }
    template <>
    inline int TBoundingBox<int>::ZStep(int numsteps) const { return numsteps >= GetZExtent() ? 1 : Round((double)GetZExtent() / numsteps); }
