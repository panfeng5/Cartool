            ...
            for (int pi = 0; pi < numpoints; pi++)
            {
                ...
                if (outputnames)
                {
                    ...
                    else
                    {
                        ...
                        if (xyzformat || elsformat)
                            IntegerToString(name, pi + 1, 0);
                        else if (sxyzformat)
                            StringCopy(name, "e", IntegerToString(buff, pi + 1, 0));
                        else if (spiformat)
                            StringCopy(name, "sp", IntegerToString(buff, pi + 1, 0));
                        else
                            IntegerToString(name, pi + 1, 0);
                    }
    ...
    */
    ...
    void TPoints::SetLineOfPoints(TPointFloat fromp, TPointFloat top, bool includingextrema, int subsampling)
    {
        ...
        for (int i = 0; i <= numsteps; i++)
            Add(fromp * (1 - (double)i / numsteps) + top * ((double)i / numsteps));
    }
    ...
    void TPoints::SetSphericalSpiral(int numpoints)
    {
        ...
        {
            if (zerotoone <= 0.0)
                return TPointDouble(0, 0, 1);
            else if (zerotoone >= 1.0)
                return TPointDouble(0, 0, -1);
            else
            {
        ...
        for (int i = 0; i < numpoints; i++)
        {
            ...
            for (; lastx < numcumulength - 1; lastx++)
                ...
                if (zerotooney >= cumulength[lastx] && zerotooney <= cumulength[lastx + 1])
                {
                    ...
                    double w = (zerotooney - cumulength[lastx]) / (cumulength[lastx + 1] - cumulength[lastx]);
        ...
        const TMatrix44 *mriabstoguillotine)
    {
        ...
        *this += centertranslate;                                   // points shift this way
        ...
        for (int pi = 0; pi < (int)origsurfp; pi++)
        {
            ...
            if (p.Y > backears && p.Y < frontears)
                continue;

            // skip hole around eyes (currently bigger than nose part)
            if (p.Y > 0 && p.Z < aboveeyes && fabs(p.X) < eyeswidth)
                continue;

            // skip hole around nose
            if (p.Y > 0 && p.Z < abovenose && fabs(p.X) < nosewidth)
                continue;

            // skip low back head
            if (p.Y <= backears && p.Z < bottomskull)
                continue;

            // skip low front head
            if (p.Y >= frontears && p.Z < bottomface)
                continue;
        ...
        for (int pi = 0; pi < NumPoints; pi++)
            ...
            if (Points[pi].Z >= 0)
                top.Add(Points[pi]); // comment this to apply a global balance instead of only on inferior points
            else if (Points[pi].Y < 0)
                belowback.Add(Points[pi]);
            else
                belowfront.Add(Points[pi]);
        ...
        if (abs(numbelowfront - numbelowback) < 50 || RelativeDifference(numbelowfront, numbelowback) < 0.01)
        {
            // back to original space
            *this -= centertranslate;
            // by safety
    ...
    void TPoints::Sort()
    {
        OmpCriticalBegin(TPointsSort)
            ...
            sort(Points.begin(), Points.end(),
                 [](const TPointFloat &p1, const TPointFloat &p2)
                 { return p1 > p2; } // '>' exists in TPointT
            );
    ...
    void TPoints::WriteFile(const char *file, const TStrings *names, TSelection *exclsel, int numpoints, ClusterType type) const
    {
        ...
        else if (elsformat)
        {
            ...
            ofs << StreamFormatGeneral;
            ofs << StreamFormatText << (type == ClusterPoint ? "Single Points" : type == ClusterLine ? "Lines"
            ...
            if (outputnames)
            {
                ...
                else
                {
                    if (xyzformat || elsformat)
                        ...
                        sprintf(name, "e%0d", i + 1);
                    else if (spiformat)
                        sprintf(name, "sp%0d", i + 1);
                    else
                        sprintf(name, "%0d", i + 1);
                }
                ...
                {                                // text file
                    ...
                    if (names)
                    {
                        ...
                        if (sscanf(buff, "%f %f %f %s", &pf.X, &pf.Y, &pf.Z, name) < 4)
                        {
                            ...
                            if (xyzformat || elsformat)
                                StringCopy(name, /*"e",*/ IntegerToString(buff, j, 0)); // !removing the 'e' for Tracks Interpolation!
                            else if (spiformat)
                                StringCopy(name, "sp", IntegerToString(buff, j, 0));
                        }
        ...
        for (int i = 0; i < NumPoints; i++)
            for (int j = i + 1; j < NumPoints; j++)
            {
                ...
                if ((Points[j] - Points[i]).Norm2() < neighdist)
                {
                    ...
                    if (neighbi(i, 0) < numsneigh)
                        neighbi(i, ++neighbi(i, 0)) = j;
                    // symmetry saves time
                    if (neighbi(j, 0) < numsneigh)
                        neighbi(j, ++neighbi(j, 0)) = i;
                }
...
#define minguillotinethreshold 1.5
            ...
            for (int i = 0; i < NumPoints; i++)
        {
            ...
            if (resurfacepoint)
            {
                ...
                { // ! gradientdirection - simply use direction from center to point
