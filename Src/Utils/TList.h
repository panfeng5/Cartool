...
namespace crtl
{
    ...
    public:
        ...
        void SetForward(TList<TypeD> &List)
        {
            if ((bool)List)
            {
                P = List.GetFirstAtom();
        ...
        void SetForward(const TList<TypeD> &List)
        {
            if ((bool)List)
            {
                P = (TListAtom<TypeD> *)List.GetFirstAtom();
                Index = 0;
            }
            else
                Reset();
        }
        void SetBackward(TList<TypeD> &List)
        {
            if ((bool)List)
            {
                P = List.GetLastAtom();
        ...
        void SetBackward(const TList<TypeD> &List)
        {
            if ((bool)List)
            {
                P = (TListAtom<TypeD> *)List.GetLastAtom();
                Index = (int)List - 1;
            }
            else
                Reset();
...
#define continueforeachin(LIST, ITERATOR) for (ITERATOR.Next(); (bool)ITERATOR; ITERATOR.Next())
    ...
    void TList<TypeD>::RemoveAtom(const TListAtom<TypeD> *tolistatom, DeallocateType deallocate)
    {
        ...
        */
        ...
        if (previousatom)
            previousatom->Next = nextatom; // could be 0 when removing last element
        if (nextatom)
            nextatom->Previous = previousatom; // could be 0 when removing first element

        if (First == tolistatom)
            First = nextatom; // could be 0 when removing last element
        if (Last == tolistatom)
            Last = previousatom; // same
