...
#define ForAllPossibleSelectionBackward for (int index = MaxSet() - 1, i = IsTemplate() ? IndexToValue[index] : index; \
    ...
    TSelection &TSelection::operator=(const TSelection &op2)
    {
        ...
        { // filter out what is not in receiving template, and sort according to receiving template
        ...
        { // if ( IsArbitrary () ) {       // arbitrary case is not so picky!
            ListSet = op2.ListSet;
            ArraySet = op2.ArraySet;
            NumSelected = op2.NumSelected;
        }
    ...
    void TSelection::Show(const char *title) const
    {
        if (IsNoneSet())
        {
            ShowMessage("- Empty -", StringIsEmpty(title) ? (IsSorted() ? "Sorted Selection" : IsTemplate() ? "Templated Selection"
                                                                                                            : "Arbitrary Selection")
                                                          : title);
            return;
        }
        ...
        ShowMessage(buff, StringIsEmpty(title) ? (IsSorted() ? "Sorted Selection" : IsTemplate() ? "Templated Selection"
                                                                                                 : "Arbitrary Selection")
                                               : title);
    }
    ...
    void TSelection::SetOrderType(TSelectionOrder ordertype)
    {
        ...
        if (ordertype == OrderArbitrary)
        { // this type is easy-going, no need to change the order
    ...
    */
    ...
    int TSelection::NumSet(int fromvalue, int tovalue) const
    {
        ...
        */
        ...
        if (IsTemplate())
        {
            ...
            ForAllValues(valuei, fromvaluei, tovaluei) if (ArraySet[IndexToValue[valuei]]) numset++;
        }
        else
            ForAllValues(value, fromvalue, tovalue) if (ArraySet[value]) numset++;
    ...
    {
        // already cleared?
        if (value < MinValue() || value > MaxValue() || !ArraySet[value])
            return;
        ...
        for (i = 0, tos = &ListSet[0]; i < NumSelected; i++, tos++)
            if (*tos == value)
                break;
            else if (IsSorted() && *tos > value || IsTemplate() && ValueToIndex[*tos] > ValueToIndex[value])
                return;
    ...
    void TSelection::Set(int value)
    {
        ...
        if (IsSorted())
        { // Sorted case will also test for existence
            int i;
            int *tos;
        ...
        { // Sorted & restricted by template
            ...
            for (i = 0, tos = &ListSet[0]; i < NumSelected; i++, tos++)
                if (*tos == value)
                    return; // shouldn't happen here
                else if (ValueToIndex[*tos] > indexvalue)
                    break; // insert here
    ...
    void TSelection::Set(const char *list, bool startfrom1, bool verbose)
    {
        ScanTextAction(list, 0,
                       TSelectionScanFlags(ScanTextActionSet | (startfrom1 ? ScanTextActionStartFrom1 : 0) | (verbose ? 0 : ScanTextActionSilent)));
    }

    void TSelection::Set(const char *list, const TStrings *dictionary, bool verbose)
    {
        ScanTextAction(list, dictionary, TSelectionScanFlags(ScanTextActionSet | (verbose ? 0 : ScanTextActionSilent)));
    }
    ...
    {
        ScanTextAction(list, 0,
                       TSelectionScanFlags(ScanTextActionClear | (startfrom1 ? ScanTextActionStartFrom1 : 0) | (verbose ? 0 : ScanTextActionSilent)));
    }

    void TSelection::Reset(const char *list, const TStrings *dictionary, bool verbose)
    {
        ScanTextAction(list, dictionary,
                       TSelectionScanFlags(ScanTextActionClear | (verbose ? 0 : ScanTextActionSilent)));
    }
        ...
        for (int toki = 0; toki < (int)tokens; toki++)
        {
            ...
            { // dictionary provided
                ...
                for (k = 0; k < maxnames; k++)
                {
                    ...
                    if (StringStartsWith(toc, (*dictionary)[k]) && (i == -1 || StringLength((*dictionary)[k]) > StringLength((*dictionary)[i])))
                    {
        ...
        for (int i = 0; i < NumSelected; i++)
        {
            ...
            */
            ...
            if (i < NumSelected - 1)
            {
                ...
                if (direction == 0 && !IsSorted() && ListSet[i + 1] == to - 1)
                    direction = -1;
                if (direction == 0 && ListSet[i + 1] == to + 1)
                    direction = 1;

                // we still go in the right direction?
                // check for forced enumeration
                if (ListSet[i + 1] == to + direction && (StringIsEmpty(alwaysenumerate) || StringIsNot(alwaysenumerate, "*") && dictionary && !IsStringAmong((*dictionary)[to + (direction < 0 ? 0 : 1)], alwaysenumerate)))
                {
            ...
            // write first limit
            if (dictionary && from < maxnames)
                StringAppend(formatted, (*dictionary)[from]);
            else
                StringAppend(formatted, IntegerToString(buff, from + 1));
            ...
            if (from != to)
            {
                ...
                if (dictionary && to < maxnames)
                    StringAppend(formatted, (*dictionary)[to]);
                else
                    StringAppend(formatted, IntegerToString(buff, to + 1));
            }
    ...
    void TSelection::Copy(int fromvalue, int tovalue, const TSelection *sel2)
    {
        ...
        if (IsTemplate())
        {
            ...
            ForAllValues(valuei, fromvaluei, tovaluei)
            {
                ...
                if (sel2->ArraySet(value) && !ArraySet(value))
                    Set(value);
                else if (!sel2->ArraySet(value) && ArraySet(value))
                    Reset(value);
            }
        }
        else
    ...
    void TSelection::Shift(int fromvalue, int tovalue, int numshift, TSelectionShiftFlags flags)
    {
        ...
        if (fromvalue == tovalue || numshift < 1)
        ...
        if (IsTemplate())
        { // we're going to work directly on the indexes
    ...
    int TIteratorSelectedForward::FirstSelected(const TSelection &sel, int fromvalue, int tovalue)
    {
        ...
        if (tosel->IsTemplate())
        {
            ...
            for (int i = 0; i < tosel->NumSelected; i++)
            {
                ...
                if (valuei >= fromvaluei && valuei <= tovaluei)
                { // if within range, store range of indexes
            ...
            for (int i = 0; i < tosel->NumSelected; i++)
            {
                ...
                if (IsInsideLimits(tosel->ListSet[i], LastSearchFromValue, LastSearchToValue))
                { // if within range, store range of indexes
