...
namespace crtl
{
    ...
    const char *ZScoreEnumToString(ZScoreType z)
    {
        ...
        case ZScoreNone:
            return "No standardization";

        case ZScorePositive_CenterScale:
            return "Z-Score ( positive data )";
        case ZScorePositive_CenterScaleOffset:
            return "Z-Score ( positive data ), then offset of " MinSDToKeepS " sigma";
        case ZScorePositive_CenterScaleAbs:
            return "Z-Score ( positive data ), then absolute value";
        case ZScorePositive_CenterScalePlus:
            return "Z-Score ( positive data ), then keeping positive part";
        case ZScorePositive_NocenterScale:
            return "Data rescaled by SD (no centering)";
        case ZScorePositive_CenterScaleInvertOffset:
            return "Z-Score ( positive data ), then inverted and offset of " MinSDToKeepS " sigma";

        case ZScoreVectorial_CenterVectors_CenterScale:
            return "Vector rescaled by ( Z-Score ( Norm ), then offset of " MinSDToKeepS " sigma )";
        case ZScoreVectorial_CenterVectors_Scale:
            return "Vector rescaled by its Norm's SD (no centering)";
        case ZScoreVectorial_CenterScaleByComponent:
            return "Vector with Z-Score done on each component";

        case ZScoreSigned_CenterScale:
            return "Z-Score";

        default:
            return "Unknwown standardization";
        }
    }
    ...
    const char *ZScoreEnumToInfix(ZScoreType z)
    {
        ...
        case ZScorePositive_CenterScale:
            return PostfixStandardizationZScore "PS";
        case ZScorePositive_CenterScaleOffset:
            return PostfixStandardizationZScore "PO";
        case ZScorePositive_CenterScaleAbs:
            return PostfixStandardizationZScore "PA";
        case ZScorePositive_CenterScalePlus:
            return PostfixStandardizationZScore "P+";
        case ZScorePositive_NocenterScale:
            return PostfixStandardizationZScore "PK";
        case ZScorePositive_CenterScaleInvertOffset:
            return PostfixStandardizationZScore "PIO";

        case ZScoreVectorial_CenterVectors_CenterScale:
            return PostfixStandardizationZScore "VN";
        case ZScoreVectorial_CenterVectors_Scale:
            return PostfixStandardizationZScore "VK";
        case ZScoreVectorial_CenterScaleByComponent:
            return PostfixStandardizationZScore "VC";

        case ZScoreSigned_CenterScale:
            return PostfixStandardizationZScore "SS";

        default:
            return PostfixStandardizationZScore;
        }
    }
    ...
    const char *ZScoreEnumToFactorFileInfix(ZScoreType z)
    {
        ...
        case ZScoreNone:
            return "";
        default:
            return InfixStandardizationZScoreFactors;
        }
    }
                              ...
                              bool *askedsignedzscore, ZScoreType &timezscore)
    {
        ...
        if (IsVector(datatype))
        {
            ...
            if (contenttype == TracksContentUnknown && asking)
            {
                ...
                char answer = GetOptionFromUser("Data is vectorial, select the type of standardization:" NewLine
                                                    NewLine Tab "(N)orm of the vectors" NewLine Tab "(E)ach X,Y,Z component independently",
        ...
        else if (IsPositive(datatype))
        {
            ...
            if (contenttype == TracksContentERP || contenttype == TracksContentFrequency || contenttype == TracksContentEEGRecording)

                zsp = ZScorePositive_Default;

            else
            { // if ( contenttype == TracksContentUnknown ) {
              ...
              // asking all the time if control variable not provided, once if it is
                if (asking)
                {
                    ...
                    char answer = GetOptionFromUser("Data is positive, select the type of standardization:" NewLine
                                                        NewLine Tab "(O)ffseted Z-Score   which remain positive" NewLine Tab "(S)igned Z-Score   OK but results are not positive anymore" NewLine Tab "(A)bsolute Z-Score" NewLine Tab "(R)escaling only without centering",
    ...
    const char *TDataFormat::GetContentTypeName(char *s) const
    {
        ...
        if (ExtraContentType)
            StringCopy(s, ExtraContentTypeNames[ExtraContentType], " ", ContentNames[ContentType]);
        else
            StringCopy(s, ContentNames[ContentType]);
