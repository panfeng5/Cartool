...
namespace crtl
{
        ...
        else if (type == 2 /*ClusterGrid*/)
        {
            ...
            for (int d2 = 0; d2 < Dim2 - 1; d2++)
                for (int d1 = 0; d1 < Dim1 - 1; d1++)
                {
                    ...
                    ListTriangles[NumPoints] = listp[i11];
                    ListTrianglesIndexes[NumPoints++] = i11;
                    ListTriangles[NumPoints] = listp[i12];
                    ListTrianglesIndexes[NumPoints++] = i12;
                    ListTriangles[NumPoints] = listp[i22];
                    ListTrianglesIndexes[NumPoints++] = i22;

                    ListTriangles[NumPoints] = listp[i11];
                    ListTrianglesIndexes[NumPoints++] = i11;
                    ListTriangles[NumPoints] = listp[i22];
                    ListTrianglesIndexes[NumPoints++] = i22;
                    ListTriangles[NumPoints] = listp[i21];
                    ListTrianglesIndexes[NumPoints++] = i21;
    ...
    void TTriangleSurface::ComputeDelaunay3D()
    {
        ...
        datacenter = (datacenter / NumPoints + (limitmin + limitmax) / 2) / 2;
        ...
        auto IsBadEdge = [maxdist](const D3Dfloat &vn)
        { return vn / maxdist < 1e-6 || vn > maxdist; };

        auto IsCircleRadiusTooBig = [maxradius](const D3Dfloat &R)
        { return R > maxradius; };
        ...
        */
        ...
        {
            for (int i = 0; i < NumPoints; i++)
                if (ListPoints[i].Index == orgindex)
                    return i;
            return TIndexNull;
        };
        ...
        while ((nextedge = triangulation.GetNextBorder()) != 0)
        {
            ...
            for (ci = 0; ci < NumPoints; ci++)
            {
                ...
                if (triangulation.GetTriangle(nextedge, c))
                    continue;
                ...
                //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // circumradius
                temp = (normab + normbc + normac) * (normbc + normac - normab) * (normab + normbc - normac) * (normab - normbc + normac);
                ...
                if (IsBadEdge(normac)          // non non-sense edge
                    ...
                    || IsTriangleAreaTooBig(normab, normbc, normac, R))
                    continue; // avoiding big triangles: underneath the cap, or around the ears
                ...
                center = ((mab + voab * trilab) + (mac + voac * trilac)) / (D3Dfloat)2;
            ...
            if (!pointisin)
            {
                ...
                if (circumdi != TIndexNull)
                {
                    ...
                    if (vad.IsOppositeDirection(voac))
                        triangulation.AddTriangle(a, c, circumd);
                    else if (vad.IsOppositeDirection(voab))
                        triangulation.AddTriangle(a, circumd, b);
                    else
                        triangulation.AddTriangle(b, circumd, c);
                }
            }
    ...
    void TTriangleSurface::ComputeLine(const TPoints &listp, const TSelection &sel, double mindist)
    {
        ...
        for (TIteratorSelectedForward seli(sel); (bool)seli; ++seli)
        {
            ...
            // on first axis
            ListTriangles[NumPoints] = p11;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p12;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p14;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p11;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p14;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p13;
            ListTrianglesIndexes[NumPoints++] = i;
            // on second axis
            ListTriangles[NumPoints] = p21;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p22;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p24;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p21;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p24;
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p23;
            ListTrianglesIndexes[NumPoints++] = i;
            ...
            ListTrianglesIndexes[NumPoints++] = i;
            ListTriangles[NumPoints] = p21;
            ListTrianglesIndexes[NumPoints++] = i + 1;
            ListTriangles[NumPoints] = p12;
            ListTrianglesIndexes[NumPoints++] = i;
