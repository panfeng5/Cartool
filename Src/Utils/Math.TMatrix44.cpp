...
namespace crtl
{
    ...
    bool TMatrix44::IsIdentity(double epsilon) const
    {
        ...
        return m[0] == 1 && m[4] == 0 && m[8] == 0 && m[12] == 0 && m[1] == 0 && m[5] == 1 && m[9] == 0 && m[13] == 0 && m[2] == 0 && m[6] == 0 && m[10] == 1 && m[14] == 0 && m[3] == 0 && m[7] == 0 && m[11] == 0 && m[15] == 1;
    }
    ...
    bool TMatrix44::IsTranslation(double epsilon) const
    {
        ...
        return m[0] == 1 && m[4] == 0 && m[8] == 0 && m[1] == 0 && m[5] == 1 && m[9] == 0 && m[2] == 0 && m[6] == 0 && m[10] == 1 && m[3] == 0 && m[7] == 0 && m[11] == 0 /*&& m[ 15 ] == 1*/
    ...
    void ShowMatrixAnalysis(MatrixAnalysis ma, const char *title)
    {
        ...
        else
        {
            ...
            if (ma & HasRotation)
                StringAppend(text, (ma & HasOrtho) ? "Rotation Ortho" : "Rotation", "\n");
            if (ma & HasSymmetry)
                StringAppend(text, (ma & HasOrtho) ? "Symmetry Ortho" : "Symmetry", "\n");
            if (ma & HasScaling)
                StringAppend(text, (ma & HasIntegerScaling) ? "Integer Scaling" : "Scaling", "\n");
            if (ma & HasTranslation)
                StringAppend(text, (ma & HasIntegerTranslation) ? "Integer Translation" : "Translation", "\n");
            if (ma & HasShearing)
                StringAppend(text, "Shearing", "\n");
        }
    ...
    MatrixAnalysis TMatrix44::Analyze(double epsilon) const
    {
        ...
        if (m(0, 3) != 0 || m(1, 3) != 0 || m(2, 3) != 0)
        {
            ...
            if (IsInteger(m(0, 3)) && IsInteger(m(1, 3)) && IsInteger(m(2, 3)))
        ...
        if (normx != 1 || normy != 1 || normz != 1)
        {

            ma |= HasScaling;

            if (IsInteger(normx) && IsInteger(normy) && IsInteger(normz))
        ...
        bool isortho = mx.IsAxisAligned(epsilon) && my.IsAxisAligned(epsilon) && mz.IsAxisAligned(epsilon);
        ...
        else if (mx != TVector3Double(1, 0, 0) || my != TVector3Double(0, 1, 0) || mz != TVector3Double(0, 0, 1))
        {
    ...
    TPointDouble TMatrix44::IsOrthogonal(const TPointDouble &inputvoxel, double epsilon) const
    {
        ...
        if (outputvoxel[0].IsAxisAligned(epsilon) && outputvoxel[1].IsAxisAligned(epsilon) && outputvoxel[2].IsAxisAligned(epsilon))
        {
    ...
    bool TMatrix44::ApplyVoxel(const TPointDouble &inputvoxel, TPointDouble (&outputvoxel)[3]) const
    {
        ...
        if (inputvoxel.X <= 0 || inputvoxel.Y <= 0 || inputvoxel.Z <= 0)
        ...
        //|| p2.Norm () < 1e-6
        //  || p3.Norm () < 1e-6 )
        if (p1.Norm() == 0 || p2.Norm() == 0 || p3.Norm() == 0)
    ...
    {
        return Matrix[12] * Matrix[9] * Matrix[6] * Matrix[3] - Matrix[8] * Matrix[13] * Matrix[6] * Matrix[3] - Matrix[12] * Matrix[5] * Matrix[10] * Matrix[3] + Matrix[4] * Matrix[13] * Matrix[10] * Matrix[3] +
