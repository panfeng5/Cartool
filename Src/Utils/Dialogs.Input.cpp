...
namespace crtl
{
    ...
    bool GetInputFromUser(const char *text, const char *title, char *answer, const char *defaultanswer, TWindow *that)
    {
        ...
        if (StringIsEmpty(defaultanswer))
            ClearString(buff);
        else
            StringCopy(buff, defaultanswer);
    ...
    SkullStrippingType GetSkullStrippingTypeFromUser(const char *title, const char *defaultanswer, TWindow *that)
    {
        ...
        if (!GetValueFromUser("Skull-Stripping method:" NewLine
                                  NewLine Tab "1) Multiple Region Growing - Recommended" NewLine Tab "2) Single Region Growing   - If 1) failed" NewLine Tab "3) Multiple Masking           - If 1) and 2) failed",
    ...
    GetFileFromUser::GetFileFromUser(const char *title, const char *filefilter, int filefilterindex, GetFileFlags flags)
    {
        ...
        else if (Flags == GetFileRead)
            StringCopy(OFNDialogTitle, "Open File");
        else if (Flags == GetFileWrite)
            StringCopy(OFNDialogTitle, "Save File");
        else if (Flags == GetFilePath)
            StringCopy(OFNDialogTitle, "Get Path");
        else if (Flags == GetFileMulti)
            StringCopy(OFNDialogTitle, "Open Multiple Files");
        else if (Flags == GetFileDirectory)
            StringCopy(OFNDialogTitle, "Open Directory");
        else
            ClearString(OFNDialogTitle);
        ...
        if (Flags == GetFileRead)
            OFNDialogFlags |= OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
        else if (Flags == GetFileWrite)
            OFNDialogFlags |= OFN_EXTENSIONDIFFERENT | OFN_OVERWRITEPROMPT;
        else if (Flags == GetFilePath)
            OFNDialogFlags |= 0;
        else if (Flags == GetFileMulti)
            OFNDialogFlags |= OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST | OFN_ALLOWMULTISELECT;
        else if (Flags == GetFileDirectory)
            OFNDialogFlags |= 0;
        else
            OFNDialogFlags |= OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
    ...
    bool GetFileFromUser::Execute(const char *fileinit)
    {
        ...
        {
            if (StringIsEmpty(fileinit))
                StringCopy(files, GetFile()); // recall stored string
            else
                StringCopy(files, fileinit); // use parameter string
        }
        else
            ClearString(files); // IsMultiFiles (), no pre-fill
    ...
    */
    ...
    void GetFileFromUser::CheckMissingExtension(char *file)
    {
        ...
        if (StringIsSpace(allexts) || StringContains((const char *)allexts, "*", CaseSensitive)) // included an "*.*"
            return;
        ...
        if (StringContains((const char *)firstext, "*", CaseSensitive) || StringStartsWith(firstext, "."))
            ClearString(firstext);
