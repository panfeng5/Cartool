...
namespace crtl
{
                                          ...
                                          double &curvesize, double &curvemin, double &curveratio)
    {
        ...
        double curvemax = maxdata + marginfactor * kerneldensity;
                                      ...
                                      double *usercurvesize)
    {
        ...
        if (IsFlag(options, HistogramNormMax))
            NormalizeMax(); // setting the max to 1 is easier for visual exploration
        else if (IsFlag(options, HistogramNormArea))
            NormalizeArea(); // the real mathematical formula, total area / probability == 1
                                      ...
                                      HistogramOptions options)
    {
        ...
        for (int i = 0, j = 0; i < (int)data; i++)
            ...
            if (!mask || (*mask)(i))
            {
                ...
                if (ignorenulls && v == 0.0 || ignorepositive && v > 0.0 || ignorenegative && v < 0.0)
                    continue;
    ...
    */
    ...
    {
        return GetLastPosition(unit) - GetFirstPosition(unit) + (unit == BinUnit || IsStepInteger() ? 1 : 0); // integer cases include bin at the border
    }
    ...
    */
    ...
    int THistogram::GetNextInflexionPosition(int from, bool forward) const
    {
        ...
        for (; pos >= posmin && pos <= posmax; pos += inc)
        {
            ...
            d12 = max((double)(-2 * Array[pos] + Array[pos + inc] + Array[pos - inc]),
                      ...
                      (-2 * Array[pos] + Array[pos + 4 * inc] + Array[pos - step4 * inc]) / 4.0) /
                  2;
    ...
    int THistogram::GetNextLCornerPosition(int from) const
    {
        ...
        for (; pos >= posmin && pos <= posmax; pos += inc)
        {
            ...
            //                             ( Array[ pos ] - Array[ pos - step2 * inc ] ) / (double) step2,
            //                             ( Array[ pos ] - Array[ pos - step4 * inc ] ) / (double) step4 );
            d1 = ((double)(Array[pos] - Array[pos - inc]) + (Array[pos] - Array[pos - step2 * inc]) / (double)step2 + (Array[pos] - Array[pos - step4 * inc]) / (double)step4) / 3.0;
            ...
            //                             ( Array[ pos + 2 * inc ] - Array[ pos ] ) / 2.0,
            //                             ( Array[ pos + 4 * inc ] - Array[ pos ] ) / 4.0 );
            d2 = ((double)(Array[pos + inc] - Array[pos]) + (Array[pos + 2 * inc] - Array[pos]) / 2.0 + (Array[pos + 4 * inc] - Array[pos]) / 4.0) / 3;

            // max curvature
            //    d12     = max ( (double) ( - 2 * Array[ pos ] + Array[ pos +     inc ] + Array[ pos -         inc ] ),
            //                             ( - 2 * Array[ pos ] + Array[ pos + 2 * inc ] + Array[ pos - step2 * inc ] ) / 2.0,
            //                             ( - 2 * Array[ pos ] + Array[ pos + 4 * inc ] + Array[ pos - step4 * inc ] ) / 4.0 ) / 2;
            d12 = ((double)(-2 * Array[pos] + Array[pos + inc] + Array[pos - inc]) + (-2 * Array[pos] + Array[pos + 2 * inc] + Array[pos - step2 * inc]) / 2.0 + (-2 * Array[pos] + Array[pos + 4 * inc] + Array[pos - step4 * inc]) / 4.0) / 2 / 3;
            ...
            if (d1 >= 0 || d2 >= 0 || d12 > 0.005 && (d12 < maxd * 0.75 || RelativeDifference(d12, maxd) > 0.20))
                break;
...
#define NumHistoDegrade 100
        ...
        else if (nummodesfirsthalf == 2)
        {
            ...
            */
            ...
            whichmode = mode2 > mode1 ? 1
                        ...
                        : mode2 / (mode1 + 1e-6) < 1.5 * modepos2 / (h.GetExtent(BinUnit) + 1e-6) ? 1
                                                                                                  : 2;
        }

        else if (nummodesfirsthalf >= 3)
        { // could be more than 3, if the loop above failed to converge
    ...
    */
    ...
    void THistogram::ComputeZScoreFactors(int maxbinspertf,
                                          double &center, double &spreadleft, double &spreadright) const
    {
        center = 0;
        spreadleft = 0;
        spreadright = 0;
    ...
    void THistogram::WriteFileVerbose(char *file, bool logplot) const
    {
        ...
        for (int b = 0; b < Dim1; b++)
        {
            ...
            if (b == maxpos)
                o << "M";
            else if (b > 0 && Array[b - 1] == Array[b] || b < Dim1 - 1 && Array[b + 1] == Array[b])
                o << "|";
            else if (b > 0 && Array[b - 1] < Array[b] && b < Dim1 - 1 && Array[b + 1] < Array[b])
                o << ">";
            else if (b > 0 && Array[b - 1] > Array[b] && b < Dim1 - 1 && Array[b + 1] > Array[b])
