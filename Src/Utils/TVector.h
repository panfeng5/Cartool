...
namespace crtl
{
    ...
    {
    public:
        FctParams() : TArray1<double>(MaxNumParams) {}
    ...
    public:
        ...
        TVector<TypeD> &operator=(const TVector<TypeD> &op2)
        {
            TArray1<TypeD>::operator=(op2);
            Index1 = op2.Index1;
            return *this;
        }
    ...
    TypeD TVector<TypeD>::GetValueChecked(double x, InterpolationType interpolate) const
    {
        ...
        else if (IsFlag(interpolate, InterpolateUniformCubicBSpline) || IsFlag(interpolate, InterpolateCubicHermiteSpline))
        {
    ...
    void TVector<TypeD>::Sort(SortDirection direction, int size)
    {
        ...
        if (direction == Ascending)
            SortAscending(0, IsInsideLimits(size, 1, Dim1) ? size - 1 : Dim1 - 1);
        else
            SortDescending(0, IsInsideLimits(size, 1, Dim1) ? size - 1 : Dim1 - 1);
    }
    ...
    void TVector<TypeD>::Filter(FilterTypes filtertype, int size)
    {
        ...
        for (int s = 0; s < numfilters; s++)
        {
            ...
            for (i1 = 1; i1 < Dim1 - 1; i1++)
            {
                ...
                sum = (TypeD)(w[Xm] * safecopy[i1 - 1] + w[X0] * safecopy[i1] + w[Xp] * safecopy[i1 + 1]);
            ...
            i1 = 0;
            sum = (TypeD)(w[X0] * safecopy[i1] + w[Xp] * safecopy[i1 + 1]);
            Array[i1] = (TypeD)(sum / sumw);

            i1 = Dim1 - 1;
            sum = (TypeD)(w[Xm] * safecopy[i1 - 1] + w[X0] * safecopy[i1]);
            Array[i1] = (TypeD)(sum / sumw);
        } // for s
    }
    ...
    {
        if (filtertype == FilterTypeGaussian || filtertype == FilterTypeHighPassLight || filtertype == FilterTypeLoG || filtertype == FilterTypeMean)
        ...
        else if (filtertype == FilterTypeThreshold || filtertype == FilterTypeThresholdAbove || filtertype == FilterTypeThresholdBelow)
            FilterThreshold(filtertype, params);
        else if (filtertype == FilterTypeBinarize)
            FilterBinarize(params);

        else if (filtertype == FilterTypeMin || filtertype == FilterTypeMax || filtertype == FilterTypeMinMax || filtertype == FilterTypeRange || filtertype == FilterTypeMedian || filtertype == FilterTypeInterquartileMean
                 //     || filtertype == FilterTypeMeanSub
                 //     || filtertype == FilterTypeMAD
                 || filtertype == FilterTypeSD || filtertype == FilterTypeSDInv || filtertype == FilterTypeCoV || filtertype == FilterTypeLogSNR
    ...
    void TVector<TypeD>::FilterLinear(FilterTypes filtertype, FctParams &params, bool showprogress)
    {
        ...
        double resamplegaussian = filtertype == FilterTypeGaussian || filtertype == FilterTypeLoG ? 6 / 2 : 1;
        ...
        for (int xk = 0; xk < Kf.GetDim1(); xk++)
        {
            ...
            if (filtertype == FilterTypeGaussian)
                v = gauss(kpx);
            else if (filtertype == FilterTypeLoG)
                v = (Square((double)kpx) - Square(sigma)) / Power(sigma, 4) * gauss(kpx); // skipping the cst because we are normalizing after

            // these filters size IS NOT modified
            else if (filtertype == FilterTypeHighPassLight)
                v = (kpx ? -1 : 4) * gauss(kpx); // very rough, only for 3
            else                                 /*if ( filtertype == FilterTypeMean        )*/
                v = 1;
    ...
    void TVector<TypeD>::FilterStat(FilterTypes filtertype, FctParams &params, bool showprogress)
    {
        ...
        TEasyStats stat(filtertype == FilterTypeMin || filtertype == FilterTypeMax
        ...
        */
        ...
        for (int x = 0; x < Dim1; x++)
        {
            ...
            if (filtertype == FilterTypeMin)
                ...
                v = stat.Median();
            else if (filtertype == FilterTypeInterquartileMean)
                v = stat.InterQuartileMean();
            else if (filtertype == FilterTypeSD)
                v = stat.SD();
            else if (filtertype == FilterTypeSDInv)
                v = (v = stat.SD()) != 0 ? crtl::NoMore(1e10, 1 / v) : 0; // capping the max(?)
                                                                          // give more resolution to lower values (Gamma in option?)
            else if (filtertype == FilterTypeCoV)
                v = stat.CoefficientOfVariation(); //  * covrescale;
...
// should be a known format!
    ...
    double TVector<TypeD>::CorrelationLinearCircular(const TVector<TypeD> &mapcirc, bool centeraverage, TVector<TypeD> &mapcos, TVector<TypeD> &mapsin) const
    {
        ...
        double circcorr = sqrt((Square(rcx) + Square(rsx) - 2 * rcx * rsx * rcs) / NonNull(1 - Square(rcs)));
    ...
    double TVector<TypeD>::Dissimilarity(const TVector<TypeD> &v, bool centeraverage, bool vectorial) const
    {
        ...
        for (int i = 0; i < Dim1; i++)
            ...
            sum += Square((Array[i] - avg1) / gfp1 - (v.Array[i] - avg2) / gfp2);
    ...
    double TVector<TypeD>::Dissimilarity(const TVector<TypeD> &v, const TSelection &subset, bool centeraverage, bool vectorial) const
    {
        ...
        for (TIteratorSelectedForward seli(subset); (bool)seli; ++seli)
            ...
            if (seli() < Dim1)
                // Euclidean distance between standardized vectors
                sum += Square((Array[seli()] - avg1) / gfp1 - (v.Array[seli()] - avg2) / gfp2);
    ...
    void TVector<TypeD>::ZScoreAsym()
    {
        ...
        double spreadleft = (sdleft + 2 * madleft) / 3;

        double spreadright = (sdright + 2 * madright) / 3;
                                          ...
                                          TMap *brainstate)
    {
        ...
        for (int sourcei = 0; sourcei < numsources; sourcei++)
        {
            ...
            do
                positioni = randunif((UINT)numsolp);
    ...
    */
    ...
    void TVector<TypeD>::Cumulate(const TVector<TypeD> &v, PolarityType polarity)
    {
        ...
        if (polarity == PolarityEvaluate && IsOppositeDirection(v))
            (*this) -= v;
        else
            (*this) += v;
    }
