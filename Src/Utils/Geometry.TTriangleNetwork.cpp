...
// DBGV2 ( NumTriangles, MaxTriangles, "NumTriangles, MaxTriangles" );
    ...
    TTriangle *TTriangleNetwork::GetTriangle(const TEdge *e, const D3DPointI &v)
    {
        ...
        if (e->Triangle1 != 0 && e->Triangle1->HasVertex(&v))
            return e->Triangle1;
        else if (e->Triangle2 != 0 && e->Triangle2->HasVertex(&v))
            return e->Triangle2;
        else
            return 0;
    }
    ...
    void TTriangleNetwork::GetTriangleVertices(int ti, const D3DPointI *v[3]) const
    {
        ...
        v[2] = v[0] != t.Edge2->Vertex1 && v[1] != t.Edge2->Vertex1 ? t.Edge2->Vertex1
                                                                    : t.Edge2->Vertex2;
    }
    ...
    TTriangle *TTriangleNetwork::InsertTriangle(TEdge *e1, TEdge *e2, TEdge *e3)
    {
        ...
        if (!e1->Triangle1)
            e1->Triangle1 = &t; // !cautious which triangle to update!
        else if (!e1->Triangle2)
            e1->Triangle2 = &t;

        if (!e2->Triangle1)
            e2->Triangle1 = &t;
        else if (!e2->Triangle2)
            e2->Triangle2 = &t;

        if (!e3->Triangle1)
            e3->Triangle1 = &t;
        else if (!e3->Triangle2)
            e3->Triangle2 = &t;
    ...
    void TTriangleNetwork::AddTriangle(const D3DPointI &a, const D3DPointI &b, const D3DPointI &c)
    {
        ...
        if (e1 && !CanConnect(e1, e1->GetVertexNotBelonging(v1, v2, v3)))
            return;

        if (e2 && !CanConnect(e2, e2->GetVertexNotBelonging(v1, v2, v3)))
            return;

        if (e3 && !CanConnect(e3, e3->GetVertexNotBelonging(v1, v2, v3)))
            return;
        ...
        else if (numexistingedges == 2)
        {
            ...
            //                          e2:  1 0 1  ->  x2 ei x1
            //                          e3:  0 1 1  ->  ei x2 x2
            TEdge *&x1 = !e3 ? e1 : !e2 ? e1
                                        : e2;
            TEdge *&x2 = !e3 ? e2 : !e2 ? e3
                                        : e3;
            TEdge *&ei = !e3 ? e3 : !e2 ? e2
                                        : e1;

            // 1 edge to insert
            if (x1->Vertex1 == x2->Vertex1)
                ei = InsertEdge(x1->Vertex2, x2->Vertex2);
            else if (x1->Vertex1 == x2->Vertex2)
                ei = InsertEdge(x1->Vertex2, x2->Vertex1);
            else if (x1->Vertex2 == x2->Vertex1)
                ei = InsertEdge(x1->Vertex1, x2->Vertex2);
            else if (x1->Vertex2 == x2->Vertex2)
                ei = InsertEdge(x1->Vertex1, x2->Vertex1);
        }
        ...
        else if (numexistingedges == 1)
        {
            ...
            TEdge *&x = e1 ? e1 : e2 ? e2
                                     : e3;
            TEdge *&ei1 = e1 ? e2 : e2 ? e1
                                       : e1;
            TEdge *&ei2 = e1 ? e3 : e2 ? e3
                                       : e2;

            // 2 edges to insert
            if (!x->HasVertex(v1))
            {
                ei1 = InsertEdge(v1, x->Vertex1);
                ei2 = InsertEdge(v1, x->Vertex2);
            }
            else if (!x->HasVertex(v2))
            {
                ei1 = InsertEdge(v2, x->Vertex1);
                ei2 = InsertEdge(v2, x->Vertex2);
            }
            else
            {
                ei1 = InsertEdge(v3, x->Vertex1);
                ei2 = InsertEdge(v3, x->Vertex2);
            }
        }
    ...
    void TTriangleNetwork::FillHoles()
    {
        ...
        {
            if (e1->Vertex1 == e2->Vertex1 || e1->Vertex1 == e2->Vertex2)
                return e1->Vertex1;
            else if (e1->Vertex2 == e2->Vertex1 || e1->Vertex2 == e2->Vertex2)
                return e1->Vertex2;
            else
                return (const D3DPointI *)0;
        };
