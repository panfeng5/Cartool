...
namespace crtl
{
                                  ...
                                  const char *filename)
    {
        ...
        for (int el = 0; el < numel; el++)
        {
            ...
            if (el < (int)xyznames)
                StringCopy(buff, xyznames[el]);
            else
                StringCopy(buff, "e", IntegerToString(el + 1));
        ...
        for (int sp = 0; sp < numsolp; sp++)
        {
            ...
            if (sp < (int)spnames)
                ...
                StringCopy(buff, "sp", IntegerToString(sp + 1));
                                ...
                                const char *filename)
    {
        ...
        int numsolp = (numrows / dimsp) + (spsrejected ? spsrejected->NumSet() : 0); // adding optional rejected points to get back to the original count
