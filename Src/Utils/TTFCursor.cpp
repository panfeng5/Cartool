...
namespace crtl
{
        ...
        : EEGDoc(eegdoc)
    {
        ...
        if (IsInsideLimits(initmin, LimitMin, LimitMax))
            PosMin = initmin;
        else
            PosMin = LimitMin;

        if (IsInsideLimits(initmax, LimitMin, LimitMax))
            PosMax = initmax;
        else
            PosMax = PosMin;
    ...
    {
        if (PosMin + delta < 0)
            SetPos(0, PosMax - PosMin);
        else if (PosMax + delta > LimitMax)
            SetPos(LimitMax - PosMax + PosMin, LimitMax);
        else
            SetPos(PosMin + delta, PosMax + delta);
    }
    ...
    void TTFCursor::SetExtendingPos(long pos)
    {
        ...
        {
            Extending = true;
            if (abs(pos - PosMin) < abs(pos - PosMax))
                PosPivot = PosMax;
            else
                PosPivot = PosMin;
        }

        if (PosExtend < PosPivot)
        {
            PosMin = PosExtend;
            PosMax = PosPivot;
        }
    ...
    {
        return PosMin == TranslateCursorTF(&op2, op2.PosMin) && PosMax == TranslateCursorTF(&op2, op2.PosMax);
    }
