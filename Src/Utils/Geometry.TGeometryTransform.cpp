...
namespace crtl
{
                                   ...
                                   TMatrix44 *AbsNorm_to_RelVoxel, TMatrix44 *RelVoxel_to_AbsNorm)
    {
        ...
        if (AbsNorm_to_RelVoxel)
            AbsNorm_to_RelVoxel->SetIdentity();
        if (RelVoxel_to_AbsNorm)
            RelVoxel_to_AbsNorm->SetIdentity();
        ...
        if (estimatecenter || MriCenter.IsNull())
        {
    ...
    void TGeometryTransform::ConvertTo(TPointFloat &p, const TBoundingBox<double> *bounding) const
    {
        ...
        // set origin
        if (Type == TalairachAvg152T1Fld || Type == TalairachAvg152T1_smac || Type == TalairachAve152_sym)
        {
            pmni.X = p.Z - 45;
            pmni.Y = -p.X + 45;
            pmni.Z = -p.Y + 55;
        ...
        else if (Type == TalairachAvg152T1Fld || Type == TalairachAvg152T1_smac || Type == TalairachAve152_sym)
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // MNI to Talairach
        if (Type == TalairachAvg152T1Fld || Type == TalairachAvg152T1_smac || Type == TalairachAve152_sym || Type == TalairachMNI152_2001 || Type == TalairachMNI152_2009 || Type == TalairachNIHPD_Infant)
        {
    ...
    void TGeometryTransform::ConvertTo(char *filein, const TBoundingBox<double> *bounding) const
    {
        ...
        // files ok?
        if (!CanOpenFile(filein, CanOpenFileRead) || !CanOpenFile(fileout, CanOpenFileWrite))
        {
            return;
        }
