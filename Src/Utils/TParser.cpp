...
namespace crtl
{
    ...
    bool TTokenVariable::CreateGof(const char *path, int numfiles)
    {
        ...
        while (StringContains((const char *)newpath, "temp_"));
    ...
    TTokenOperator::TTokenOperator(const char *tokenchar)
    {
        ...
        if (opi >= 0)
        { // found a matching operator name
        ...
        for (int i = 0; i <= l; i++, toc++)
        {
            ...
            if (isalpha(*start))
            {
                ...
                else
                {
                    ...
                    if (StringIsAnOperator(buff) != -1)
                        tokens.Push(new TTokenOperator(buff));

                    else if (variables.GetVariable(buff) != 0)
                        tokens.Push(new TTokenVariable(variables.GetVariable(buff)));

                    else
                        tokens.Push(new TTokenIdentifier(buff));
            ...
            { // other cases: special chars, like operators, or errors
    ...
    void TParser::SimplifiesUnaryOperators(TTokensStack &tokenstack)
    {
        for (int i = 0; i < tokenstack.GetNumTokens(); i++)
        {
            ...
            if (!(tok->IsOperator(OpBinaryMinus) || tok->IsOperator(OpBinaryPlus)))
                continue;
            ...
            for (int j = i; j < tokenstack.GetNumTokens(); j++)
            {
                ...
                if (tokenstack[j]->IsOperator(OpBinaryMinus))
                    countminus++;
                else if (tokenstack[j]->IsOperator(OpBinaryPlus))
                    countplus++;
                else
                    break;
            }
                ...
                for (insertposti = i + 3; insertposti < tokenstack.GetNumTokens() /*- 1*/; insertposti++)
                {
                    ...
                    else if (tokenstack[insertposti]->IsOperator(OpParenthesisOpen))
                    {
                        ...
                        while (parenthesislevel != 0 && insertposti < tokenstack.GetNumTokens())
                        {
                            ...
                            if (tokenstack[insertposti]->IsOperator(OpParenthesisOpen))
                                parenthesislevel++;
                            else if (tokenstack[insertposti]->IsOperator(OpParenthesisClose))
                                parenthesislevel--;
    ...
    bool TParser::Parse(const char *expression, const TTokensStack &variables, bool verbose)
    {
        ...
        for (int tokeni = 0; tokeni < tokensinfix.GetNumTokens(); tokeni++)
        {
            ...
            if (tok->IsOperator())
            {
                ...
                { // equal or less priority: pop all last, push the new

                    while (operators.IsNotEmpty() && operators.GetLastOperator()->Priority >= tokop->Priority && operators.GetLastOperator()->Priority < ParenthesisPriority)
                        Tokens.Push(operators.Pop());
        ...
        for (int tokeni = 0; tokeni < Tokens.GetNumTokens(); tokeni++)
        {
            ...
            case OpSqrt:
                ...
                if (nummemory == 1)
                {
                    ...
                    if (numfloat == 1)
                    {
                        ...
                        if (tokop->Code == OpAbs)
                            *tokvartemp->DataFloat = fabs(*tokvar1->DataFloat);
                        else if (tokop->Code == OpScalar)
                            *tokvartemp->DataFloat = *tokvar1->DataFloat;
                        else if (tokop->Code == OpSqr)
                            *tokvartemp->DataFloat = *tokvar1->DataFloat * *tokvar1->DataFloat;
                        else if (tokop->Code == OpSqrt)
                            *tokvartemp->DataFloat = sqrt(*tokvar1->DataFloat);
                    }
                    ...
                    else if (numvector == 1)
                    {
                        ...
                        // single variable now - generalize to use dimensions
                        if (tokop->Code == OpAbs)
                            *tokvartemp->DataFloat = tokvar1->DataVector->Norm();
                        else if (tokop->Code == OpScalar)
                            *tokvartemp->DataFloat = tokvar1->DataVector->Norm();
                        else if (tokop->Code == OpSqr)
                            *tokvartemp->DataFloat = tokvar1->DataVector->Norm2();
                        else
                        {
                            if (verbose)
                            {
                    ...
                    for (int i = 0; i < tokvar1->NumFiles(); i++)
                    {
                        ...
                        for (int tf = 0; tf < numtf; tf++)
                        {
                            ...
                            for (int e = 0; e < fileout.NumTracks; e++)
                                if (tokop->Code == OpAbs)
                                    fileout.Write(fabs(eegb(e, 0)));
                                // reading has already done the job to go down to scalar
                                else if (tokop->Code == OpScalar)
                                    fileout.Write(eegb(e, 0));
                                // its either a scalar, or the norm
                                else if (tokop->Code == OpSqr)
                                    fileout.Write(eegb(e, 0) * eegb(e, 0));
                                // only for scalar
                                else if (tokop->Code == OpSqrt)
                                    fileout.Write(sqrt(eegb(e, 0)));
                        }

                        if (file1->CanClose(true))
                            docmanager->CloseDoc(file1);
                    }
            ...
            case OpBinaryMinus:
                ...
                if (nummemory == 2)
                {
                    ...
                    if (numfloat == 2)
                    {
                        tokvartemp->Allocate(CombineFlags(AllocateMemory, AllocateFloat), operands);
                        // single variable now - generalize to use dimensions
                        if (tokop->Code == OpBinaryPlus)
                            *tokvartemp->DataFloat = *tokvar1->DataFloat + *tokvar2->DataFloat;
                        else if (tokop->Code == OpBinaryMinus)
                            *tokvartemp->DataFloat = *tokvar1->DataFloat - *tokvar2->DataFloat;
                    }
                    ...
                    else if (numvector == 2)
                    {
                        ...
                        // single variable now - generalize to use dimensions
                        if (tokop->Code == OpBinaryPlus)
                            *tokvartemp->DataVector = *tokvar1->DataVector + *tokvar2->DataVector;
                        else if (tokop->Code == OpBinaryMinus)
                            *tokvartemp->DataVector = *tokvar1->DataVector - *tokvar2->DataVector;
                    }
                            ...
                            if (fileout.IsScalar(AtomTypeUseOriginal))
                            {
                                ...
                                for (int e = 0; e < numel; e++)
                                    if (tokop->Code == OpBinaryPlus)
                                        ...
                                        fileout.Write(operandswitch ? *tokvar2->DataFloat - eegb(e, 0) : eegb(e, 0) - *tokvar2->DataFloat);
                            }
                            else
                            {
                                ...
                                for (int e = 0; e < numel; e++)
                                    if (tokop->Code == OpBinaryPlus)
                                        fileout.Write(vec1[e] + *tokvar2->DataVector);
                                    else if (tokop->Code == OpBinaryMinus)
                                        fileout.Write(operandswitch ? *tokvar2->DataVector - vec1[e] : vec1[e] - *tokvar2->DataVector);
                            }
                        }
                ...
                else if (numgof == 2)
                {
                    ...
                    // check operands types
                    if (tokvar1->Content == ContentMatrix || tokvar2->Content == ContentMatrix || tokvar1->DataType != tokvar2->DataType)
                    {
                        if (verbose)
                        {
                            sprintf(buff, "Can not apply operation \"%s\" on these types of files.", tokop->Name);
                    ...
                    for (int i = 0; i < tokvar1->NumFiles(); i++)
                    {
                        ...
                        for (int tf = 0; tf < numtf; tf++)
                        {
                            ...
                            if (fileout.IsScalar(AtomTypeUseOriginal))
                            {
                                ...
                                for (int e = 0; e < numel; e++)
                                    if (tokop->Code == OpBinaryPlus)
                                        fileout.Write(eegb(e, 0) + eegb(e, 1));
                                    else if (tokop->Code == OpBinaryMinus)
                                        fileout.Write(eegb(e, 0) - eegb(e, 1));
                            }
                            else
                        ...
                        if (file1->CanClose(true))
                            docmanager->CloseDoc(file1);
                        if (file2->CanClose(true))
                            docmanager->CloseDoc(file2);
                    }
            ...
            case OpBinaryMult:
                ...
                else if (numgof == 2)
                {
                    ...
                    for (int i = 0; i < tokvar1->NumFiles(); i++)
                    {
                        ...
                        if (isdoc->CanClose(true))
                            docmanager->CloseDoc(isdoc);
                    } // for isdoc
            ...
            case OpAssign:
                ...
                if (tokvar2->IsSingleFloat())
                {
                    ...
                    if (tokvar1->IsSingleFloat())
                        *tokvar1->DataFloat = *tokvar2->DataFloat;
                    else
                    {
                        if (verbose)
                        {
                ...
                else if (tokvar2->IsMemory() && tokvar2->IsVector())
                {
                    ...
                    if (tokvar1->IsMemory() && tokvar1->IsVector())
                        *tokvar1->DataVector = *tokvar2->DataVector;
                    else
                    {
                        if (verbose)
                        {
