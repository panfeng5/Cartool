...
#define DoubleQuoteS "\""
    ...
    void TSplitStrings::Add(const char *str, StringsUnicity unicity, const char *separators)
    {
        ...
        for (char *totok = (char *)buff; *totok; totok++)
        {
            ...
            if (!withinquote)
            {
                ...
                if (begin == 0 && *totok == DoubleQuoteC)
                { // note: for the moment, a quote is "allowed" inside a token
    ...
    {
        return Tokens.Contains(str) || Tokens.Contains("*"); // a single wildchar '*' will match everything
    }

    bool TSplitStrings::PartiallyContains(const char *str) const
    {
        return Tokens.PartiallyContains(str) || Tokens.Contains("*"); // a single wildchar '*' will match everything
    }
    ...
    void TSplitStrings::ExpandWildchars(TStrings &strs, StringsUnicity unicity)
    {
        ...
        for (int i = 0; i < GetNumTokens(); i++)
        {
            ...
            foreachin(strs, iterator) if (!Contains(iterator()) && StringStartsWith(iterator(), buff))
                AddToken(iterator());
        }
