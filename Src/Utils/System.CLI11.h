...
namespace crtl
{
        ...
        inline CLI::Option *ExcludeCLIOptions(CLI::App *app, const char *option, const char *excluded1, const char *excluded2 = 0, const char *excluded3 = 0)
        {
                ...
                if (StringIsNotEmpty(excluded1))
                        opt->excludes(GetCLIOption(app, excluded1));
                if (StringIsNotEmpty(excluded2))
                        opt->excludes(GetCLIOption(app, excluded2));
                if (StringIsNotEmpty(excluded3))
                        opt->excludes(GetCLIOption(app, excluded3));
