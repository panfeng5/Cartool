...
namespace crtl
{
    ...
    public:
        ...
        {
            if (IsExtendedPath())
                StringClip(String, StringLength(ExtendedPathPrefix), StringLength(String) - 1);
        ...
        void Open()
        {
            if (IsNotEmpty() && CanOpenFile())
                CartoolObjects.CartoolDocManager->OpenDoc(String, dtOpenOptions);
        }
    ...
    void TFileName::CheckFileName(TFilenameFlags flags)
    {
        ...
        if (IsFlag(flags, TFilenameMsDosToWindows))
            MsDosPathToWindowsPath(); // can expand to a longer string - though is currently prevented to be longer than 256
        if (IsFlag(flags, TFilenameAbsolutePath))
            CheckAbsolutePath(); // convert a relative path to an absolute one
        if (IsFlag(flags, TFilenameExtendedPath))
            CheckExtendedPath(); // add special prefix for path longer than 256
        if (IsFlag(flags, TFilenameSibling))
            CheckSiblingFile(); // convert to another extension / sibling file for Cartool purpose (.img to .hdr f.ex.)
        if (IsFlag(flags, TFilenameDirectory))
            RemoveFilename(String, false); // keeping only the directory part - trailing \ is removed
        if (IsFlag(flags, TFilenameNoOverwrite))
            CheckNoOverwrite(); // avoiding overwrite by generating a variation of given file name
    }
