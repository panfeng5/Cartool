...
namespace crtl
{
    ...
    void TEasyStats::Set(const Volume &array3, const Volume *mask, bool ignorenulls, bool allocate, int numdownsamples)
    {
        ...
        for (int i = 0, j = 0; i < array3.GetLinearDim(); i++)
        {
            ...
            if (isdown && (j++ % down.Step))
                continue; // test must be on a separate line, downsampling count is after masking
    ...
    void TEasyStats::KeepWithin(double minvalue, double maxvalue)
    {
        ...
        if (Data[0] <= maxvalue && Data[lastitem] >= minvalue)

            return;

        // All data outside range?
        if (Data[0] < minvalue || Data[lastitem] > maxvalue)
        {
            // Remove everything
            Reset();
                                 ...
                                 TArray1<double> *params)
    {
        ...
        for (int i = 0; i < numresampling; i++)
        {
            ...
            if (how == TEasyStatsFunctionMaxMode)
                var += substats.MaxModeHSM(); // set a default mode
            else if (how == TEasyStatsFunctionMaxModeHistogram)
                var += substats.MaxModeHistogram();
            else if (how == TEasyStatsFunctionMaxModeHSM)
                var += substats.MaxModeHSM();
            else if (how == TEasyStatsFunctionMaxModeHRM)
                var += substats.MaxModeHRM();

            else if (how == TEasyStatsFunctionSD)
                var += substats.SD();
            else if (how == TEasyStatsFunctionInterQuartileRange)
                var += substats.InterQuartileRange();
            else if (how == TEasyStatsFunctionMAD)
                var += substats.MAD(CanAlterData);
    ...
    void TEasyStats::AddAngle(double a, ThreadSafety safety)
    {
        if (NumItems)
        {
            ...
            if (a < avg - Pi)
                a += TwoPi; // do a better formula for more than 2Pi distance
            else if (a > avg + Pi)
                a -= TwoPi;
    ...
    */
    ...
    void TEasyStats::VarianceAsym(double center, double &varleft, double &varright)
    {
        ...
        OmpParallelForSum(vl, vr, nl, nr)
            ...
            for (int i = 0; i < NumItems; i++)
                ...
                if (Data[i] > center)
        {
            vr += Square(Data[i] - center);
            nr++;
        }
        else
        {
            vl += Square(Data[i] - center);
            nl++;
        }
    ...
    void TEasyStats::MADAsym(double center, double &madleft, double &madright)
    {
        ...
        for (int i = 0; i < NumItems; i++)
        {
            ...
            if (d < 0)
                deltal.Add(d, ThreadSafetyIgnore);
            else
                deltar.Add(d, ThreadSafetyIgnore);
        }
    ...
    double TEasyStats::MAD(double center, StorageDataLife datalife)
    {
        ...
        { // Caller wished to preserve the data - We will need some temp stats, which is costly
    ...
    double TEasyStats::Quantile(double p)
    {
        ...
        if (frac == 0)
            ...
            return (1 - frac) * Data[(int)cut] + frac * Data[(int)cut + 1]; // fractional part -> there are neighbors on each side
    }
    ...
    void TEasyStats::InterQuartileRangeAsym(double center, double &iqrleft, double &iqrright)
    {
        ...
        for (int i = 0; i < NumItems; i++)
            ...
            if (Data[i] < center)
                left.Add(Data[i], ThreadSafetyIgnore);
            else
                right.Add(Data[i], ThreadSafetyIgnore);
    ...
    double TEasyStats::SkewnessPearson(double center)
    {
        ...
        // unbiased estimator
        return skew / den * NumItems / (double)((NumItems - 1) * (NumItems - 2));
    }
    ...
    void TEasyStats::SkewnessPearsonAsym(double center, double &skewleft, double &skewright)
    {
        ...
        OmpParallelForSum(sl, sr, nl, nr)
            ...
            for (int i = 0; i < NumItems; i++)
                ...
                if (Data[i] > center)
        {
            sr += Cube(Data[i] - center);
            nr++;
        }
        else
        {
            sl += Cube(center - Data[i]);
            nl++;
        } // to keep a positive value
    ...
    int TEasyStats::DeSkew(DeSkewType how)
    {
        ...
        if (how == DeSkewRight || how == DeSkewAuto && isskewed == 1)
        {
        ...
        else if (how == DeSkewLeft || how == DeSkewAuto && isskewed == -1)
        {
        ...
        else if (how == DeSkewNone || how == DeSkewAuto && isskewed == 0)
        {
    ...
    double TEasyStats::Kurtosis()
    {
        ...
        // unbiased estimator
        return kurt / den * (NumItems * (NumItems + 1)) / (double)((NumItems - 1) * (NumItems - 2) * (NumItems - 3)) - 3 * Square((double)(NumItems - 1)) / (double)((NumItems - 2) * (NumItems - 3));
    }
    ...
    void TEasyStats::KurtosisAsym(double center, double &kurtleft, double &kurtright)
    {
        ...
        OmpParallelForSum(kl, kr, nl, nr)
            ...
            for (int i = 0; i < NumItems; i++)
                ...
                if (Data[i] > center)
        {
            kr += pow(Data[i] - center, 4);
            nr++;
        }
        else
        {
            kl += pow(Data[i] - center, 4);
            nl++;
        }

        // unbiased estimator
        kurtleft = nl < 4 ? 0
                          : kl / denleft * (nl * (nl + 1)) / (double)((nl - 1) * (nl - 2) * (nl - 3)) - 3 * Square((double)(nl - 1)) / (double)((nl - 2) * (nl - 3));

        kurtright = nr < 4 ? 0
                           : kr / denright * (nr * (nr + 1)) / (double)((nr - 1) * (nr - 2) * (nr - 3)) - 3 * Square((double)(nr - 1)) / (double)((nr - 2) * (nr - 3));
    }
    ...
    double TEasyStats::RobustKurtosis()
    {
        ...
        return (kurt / den * (NumItems * (NumItems + 1)) / (double)((NumItems - 1) * (NumItems - 2) * (NumItems - 3)) - 3 * Square((double)(NumItems - 1)) / (double)((NumItems - 2) * (NumItems - 3))) / sqrt((double)NumItems);
    }
...
#define CSKurtosisBeta 0.250
    ...
    double TEasyStats::GaussianKernelDensity(KernelDensityType kdetype, int numgaussians)
    {
        ...
        if (kdetype == KernelDensityDiscrete)
        {
            ...
            OmpParallelForSum(numunique)
                ...
                for (int i = 0; i < NumItems; i++) if (!i || Data[i] != Data[i - 1])
                    numunique++;
        ...
        else if (kdetype == KernelDensitySilverman || kdetype == KernelDensitySilvermanRobust)
        {
    ...
    // Bickel, Fr�hwirth "On a fast, robust estimator of the mode"
    double TEasyStats::MaxModeHSM()
    {
        ...
        if (N == 1)
            maxmodeposition = Data[imin];
        else if (N == 2)
            maxmodeposition = (Data[imin] + Data[imin + 1]) / 2;
        else /*if ( N == 3 )*/
            maxmodeposition = fabs(Data[imin] - Data[imin + 1]) < fabs(Data[imin + 1] - Data[imin + 2])
                                  ? (Data[imin] + Data[imin + 1]) / 2
                                  : (Data[imin + 1] + Data[imin + 2]) / 2;
    ...
    double TEasyStats::MaxModeHRM()
    {
        ...
        if (density == 1)
            ...
            maxmodeposition = (Data[imin] + Data[imax]) / 2;
    ...
    double TEasyStats::LastMode(double noisethreshold)
    {
        ...
        for (int i = (int)H - 2; i >= 1; i--)
            ...
            if (H[i] > minv && H[i] > H[i - 1] && H[i] > H[i + 1])
        ...
        for (int iter = 1; iter <= MaxGaussiantMixtureIter; iter++)
        {
            ...
            for (int k = 0; k < numgaussian; k++)
            {
                ...
                if (dofaster)
                {
                    ...
                    if (!(alpha(k) > 0 && IsInsideLimits(mean(k), Min(), Max())))
                    {
