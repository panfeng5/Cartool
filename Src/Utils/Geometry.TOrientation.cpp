...
namespace crtl
{
    ...
    bool IsOrientationOK(TOrientationType xaxis, TOrientationType yaxis, TOrientationType zaxis)
    {
        ...
        if (xaxis == ToBack)
            backvector.X = 1;
        else if (xaxis == ToFront)
            backvector.X = -1;
        else if (yaxis == ToBack)
            backvector.Y = 1;
        else if (yaxis == ToFront)
            backvector.Y = -1;
        else if (zaxis == ToBack)
            backvector.Z = 1;
        else if (zaxis == ToFront)
            backvector.Z = -1;
        ...
        if (xaxis == ToRight)
            rightvector.X = 1;
        else if (xaxis == ToLeft)
            rightvector.X = -1;
        else if (yaxis == ToRight)
            rightvector.Y = 1;
        else if (yaxis == ToLeft)
            rightvector.Y = -1;
        else if (zaxis == ToRight)
            rightvector.Z = 1;
        else if (zaxis == ToLeft)
            rightvector.Z = -1;
    ...
    bool TOrientation::SetOrientation(TOrientationType xaxis, TOrientationType yaxis, TOrientationType zaxis)
    {
        ...
        {
            BackVector.X = 1;
            BoxSides[XMinSide] = ToFront;
            BoxSides[XMaxSide] = ToBack;
            BoxSides[XAxis] = FrontBack;
        }
        else if (xaxis == ToFront)
        {
            BackVector.X = -1;
            BoxSides[XMinSide] = ToBack;
            BoxSides[XMaxSide] = ToFront;
            BoxSides[XAxis] = FrontBack;
        }
        else if (yaxis == ToBack)
        {
            BackVector.Y = 1;
            BoxSides[YMinSide] = ToFront;
            BoxSides[YMaxSide] = ToBack;
            BoxSides[YAxis] = FrontBack;
        }
        else if (yaxis == ToFront)
        {
            BackVector.Y = -1;
            BoxSides[YMinSide] = ToBack;
            BoxSides[YMaxSide] = ToFront;
            BoxSides[YAxis] = FrontBack;
        }
        else if (zaxis == ToBack)
        {
            BackVector.Z = 1;
            BoxSides[ZMinSide] = ToFront;
            BoxSides[ZMaxSide] = ToBack;
            BoxSides[ZAxis] = FrontBack;
        }
        else if (zaxis == ToFront)
        {
            BackVector.Z = -1;
            BoxSides[ZMinSide] = ToBack;
            BoxSides[ZMaxSide] = ToFront;
            BoxSides[ZAxis] = FrontBack;
        }
        ...
        {
            DownVector.X = 1;
            BoxSides[XMinSide] = ToUp;
            BoxSides[XMaxSide] = ToDown;
            BoxSides[XAxis] = UpDown;
        }
        else if (xaxis == ToUp)
        {
            DownVector.X = -1;
            BoxSides[XMinSide] = ToDown;
            BoxSides[XMaxSide] = ToUp;
            BoxSides[XAxis] = UpDown;
        }
        else if (yaxis == ToDown)
        {
            DownVector.Y = 1;
            BoxSides[YMinSide] = ToUp;
            BoxSides[YMaxSide] = ToDown;
            BoxSides[YAxis] = UpDown;
        }
        else if (yaxis == ToUp)
        {
            DownVector.Y = -1;
            BoxSides[YMinSide] = ToDown;
            BoxSides[YMaxSide] = ToUp;
            BoxSides[YAxis] = UpDown;
        }
        else if (zaxis == ToDown)
        {
            DownVector.Z = 1;
            BoxSides[ZMinSide] = ToUp;
            BoxSides[ZMaxSide] = ToDown;
            BoxSides[ZAxis] = UpDown;
        }
        else if (zaxis == ToUp)
        {
            DownVector.Z = -1;
            BoxSides[ZMinSide] = ToDown;
            BoxSides[ZMaxSide] = ToUp;
            BoxSides[ZAxis] = UpDown;
        }
        ...
        {
            RightVector.X = 1;
            BoxSides[XMinSide] = ToLeft;
            BoxSides[XMaxSide] = ToRight;
            BoxSides[XAxis] = LeftRight;
        }
        else if (xaxis == ToLeft)
        {
            RightVector.X = -1;
            BoxSides[XMinSide] = ToRight;
            BoxSides[XMaxSide] = ToLeft;
            BoxSides[XAxis] = LeftRight;
        }
        else if (yaxis == ToRight)
        {
            RightVector.Y = 1;
            BoxSides[YMinSide] = ToLeft;
            BoxSides[YMaxSide] = ToRight;
            BoxSides[YAxis] = LeftRight;
        }
        else if (yaxis == ToLeft)
        {
            RightVector.Y = -1;
            BoxSides[YMinSide] = ToRight;
            BoxSides[YMaxSide] = ToLeft;
            BoxSides[YAxis] = LeftRight;
        }
        else if (zaxis == ToRight)
        {
            RightVector.Z = 1;
            BoxSides[ZMinSide] = ToLeft;
            BoxSides[ZMaxSide] = ToRight;
            BoxSides[ZAxis] = LeftRight;
        }
        else if (zaxis == ToLeft)
        {
            RightVector.Z = -1;
            BoxSides[ZMinSide] = ToRight;
            BoxSides[ZMaxSide] = ToLeft;
            BoxSides[ZAxis] = LeftRight;
        }

        StandardOrientMatrix[0] = xaxis == ToBack ? 1 : xaxis == ToFront ? -1
                                                                         : 0;
        StandardOrientMatrix[1] = xaxis == ToDown ? 1 : xaxis == ToUp ? -1
                                                                      : 0;
        StandardOrientMatrix[2] = xaxis == ToRight ? 1 : xaxis == ToLeft ? -1
                                                                         : 0;

        StandardOrientMatrix[4] = yaxis == ToBack ? 1 : yaxis == ToFront ? -1
                                                                         : 0;
        StandardOrientMatrix[5] = yaxis == ToDown ? 1 : yaxis == ToUp ? -1
                                                                      : 0;
        StandardOrientMatrix[6] = yaxis == ToRight ? 1 : yaxis == ToLeft ? -1
                                                                         : 0;

        StandardOrientMatrix[8] = zaxis == ToBack ? 1 : zaxis == ToFront ? -1
                                                                         : 0;
        StandardOrientMatrix[9] = zaxis == ToDown ? 1 : zaxis == ToUp ? -1
                                                                      : 0;
        StandardOrientMatrix[10] = zaxis == ToRight ? 1 : zaxis == ToLeft ? -1
                                                                          : 0;
