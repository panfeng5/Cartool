...
namespace crtl
{
    ...
    void TOpenDoc<TypeD>::Close(TOpenDocType option)
    {
        ...
        bool letopen = option == CloseDocLetOpen || option == CloseDocRestoreAsBefore && AlreadyOpen;

        // close doc if it is open and we want to restore the orginal state
        if (!letopen && Doc->TBaseDoc::CanClose(true)) // TRisDoc doesn't seem to see this function?!
    ...
    bool TOpenDoc<TypeD>::Open(const char *filepath, TOpenDocType option)
    {
        ...
        if (StringIsEmpty(path) || !CanOpenFile(path)) // test beforehand if file exists, so we can gracefully return before requesting the main application - this could be restricted to OpenDocHidden only?
            return false;
        ...
        if (option == OpenDocVisibleLocked)
            Lock();
        else
            Unlock();
