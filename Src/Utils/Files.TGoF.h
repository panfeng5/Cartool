...
namespace crtl
{
    ...
    public:
        ...
        TGoF(const char *file, TFilenameFlags flags = TFilenameNoPreprocessing)
        {
            SetOnly(file);
            CheckFileNames(flags);
        }
        TGoF(const TStrings &files, TFilenameFlags flags = TFilenameNoPreprocessing)
        {
            Set(files);
            CheckFileNames(flags);
        } // TStrings "copy" constructor
        TGoF(const std::vector<std::string> &files, TFilenameFlags flags = TFilenameNoPreprocessing)
        {
            Set(files);
            CheckFileNames(flags);
        } // Used by CLI
        TGoF(const owl::TStringArray &files, TFilenameFlags flags = TFilenameNoPreprocessing)
        {
            Set(files);
            CheckFileNames(flags);
        } // Used by OwlNext dialogs
        TGoF(const TArray2<char> &files, TFilenameFlags flags = TFilenameNoPreprocessing)
        {
            Set(files);
            CheckFileNames(flags);
        } // Can be handy
        ...
        TGoF(const TGoF &files, TFilenameFlags flags = TFilenameNoPreprocessing)
        {
            Set(files);
            CheckFileNames(flags);
        } // TGoF copy constructor
