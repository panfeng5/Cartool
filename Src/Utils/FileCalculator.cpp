...
namespace crtl
{
    ...
    {
        if (StringIsEmpty(expr) || StringIsEmpty(basedir) || StringIsEmpty(fileext) || gogof.IsEmpty())
        ...
        if (StringContains((const char *)expr, "result"))
            StringCopy(checkedexpr, expr);
        else
            StringCopy(checkedexpr, "result = ", expr);
        ...
        { // create a new list of the renamed files
            TGoF renamed;
            TFileName dir;
            TFileName filename;
