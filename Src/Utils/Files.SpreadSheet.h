...
#define SpreadSheetFilesExt FILEEXT_CSV " " FILEEXT_TXT
    ...
    CsvType TSpreadSheet::GetType() const
    {
        ...
        bool isstatfiles = HasAttribute("numfiles") && HasAttribute("timemin") // Only in stat files
                           && HasAttribute("timemax") && HasAttribute("timemode") && HasAttribute("file1");

        bool issegmentation = HasAttribute("numfiles") && !HasAttribute("timemin") // NOT like stat files
                              && StringGrep(GetHeader(1), "file[0-9]+", GrepOptionDefault);

        bool istimefreqepochs = HasAttribute("Track Name") && HasAttribute("TF From") && HasAttribute("TF To") && HasAttribute("FreqCentr");

        bool isaveraging = HasAttribute("file") && HasAttribute("session") && HasAttribute("tva") && HasAttribute("triggers");
        ...
        if (isstatfittingcartool)
            csvtype = (CsvType)(csvtype | (StringGrep(GetHeader(1), FitGroupNameLong "[0-9]+_" FitMapNameLong "[0-9]+_.+", GrepOptionDefault) ? CsvLongNames : CsvShortNames));
        // The short case shouldn't really occur, still testing
        if (isstatfittingR)
            csvtype = (CsvType)(csvtype | (StringLength(GetHeader(3)) > 1 ? CsvLongNames : CsvShortNames));

        if (isstatmarkov)
            csvtype = (CsvType)(csvtype | (StringGrep(GetHeader(1), FitGroupNameLong "[0-9]+_" FitFromMapNameLong "[0-9]+_" FitToMapNameLong "[0-9]+", GrepOptionDefault) ? CsvLongNames : CsvShortNames));
                                      ...
                                      int &numfitvars, TStrings &fitvarnames) const
    {
        ...
        for (int i = 0; i < GetNumAttributes(); i++)
        {
            ...
            if (StringStartsWith(attr, FitGroupNameLong))
            {
                ...
                if (StringIsEmpty(csvfirstgroup))
                    sprintf(csvfirstgroup, FitGroupNameLong "%d_", gi);
            }
            ...
            else if (StringStartsWith(attr, FitGroupNameShort))
            {
                ...
                if (StringIsEmpty(csvfirstgroup))
                    sprintf(csvfirstgroup, FitGroupNameShort "%d_", gi);
            }
        ...
        for (int i = 0; i < GetNumAttributes(); i++)
        {
            ...
            if (IsLongNames(csvtype))
                sscanf(attr, FitGroupNameLong "%*d_%s", varname);
            else if (IsShortNames(csvtype))
                sscanf(attr, FitGroupNameShort "%*d_%s", varname);
        ...
        for (int i = 0; i < numvarspergroup; i++)
        {
            ...
            if (IsCsvStatMarkov(csvtype) && IsLongNames(csvtype))
                StringReplace(mapname, FitFromMapNameLong, FitMapNameLong);
            else if (IsCsvStatMarkov(csvtype) && IsShortNames(csvtype))
                StringReplace(mapname, FitFromMapNameShort, FitMapNameShort);
    ...
    bool TSpreadSheet::InitFile(char *file, char *forcedelimiter)
    {
        ...
        // get delimiter
        if (StringIsNotEmpty(forcedelimiter))
            StringCopy(OutListSeparator, forcedelimiter, SpreadSheetSeparatorListSize - 1);
        else
        {
            if (IsExtension((char *)getfile, FILEEXT_CSV))
                StringCopy(OutListSeparator, LocaleListSeparator, SpreadSheetSeparatorListSize - 1);
            else if (IsExtension((char *)getfile, FILEEXT_TXT))
                StringCopy(OutListSeparator, Tab, SpreadSheetSeparatorListSize - 1);
            else
                StringCopy(OutListSeparator, " ", SpreadSheetSeparatorListSize - 1);
        }
    ...
    bool TSpreadSheet::ReadFile(const char *file)
    {
        ...
        if (StringIs(toext, FILEEXT_CSV))
        { // actually, this case is tricky, it can either be , or ;

            is.getline(buffline, bufflinesize, '\n');

            if (StringContains(buffline, ';'))
                delimiter = ";";
            else if (StringContains(buffline, ','))
                delimiter = ",";
            else
                delimiter = LocaleListSeparator;
        }
        else if (StringIs(toext, "txt"))
            delimiter = Tab;
        else
            delimiter = " ";
    ...
    bool TSpreadSheet::GetRecord(int record, int attribute, char *answer) const
    {
        ...
        if (!(IsInsideLimits(record, 0, NumRecords - 1) && IsInsideLimits(attribute, 0, NumAttributes - 1)))
            return false;
