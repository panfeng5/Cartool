...
namespace crtl
{
    ...
    inline TypeD SwapBytes(TypeD v, bool swap = true)
    {
        if (swap)
        {
            if (sizeof(TypeD) == 1)
                ;
            else if (sizeof(TypeD) == 2)
                v = (TypeD)_byteswap_ushort(v);
            else if (sizeof(TypeD) == 4)
                v = (TypeD)_byteswap_ulong(v);
            else if (sizeof(TypeD) == 8)
                v = (TypeD)_byteswap_uint64(v);
        }
    ...
    inline void Permutate(TypeD &a, TypeD &b)
    {
        TypeD t(a);
        a = b;
        b = t;
    ...
    template <class TypeD>
    inline void CheckOrder(TypeD &low, TypeD &high)
    {
        if (low > high)
            Permutate(low, high);
    }
    ...
    template <class TypeD>
    inline void CheckOrder(TypeD &low, TypeD &mid, TypeD &high)
    {
        CheckOrder(mid, high);
        CheckOrder(low, mid);
        CheckOrder(mid, high);
    ...
    template <typename TypeD>
    inline TypeD Clip(TypeD v, TypeD limitmin, TypeD limitmax) { return v < limitmin ? limitmin : v > limitmax ? limitmax
                                                                                                               : v; }
...
#undef max
    ...
    template <typename T>
    inline T AbsNonNull(T v) { return v > 0 ? v : v < 0 ? -v
                                                        : 1; } // v non-null -> |v|, else 1
    template <typename T>
    inline void NonNulled(T &v)
    {
        if (v == 0)
            v = 1;
    }
    template <typename T>
    inline void AbsNonNulled(T &v)
    {
        if (v < 0)
            v = -v;
        else if (v == 0)
    ...
    inline void LinearIndexToIndexes(int li, int &i1, int &i2, int dim2)
    {
        i1 = li / dim2;
        i2 = li % dim2;
    }
