...
namespace crtl
{
    ...
    void CheckExtendedPath(char *path, bool force)
    {
        ...
        if ((force || StringLength(path) >= WindowsMaxPath) && !IsExtendedPath(path))
    ...
    void NukeDirectory(const char *dir, bool confirm)
    {
        ...
        if (dirhaswildchar)
        { // search will begin at directory level
        ...
        while (findfile.SearchFile(file))
        {
            ...
            if (findfile.IsOwnDirectory() || findfile.IsAboveDirectory())
                continue;
    ...
    {
        if (StringIsEmpty(filefrom) || StringIsEmpty(filedest))
            return;
    ...
    void _CopyFileToDirectory(const char *filetocopy, const char *todir, bool deletesource, char *finalfile)
    {
        ...
        if (deletesource)
            MoveFileExtended(CopyFromFile, CopyToFile);
        else
            CopyFileExtended(CopyFromFile, CopyToFile);
    ...
    */
    ...
    bool IsAbsoluteFilename(const char *filename)
    {
        ...
        return grepdoswinunc1.Matched(filename) || grepwinunc234.Matched(filename)
...
#define nameseparator "(\\.|_|-)"
        ...
        if (IsExtensionAmong(filepath, AllMriFilesExt " " FILEEXT_MRIAVW_IMG))
        {
            ...
            if (StringGrep(filepath, nameseparator "ROI" nameseparator, options))
                return (MriContentType)(MriContentTypeSegmented | MriContentTypeRoiMask);
            if (StringGrep(filepath, nameseparator "mask" nameseparator, options))
                return (MriContentType)(MriContentTypeSegmented | MriContentTypeBinaryMask);
            if (StringGrep(filepath, nameseparator "(gray|grey|class-GM)" nameseparator, options))
                return (MriContentType)(MriContentTypeSegmented | MriContentTypeGrey);
            if (StringGrep(filepath, nameseparator "(white|class-WM)" nameseparator, options))
                return (MriContentType)(MriContentTypeSegmented | MriContentTypeWhite);
            if (StringGrep(filepath, nameseparator "(brain|seg|full brain)" nameseparator, options))
                return (MriContentType)(MriContentTypeSegmented | MriContentTypeBrain);
            if (StringGrep(filepath, nameseparator "(head|full|original)" nameseparator, options))
                return MriContentTypeFullHead;
            if (StringGrep(filepath, nameseparator "tissues" nameseparator, options))
                return (MriContentType)(MriContentTypeSegmented | MriContentTypeRoiMask);
        } // MRI type
    ...
    bool IsTemplateFileName(const char *filepath)
    {
        ...
        bool istemplate = StringStartsWith(filename, "MNI152") // T1/T2/PD
                          || StringStartsWith(filename, "MNI305") || StringStartsWith(filename, "avg152") || StringStartsWith(filename, "ave152");
