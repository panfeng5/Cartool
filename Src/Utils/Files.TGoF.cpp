...
namespace crtl
{
    ...
    void TracksCompatibleClass::Show(const char *title) const
    {
        ...
        ClearString(buff);
        StringAppend(buff, "NumTracks" Tab Tab "= ", NumTracks == CompatibilityIrrelevant ? "Irrelevant" : NumTracks == CompatibilityNotConsistent ? "Inconsistent"
                                                                                                                                                   : IntegerToString(NumTracks),
                     NewLine);
        StringAppend(buff, "NumAuxTracks" Tab Tab "= ", NumAuxTracks == CompatibilityIrrelevant ? "Irrelevant" : NumAuxTracks == CompatibilityNotConsistent ? "Inconsistent"
                                                                                                                                                            : IntegerToString(NumAuxTracks),
                     NewLine);
        StringAppend(buff, "NumSolPoints" Tab Tab "= ", NumSolPoints == CompatibilityIrrelevant ? "Irrelevant" : NumSolPoints == CompatibilityNotConsistent ? "Inconsistent"
                                                                                                                                                            : IntegerToString(NumSolPoints),
                     NewLine);
        StringAppend(buff, "NumTF" Tab Tab Tab "= ", NumTF == CompatibilityIrrelevant ? "Irrelevant" : NumTF == CompatibilityNotConsistent ? "Inconsistent"
                                                                                                                                           : IntegerToString(NumTF),
                     NewLine);
        StringAppend(buff, "NumFreqs" Tab Tab "= ", NumFreqs == CompatibilityIrrelevant ? "Irrelevant" : NumFreqs == CompatibilityNotConsistent ? "Inconsistent"
                                                                                                                                                : IntegerToString(NumFreqs),
                     NewLine);
        StringAppend(buff, "SamplingFrequency" Tab "= ", SamplingFrequency == CompatibilityIrrelevant ? "Irrelevant" : SamplingFrequency == CompatibilityNotConsistent ? "Inconsistent"
                                                                                                                                                                       : FloatToString(SamplingFrequency));
    ...
    void TGoF::Sort()
    {
        ...
        for (int l = 0; l < minlength; l++)
        {
            ...
            if (i == numstrings)
                commonstartlength++; // all files matched current char
            else
                break; // stop now
        }
    ...
    TGoF::TGoF(const owl::TDropInfo &drop, const char *filesext, TPointInt *where, bool doesnotbelong)
    {
        ...
        if (where != 0)
        {
            ...
            if (droppedinside)
                where->Set(droppedposition.X(), droppedposition.Y(), 0);
            else
                where->Reset();
        }
        ...
        for (int i = 0; i < dropcount; i++)
        {
            ...
            for (int si = 0; si < (int)siblingfiles; si++)
                ...
                if (StringIsEmpty(filesext) || (IsExtensionAmong(siblingfiles[si], filesext) ^ doesnotbelong))
    ...
    void TGoF::CopyFilesTo(const char *newdir, CopyToFlags flags, const char *buddyexts)
    {
        ...
        for (int i = 0; i < (int)Strings; i++)
        {
            ...
            for (int i = -1; i < (int)exts; i++)
            {
                ...
                if (flags & CopyAndDeleteOriginals)
                    MoveFileExtended(oldfilename, newfilename); // copy & delete original == moving
                else
                    CopyFileExtended(oldfilename, newfilename); // make a separate copy
    ...
    int TGoF::FindFiles(const char *templ, bool searchfiles)
    {
        ...
        while (findfile.SearchFile(templ))
        {
            ...
            if (searchfiles && !findfile.IsFile() || !searchfiles && !findfile.IsDirectory())
                continue;
    ...
    void TGoF::KeepLatestFile()
    {
        ...
        for (int i = 0; i < (int)Strings; i++)
        {
            ...
            else if (LastWriteTime.dwHighDateTime > BestLastWriteTime.dwHighDateTime || LastWriteTime.dwHighDateTime == BestLastWriteTime.dwHighDateTime && LastWriteTime.dwLowDateTime > BestLastWriteTime.dwLowDateTime)
            {
    ...
    */
    ...
    bool TGoF::AllExtensionsAre(const char *exts, int atomtype) const
    {
        ...
        for (int i = 0; i < (int)Strings; i++)
        {
            ...
            if (atomtype != UnknownAtomType)
            {
                ...
                if (atomtype == AtomTypeScalar && !isscalar || atomtype == AtomTypeVector && isscalar)
                    return false;
            }
        }
    ...
    bool TGoF::AllFreqsGroup() const
    {
        ...
        if (!(AnyTracksGroup(tg) && tg.allfreq))
        ...
        return tc.NumTracks >= CompatibilityConsistent && tc.NumTracks > 0 && tc.NumTF >= CompatibilityConsistent && tc.NumTF > 0 && tc.NumFreqs >= CompatibilityConsistent && tc.NumFreqs > 0;
    }
    ...
    bool TGoF::AnyTracksGroup(TracksGroupClass &tg) const
    {
        ...
        for (int i = 0; i < (int)Strings; i++)
        {
            ...
            if (IsExtensionAmong(Strings[i], AllEegFilesExt))
                eegcount++;
            else if (IsExtensionAmong(Strings[i], AllFreqFilesExt))
                freqcount++;
            //  else if ( IsExtensionAmong ( Strings[ i ], AllSegDataExt   ) )      segdatacount++;
            else if (IsExtensionAmong(Strings[i], AllDataExt))
                datacount++;
            else if (IsExtensionAmong(Strings[i], AllRisFilesExt))
            {
                ...
                if (ReadFromHeader(Strings[i], ReadInverseScalar, &isscalar) && !isscalar)
                        ...
                        TGoGoF &resgogof, TGoGoF *resgogofalt)
    {
        ...
        if (IsEmpty() || resampling == NoTimeResampling || numresamples <= 0 || resamplingsize <= 0)
    ...
    void TGoF::SplitFreqFiles(SplitFreqFlags how, TGoGoF *gogofout, bool showgauge)
    {
        ...
        for (int i = 0; i < NumFiles(); i++)
        {
            ...
            int readtf = (512 * (1 << 20)) / NonNull(numel * numfreqs * (FreqDoc->IsComplex(AtomTypeUseOriginal) ? 2 : 1) * sizeof(float)); // space for "1 slice" of electrodes x frequencies
            ...
            if (i == 0)
                if (how == SplitFreqByFrequency)
                    Gauge.SetRange(0, NumFiles() * numblocks * numfreqs * numparts);
                else if (how == SplitFreqByElectrode)
                    Gauge.SetRange(0, NumFiles() * numblocks * numel * numparts);
                else if (how == SplitFreqByTime)
                    Gauge.SetRange(0, NumFiles() * numtf * numparts);
            ...
            for (long blocktf = 0; blocktf < numtf; blocktf += readtf)
            {
                ...
                if (numparts == 2)
                    FreqDoc->ReadFrequencies(blocktf, blocktf + blocklen - 1, realpart, imagpart);
                else
                    FreqDoc->ReadFrequencies(blocktf, blocktf + blocklen - 1, realpart);
                            ...
                            for (int freq = 0; freq < numfreqs; freq++)
                                for (int e = 0; e < numel; e++)
                                {
                                    ...
                                    if (FreqDoc->IsFreqTypeFFTApproximation())
                                        v = Square(realpart(freq, e, tf0)); // we could also output fabs(fftapprox)
                                    else if (part == 0)
                                        v = realpart(freq, e, tf0);
                                    else
                                        v = imagpart(freq, e, tf0);
    ...
    int TGoF::SplitByNames(const char *splitwith,
                           TGoGoF &gogofout, TStrings *groupnames) const
    {
        gogofout.Reset();
                              ...
                              TStrings *diffs) const
    {
        ...
        for (int l = 1; l <= minlength; l++)
        {
            ...
            if (i == numstrings)
                commonendlength++; // all files matched current char
            else
                break; // stop now
        }
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // there they are:
        if (commonstart && commonstartlength > 0)
            StringCopy(commonstart, str[0], commonstartlength);

        if (commonend && commonendlength > 0)
            StringCopy(commonend, StringEnd(str[0]) - commonendlength, commonendlength);
...
#define InsignificantWords "From Into To The In Export"
        ...
        else if (numstrings < 50)
        {
            ...
            for (int i = 0; i < numstrings - 1; i++)
                for (int itok = 2; itok < splitnames[i].GetNumTokens(); itok++)
                {
                    ...
                    if (found)
                    { // also remove the first occurence
        ...
        { // too much files, go for a faster cleansing
        ...
        */
        ...
        while (match.IsNotEmpty() &&
               (StringStartsWith(match, " ") || StringStartsWith(match, ".") || StringStartsWith(match, "_")))
        {
        ...
        while (match.IsNotEmpty() &&
               (StringEndsWith(match, " ") || StringEndsWith(match, ".") || StringEndsWith(match, "_")))
        {
