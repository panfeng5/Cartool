...
namespace crtl
{
    ...
    {
        if (!(Processing == CreateRoisInteractiveTracks || Processing == CreateRoisInteractiveXyz || Processing == CreateRoisInteractiveSp) || Rois.IsNotAllocated() || Rois.IsEmpty() || BaseDoc == 0 || StringIsEmpty(BaseFileName))
        ...
        verbose.NextTopic("Input Files:");
        {
            if (Processing == CreateRoisInteractiveTracks)
                verbose.Put("Tracks file:", BaseDoc->GetDocPath());
            else if (Processing == CreateRoisInteractiveXyz)
                verbose.Put("Electrodes Coordinates file:", BaseDoc->GetDocPath());
            else if (Processing == CreateRoisInteractiveSp)
                verbose.Put("Solution Points file:", BaseDoc->GetDocPath());
    ...
    bool GenerateRoisFromSpAndMri(
        ...
        const char *BaseFileName)
    {
        if (!(Processing == CreateRoisAutomaticSpMri || Processing == CreateRoisAutomaticSpAAL) || SPDoc == 0 || MRIDoc == 0 || StringIsEmpty(BaseFileName))
        ...
        do
        {
            ...
            if (genroisfileok)
                ifs.getline(buff, KiloByte); // read next roi from file
            else
                IntegerToString(buff, ++curroi); // or simply generate one
            ...
            for (int i = 0; i < (int)tokens; i++)
            {
                ...
                OmpParallelFor
                    ...
                    for (int x = mrixmin; x <= mrixmax; x++) for (int y = mriymin; y <= mriymax; y++) for (int z = mrizmin; z <= mrizmax; z++)
                {
                    ...
                    for (int sp = 0; sp < numsp; sp++)
                    {
                        // shift of 0.5 or not?
                        // mribackgdata.ToAbs ()
                        if (Truncate(points[sp].X + 0.5) == Truncate(x - MRIOrigin.X) && Truncate(points[sp].Y + 0.5) == Truncate(y - MRIOrigin.Y) && Truncate(points[sp].Z + 0.5) == Truncate(z - MRIOrigin.Z))
                        {
                            // thread-safe
                            SelForRoi.Set(sp);
        ...
        verbose.NextTopic("Processing:");
        {
            ...
            if (Processing == CreateRoisAutomaticSpMri)
                verbose.Put("Mode:", "Automatic, Solution Points intersecting a Volume Mask");
            else if (Processing == CreateRoisAutomaticSpAAL)
                verbose.Put("Mode:", "Automatic, Solution Points intersecting the AAL Mask");
    ...
    bool GenerateRoisFromSpAndTalairach(
        ...
        const char *BaseFileName)
    {
        if (Processing != CreateRoisAutomaticSpTalairach || SPDoc == 0 || StringIsEmpty(GenerateRoisFile) || StringIsEmpty(BaseFileName))
        ...
        if (MRIDoc)
        { // allocate and prepare some working volumes
            ...
            for (int i = 0; i < numfields; i++)
            {
                ...
                { // not a known talairach name?
                    ...
                    if (firstfield && (StringStartsWith(l[i], "Right") || StringStartsWith(l[i], "Left")))
                    {
                        ...
                        if (!talcode)
                        { // really continue if the remaining part has not been recognized
