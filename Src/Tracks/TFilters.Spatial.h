...
namespace crtl
{
    ...
    inline SpatialFilterType TextToSpatialFilterType(const char *text)
    {
        ...
        for (int i = 0; i < NumSpatialFilterTypes; i++)
            ...
            if (StringContains(text, SpatialFilterLongName[i]) || StringContains(text, SpatialFilterShortName[i]))
    ...
    void TFilterSpatial<TypeD>::Set(SpatialFilterType how, const char *file, int maxnumneigh, double maxdistneigh)
    {
        ...
        if (How != SpatialFilterNone && How == how && IsAllocated()
            //&& GetNumPoints () == dim
            && File == file)
    ...
    bool TFilterSpatial<TypeD>::Apply(TVector<TypeD> &map)
    {
        ...
        if (How == SpatialFilterInterseptileWeightedMean || How == SpatialFilterInterseptileGaussianMean)
            return SpatialFilteringInterseptile(map);

        else if (How == SpatialFilterOutlier)
            return SpatialFilteringInterpolate(map);

        else if (How == SpatialFilterOutliersGaussianMean)
            return SpatialFilteringInterpolateSmooth(map);
        else if (How == SpatialFilterOutliersWeightedMean)
            return SpatialFilteringInterpolateSmooth(map);

        else if (How == SpatialFilterMedian || How == SpatialFilterInterquartileMean || How == SpatialFilterMinMax)
            return SpatialFilteringStat(map);
    ...
    {
        if (!IsDimensionOK(data) || How == SpatialFilterNone)
            return;
    ...
    {
        if (!IsDimensionOK(maps) || How == SpatialFilterNone)
            return;
    ...
    */
    ...
    {
        if (!(How == SpatialFilterMedian || How == SpatialFilterInterquartileMean || How == SpatialFilterMinMax))
            return false;
        ...
        for (int i = 0; i < numpoints; i++)
        {
            ...
            if (How == SpatialFilterMedian)
                map[i] = stat.Median(false);
            //  else if ( How == SpatialFilterMean               )  map[ i ]    = stat.Mean ();
            else if (How == SpatialFilterInterquartileMean)
                map[i] = stat.InterQuartileMean();
            else if (How == SpatialFilterMinMax)
                map[i] = stat.MinMax(stat[0]); // !stat not sorted yet, so first element is the central one (distance == 0)!
    ...
    {
        if (!(How == SpatialFilterOutliersGaussianMean || How == SpatialFilterOutliersWeightedMean))
            return false;
