...
namespace crtl
{
    ...
    public:
        ...
        int CheckNumMaps(int &nummaps) const
        {
            nummaps = nummaps < 0 ? NumMaps : crtl::NoMore(NumMaps, (const int)nummaps);
            return nummaps;
        } // passing -1 for all maps, otherwise doing a safe range clipping
