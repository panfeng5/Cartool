...
namespace crtl
{
                                 ...
                                 const TPoints &frompoints)
    {
        ...
        if (interpolationtype == Interpolation2DSurfaceSpline)
            esize = (mdegree * (mdegree + 1)) / 2;
        else if (interpolationtype == InterpolationSphericalSpline)
            esize = 1;
        else if (interpolationtype == InterpolationSphericalCurrentDensitySpline)
            esize = 1;
        else if (interpolationtype == Interpolation3DSpline)
            esize = (mdegree * (mdegree + 1) * (mdegree + 2)) / (3 * 2);
        else
            esize = 0;
                ...
                for (int d = 0, row = maxelfrom; d < mdegree; d++)
                    for (int k = 0; k <= d; k++)
                    {
                        ...
                        if (d - k == 0)
                            ...
                            e = pow(frompoints[i].X, d - k);

                        if (k != 0)
                            e *= pow(frompoints[i].Y, k);
        ...
        else if (interpolationtype == InterpolationSphericalSpline || interpolationtype == InterpolationSphericalCurrentDensitySpline)
        {
                ...
                for (int d = 0, row = maxelfrom; d < mdegree; d++)
                    for (int k = 0; k <= d; k++)
                        for (int g = 0; g <= k; g++)
                        {
                            ...
                            if (k - g != 0)
                                e *= pow(frompoints[i].Y, k - g);

                            if (g != 0)
                                e *= pow(frompoints[i].Z, g);
                                 ...
                                 bool silent)
    {
        ...
        if (FromNormalized == FiducialNormalization && FromFiducials.SomeStringsGrep(GrepEmptyInput, GrepOptionDefault))
        {
        ...
        // checking for empty fields
        if (DestNormalized == FiducialNormalization && DestFiducials.SomeStringsGrep(GrepEmptyInput, GrepOptionDefault))
        {
    ...
    bool TInterpolateTracks::SetTransformMatrices()
    {
        ...
        { // AlreadyNormalized - trust From electrodes are in a normalized, fiducial-like space
        ...
        if (TargetSpace == BackToOriginalElectrodes || (Dest_To_Fid.IsIdentity() && Fid_To_From.IsIdentity()))
        {
        ...
        */
        ...
        verbose.NextTopic("'From' Space:");
        {
            ...
            verbose.Put("Re-orient, center & normalize:", FromNormalized == AlreadyNormalized ? "Using electrodes coordinates from file 'as is'"
                                                                                              ...
                                                                                              : "Transforming to a Fiducial coordinate system");
        ...
        verbose.NextTopic("Interpolation:");
        {
            ...
            if (InterpolationType == Interpolation2DSurfaceSpline)
                (ofstream &)verbose << "Surface Spline" << NewLine;
            else if (InterpolationType == InterpolationSphericalSpline)
                (ofstream &)verbose << "Spherical Spline" << NewLine;
            else if (InterpolationType == InterpolationSphericalCurrentDensitySpline)
                (ofstream &)verbose << "Current Density with Spherical Spline" << NewLine;
            else if (InterpolationType == Interpolation3DSpline)
                (ofstream &)verbose << "3D Spline" << NewLine;
        ...
        for (int toei = 0; toei < maxelto; toei++)
            ...
            if (ToHasSamePositionAsFrom[toei] == InvalidIndex || fromgoodsel.IsNotSelected(ToHasSamePositionAsFrom[toei]) || InterpolationType == InterpolationSphericalCurrentDensitySpline)
            {
        ...
        auto Interpolate2DSurfaceSpline = [this](int toei, const AVector &pq)
        {
            ...
            for (int d = 0, row = maxelfrom; d < SplineDegree; d++)
                for (int k = 0; k <= d; k++)
                    ...
                    v += pq[row++] * Dest_To_FidPoints_Power[d - k][toei].X * Dest_To_FidPoints_Power[k][toei].Y;
        ...
        auto Interpolate3DSpline2 = [this](int toei, const AVector &pq)
        {
            ...
            v += pq[row] * Dest_To_FidPoints_Power[0][toei].X * Dest_To_FidPoints_Power[0][toei].Y * Dest_To_FidPoints_Power[0][toei].Z

                 + pq[row + 1] * Dest_To_FidPoints_Power[1][toei].X * Dest_To_FidPoints_Power[0][toei].Y * Dest_To_FidPoints_Power[0][toei].Z

                 + pq[row + 2] * Dest_To_FidPoints_Power[0][toei].X * Dest_To_FidPoints_Power[1][toei].Y * Dest_To_FidPoints_Power[0][toei].Z

                 + pq[row + 3] * Dest_To_FidPoints_Power[0][toei].X * Dest_To_FidPoints_Power[0][toei].Y * Dest_To_FidPoints_Power[1][toei].Z;
            ...
            for (int d = 0, row = maxelfrom; d < SplineDegree; d++)
                for (int k = 0; k <= d; k++)
                    for (int g = 0; g <= k; g++)
                        ...
                        v += pq[row++] * Dest_To_FidPoints_Power[d - k][toei].X * Dest_To_FidPoints_Power[k - g][toei].Y * Dest_To_FidPoints_Power[g][toei].Z;
        ...
        if (InterpolationType == Interpolation2DSurfaceSpline)
            interpolate = Interpolate2DSurfaceSpline;
        else if (InterpolationType == InterpolationSphericalSpline)
            interpolate = InterpolateSphericalSpline;
        else if (InterpolationType == InterpolationSphericalCurrentDensitySpline)
            interpolate = InterpolateSphericalCurrentDensitySpline;
        else if (InterpolationType == Interpolation3DSpline && SplineDegree == 2)
            interpolate = Interpolate3DSpline2; // optimized version
        else if (InterpolationType == Interpolation3DSpline && SplineDegree > 2)
            interpolate = Interpolate3DSpline; // works for all degrees, including 2
            ...
            for (int tf = 0; tf < maxtf; tf++)
        {
            ...
            { // faster by bypassing fromgoodsel
            ...
            for (int toei = 0; toei < maxelto; toei++)
            {
                ...
                if (dosolve)
                    dataout(tf, toei) = interpolate(toei, X); // do the actual interpolation now
                else
                    dataout(tf, toei) = datain(tf, desttofromindex); // plain copy of original data: faster and will not introduce rounding errors
            } // for electrode
        } // for tf
