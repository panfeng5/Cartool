...
namespace crtl
{
    ...
    void TFilterButterworthHighPass<TypeD>::Apply(TypeD *data, int numpts)
    {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // init W's to 0
        auto ResetWs = [&](int order2)
        { for ( int k = 0; k < order2; k++ )  w2[ k ] = w1[ k ] = 0; };
    ...
    void TFilterButterworthBandPass<TypeD>::Set(int order, FilterCausality causal, double fsamp, double freq1, double freq2)
    {
        ...
        int order4 = Order / 4;
        // center of pass band
        double center = cos(Pi * (FrequencyCutHigh + FrequencyCutLow) / SamplingFrequency) / cos(Pi * (FrequencyCutHigh - FrequencyCutLow) / SamplingFrequency);
    ...
    void TFilterButterworthBandPass<TypeD>::Apply(TypeD *data, int numpts)
    {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // init W's to 0
        auto ResetWs = [&](int order4)
        { for ( int k = 0; k < order4; k++ )  w4[ k ] = w3[ k ] = w2[ k ] = w1[ k ] = 0; };
