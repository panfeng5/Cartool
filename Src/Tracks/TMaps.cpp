...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    {
        if (maps1.IsNotAllocated() || maps2.IsNotAllocated() || maps1.GetDimension() != maps2.GetDimension())
        {
    ...
    void TMaps::CentroidsToLabeling(const TMaps &data,
                                    ...
                                    PolarityType polarity, double limitcorr) const
    {
        // if ( IsNotAllocated () )
        //     return;
                         ...
                         TLabeling *labels, int l, const TMap *ref)
    {
        ...
        if (NumMaps == 0)
            ...
            return *allmaps[0]; // return single existing map
        ...
        { // NumMaps >= 2
          ...
          // Some functions will may need to estimate the best referential maps in case of evaluating polarities
            ...
            case MeanCentroid:
                return ComputeMeanCentroid(allmaps, datatype, polarity, labels, l);
            case WeightedMeanCentroid:
                return ComputeWeightedMeanCentroid(allmaps, datatype, polarity, labels, l, ref);
            case MedianCentroid:
                return ComputeMedianCentroid(allmaps, datatype, polarity, labels, l);
            case MedoidCentroid:
                return ComputeMedoidCentroid(allmaps, datatype, polarity, maxsamples, labels, l);
            case EigenVectorCentroid:
                return ComputeEigenvectorCentroid(allmaps, datatype, polarity, maxsamples, labels, l);
            case MaxCentroid:
                return ComputeMaxCentroid(allmaps, datatype, polarity, labels, l);
            default:
                return map; // null map
            }
        }
    ...
    TMap TMaps::ComputeCentroid(CentroidType centroid,
                                AtomType datatype, PolarityType polarity,
                                int maxsamples,
                                TLabeling *labels, int l, const TMap *ref) const
    {
        TArray1<TMap *> allmaps;
        ...
        TLabeling *labels, int l)
    {
        ...
        for (int mi = 0; mi < NumMaps; mi++)
        {
            ...
            { // no polarity evaluation, do a direct max
    ...
    */
                              ...
                              TLabeling *labels, int l)
    {
        ...
        for (int spi = 0; spi < DimensionSP; spi++)
        {
            ...
            if (fabs(v.X) > fabs(v.Y) && fabs(v.X) > fabs(v.Z) && v.IsOppositeDirection(TVector3Float(1, 0, 0)) || fabs(v.Y) > fabs(v.X) && fabs(v.Y) > fabs(v.Z) && v.IsOppositeDirection(TVector3Float(0, 1, 0)) || fabs(v.Z) > fabs(v.X) && fabs(v.Z) > fabs(v.Y) && v.IsOppositeDirection(TVector3Float(0, 0, 1)))
...
#define EigenvaluesConvergence 1e-6
        ...
        bool updatepolarity)
    {
        ...
        if (ranking)
            ToRank(AtomTypeScalar, RankingOptions(RankingAccountNulls | RankingMergeIdenticals));
                                 ...
                                 LabelType &index1, LabelType &index2)
    {
        ...
        OmpParallelFor
            ...
            for (int nc1 = 0; nc1 < nclusters; nc1++) for (int nc2 = nc1 + 1; nc2 < nclusters; nc2++)
        {
    ...
    void TMaps::FilterTime(FilterTypes filtertype, FctParams &params, bool showprogress)
    {
        ...
        if (filtertype == FilterTypeLowPass)
            filter = new TFilterButterworthLowPass<double>(params[FilterParamOrder], FilterNonCausal, SamplingFrequency, params[FilterParamFreqCut]);
        else if (filtertype == FilterTypeHighPass)
            filter = new TFilterButterworthHighPass<double>(params[FilterParamOrder], FilterNonCausal, SamplingFrequency, params[FilterParamFreqCut]);
        else if (filtertype == FilterTypeBandPass)
            filter = new TFilterButterworthBandPass<double>(params[FilterParamOrder], FilterNonCausal, SamplingFrequency, params[FilterParamFreqCutMin], params[FilterParamFreqCutMax]);
        else if (filtertype == FilterTypeBandStop)
            filter = new TFilterButterworthBandStop<double>(params[FilterParamOrder], FilterNonCausal, SamplingFrequency, params[FilterParamFreqCutMin], params[FilterParamFreqCutMax]);
        else if (filtertype == FilterTypeEnvelopeSlidingWindow)
            filter = new TFilterEnvelope<double>(FilterTypeEnvelopeSlidingWindow, SamplingFrequency, params[FilterParamEnvelopeWidth]);
        else if (filtertype == FilterTypeEnvelopeGapBridging)
            filter = new TFilterEnvelope<double>(FilterTypeEnvelopeGapBridging, SamplingFrequency, params[FilterParamEnvelopeWidth]);
        else if (filtertype == FilterTypeEnvelopeAnalytic)
            filter = new TFilterEnvelope<double>(FilterTypeEnvelopeAnalytic, SamplingFrequency, params[FilterParamEnvelopeWidth]);
        ...
        for (int i = 0; i < GetDimension(); i++)
        {
            ...
            if (filter)
                filter->Apply(line.GetArray(), line.GetDim());
            else
                                         ...
                                         TMaps &ESI) const
    {
        ...
        for (int nc = 0; nc < NumMaps; nc++)
            ...
            if (vectorial)
                ISDoc->MultiplyMatrix(regularization, Maps[nc], *(TArray1<TVector3Float> *)&ESI[nc]);
            else
                ISDoc->MultiplyMatrix(regularization, Maps[nc], ESI[nc]);
    ...
    //----------------------------------------------------------------------------
    bool TMaps::Covariance3DVectorial(AMatrix33 &Cov, const TSelection *tfok) const
    {
        if (IsNotAllocated())
            return false;
        ...
        for (int e = 0; e < dimension; e++)
        {
            ...
            if (scanmaxes)
            {
                ...
                for (int nc = 1; nc < NumMaps - 1; nc++)
                {
                    ...
                    if (e0 > 0 && e0 > ep && e0 > en || e0 < 0 && e0 < ep && e0 < en)
    ...
    void TMaps::ApplyZScoreSigned(ZScoreType how, const TArray2<float> &zscorevalues)
    {
        ...
        OmpParallelFor
            ...
            for (int nc = 0; nc < NumMaps; nc++) for (int e = 0; e < dimension; e++)
        {
    ...
    void TMaps::ApplyZScorePositive(ZScoreType how, const TArray2<float> &zscorevalues)
    {
        ...
        OmpParallelFor
            ...
            for (int nc = 0; nc < NumMaps; nc++) for (int e = 0; e < dimensionsp; e++)
        {
    ...
    void TMaps::ComputeZScoreVectorial(ZScoreType how, TArray2<float> &zscorevalues) const
    {
        ...
        for (int e1 = 0, e3 = 0; e1 < dimensionsp; e1++, e3 += 3)
        {
            ...
            for (int nc = 0; nc < NumMaps; nc++)
            {
                ...
                norm = sqrt(Square(Maps[nc][e3]) + Square(Maps[nc][e3 + 1]) + Square(Maps[nc][e3 + 2]));
    ...
    void TMaps::ApplyZScoreVectorial(ZScoreType how, const TArray2<float> &zscorevalues)
    {
        ...
        OmpParallelFor
            ...
            for (int nc = 0; nc < NumMaps; nc++) for (int e1 = 0, e3 = 0; e1 < dimensionsp; e1++, e3 += 3)
        {
            ...
            // recover & Deskew the norms of the vectors
            double norm = sqrt(Square(Maps[nc][e3]) + Square(Maps[nc][e3 + 1]) + Square(Maps[nc][e3 + 2]));
    ...
    void TMaps::ComputeZScore(ZScoreType how, TArray2<float> &zscorevalues) const
    {
        ...
        if (IsZScoreVectorial(how))
            ComputeZScoreVectorial(how, zscorevalues);
        else if (IsZScorePositive(how))
            ComputeZScorePositive(how, zscorevalues);
        else if (IsZScoreSigned(how))
            ComputeZScoreSigned(how, zscorevalues);
    ...
    void TMaps::ApplyZScore(ZScoreType how, const TArray2<float> &zscorevalues)
    {
        ...
        if (IsZScoreVectorial(how))
            ApplyZScoreVectorial(how, zscorevalues);
        else if (IsZScorePositive(how))
            ApplyZScorePositive(how, zscorevalues);
        else if (IsZScoreSigned(how))
            ApplyZScoreSigned(how, zscorevalues);
    }
    ...
    void TMaps::ZPositiveToZSigned()
    {
        ...
        OmpParallelFor
            ...
            for (int nc = 0; nc < NumMaps; nc++) for (int dim = 0; dim < Dimension; dim++)
    ...
    void TMaps::WriteFileReordered(const char *filename,
                                   ...
                                   int dimsize, bool updatingcall) const
    ...
    void TMaps::WriteFile(const char *filename,
                          ...
                          const TStrings *tracksnames) const
