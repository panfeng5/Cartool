...
namespace crtl
{
                              ...
                              bool openresults, bool showgauge)
    {
        ...
        if (nonrobust)
        {
            ...
            if (sdfile)
                sum2.Resize(numtracks, numtf);
            if (snrfile)
                snr.Resize(numtracks, numtf);
        }
        ...
        for (int eegi = 0; eegi < (int)gof; eegi++)
        {
            ...
            if (robust)
                ...
                OmpParallelFor
                    ...
                    for (int e = 0; e < numtracks; e++) for (int tf = 0; tf < numtf; tf++)
            ...
            if (sdfile)
            {
                ...
                OmpParallelFor
                    ...
                    for (int i = 0; i < lineardim; i++)
                        ...
                        sum2.GetValue(i) = sqrt(fabsl((sum2.GetValue(i) - Square(sum.GetValue(i)) / numfiles) / NonNull(numfiles - 1)));
            }
        ...
        for (int eegi = 0; eegi < numfiles; eegi++)
        {
            ...
            for (int e = 0; e < numtracks; e++)
                for (int tf = 0; tf < numtf; tf++)
                {
                    ...
                    if (critselmeancrit[e])
                        errdat(ranki, tf)++;

                    // for the average meta-criterion -
                    if (critselmetacrit[e])
                        errdat(metacriti, tf)++;
                }
                                 ...
                                 bool openresults, bool showgauge)
    {
        ...
        // for circular stats
        if (sphstats)
            sumn.Resize(risbuff.GetNumMaps(), risbuff.GetDimension());
        ...
        if (dosphsd)
        {
            ...
            OmpParallelFor
                ...
                for (int tf = 0; tf < numtf; tf++) for (int spi = 0; spi < numtracks; spi++)
            {
                            ...
                            bool openresults, bool showgauge)
    {
        ...
        if (poleval)
        {
            ...
            OmpParallelFor
                ...
                for (int fi = 0; fi < numfreqs; fi++) for (int tfi = 1; tfi < numtf; tfi++)
            ...
            OmpParallelFor
                ...
                for (int fi = 0; fi < numfreqs; fi++) for (int tfi = 0; tfi < numtf; tfi++) for (int ei = 0; ei < numtracks; ei++)
            {

                sum(fi, ei, tfi) = summaps(fi, tfi, ei);

                if (sdfile)
                    sum2(fi, ei, tfi) = Square(summaps(fi, tfi, ei));
            }
