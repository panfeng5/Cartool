...
#define maxconst 1000
    ...
    *toooo << StreamFormatFloat32 << ( orgx + cstl[ eli ].real () ) * maxw ) << "\t"
                           ...
                           bool silent)
    {
        ...
        if (StringIsEmpty(buff) || StringIs(buff, "*") // bypassing the '*' of TSelection
        ...
        bool nofreqavg = outputatomtype == OutputAtomComplex                                           // don't average complex values
                         ...
                         || outputbands == OutputLinearInterval || analysis == FreqAnalysisSTransform; // never for S-Transform, smooth enough all the time(?)
        ...
        else if (outputbands == OutputLogInterval)
        {
            ...
            for (int fbi = 0; fbi < numfreqbands; fbi++, fb++)
            {
                ...
                if (fbi > 0 && fb->SaveFreqMin_i == (fb - 1)->SaveFreqMin_i && fb->SaveFreqMax_i == (fb - 1)->SaveFreqMax_i)
                {
        ...
        else if (outputbands == OutputLinearInterval)
        {
            ...
            // test limits
            if (fb->SaveFreqMax_i < 0)
                fb->SaveFreqMax_i = 0;
            else if (fb->SaveFreqMax_i > limitfreqmax_i - fb->SaveFreqStep_i + 1)
                fb->SaveFreqMax_i = fb->SaveFreqMin_i + TruncateTo(limitfreqmax_i - fb->SaveFreqStep_i + 1 - fb->SaveFreqMin_i, fb->SaveFreqStep_i);
        ...
        for_each(freqbands.cbegin(), freqbands.cend(),
                 [&numsavedfreqs, &maxfftafreqs](const TOneFrequencyBand &fb)
                 { numsavedfreqs      += fb.SaveNumFreqs; 
        ...
        if (optimaldownsampling && analysis == FreqAnalysisSTransform
        ...
        else
        {
            ...
            StringCopy(BaseDir, buff, ".", analysis == FreqAnalysisFFT ? InfixFft : analysis == FreqAnalysisPowerMaps      ? InfixPowerMaps
                                                                                : analysis == FreqAnalysisFFTApproximation ? InfixFftApprox
                                                                                : analysis == FreqAnalysisSTransform       ? InfixSTransform
                                                                                                                           : "Other");

            if (outputbands == OutputLinearInterval || outputbands == OutputLogInterval)
            ...
            if (analysis != FreqAnalysisSTransform)
                if (outputsequential)
                    StringAppend(BaseDir, "." InfixSeq);
                else
                    StringAppend(BaseDir, "." InfixAvg);
        }
        ...
        if (createsubdir)
        {
            ...
            if (splitspectrum)
                CreatePath(BaseDirSpectrum, false);
            if (splitspectrum && savefftapprox)
                CreatePath(BaseDirApprox, false);
            if (splitfrequency)
                CreatePath(BaseDirSplitFreqs, false);
            if (splitelectrode)
                CreatePath(BaseDirSplitElecs, false);
        }
        ...
        if (savefreq)
            StringCopy(fileoutfreq, BaseFileName, FILEEXT_FREQ);
        if (savefreq)
            StringCopy(fileoutmrk, fileoutfreq, "." FILEEXT_MRK);
        if (splitelectrode)
            StringCopy(fileoutsplitelec, BaseFileNameSplitElecs, "*.", FILEEXT_EEGSEF /*FILEEXT_EEGEP*/);
        if (splitfrequency)
            StringCopy(fileoutsplitfreq, BaseFileNameSplitFreqs, "*.", FILEEXT_EEGSEF /*FILEEXT_EEGEP*/);
        if (splitspectrum)
            StringCopy(fileoutspectrum, BaseFileNameSpectrum, outputsequential ? InfixWindow " *." : "", InfixSpectrum ".", FILEEXT_EEGSEF /*FILEEXT_EEGEP*/);
        if (savefftapprox)
            StringCopy(fileoutapprfreqs, BaseFileNameApprox, outputsequential ? InfixWindow " *." : "", InfixApproxEeg ".", FILEEXT_EEGSEF /*FILEEXT_EEGEP*/);
        ...
        verbose.NextTopic("Frequencies:");
        {
            ...
            if (outputbands == OutputLinearInterval)
                verbose.Put("Saving:", "Interval");
            else if (outputbands == OutputLogInterval)
                verbose.Put("Saving:", "Log Interval");
            else if (outputbands == OutputBands)
                verbose.Put("Saving:", "Frequency Bands");
            else
                verbose.Put("Saving:", "Unknown");
            ...
            else if (outputbands == OutputBands || outputbands == OutputLogInterval)
            {
        ...
        {
            if (analysis == FreqAnalysisFFT)
                verbose.Put("Type of frequency analysis:", "FFT");
            else if (analysis == FreqAnalysisPowerMaps)
                verbose.Put("Type of frequency analysis:", "FFT (Power Maps)");
            else if (analysis == FreqAnalysisFFTApproximation)
                verbose.Put("Type of frequency analysis:", "FFT Approximation");
            else if (analysis == FreqAnalysisSTransform)
                verbose.Put("Type of frequency analysis:", "Wavelet (S-Transform)");
            else
                verbose.Put("Type of frequency analysis:", "Unknown");
        ...
        {
            if (ref == ReferenceAsInFile)
                verbose.Put("Reference:", "No reference, as in file");
            else if (ref == ReferenceAverage)
                verbose.Put("Reference:", "Average reference");
            else if (ref == ReferenceArbitraryTracks)
                verbose.Put("Reference:", reflist);
            else if (ref == ReferenceUsingCurrent)
            {

                if (eegdoc->GetReferenceType() == ReferenceAverage)
                    verbose.Put("Current reference:", "Average reference");
                else if (eegdoc->GetReferenceType() == ReferenceAsInFile)
                    verbose.Put("Current reference:", "No reference, as in file");
                else
                {
        ...
        if (analysis == FreqAnalysisFFTApproximation)
            StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqFFTApproximation]);

        else if (analysis == FreqAnalysisSTransform)
        {

            if (outputatomtype == OutputAtomNorm)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqSTransformNorm]);
            else if (outputatomtype == OutputAtomNorm2)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqSTransformNorm2]);
            else if (outputatomtype == OutputAtomComplex)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqSTransformComplex]);
            else if (outputatomtype == OutputAtomPhase)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqSTransformPhase]);
        }

        else if (analysis == FreqAnalysisPowerMaps || analysis == FreqAnalysisFFT)
        {

            if (outputatomtype == OutputAtomNorm)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqFFTNorm]);
            else if (outputatomtype == OutputAtomNorm2)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqFFTNorm2]);
            else if (outputatomtype == OutputAtomComplex)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqFFTComplex]);
            else if (outputatomtype == OutputAtomPhase)
                StringCopy(expfile.FreqTypeName, FrequencyAnalysisNames[FreqFFTPhase]);
        }
        ...
        else if (outputbands == OutputBands || outputbands == OutputLogInterval)
        {
            ...
            for (int fbi = 0; fbi < numfreqbands; fbi++, fb++)
            {
                ...
                if (Fraction(RoundTo(fb->SaveFreqMin_hz, FrequencyResolution)) != 0)
                    nofractions = false;
                if (Fraction(RoundTo(fb->SaveFreqMax_hz, FrequencyResolution)) != 0)
                    nofractions = false;
        ...
        { // FFT, Power Maps (also FFT), FFT Approximation
        ...
        for (int blocki0 = 0, firsttf = timemin; blocki0 < numblocks; blocki0++, firsttf += blockstep)
        {
            ...
            OmpParallelBegin
                ...
                TVector<AReal>
                    X; // current block of input, real data
                ...
                for (int eli = 0; eli < numelsave; eli++)
            {
                ...
                if (windowing == FreqWindowingHanning)
                    ...
                    for (int i = 0; i < blocksize; i++)
                        X(i) = blockeeg(el, i) * windowingtable(i);
                    ...
                    for (int fbi = 0, savedfi0 = 0; fbi < numfreqbands; fbi++, fb++)
                    {
                        ...
                        for (int fi = fb->SaveFreqMin_i; fi <= fb->SaveFreqMax_i; fi += fb->SaveFreqStep_i, savedfi0++)
                        {
                            ...
                            for (int downf = 0, fi2 = fi; downf < fb->AvgNumFreqs; downf++, fi2 += fb->AvgFreqStep_i)
                            {
                                ...
                                if (fi2 == 0)
                                { // 0 Hz - manually computing the mean of signal
                                    ...
                                    if (outputatomtype == OutputAtomComplex)
                                    {
                                        SumST = (AComplex)mean;
                                    }
                                    ...
                                    {
                                        SumST = (AComplex)0;
                                    } // no imaginary -> phase is 0
                                    else if (outputatomtype == OutputAtomNorm)
                                    {
                                        SumST = (AComplex)abs(mean);
                                    }
                                    else if (outputatomtype == OutputAtomNorm2)
                                    {
                                        SumST = (AComplex)Square(mean);
                                    }
                                    else if (outputatomtype == OutputAtomReal)
                                    {
                                        SumST = (AComplex)mean;
                                    } // shouldn't occur
                                }
                                ...
                                { // freqs > 0 [Hz]
                                    ...
                                    if (windowing == FreqWindowingHanningBorder && outputatomtype != OutputAtomPhase)
                                    {
...
#define numcyclesinhanning 2
                                    ...
                                    for (int i = 0; i < blocksize; i++)
                                    {
                                        ...
                                        {
                                            SumST(i) += ST(i).real();
                                        } // saving ST ( i ) is the original data + filter; abs ( ST ( i ) ) is the envelope
                                    } // for i
                            ...
                            for (int i = 0, i0 = 0; i < blocksize; i += timedown, i0++) // do all TFs
                                ...
                                if (datatypeout == AtomTypeComplex)
                                {
                                    results(i0, eli, 2 * savedfi0) = SumST(i).real();
                                    results(i0, eli, 2 * savedfi0 + 1) = SumST(i).imag();
                                }
                                else
                                    results(i0, eli, savedfi0) = SumST(i).real();
                ...
                { // all other FFT analysis
                    ...
                    for (int fbi = 0, savedfi0 = 0; fbi < numfreqbands; fbi++, fb++)
                    {
                        ...
                        for (int fi = fb->SaveFreqMin_i; fi <= fb->SaveFreqMax_i; fi += fb->SaveFreqStep_i, savedfi0++)
                        {
                            ...
                            if (outputatomtype == OutputAtomComplex)
                            {
                                blockfft(savedfi0, eli) = F(fi);
                            } // OK when dealing with norm of complex, but not norm^2
                            else if (outputatomtype == OutputAtomPhase)
                            {
                                blockfft(savedfi0, eli) = ArcTangent(F(fi));
                            }
                            else if (outputatomtype == OutputAtomNorm)
                            {
                                blockfft(savedfi0, eli) = abs(F(fi));
                            }
                            else if (outputatomtype == OutputAtomNorm2)
                            {
                                blockfft(savedfi0, eli) = norm(F(fi));
                            }
                            else if (outputatomtype == OutputAtomReal)
                            {
                                blockfft(savedfi0, eli) = abs(F(fi));
                            } // this case shouldn't happen, but let's feed it something anyway

                            // allow frequency averaging only if not complex - AvgNumFreqs must be set to 1
                            bool avgfreqs = fb->AvgNumFreqs > 1 && !nofreqavg;

                            if (avgfreqs)
                            { // sum following frequencies
                                ...
                                for (int downf = 1, fi2 = fi + fb->AvgFreqStep_i; downf < fb->AvgNumFreqs; downf++, fi2 += fb->AvgFreqStep_i)
                                {
                                    ...
                                    if (outputatomtype == OutputAtomNorm)
                                    {
                                        blockfft(savedfi0, eli) += abs(F(fi2));
                                    }
                                    else if (outputatomtype == OutputAtomNorm2)
                                    {
                                        blockfft(savedfi0, eli) += norm(F(fi2));
                                    }
                                    else if (outputatomtype == OutputAtomReal)
                                    {
                                        blockfft(savedfi0, eli) += abs(F(fi2));
                    ...
                    for (int fi0 = 0; fi0 < numsavedfreqs; fi0++)
                    {
                        ...
                        { // write directly to file
                            if (datatypeout == AtomTypeComplex)
                            {
                                results(blocki0, eli, 2 * fi0) = blockfft(fi0, eli).real();
                                results(blocki0, eli, 2 * fi0 + 1) = blockfft(fi0, eli).imag();
                            }
                            else
                                results(blocki0, eli, fi0) = blockfft(fi0, eli).real();
                        } // if outputsequential
                        ...
                        { // sum up in buffer
                            if (datatypeout == AtomTypeComplex)
                            {
                                sumfft(fi0, eli) += blockfft(fi0, eli);
                            } // shouldn't happen, not meaningful to average complex numbers!
                            else
                            {
                                sumfft(fi0, eli) += blockfft(fi0, eli).real();
                            } // norm/norm2 has already been computed, and is stored in the Real component
                        } // if average freqs
                    ...
                    for (int fi0 = 0; fi0 < fb->SaveNumFreqs; fi0++, savedfi0++)
                    {
                        ...
                        for (int eli2 = 0; eli2 < numelsave; eli2++)
                        {
                            ...
                            { // write directly to file
                                ...
                                if (datatypeout == AtomTypeComplex)
                                {
                                    results(blocki0, eli2, 2 * savedfi0) = blockfft(fi0, eli2).real();
                                    results(blocki0, eli2, 2 * savedfi0 + 1) = blockfft(fi0, eli2).imag();
                                }
                                else
                                    results(blocki0, eli2, savedfi0) = blockfft(fi0, eli2).real();
                            } // if outputsequential

                            else
                            { // one final step is to do a temporal averaging
                              // again, be careful to polarity if time averaging FFTA
                                ...
                                if (datatypeout == AtomTypeComplex)
                                {
                                    sumfft(savedfi0, eli2) += blockfft(fi0, eli2);
                                } // shouldn't happen, not meaningful to average in C!
                                else
                                {
                                    sumfft(savedfi0, eli2) += blockfft(fi0, eli2).real();
                                } // norm, norm2 have already been computed, in the Real part
                            }
                        }
                    } // for freq
                } // for freqband
        ...
        { // writing temporal mean
            ...
            for (int eli = 0; eli < numelsave; eli++)
            {
                ...
                for (int fi0 = 0; fi0 < numsavedfreqs; fi0++)
                    ...
                    if (datatypeout == AtomTypeComplex)
                        expfile.Write(sumfft(fi0, eli), 0, eli, fi0);
                    else
                        expfile.Write(sumfft(fi0, eli).real(), 0, eli, fi0);
            } // for electrode
        }
        ...
        if (outputmarkers)
        {
            ...
            long mrktimemin = timemin + margin;
            // same on the other side
            long mrktimemax = ComputeTimeMax(isstransform, windowoverlapmax, mrktimemin, blocksize, blocksoverlap, numblocks) - margin;
