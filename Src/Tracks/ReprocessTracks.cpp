...
namespace crtl
{
    ...
    bool ReprocessTracks(
        ...
        bool silent)
    {
        if (EEGDoc == 0 || tracksoptions == ProcessUnknown || timeoptions == UnknownTimeProcessing || sequenceoptions == UnknownSequenceProcessing)
        ...
        else if (numaddedels > 0)
        {
            ...
            if (hasnulls && haspseudos // user explicitly specified nulls AND pseudos
                || hasnulls && !haspseudos)
            { // user specified (some) nulls, fair enough
            ...
            else if (hasregular && !hasnulls && haspseudos)
            {
                ...
                TSelection userselreg(usersel);
                userselreg.Reset(lastorig + 1, numtotalels - 1);
                TSelection userselpseudo(usersel);
                userselpseudo.Reset(firstorig, firstpseudo - 1);
            ...
            { // only some pseudos, no regular and no nulls - don't add the nulls!
                elsel = usersel;
            }
        ...
        bool downsample = downsampleratio > 1 && timeoptions == ExportTimeInterval && sequenceoptions == SequenceProcessing;
        ...
        if (baselinecorr)
        {
            ...
            if (baselinecorrpre < 0 && baselinecorrpost < 0 || baselinecorrpre > lasttimeframe && baselinecorrpost > lasttimeframe)
        ...
        bool dopseudos = haspseudos                                                                  // we could totally bypass this, as we are computing the pseudo-tracks locally
                         && tracksoptions == ProcessTracks && elsel.NumSet(firstpseudo, lastpseudo); // can we / do we need to handle the pseudos?

        // triggers only allowed to mimick the original file, i.e. time intervals or excluded markers
        bool outputtriggers = outputmarkers && timeoptions != ExportTimeTriggers && sequenceoptions == SequenceProcessing;
        ...
        bool samplingfreqout = filetype != PresetFileTypeEp && filetype != PresetFileTypeTxt;
        ...
        if (timeoptions == ExportTimeInterval)
        {
            ...
            if (downsample)
            { // force the time interval to be an exact multiple of the downsampling ratio
            ...
            if (sequenceoptions == SequenceProcessing && intimenum > 0)
            {
                ...
                if (intimemin > 0)
                    remaintags.ClipMarkers(0, intimemin - 1, AllMarkerTypes);
                if (intimemax < lasttimeframe)
                    remaintags.ClipMarkers(intimemax + 1, lasttimeframe, AllMarkerTypes);
        ...
        if (intimenum == 0)
        {
            ...
            if (!silent)
            {
                if (timeoptions == ExportTimeTriggers)
                    ShowMessage("No triggers were found that match your input list," NewLine "skipping current file..", EEGDoc->GetTitle(), ShowMessageWarning);
                else if (timeoptions == ExcludeTimeTriggers)
                    ShowMessage("Your list of triggers excluded the whole file," NewLine "skipping current file..", EEGDoc->GetTitle(), ShowMessageWarning);
                else if (timeoptions == ExportTimeInterval)
                    ShowMessage("The interval duration appears to be less than 1 TF," NewLine "skipping current file..", EEGDoc->GetTitle(), ShowMessageWarning);
            }
            ...
            if (!silent)
            {
                ...
                StringCopy(buff,
                           "Spatial Filter has been disabled due to dimensions mismatch:" NewLine
                               NewLine Tab "Spatial Filter Dimension = ",
                           IntegerToString(filters.GetSpatialFilterDim()), NewLine Tab "Actual Number of Tracks  = ", IntegerToString(numels), NewLine NewLine "Try using the  'Other Filters'  option with an appropriate XYZ file.");
        ...
        auto GetTracks = [&EEGDoc,
                          ...
                          &filters, &ref, &refsel, &validsel, &auxsel](long tf1, long tf2, long numtf, int tfoffset)
        {
            ...
            if (doanyfilter || IsEffectiveReference(ref))
            { // !filters semantic do not include the reference for the moment!
        ...
        if (rescalingoptions == GfpRescaling || createfile && IsExtension(filename, FILEEXT_EEGEDF))
        {
        ...
        for (int i = 0; i < timechunks.GetNumMarkers(); i++)
        {
            ...
            else if (/*sequenceoptions == SequenceProcessing &&*/ downsample)
            {
                ...
                for (int e = 0; e < numels; e++)
                    for (long tfi = 0; tfi < numtf; tfi++)
                        ...
                        if ((tfi % downsampleratio) == 0)
                            eegb[e][tfi / downsampleratio] = eegb(e, tfi); // set @ beginning of bucket
                        else
                            eegb[e][tfi / downsampleratio] += eegb(e, tfi); // cumulate the other values
            ...
            if (dopseudos)
            {
                ...
                for (long tfi = 0; tfi < numtf; tfi++)
                {
                    ...
                    if (elsel[gfpoff])
                        eegb(gfpoff, tfi) = map1.GlobalFieldPower(validsel, !IsAbsolute(datatype), IsVector(datatype));
                    if (elsel[disoff])
                        eegb(disoff, tfi) = map1.Dissimilarity(map2, validsel, !IsAbsolute(datatype), IsVector(datatype));
                    if (elsel[avgoff])
                        eegb(avgoff, tfi) = map1.Average(validsel);
                }
        ...
        if (closefile)
        {
            ...
            {
                TSelection insel(numels, OrderSorted);
                EEGDoc->SetRegular(insel);
            ...
            verbose.NextTopic("Processing Parameters:");
            {
                ...
                verbose.NextLine();
                if (filtersoptions == UsingCurrentFilters)
                    verbose.Put("Current filters:", filters.ParametersToText(buff));
                else if (filtersoptions == UsingOtherFilters)
                    verbose.Put("Other filters:", filters.ParametersToText(buff));
                else
                ...
                if (spatialfiltererror)
                    verbose.Put("", "Spatial filter has been disabled due to dimensions mismatch");

                verbose.NextLine();
                if (datatype == AtomTypePositive || ref == ReferenceAsInFile)
                    verbose.Put("Reference:", "No reference, as in file");
                else if (ref == ReferenceAverage)
                    verbose.Put("Reference:", "Average reference");
                else if (ref == ReferenceArbitraryTracks)
                    verbose.Put("Reference:", refsel.ToText(buff, &ElectrodesNames, auxsandnulls.c_str()));
                else if (ref == ReferenceUsingCurrent)
                {
                    ...
                    if (EEGDoc->GetReferenceType() == ReferenceAverage)
                        ...
                        verbose.Put("Current reference:", EEGDoc->GetReferenceTracks().ToText(buff, &ElectrodesNames, auxsandnulls.c_str()));
                }
                else
