...
namespace crtl
{
    ...
    int TTracksFilters<TypeD>::GetSafeMargin() const
    {
        ...
        if (HasButterworthHigh())
            ...
            marginhigh = Round(FrequencyToTimeFrame(ButterworthHigh, SamplingFrequency) * NoMore((const int)10, OrderHigh) // Higher orders need more margin, but we cap it up to 10 as there doesn't seem any real gain above, while computation is really getting worse
        ...
        if (HasButterworthLow())
            ...
            marginlow = AtLeast(EegMinLowFilterSide, Round(FrequencyToTimeFrame(ButterworthLow, SamplingFrequency) * OrderLow // Higher orders need more margin
                                                           ));
        ...
        if (HasNotches())
        {
            ...
            { // using fundamental frequency is enough

                mn = Round(FrequencyToTimeFrame(Notches[n] - TFilterNotchWidth / 2, SamplingFrequency) * TFilterNotchOrder * NoMore(6, FilterNotches.GetNumFilters()) // we can save some computation when no harmonics are being requested
                );
    ...
    void TTracksFilters<TypeD>::SetFilters(double fsamp, bool silent)
    {
        ...
        if (HasThresholdKeepAbove() && HasThresholdKeepBelow())
        {
            ...
            if (ThresholdKeepAboveValue > ThresholdKeepBelowValue)
                FilterThreshold.Set(FilterClipAboveAndBelow, ThresholdKeepAboveValue, ThresholdKeepBelowValue);

            else
                FilterThreshold.Set(FilterClipAboveOrBelow, ThresholdKeepAboveValue, ThresholdKeepBelowValue);
        }
        else if (HasThresholdKeepAbove() && !HasThresholdKeepBelow())
            FilterThreshold.Set(FilterClipBelow, ThresholdKeepAboveValue);

        else if (!HasThresholdKeepAbove() && HasThresholdKeepBelow())
            FilterThreshold.Set(FilterClipAbove, ThresholdKeepBelowValue);
        ...
        if (HasButterworthBand())
            FilterButterworthBandPass.Set(GetOrder(), Causal, SamplingFrequency, ButterworthLow, ButterworthHigh);
        else if (HasButterworthHigh())
            FilterButterworthHighPass.Set(GetOrder(), Causal, SamplingFrequency, ButterworthHigh);
        else if (HasButterworthLow())
            FilterButterworthLowPass.Set(GetOrder(), Causal, SamplingFrequency, ButterworthLow);
        ...
        if (freqadjusted && !silent)
        {
            ...
            if (ButterworthLow != ButterworthLowRequested && ButterworthHigh != ButterworthHighRequested)
                StringCopy(buff, "Frequencies have been clipped to  ", FloatToString(FreqMin, 3), " .. ", FloatToString(FreqMax, 3), " [Hz]");
            else if (ButterworthLow != ButterworthLowRequested)
                StringCopy(buff, "Low pass frequency has been clipped to  ", FloatToString(FreqMax, 3), " [Hz]");
            else if (ButterworthHigh != ButterworthHighRequested)
                StringCopy(buff, "High pass frequency has been clipped to  ", FloatToString(FreqMin, 3), " [Hz]");
                                             ...
                                             FilterPrecedence filterprecedence)
    {
            ...
            if (IsFlag(filterprecedence, FilterTemporal) && (HasBaseline() || HasButterworthBand() || HasButterworthHigh() || HasButterworthLow() || HasNotches()))
        {
                ...
                for (int el = 0; el < numel; el++)
            {
                ...
                if (HasBaseline())
                    FilterBaseline.Apply(data[el] + tfoffset, numtf);

                // force Band Pass
                if (HasButterworthBand())
                    FilterButterworthBandPass.Apply(data[el] + tfoffset, numtf);

                else if (HasButterworthHigh())
                    FilterButterworthHighPass.Apply(data[el] + tfoffset, numtf);

                else if (HasButterworthLow())
                    FilterButterworthLowPass.Apply(data[el] + tfoffset, numtf);

                if (HasNotches())
                    FilterNotches.Apply(data[el] + tfoffset, numtf);
            } // for el
        ...
        if (IsFlag(filterprecedence, FilterNonTemporal) && (HasSpatialFilter() || HasRanking()))
        {
                ...
                for (long tf0 = 0; tf0 < numtf; tf0++)
            {
                ...
                if (HasSpatialFilter())
                    FilterSpatial.Apply(temp); // could also make use of auxtracks?

                if (HasRanking())
                    FilterRanking.Apply(temp, RankingOptions(RankingAccountNulls | RankingMergeIdenticals)); // ignoring null values or not? or ignoring only for positive data?
        ...
        if (HasReference(reference))
        {
                ...
                for (long tf0 = 0; tf0 < numtf; tf0++)
            {
                ...
                if (HasReference(reference))
                    FilterReference.Apply(temp, reference, referencetracks, validtracks, auxtracks);
        ...
        if (IsFlag(filterprecedence, FilterPrecedence(FilterTemporal | FilterNonTemporal)) && (HasRectification() || HasEnvelope() || HasThresholdFilter()))
        {
                ...
                for (int el = 0; el < numel; el++)
            {
                ...
                if (IsFlag(filterprecedence, FilterNonTemporal) && HasRectification())
                    FilterRectification.Apply(data[el] + tfoffset, numtf);

                // AFTER Rectification
                if (IsFlag(filterprecedence, FilterTemporal) && HasEnvelope())
                    FilterEnvelope.Apply(data[el] + tfoffset, numtf);

                // !We call with a vector of temporal data, but it does not matter for this operator!
                if (IsFlag(filterprecedence, FilterNonTemporal) && HasThresholdFilter())
                    FilterThreshold.Apply(data[el] + tfoffset, numtf);
            } // for el
