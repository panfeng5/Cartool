...
namespace crtl
{
            ...
            */
            ...
            do
            {
                ...
                if (!StringIsSpace(rw))
                {
                    ...
                    os << ((StringContains((const char *)rw, "REJECT") || StringContains((const char *)rw, "WRONG")) ? "0"
                                                                                                                     : "1")
                       << Tab "0"
                       << Tab << code
                       << fastendl;
            ...
            } while (StringIsSpace(rw) || StringContains((const char *)rw, "Epoch") || StringContains((const char *)rw, "Restarting"));
    ...
    */
    ...
    void MergeTracksToFreqFiles(const TGoF &gof, FrequencyAnalysisType freqtype, char *returnfreqfile, bool showgauge)
    {
        ...
        // skip the "Hz"
        if (StringStartsWith(commonend, " Hz."))
            StringCopy(commonend, commonend + 4);
        else if (StringStartsWith(commonend, "Hz."))
            StringCopy(commonend, commonend + 3);
        else if (StringStartsWith(commonend, " Hz"))
            StringCopy(commonend, commonend + 3);
        else if (StringStartsWith(commonend, "Hz"))
            StringCopy(commonend, commonend + 2);
        ...
        if (nzeroes > 1)
        {
            ...
            if (!StringEndsWith(OutFileName, "."))
                StringAppend(OutFileName, ".");

            StringAppend(OutFileName, "Merged");

            if (!StringStartsWith(commonend, "."))
                StringAppend(OutFileName, ".");
        ...
        if (StringEndsWith(commonend, FILEEXT_EEGEPSD))
            PostfixFilename(OutFileName, "." /*FILEEXT_EEGEPSD*/ InfixSD);
        else if (StringEndsWith(commonend, FILEEXT_EEGEPSE))
            PostfixFilename(OutFileName, "." /*FILEEXT_EEGEPSE*/ InfixSE);
        else if (StringEndsWith(commonend, FILEEXT_RIS))
            PostfixFilename(OutFileName, "." FILEEXT_RIS);
...

    {
        ...
        { // Otherwise, create our own selection
            spsel = TSelection(numsp, OrderSorted);
            // Caller might have specified an optional list..
            if (StringIsNotEmpty(splist))
                spsel.Set(splist, risdoc->GetElectrodesNames());
            // ..or not
            else
                spsel.Set();
        }
        ...
        for (TIteratorSelectedForward spi(spsel); (bool)spi; ++spi)
        {
            ...
            if (Gauge.IsAlive())
                Gauge.Next(2, SuperGaugeUpdateTitle);
            else
                UpdateApplication;
            ...
            if (spontaneous)
            { // To emphasize the cloud main direction, we can duplicate each vector on its opposite side
