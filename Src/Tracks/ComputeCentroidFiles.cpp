...
namespace crtl
{
                        ...
                        bool normalize)
    {
        ...
        if (ranking)
            data.ToRank(datatype, RankingOptions(RankingAccountNulls | RankingCountIdenticals), -1);
        // handling both vectorial and scalar data
        if (thresholding)
            data.Thresholding(threshold, datatype);

        if (reference != ReferenceNone)
            data.SetReference(reference, datatype);

        if (normalize)
            data.Normalize(datatype);
    };
                              ...
                              bool showprogress)
    {
        ...
        for (int i = 0; i < (int)gof; i++)
        {
            ...
            if (layout == AllFilesOneCentroid)
                mapsin.ReadFiles(gof, datatype, ReferenceNone);      // reading & concatenating all files at once
            else                                                     /*OneFileOneCentroid*/
                mapsin.ReadFile(gof[i], 0, datatype, ReferenceNone); // reading a single file
            ...
            if (spatialfilter != SpatialFilterNone)
                mapsin.FilterSpatial(SpatialFilterDefault, xyzfile);
                              ...
                              bool showprogress)
    {
        ...
        if (StringIsNotEmpty(alldiffs) && StringLength(filename) + StringLength(alldiffs) + 4 <= NoMore(WindowsMaxComponentLength, MaxPathShort))
            // we're good here
