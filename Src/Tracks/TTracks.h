...
namespace crtl
{
    ...
    void TTracks<TypeD>::Filter(FilterTypes filtertype, FctParams &params, bool showprogress)
    {
        ...
        else if (filtertype == FilterTypeMin || filtertype == FilterTypeMax || filtertype == FilterTypeMinMax || filtertype == FilterTypeRange || filtertype == FilterTypeMedian
        ...
        for (int xk = 0; xk < Kf.GetDim1(); xk++)
            for (int yk = 0; yk < Kf.GetDim2(); yk++)
            {
                ...
                if (filtertype == FilterTypeGaussian)
                    ...
                    k = (kp.X || kp.Y ? -1 : 4) * gauss(kp.Norm()); // very rough, only for 3x3x3
        ...
        for (int x = 0; x < Dim1; x++)
        {
            ...
            for (int y = 0; y < Dim2; y++)
            {
                ...
                // complex compatible formula:
                    ...
                    v = fabs(GetValue(x, y) - stat.Min()) < fabs(GetValue(x, y) - stat.Max()) ? stat.Min() : stat.Max();
        ...
        do
        {
            ...
            for (int x = xi1; x <= xi2; x++)
                for (int y = yi1; y <= yi2; y++)
                {
                    ...
                    if (y > 0 && (v0n = GetValue(x, y - 1)) >= maxthresh && v0n <= v00 /* * 1.05 */)
                    {
                        GetValue(x, y - 1) = -v0n;
                        if (y - 1 < ybox1)
                            ybox1 = y - 1;
                    ...
                    if (y < Dim2 - 1 && (v0p = GetValue(x, y + 1)) >= maxthresh && v0p <= v00 /* * 1.05 */)
                    {
                        GetValue(x, y + 1) = -v0p;
                        if (y + 1 > ybox2)
                            ybox2 = y + 1;
                    ...
                    if (x > 0 && (vn0 = GetValue(x - 1, y)) >= maxthresh && vn0 <= v00 /* * 1.05 */)
                    {
                        GetValue(x - 1, y) = -vn0;
                        if (x - 1 < xbox1)
                            xbox1 = x - 1;
                    ...
                    if (x < Dim1 - 1 && (vp0 = GetValue(x + 1, y)) >= maxthresh && vp0 <= v00 /* * 1.05 */)
                    {
                        GetValue(x + 1, y) = -vp0;
                        if (x + 1 > xbox2)
                            xbox2 = x + 1;
