...
namespace crtl
{
                    ...
                    TSuperGauge *gauge)
    {
        ...
        */
        ...
        if (savematrix)
        {
            ...
            if (StringIsNotEmpty(prefix))
                PrefixFilename(MatrixFileName, prefix, ".");
        ...
        */
        ...
        if (savetopo)
        {
            ...
            if (StringIsNotEmpty(prefix))
                PrefixFilename(TopoFileName, prefix, ".");
        ...
        if (saveeigenvalues)
        {
            ...
            if (StringIsNotEmpty(prefix))
                PrefixFilename(EigenvaluesFileName, prefix, ".");
        ...
        if (savepcatracks)
        {
            ...
            if (StringIsNotEmpty(prefix))
                PrefixFilename(PcaDataFileName, prefix, ".");
        ...
        if (savepcapoints)
        {
            ...
            for (int i = 0; i < AtMost(12, pcanumcomp - 2); i += 3)
            {
                ...
                if (StringIsNotEmpty(prefix))
                    PrefixFilename(CloudFileName, prefix, ".");
                RemoveExtension(CloudFileName);
    ...
    */
             ...
             TSuperGauge *gauge)
    {
        ...
        if (pcaresults == Vt_X)
            T = topca.head_rows(maxeigen);
        // results can have less rows than columns
        else if (pcaresults == InvSqrtD_Vt_X)
            T = towhite.head_rows(maxeigen);
        // results are back to the original number of rows
        else if (pcaresults == V_InvSqrtD_Vt_X)
            T = V * towhite;
                    ...
                    ASymmetricMatrix &Cov)
    {
        ...
        OmpParallelFor
            ...
            for (int i = 0; i < numdim; i++) for (int j = 0; j <= i; j++)
        {
    ...
    */
                                   ...
                                   ASymmetricMatrix &Cov)
    {
        ...
        OmpParallelFor
            ...
            for (int i = 0; i < numdim; i++) for (int tf = 0; tf < numsamples; tf += step)

                stats[i]
                    .Add(data(tf, i), ThreadSafetyIgnore);
        ...
        */
        ...
        for (int tf = 0; tf < numsamples; tf++)
            ...
            if (goodtf[tf] && (tf == 0 || !goodtf[tf - 1]) && (tf == numsamples - 1 || !goodtf[tf + 1]))
                                     ...
                                     ASymmetricMatrix &Cov)
    {
            ...
            for (int j = 0; j < i; j++)
            {
                ...
                if ((i + j + 1) % modulout == 0 && numout++ < maxout)
                {
                    ...
                    for (int tf = 0; tf < numsamples; tf += step)
                    {
                        ...
                        dmaha = sqrt(S(0, 0) * data(tf, i) * data(tf, i) + S(1, 1) * data(tf, j) * data(tf, j) + 2 * S(0, 1) * data(tf, i) * data(tf, j));
                ...
                for (int tf = 0; tf < numsamples; tf++)
                {
                    ...
                    double dmaha = sqrt(S(0, 0) * data(tf, i) * data(tf, i) + S(1, 1) * data(tf, j) * data(tf, j) + 2 * S(0, 1) * data(tf, i) * data(tf, j));
            ...
            for (int i = 0; i < numdim; i++)
        {
            ...
            for (int j = 0; j < numdim; j++)
                ...
                if (i != j && !skiptracks[j] && covcount(i, j) > 0)
