...
namespace crtl
{
    ...
    {
        if ((mapsin == 0 && StringIsEmpty(filename)) || criteria == 0)
            return;
...
#define AutoCorrWindowSize3 62.0
        ...
        for (int tfi = 0; tfi < numtf; tfi++)
        {
            ...
            for (int aci = 1; aci <= autocorrwind; aci += autocorrstep)
            {
                ...
                // deskew data
                corrl = PearsonToFisher(fabs(corr1)) + PearsonToFisher(fabs(corr2));
                ...
                convl = PearsonToFisher(fabs(maps(LeftMirroring(tfi, aci, numtf - 1)).CorrelationSpearman(maps(RightMirroring(tfi, aci, numtf - 1)), true)));
        ...
        {
            // done later
            if (beci == BadEpochsAutoAvgCorrelComp || beci == BadEpochsAutoAvgCorrelNoise || beci == BadEpochsAutoAvgConvolComp || beci == BadEpochsAutoAvgConvolNoise)
                continue;
                      ...
                      double samplingfrequency)
    {
        ...
        for (int tfi = 0, tfli = 0, tfri = numtf - 1; tfi < borderttf; tfi++, tfli++, tfri--)
        {
            ...
            if (closeleft)
                track(tfli) = closeleft;
            if (closeright)
                track(tfri) = closeright;
        }
    }
                                  ...
                                  double &falsenegativerate) const
    {
        ...
        for (long tf = 0; tf < numtf; tf++)
        {
            ...
            if (projecttest[tf] && projecttraining[tf])
                numtruepos++;
            if (projecttest[tf] && !projecttraining[tf])
                numfalsepos++;
            if (!projecttest[tf] && !projecttraining[tf])
