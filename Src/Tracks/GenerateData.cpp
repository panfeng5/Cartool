...
namespace crtl
{
    ...
    {
        if (what == GenerateEeg && !leadfield.IsOpen() || what == GenerateRis && solp.IsEmpty() || what == GenerateUnknown)
        ...
        if (what == GenerateEeg)
            StringCopy(ext, FILEEXT_EEGSEF);
        else /*if ( what == GenerateRis )*/
            StringCopy(ext, FILEEXT_RIS);
        ...
        bool alternatesign = ignorepolarity && what == GenerateEeg // currently, RIS maps are only norms, and not dipoles
        ...
        if (runsegmentation)
        {
            ...
            if (what == GenerateEeg)
                analysis = ignorepolarity ? AnalysisRestingStatesIndiv : AnalysisERP;
            else if (what == GenerateRis)
                analysis = ignorepolarity ? AnalysisRestingStatesIndiv : AnalysisERP;

            if (what == GenerateEeg)
                modality = ModalityEEG;
            else if (what == GenerateRis)
                modality = ModalityESI;
            else
            ...
            if (what == GenerateEeg)
                polarity = ignorepolarity ? PolarityEvaluate : PolarityDirect;
            else if (what == GenerateRis)
                    ...
                    for (double noise = mapnoisemin; noise <= mapnoisemax; noise += mapnoisestep)
                    {
                        ...
                        else
                        {
                            ...
                            for (int tf0 = -border; tf0 < duration + border; tf0 += segduration)
                            {
                                ...
                                { // randomize all maps appearance
                                    int newmi;
                                    // make sure we get a totally NEW random map
                                    do
                                        newmi = randunif((UINT)nummaps);
                                    while (newmi == mi);
                        ...
                        */
                        ...
                        // saving maps
                        if (savemaps)
                            data.WriteFile(mapsfile);
                        // saving root maps
                        if (savetemplatemaps)
                            seedmaps.WriteFile(seedsfile);
                        // saving the root maps' sources only
                        if (savetemplateris)
                            seedris.WriteFile(templaterisfile, true);
