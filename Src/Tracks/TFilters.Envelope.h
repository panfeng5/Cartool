...
namespace crtl
{
    ...
    {
        if (!(how == FilterTypeEnvelopeSlidingWindow || how == FilterTypeEnvelopeGapBridging || how == FilterTypeEnvelopeAnalytic))
    ...
    FilterTypes GetEnvelopeTypeFromUser()
    {
        char option = GetOptionFromUser("Envelope filter (NOT Analytic for positive data):" NewLine
                                            NewLine Tab "- (S)liding Window - Signed or positive data" NewLine Tab "- (G)ap bridging    - Signed or positive data" NewLine Tab "- (A)nalytic           - Signed data only",
                                        "Envelope Filter", "S G A", "G");

        if (option == 'S')
            return FilterTypeEnvelopeSlidingWindow;
        else if (option == 'G')
            return FilterTypeEnvelopeGapBridging;
        else if (option == 'A')
            return FilterTypeEnvelopeAnalytic;
        else
    ...
    double TFilterEnvelope<TypeD>::GetEnvelopeWidthTF(ArraySizeType sizeconstraint) const
    {
        ...
        if (sizeconstraint == OddSize)
            wtf = RoundToOdd(Round(wtf));
        else if (sizeconstraint == EvenSize)
            wtf = RoundToEven(Round(wtf));
    ...
    void TFilterEnvelope<TypeD>::Apply(TypeD *data, int numpts)
    {
        ...
        if (How == FilterTypeEnvelopeSlidingWindow)
            ApplySlidingWindow(data, numpts);
        else if (How == FilterTypeEnvelopeGapBridging)
            ApplyGapBridging(data, numpts);
        else if (How == FilterTypeEnvelopeAnalytic)
            ApplyAnalytic(data, numpts);
    }
