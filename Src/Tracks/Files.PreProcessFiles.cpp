...
namespace crtl
{
    ...
    {
        if (usedregularization)
            *usedregularization = regularization;
        ...
        bool subsamplesallfiles = gfpnormalize || regularization == RegularizationAutoGlobal || regularization == RegularizationAutoLocal || backnorm == BackgroundNormalizationComputingZScore;

        bool timelinedisrupted = gfppeaks != NoGfpPeaksDetection;

        // processings that alter the timeline:
        bool timecropping = epochs == EpochsFromList || epochs == EpochsPeriodic || gfppeaks != NoGfpPeaksDetection || badepochs != NoSkippingBadEpochs;

        // all processings:
        bool ispreprocessing = spatialfilter != SpatialFilterNone || computeesi || gfpnormalize || backnorm == BackgroundNormalizationComputingZScore || ranking || thresholding || envelope != FilterTypeNone || rois || savezscore || timecropping;
        ...
        if (gauge)
            gauge->SetRange(SuperGaugeDefaultPart, gauge->GetRange(SuperGaugeDefaultPart) + (subsamplesallfiles ? gofin.NumFiles() : 0) + (numextrafiles ? numextrafiles * 8 : 0));
        ...
        for (int fi = -numextrafiles; fi < numprocfiles; fi++)
        {
            ...
            isfirstfile = !mergecomplex && fi == 0 || mergecomplex && fi <= 1;
            ...
            if (backnorm != BackgroundNormalizationNone)
            {
                ...
                if (GetZScoreProcessing(zscoremethod) == ZScorePositive_CenterScale || GetZScoreProcessing(zscoremethod) == ZScoreSigned_CenterScale)
            ...
            if (thresholding)
            {
                ...
                StringAppend(infixfile, "."
                                        "Clip",
                             FloatToString(buff, threshold, 2));
            } // BackgroundNormalizationNone
            ...
            for (int epoch = 0; epoch < AtLeast(1, (int)epochlist); epoch++)
            {
                ...
                if (badepochs != NoSkippingBadEpochs)
                {
                    ...
                    if (gfppeaks == NoGfpPeaksDetection)
                        goodepochslist.ClipMarkers(badepochslist, AllMarkerTypes);
                    else
                        goodepochslist.RemoveMarkers(badepochslist, AllMarkerTypes); // we could simply use ClipMarkers here, too
                }
                ...
                if (epochs == EpochsFromList || epochs == EpochsPeriodic)
                    StringAppend(filenameout, ".", IntegerToString(fromtf, 0), "_", IntegerToString(totf, 0)); // !separator "_" is used to retrieve epoch (for verbose purpose only) in segmentation!
                if (gfppeaks != NoGfpPeaksDetection)
                    StringAppend(filenameout, "." PostfixGfpMax);
                if (badepochs != NoSkippingBadEpochs)
                    StringAppend(filenameout, "." PostfixSkipBadEpochs);
                ...
                if (isfirstfile)
                {
                    ...
                    if (epochs == EpochsFromList || epochs == EpochsPeriodic)
                        StringAppend(infixbase, ".", IntegerToString(fromtf, 0), "_", IntegerToString(totf, 0)); // !separator "_" is used to retrieve epoch (for verbose purpose only) in segmentation!
        ...
        if (savezscore && backnorm != BackgroundNormalizationNone && ZScoreFactors.IsAllocated())
        {
