...
namespace crtl
{
    ...
    void TExportTracks::CloneParameters(const char *ext, const char *file1, const char *file2)
    {
        ...
        if (Type == ExportTracksUnknown)
            ...
            if (risdoc && (!file2 || risdoc2))
                Type = ExportTracksRis;
            else if (freqdoc && (!file2 || freqdoc2))
                Type = ExportTracksFreq;
            else
                Type = ExportTracksSef;
    ...
    void TExportTracks::PreFillFile()
    {
        ...
        if (Type == ExportTracksRis)
            sizetoreset = NumTracks * NumTime * (IsVector(AtomTypeUseOriginal) ? 3 : 1) * sizeof(float);
        else if (Type == ExportTracksSef)
            sizetoreset = NumTracks * NumTime * sizeof(float);
        else if (Type == ExportTracksBv)
            sizetoreset = NumTracks * NumTime * sizeof(float); // BVTypeFloat32
            ...
            sizetoreset = ((NumTime + EdfTrailingTF) / EdfTfPerRec) * EdfBlockSize;
        else if (Type == ExportTracksFreq)
            sizetoreset = NumTracks * NumTime * NumFrequencies * (IsComplex(AtomTypeUseOriginal) ? sizeof(complex<float>) : sizeof(float));
        else
            sizetoreset = 0;
    ...
    {
        return EdfDataOrg + (tf / EdfTfPerRec) * EdfBlockSize            // block position
    ...
    void TExportTracks::Begin(bool dummyheader)
    {
        ...
        if (Filename.IsEmpty())
        { // no filename is kind of a problem...
            Reset();
            return;
        }
    ...
    {
        if (Markers.IsEmpty() || OutputTriggers == NoTriggersNoMarkers || !HasNativeTriggers())
    ...
    void TExportTracks::WriteHeader(bool overwrite)
    {
        ...
        else if (Type == ExportTracksSeg || Type == ExportTracksData)
        {
        ...
        else if (Type == ExportTracksSef)
        {
            ...
            for (int i = SelTracks.IsNotAllocated() ? 0 : seli(); i >= 0 && (SelTracks.IsAllocated() || i < NumTracks); i = SelTracks.IsNotAllocated() ? i + 1 : ++seli)
            {
                ...
                if (AuxTracks.IsAllocated() && (i >= AuxTracks.Size() || AuxTracks[i]) && !IsElectrodeNameAux(elname))
                {
        ...
        else if (Type == ExportTracksBv)
        {
            ...
            for (int i = SelTracks.IsNotAllocated() ? 0 : seli(), j = 0; i >= 0 && (SelTracks.IsAllocated() || i < NumTracks); i = SelTracks.IsNotAllocated() ? i + 1 : ++seli, j++)
            {
                ...
                sprintf(buff, "Ch%0d=%s,,,%s", j + 1, elname, "�V");
                ofhdr << buff << fastendl;
            }
        ...
        else if (Type == ExportTracksEdf)
        {
            ...
            *StringEnd(buff) = ' ';
            ...
            for (int i = SelTracks.IsNotAllocated() ? 0 : seli(); i >= 0 && (SelTracks.IsAllocated() || i < NumTracks); i = SelTracks.IsNotAllocated() ? i + 1 : ++seli)
            {
                ...
                SetString(buff, ' ', 80);
                if (AuxTracks.IsAllocated() && AuxTracks[i])
                    CopyVirtualMemory(buff, "Auxiliary channel", 17);
                //      else if ( i >= EEGDoc->GetNumElectrodes() )                                 CopyVirtualMemory ( buff, "Computed channel",  16 );
                else if (ElectrodesNames.IsNotEmpty() && IsStringAmong(ElectrodesNames[i], ComputedTracksNames))
                    CopyVirtualMemory(buff, "Computed channel", 16);
                else
                    CopyVirtualMemory(buff, "Regular channel", 15);
            ...
            auto FloatToFixedWidthString = [](char *string, double value, int totalwidth)
            { sprintf(string, "%*lg", totalwidth, value); };
    ...
    void TExportTracks::Write(float value)
    {
        ...
        if (Type == ExportTracksEp || Type == ExportTracksEph || Type == ExportTracksSeg || Type == ExportTracksData)
        {
        ...
        else if (Type == ExportTracksSef || Type == ExportTracksBv)
        {
        ...
        else if (Type == ExportTracksRis)
        {
            ...
            { // ouput vectorial, input scalar: complement with null Y / Z component
                value = 0;
                of->write((char *)&value, sizeof(float));
                of->write((char *)&value, sizeof(float));
            }
    ...
    void TExportTracks::Write(float value, long t, int e)
    {
        ...
        if (!(IsInsideLimits(t, (long)0, NumTime - 1) && IsInsideLimits(e, 0, NumTracks - 1)))
            return;
        ...
        else if (Type == ExportTracksSef || Type == ExportTracksBv || Type == ExportTracksRis)
        {
            ...
            else if (IsVector(AtomTypeUseOriginal))
            { // ouput vectorial, input scalar: complement with null Y / Z component
    ...
    void TExportTracks::Write(float value, long t, int e, int f)
    {
        ...
        if (!(IsInsideLimits(t, (long)0, NumTime - 1) && IsInsideLimits(e, 0, NumTracks - 1) && IsInsideLimits(f, 0, NumFrequencies - 1)))
            return;
    ...
    void TExportTracks::Write(const TArray1<float> &values)
    {
        ...
        // optimized write
        if (Type == ExportTracksSef || Type == ExportTracksBv || Type == ExportTracksRis /*&& ! IsVector ( AtomTypeUseOriginal )*/ // caller's responsibility to send a multiplexed array
        )
        {
        ...
        for (long tf = 0; tf < NumTime; tf++)
        {
            ...
            if (Gauge.IsAlive())
                ...
                UpdateApplication
                    ...
                    for (int e = 0; e < NumTracks; e++) for (int freq = 0; freq < NumFrequencies; freq++)
