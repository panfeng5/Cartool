...
namespace crtl
{
    ...
    {
        return op1.From == op2.From && op1.To == op2.To
               //   && op1.Code == op2.Code        // not accounted for
               && op1.Type == op2.Type && _stricmp(op1.Name, op2.Name) == 0;
    }
    ...
    bool operator>(const TMarker &op1, const TMarker &op2)
    {
        return op1.From > op2.From || op1.From == op2.From && op1.To > op2.To || op1.From == op2.From && op1.To == op2.To && op1.Type > op2.Type || op1.From == op2.From && op1.To == op2.To && op1.Type == op2.Type && _stricmp(op1.Name, op2.Name) > 0;
    }
    ...
    void TMarker::Show(const char *title) const
    {
        ...
        StringAppend(buff, "Type" Tab Tab, IsFlag(Type, MarkerTypeTrigger) ? MarkerNameTrigger : IsFlag(Type, MarkerTypeEvent) ? MarkerNameEvent
        ...
        for (int i = 0; i < (int)Markers; i++)
            ...
            if (IsFlag(Markers[i]->Type, type))
                ...
                if ((Markers[i]->From >= timemin && Markers[i]->From <= timemax) // trigger starts within range, don't care where it ends
                    || (Markers[i]->From <= timemax && Markers[i]->To >= timemin))
                { // trigger ends past beginning of range, but starts before the end of range
                    indexmin = i;
                    break;
                }
        ...
        if (forward)
        {
            ...
            for (int i = 0; i < (int)Markers; i++)
                ...
                if (IsFlag(Markers[i]->Type, type))
                    ...
                    if (Markers[i]->From > marker.To || Markers[i]->From >= marker.From && Markers[i]->From <= marker.To && Markers[i]->To > marker.To)
                    {
        ...
        { // backward
            ...
            for (int i = (int)Markers - 1; i >= 0; i--)
                ...
                if (IsFlag(Markers[i]->Type, type))
                    ...
                    if (Markers[i]->To < marker.From || Markers[i]->To >= marker.From && Markers[i]->To <= marker.To && Markers[i]->From < marker.From)
                    {
        ...
        if (forward)
        {
            ...
            for (int i = 0; i < (int)Markers; i++)
                ...
                if (IsFlag(Markers[i]->Type, type))
                    ...
                    if (Markers[i]->From > marker.To || Markers[i]->From >= marker.From && Markers[i]->From <= marker.To && Markers[i]->To > marker.From && (Markers[i]->To > marker.To || (tfc->IsExtending() && tfc->GetFixedPos() > tfc->GetExtendingPos())))
                    {
        ...
        else
        {
            ...
            for (int i = (int)Markers - 1; i >= 0; i--)
                ...
                if (IsFlag(Markers[i]->Type, type))
                    ...
                    if (Markers[i]->To < marker.From || Markers[i]->To >= marker.From && Markers[i]->To <= marker.To && Markers[i]->From < marker.To && (Markers[i]->From < marker.From || (tfc->IsExtending() && tfc->GetFixedPos() < tfc->GetExtendingPos())))
                    {
    ...
    MarkersError TMarkers::InsertMarker(const TMarker &marker, int *indexfrom)
    {
        ...
        if (inserti == (int)Markers)
            Markers.Append(markercopy); // nothing past marker?
        else
            Markers.Insert(markercopy, Markers[inserti]);
                                   ...
                                   long periodiclength)
    {
        ...
        if (epochs == EpochsFromList && !(epochfrom && epochfrom->IsNotEmpty() && epochto && epochto->IsNotEmpty() && epochfrom->NumStrings() == epochto->NumStrings()))

            epochs = EpochWholeTime;
        //  return;

        // periodic but bad length parameter -> downgrade
        if (epochs == EpochsPeriodic && periodiclength <= 0)
                                     ...
                                     const char *name)
    {
        ...
        {
            // borders are OK
            if (EegBuff[tracki][tf0] > EegBuff[tracki][tf0 - 1] && EegBuff[tracki][tf0] > EegBuff[tracki][tf0 + 1])
            {
                                       ...
                                       const char *newmarkername)
    {
        ...
        for (long tf = 0 /*fromtf*/; tf <= /*lasttimeframes*/ totf; tf++)
        {
            ...
            if (!(tf == /*lasttimeframes*/ totf || label[tf + 1] != label[tf]))
            ...
            if (flag == KeepingMarkers && segi != 0 || flag == ExcludingMarkers && segi == 0)
        ...
        while ((bool)remaining)
        {
            ...
            do
            {
                ...
                for (iterator.SetForward(remaining); (bool)iterator;)
                {
                    ...
                    bool namesok = identicalnames && StringIs(toaggregate->Name, iterator()->Name) || !identicalnames;

                    // merge on the right?
                    if ((toaggregate->To == iterator()->From || toaggregate->To == iterator()->From - 1) && namesok)
                    {
    ...
    void TMarkers::RemoveMarkers(long from, long to, MarkerType type)
    {
        ...
        for (int i = 0; i < (int)Markers; i++)
        {
            ...
            if (IsFlag(Markers[i]->Type, type) && IsInsideLimits(Markers[i]->From, Markers[i]->To, from, to))
