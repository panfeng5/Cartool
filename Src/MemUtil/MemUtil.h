...
namespace crtl
{
    ...
    private:
        // These are �ber-confidential variables, even derived classes shouldn't access them
        void *MemoryBlock;
        size_t MemorySize;
    ...
    inline void *GetSmartMemory(void *&memoryblock, size_t numbytes, MemoryAllocationType how, MemoryAllocationType &memtype)
    {
        ...
        if (IsMemoryAuto(how))
            memtype = numbytes >= GetMemoryGranularity() ? MemoryVirtual // more than 65K, we can ignore the Windows granularity
                                                                         //                                      : numbytes >= GetPageSize ()            ? MemoryWindowsHeap     // more than 4K, go to the current process Heap
                                                                         //                                      :                                         MemoryCppHeap;        // C++ style
                                                         : MemoryCHeap;  // C style
        else
            memtype = GetMemoryType(how);

        // Allocate the amount of requested type of memory
        if (IsMemoryVirtual(memtype))
            memoryblock = GetVirtualMemory(numbytes); // virtual memory
        else if (IsMemoryWindowsHeap(memtype))
            memoryblock = GetHeapMemory(numbytes); // Windows heap
        else if (IsMemoryCppHeap(memtype))
            memoryblock = numbytes > 0 ? (void *)new char[numbytes] : 0; // C++ Heap
        else if (IsMemoryCHeap(memtype))
            memoryblock = numbytes > 0 ? malloc(numbytes) : 0; // C Heap (is cheap)
        else
            memoryblock = 0;
    ...
    inline void FreeSmartMemory(void *&memoryblock, MemoryAllocationType memtype)
    {
        ...
        if (IsMemoryVirtual(memtype))
            FreeVirtualMemory(memoryblock);
        else if (IsMemoryWindowsHeap(memtype))
            FreeHeapMemory(memoryblock);
        else if (IsMemoryCppHeap(memtype))
            delete[] memoryblock;
        else if (IsMemoryCHeap(memtype))
            free(memoryblock);
    ...
    inline void ClearSmartMemory(void *&memoryblock, size_t numbytes, MemoryAllocationType memtype)
    {
        ...
        if (IsMemoryVirtual(memtype))
            ClearVirtualMemory(memoryblock, numbytes);
        else if (IsMemoryWindowsHeap(memtype))
            ClearVirtualMemory(memoryblock, numbytes);
        else if (IsMemoryCppHeap(memtype))
            memset(memoryblock, 0, numbytes);
        else if (IsMemoryCHeap(memtype))
            memset(memoryblock, 0, numbytes);
    }
