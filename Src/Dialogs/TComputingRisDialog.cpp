...
namespace crtl
{
    ...
    TComputingRisStruct::TComputingRisStruct()
    {
        ...
        SpatialFilter = BoolToCheck(ComputingRisPresetDefault == ComputingRisPresetErpIndivMeans || ComputingRisPresetDefault == ComputingRisPresetErpIndivEpochs || ComputingRisPresetDefault == ComputingRisPresetErpFitClusters || ComputingRisPresetDefault == ComputingRisPresetIndIndivEpochs || ComputingRisPresetDefault == ComputingRisPresetSpont || ComputingRisPresetDefault == ComputingRisPresetSpontClusters);
        ClearString(XyzFile);
    ...
    TComputingRisDialog::~TComputingRisDialog()
    {
        ...
        delete NumSubjectsEdit;
        delete NumConditionsEdit;
        delete NumInversesEdit;
        ...
        delete RegFixedEdit;
        delete TimeZScore;
        delete ComputeZScore;
        delete LoadZScoreFile;
        delete Rank;
        delete Envelope;
        delete EnvelopeLowFreq;
        delete ApplyRois;
        delete RoisFile;
        delete BaseFileName;
        delete SavingIndividualFiles;
        delete SavingEpochFiles;
        delete ComputeGroupsAverages;
        delete ComputeGroupsCentroids;
    ...
    {
        tce.Enable(!(CRISPresets[EegPresets->GetSelIndex()].IsEpochs() || CRISPresets[EegPresets->GetSelIndex()].IsClusters()));
    }

    void TComputingRisDialog::CmDataTypeEnable(TCommandEnabler &tce)
    {
        tce.Enable(IsNotChecked(ApplyRois) && CRISPresets[EegPresets->GetSelIndex()].CmDataTypeEnable());
    }
    ...
    void TComputingRisDialog::ReadParams(const char *filename)
    {
        ...
        if (!firstread)
        {
            ...
            appendrows = answer == EOS || Is1Group1Subject() && answer == 'S' || Is1Group1Condition() && answer == 'C';
        }
            ...
            for (int fi = 0; fi < numfiles; fi++)
            {
                ...
                if (isrowinverse)
                {
                    if (firstread || !appendrows)
                        Inverses.Add(attr); // skip if user requested it
                }
                else if (appendrows)
                    gof.Add(attr); // usual case

                else                      /*if ( ! appendrows )*/
                    GoGoF[row].Add(attr); // permutating case - this needs some more tests
            } // for file
    ...
    void TComputingRisDialog::EvDropFiles(TDropInfo drop)
    {
        ...
        if ((bool)eegfiles && (bool)freqfiles)
            ...
            ShowMessage("You can not process EEG and Frequency files together!" NewLine
                        "Just compute one type of files at a time.",
                        ComputingRisTitle, ShowMessageWarning);

        else
        {
            // will possibly split / dispatch in more groups, in case of clusters of data
            if ((bool)eegfiles)
                AddEegGroups(eegfiles);

            if ((bool)freqfiles)
                AddEegGroup(freqfiles);
    ...
    {
        tce.Enable(CheckToBool(SpatialFilter->GetCheck()) && EegPresets->GetSelIndex() != ComputingRisPresetFreq);
    }
...
#define MaxRisClusters 1000
            ...
            for (TIteratorSelectedForward cli(selcl); (bool)cli; ++cli)
            {
                ...
                if (updatingtogroups)
                    GoGoF[cli.GetIndex()].Add(gogof[cli()]);
                else
                    AddEegGroup(gogof[cli()]);
            }
        ...
        if ((bool)GoGoF)
        {
            ...
            else if (gofeeg.NumFiles() != GoGoF[0].NumFiles())
            {
                ...
                ShowMessage(grouplayout == GroupsLayout1SubjAllCond ? "Not the same number of conditions!"
                                                                    : "Not the same number of subjects!",
                            ComputingRisTitle, ShowMessageWarning);
    ...
    void TComputingRisStructEx::CheckInverses()
    {
        ...
        if (NumInverses() == 0)
            ...
            StringCopy(InverseFile, "<Inverses Not OK!>");
        // else if ( IsEegOK && IsInverseOK && ! IsInverseAndEegOK )   ClearString ( InverseFile );
        else if (IsTemplateInverse())
            StringCopy(InverseFile, Inverses[0]);
        else if (AreIndividualInverses())
            StringCopy(InverseFile, "<Subjects Inverses>");
    }
    ...
    void TComputingRisDialog::CheckEeg()
    {
        ...
        if (acrossconditions && ComputingRisTransfer.CompatEegs.SamplingFrequency == CompatibilityNotConsistent)
        {
        ...
        if (acrossconditions && ComputingRisTransfer.CompatEegs.NumFreqs == CompatibilityNotConsistent)
        {
            // we need all files from a group to have the same length!
        ...
        if (!lastgof.AnyTracksGroup(tg))
        {
            ...
            if (tg.noneofthese)
                ShowMessage("Oops, files don't really look like tracks," NewLine "are you trying to fool me?!" NewLine "Check again your input files...", ComputingRisTitle, ShowMessageWarning);
            else if (!tg.alleeg)
                ShowMessage("Files don't seem to be consistently of the EEG type!" NewLine "Check again your input files...", ComputingRisTitle, ShowMessageWarning);
            else if (!tg.allfreq)
                ShowMessage("Files don't seem to be consistently of the Frequency type!" NewLine "Check again your input files...", ComputingRisTitle, ShowMessageWarning);
            else
        ...
        if (tg.allfreq /*or NumFreqs > CompatibilityConsistent*/)
        {
            ...
            if (acrossconditions && fc.OriginalSamplingFrequency == CompatibilityNotConsistent)
            {
            ...
            if (acrossconditions && fc.FreqType == CompatibilityNotConsistent)
            {
            ...
            if (!(IsFreqTypeComplex((FrequencyAnalysisType)fc.FreqType) || IsFreqTypeReal((FrequencyAnalysisType)fc.FreqType)))
            {
        ...
        */
        ...
        if (ComputingRisTransfer.CompatEegs.NumTracks == CompatibilityNotConsistent)
        {
            ShowMessage("Groups of Files don't seem to have the same number of electrodes/tracks!" NewLine
                        "Please do again your last group...",
                        ComputingRisTitle, ShowMessageWarning);
            return;
        }
    ...
    void TComputingRisDialog::UpdateGroupSummary()
    {
        ...
        for (int gofi = 0; gofi < (int)GoGoF; gofi++)
        {
            ...
            if (grouplayout == GroupsLayoutAllSubj1Cond)
                StringCopy(buff, CRISPresets[esicase].IsClusters() ? "Cluster" : "Condition"); // although equivalent, we can be more specific
            else if (grouplayout == GroupsLayout1SubjAllCond)
                StringCopy(buff, CRISPresets[esicase].Code == ComputingRisPresetErpGroupMeans ? "Group" : "Subject"); // special case for Grand Average
            else if (grouplayout == GroupsLayout1SubjAllCondAllEpochs)
                StringCopy(buff, "Subject");
            else
                StringCopy(buff, "Group");
            StringAppend(buff, " ", IntegerToString(gofi + 1, NumIntegerDigits((int)GoGoF)), ":");

            StringAppend(buff, "  ", IntegerToString(gof.NumFiles()), " ");

            if (grouplayout == GroupsLayoutAllSubj1Cond)
                StringAppend(buff, "Subject");
            else if (grouplayout == GroupsLayout1SubjAllCond)
                StringAppend(buff, CRISPresets[esicase].IsClusters() ? "Cluster" : "Condition"); // although equivalent, we can be more specific
            else if (grouplayout == GroupsLayout1SubjAllCondAllEpochs)
                StringAppend(buff, "Epoch File");
            else
                StringAppend(buff, "File");
            StringAppend(buff, StringPlural(gof.NumFiles()));

            if (gof.NumFiles() == 1)
                StringAppend(buff, "  ( ", ToFileName(gof.GetFirst()), " )");
            else
                StringAppend(buff, "  ( ", ToFileName(gof.GetFirst()), " .. ", ToFileName(gof.GetLast()), " )");
    ...
    void TComputingRisDialog::UpdateSubjectsConditions(bool updategroups)
    {
        ...
        if (NumInverses() == 0)
            InverseFile->SetText("");
        else if (!ComputingRisTransfer.IsInverseOK)
            InverseFile->SetText("<Inverses Not OK!>");
        else if (IsTemplateInverse())
            InverseFile->SetText(Inverses[0]);
        else if (AreIndividualInverses())
            InverseFile->SetText("<Subjects Inverses>");

        InverseFile->ResetCaret;

        if (updategroups)
            // Setting Group Averages if more than 1 subject for a few presets
            ComputeGroupsAverages->SetCheck(BoolToCheck((esicase == ComputingRisPresetErpIndivMeans || esicase == ComputingRisPresetErpIndivEpochs || esicase == ComputingRisPresetIndIndivEpochs) && NumSubjects > 1));
    }
    ...
    void TComputingRisDialog::CmOkEnable(TCommandEnabler &tce)
    {
        ...
        if (!(IsTemplateInverse() || AreIndividualInverses()) || !ComputingRisTransfer.IsInverseOK)
        {
            tce.Enable(false);
            return;
        }

        if (GoGoF.IsEmpty() || !ComputingRisTransfer.IsEegOK)
        {
            tce.Enable(false);
            return;
        }

        if (CheckToBool(ComputingRisTransfer.SpatialFilter) && (StringIsEmpty(ComputingRisTransfer.XyzFile) || !ComputingRisTransfer.IsXyzOK))
        {
            tce.Enable(false);
            return;
        }

        if (CheckToBool(ComputingRisTransfer.RegFixed) && !IsRegularRegularization(ComputingRisTransfer.RegFixedEdit))
        {
            tce.Enable(false);
            return;
        }

        if (CheckToBool(ComputingRisTransfer.Envelope) && !CheckToBool(ComputingRisTransfer.VectorData) && StringToDouble(ComputingRisTransfer.EnvelopeLowFreq) <= 0)
        {
            tce.Enable(false);
            return;
        }

        if (CheckToBool(ComputingRisTransfer.ApplyRois) && (StringIsEmpty(ComputingRisTransfer.RoisFile) || !ComputingRisTransfer.IsRoisOK))
        {
            tce.Enable(false);
            return;
        }
        ...
        // any processing?
        if (!(CheckToBool(ComputingRisTransfer.SavingIndividualFiles) || CheckToBool(ComputingRisTransfer.TimeZScore) && CheckToBool(ComputingRisTransfer.ComputeZScore) && CheckToBool(ComputingRisTransfer.SavingZScoreFactors) || CheckToBool(ComputingRisTransfer.SavingEpochFiles) || CheckToBool(ComputingRisTransfer.ComputeGroupsAverages) || CheckToBool(ComputingRisTransfer.ComputeGroupsCentroids)))
        {
            tce.Enable(false);
            return;
        }
    ...
    void TComputingRisDialog::CmOk()
    {
        ...
        StringAppend(buff, IntegerToString(numsubjects), " ", esicase == ComputingRisPresetErpGroupMeans || esicase == ComputingRisPresetErpSegClusters ? "Group" : "Subject", StringPlural(numsubjects));
        StringAppend(buff, "  x  ");
        ...
        StringAppend(buff, NewLine NewLine
                     "    ");
        ...
        if (individualinverses && numinverses > 1 && GetAnswerFromUser("Do you wish to review all subjects' EEGs and Inverse pairing?", ComputingRisTitle, this))
        {
        ...
        SpatialFilterType spatialfilter = (CheckToBool(ComputingRisTransfer.SpatialFilter) && esicase != ComputingRisPresetFreq) ? SpatialFilterDefault : SpatialFilterNone;
        TFileName xyzfile;
        ...
        bool envelope = CheckToBool(ComputingRisTransfer.Envelope) && !IsVector(datatypeproc);
        ...
        bool roiing = CheckToBool(ComputingRisTransfer.ApplyRois) && !IsVector(datatypeproc); // we have to do something to forbid ROIing on non-scalar data
        TFileName roifile;
        ...
        bool savingzscorefactors = CheckToBool(ComputingRisTransfer.SavingZScoreFactors) && backnorm != BackgroundNormalizationNone;
        ...
        if (!ComputingRis(esicase,
            ...
            ShowMessage("Something went wrong during the RIS processing!" NewLine
                        "Check your parameters as well as your input files...",
                        ComputingRisTitle, ShowMessageWarning);
    }
