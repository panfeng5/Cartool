...
namespace crtl
{
    ...
    void TExportTracksDialog::SetupWindow()
    {
        ...
        if (!BatchProcessing)
        { // in case files were provided before call (ProcessCurrent)
    ...
    TExportTracksDialog::~TExportTracksDialog()
    {
        ...
        delete RoisDocFile;
        delete TimeInterval;
        delete TriggersToKeep;
        delete TriggersToExclude;
        delete TimeMin;
        delete TimeMax;
        delete EndOfFile;
        delete TriggersToKeepList;
        delete TriggersToExcludeList;
        delete AddChannels;
        delete NoFilters;
        ...
        delete NoRescaling;
        delete RescaledBy;
        delete RescaledValue;
        delete GfpNormalization;
        delete Sequence;
        delete Mean;
        delete Downsample;
        delete DownsampleRatio;
        delete FileTypes;
    ...
    */
    ...
    {
        tce.Enable(CheckToBool(ExportTracks->GetCheck()) && !ExportTracksTransfer.XyzLinked);
    }

    void TExportTracksDialog::CmXyzDocFileEnable(TCommandEnabler &tce)
    {
        tce.Enable(CheckToBool(ExportTracks->GetCheck()) && CheckToBool(UseXyzDoc->GetCheck()));
    }

    void TExportTracksDialog::CmUseXyzDocEnable(TCommandEnabler &tce)
    {
        tce.Enable(CheckToBool(ExportTracks->GetCheck()) && CheckToBool(UseXyzDoc->GetCheck()) && !ExportTracksTransfer.XyzLinked);
    }
    ...
    void TExportTracksDialog::CmXyzChange()
    {
        ...
        if (BatchProcessing || !CheckToBool(ExportTracks->GetCheck()) || StringIsEmpty(ExportTracksTransfer.XyzDocFile) || !ExportTracksTransfer.ChannelsSel)
        {
        ...
        if (XYZDoc->GetNumElectrodes() != EEGDoc->GetNumElectrodes())
        {
            ...
            StringCopy(buff, "File and Coordinates don't have the same dimension: " NewLine NewLine "File:" Tab Tab, IntegerToString(EEGDoc->GetNumElectrodes()), NewLine "Coordinates:" Tab, IntegerToString(XYZDoc->GetNumElectrodes()));
    ...
    */
    ...
    void TExportTracksDialog::CmRoiChange()
    {
        ...
        if (BatchProcessing || !CheckToBool(ExportRois->GetCheck()) || StringIsEmpty(ExportTracksTransfer.RoisDocFile))
        {
        ...
        if (RoisDoc->ROIs->GetDimension() != EEGDoc->GetNumElectrodes())
        {
            ...
            StringCopy(buff, "File and ROIs don't have the same dimension: " NewLine NewLine "File:" Tab, IntegerToString(EEGDoc->GetNumElectrodes()), NewLine "ROIs:" Tab, IntegerToString(RoisDoc->ROIs->GetDimension()));
    ...
    {
        tce.Enable(CheckToBool(BaselineCorr->GetCheck()) && !(CheckToBool(TriggersToKeep->GetCheck()) || CheckToBool(TriggersToExclude->GetCheck())));
    }

    void TExportTracksDialog::CmBaseLineCorrectionEofEnable(TCommandEnabler &tce)
    {
        tce.Enable(CheckToBool(BaselineCorr->GetCheck()) && !(CheckToBool(TriggersToKeep->GetCheck()) || CheckToBool(TriggersToExclude->GetCheck())) && !CheckToBool(BaselineCorrEof->GetCheck()));
    }

    void TExportTracksDialog::CmTimeSequenceEnable(TCommandEnabler &tce)
    {
        tce.Enable(CheckToBool(TimeInterval->GetCheck()) && CheckToBool(Sequence->GetCheck()));
    }

    void TExportTracksDialog::CmDownsampleRatioEnable(TCommandEnabler &tce)
    {
        tce.Enable(CheckToBool(TimeInterval->GetCheck()) && CheckToBool(Sequence->GetCheck()) && CheckToBool(Downsample->GetCheck()));
    }

    //----------------------------------------------------------------------------
    bool TExportTracksDialog::CmProcessEnable()
    {
        if (CheckToBool(ExportTracks->GetCheck()) && !Tracks->GetLineLength(0))
            return false;

        if (CheckToBool(ExportTracks->GetCheck()) && CheckToBool(UseXyzDoc->GetCheck()) && !XyzDocFile->GetLineLength(0))
            return false;

        if (CheckToBool(ExportRois->GetCheck()) && !RoisDocFile->GetLineLength(0))
            return false;

        if (CheckToBool(TriggersToKeep->GetCheck()) && !TriggersToKeepList->GetLineLength(0))
            return false;

        if (CheckToBool(TriggersToExclude->GetCheck()) && !TriggersToExcludeList->GetLineLength(0))
            return false;

        if (CheckToBool(OtherRef->GetCheck()) && !RefList->GetLineLength(0))
            return false;

        if (CheckToBool(BaselineCorr->GetCheck()) && !(BaselineCorrPre->GetLineLength(0) && (CheckToBool(BaselineCorrEof->GetCheck()) || BaselineCorrPost->GetLineLength(0) /*&& StringToDouble ( TAvgTransfer.BaselineCorrPre ) <= StringToDouble ( TAvgTransfer.BaselineCorrPost )*/)))
            return false;

        if (CheckToBool(RescaledBy->GetCheck()) && !RescaledValue->GetLineLength(0))
    ...
    void TExportTracksDialog::ProcessCurrent(void *usetransfer, const char *freqinfix)
    {
        ...
        bool isfrequency = IsExtensionAmong(EEGDoc->GetDocPath(), AllFreqFilesExt) || StringIsNotEmpty(freqinfix); // this is the current frequency string, when processing frequency files
        ...
        auto CloseXyz = [&XYZDoc, letopenxyz]()
        { XYZDoc.Close(letopenxyz ? CloseDocLetOpen : CloseDocRestoreAsBefore); };

        // see if a coordinates file has been provided
        if (CheckToBool(transfer->UseXyzDoc) && CanOpenFile(transfer->XyzDocFile))
        {
        ...
        auto CloseRois = [&RoisDoc, letopenrois]()
        { RoisDoc.Close(letopenrois ? CloseDocLetOpen : CloseDocRestoreAsBefore); };
        ...
        if (tracksoptions == ProcessRois)
        {
            ...
            { // there is a problem with the roi file, get out
                CloseXyz;
                CmCancel();
            }
        }
        ...
        if (CheckToBool(transfer->CurrentRef))
            ref = ReferenceUsingCurrent;
        else if (CheckToBool(transfer->AveRef))
        ...
        bool baselinecorr = CheckToBool(transfer->BaselineCorr) && timeoptions == ExportTimeInterval;
        ...
        if (baselinecorr)
        {
            ...
            if (baselinecorrpre < 0 && baselinecorrpost < 0 || baselinecorrpre > lasttimeframe && (baselinecorreof || baselinecorrpost > lasttimeframe))
        ...
        bool downsample = CheckToBool(transfer->Downsample) && timeoptions == ExportTimeInterval && sequenceoptions == SequenceProcessing;
        ...
        bool silent = isfrequency || concatenateoptions == ConcatenateTime || NumBatchFiles() > 1;
        ...
        bool openauto = CheckToBool(transfer->OpenAuto) && !isfrequency && CartoolApplication->IsInteractive();
    ...
    void TExportTracksDialog::BatchProcessCurrent()
    {
        ...
        { // loop for each frequency, and export each of them
            ...
            for (int i = 0; i < numinfiles; i++)
            {
                ...
                if (numel != numel0)
                {
                    ...
                    ShowMessage("You are trying to concatenate files" NewLine
                                "with different number of tracks." NewLine NewLine
