...
namespace crtl
{
    ...
    TMicroStatesFitFilesDialog::~TMicroStatesFitFilesDialog()
    {
        ...
        delete NumWithinSubjects;
        delete MaxWithinSubjects;
        delete EpochFrom;
        delete EpochTo;
        delete EpochMaps;
        delete BaseFileName;
        delete WriteSegFiles;
        delete WriteClusters;
        delete DeletePreProcFiles;
    }
        ...
        if (StringIsNotEmpty(FitTransfer.TemplateFileName))
        {
            ...
            if (alldatris && allmapeeg)
            {
                ShowMessage("Hu? Are you trying to fit some EEG directly into some sorts of ESI?" NewLine
                            "Nope is the answer.",
                            templfile, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
                return false;
            }
    ...
    void TMicroStatesFitFilesDialog::EvDropFiles(TDropInfo drop)
    {
        ...
        if ((bool)tracksfiles)
        {
            ...
            if ((int)tracksfiles == 1 && where.Y < 121)
            {
        ...
        if (EnumEpochs.NumSet(ifrom, ito) > 0)
        {
            ...
            if (/*firstnotseli.IsNotOk () ||*/ ifrom < 0)
            {
                ...
                ShowMessage("Time interval completely overlaps with another interval," NewLine
                            "epoch rejected.",
                            FittingAddEpoch, ShowMessageWarning);
            ...
            ShowMessage("Time interval partly overlaps with another interval," NewLine
                        "the first eligible epoch will be suggested.",
                        FittingAddEpoch, ShowMessageWarning);
            return;
        }
    ...
    TMicroStatesFitResultsDialog::~TMicroStatesFitResultsDialog()
    {
        ...
        delete TimeCoverage;
        delete SegDensity;
        delete LinesAsSamples;
        delete ColumnsAsFactors;
        delete OneFilePerGroup;
        delete OneFilePerVariable;
        delete ShortVariableNames;
        delete MarkovSegment;
        delete MarkovNumTransitions;
        delete WriteStatDurations;
        delete WriteCorrFiles;
        delete WriteSegFrequency;
    }
    ...
    void TMicroStatesFitResultsDialog::CmAllOnOff(owlwparam w)
    {
        ...
        if (w == IDC_ALLON)
            FitTransfer.SetVariablesOn();
        else if (w == IDC_ALLOFF)
            FitTransfer.SetVariablesOff();
    ...
    void TMicroStatesFitDialog::CmToDialog(owlwparam w)
    {
        ...
        if (todialogid == IDD_FITTING1)
            TMicroStatesFitFilesDialog(CartoolMdiClient, IDD_FITTING1).Execute();
        else if (todialogid == IDD_FITTING2)
            TMicroStatesFitParamsDialog(CartoolMdiClient, IDD_FITTING2).Execute();
        else if (todialogid == IDD_FITTING3)
            TMicroStatesFitResultsDialog(CartoolMdiClient, IDD_FITTING3).Execute();
    }
    ...
    void TMicroStatesFitDialog::CheckXyzFile()
    {
        ...
        if (tc.NumTracks != xyznumel)
        {
            ...
            char buff[256];
            sprintf(buff, "Not the same amount of Electrodes:" NewLine NewLine "  - EEG files " Tab "= %0d" NewLine "  - Electrodes file " Tab "= %0d" NewLine NewLine "Please check again your files!", tc.NumTracks, xyznumel);
    ...
    void TMicroStatesFitDialog::CmOkEnable(TCommandEnabler &tce)
    {
        ...
        if (CheckToBool(FitTransfer.MarkovSegment) && mnt <= 0)
        {
            tce.Enable(false);
            return;
        }
        ...
        if (!(CheckToBool(FitTransfer.FOnset) || CheckToBool(FitTransfer.LOffset) || CheckToBool(FitTransfer.NumTf) || CheckToBool(FitTransfer.Gev) || CheckToBool(FitTransfer.BestCorr) || CheckToBool(FitTransfer.TfBestCorr) || CheckToBool(FitTransfer.GfpTfBestCorr) || CheckToBool(FitTransfer.MaxGfp) || CheckToBool(FitTransfer.TfMaxGfp) || CheckToBool(FitTransfer.MeanGfp) || CheckToBool(FitTransfer.TfCentroid) || CheckToBool(FitTransfer.MeanCorr) || CheckToBool(FitTransfer.MeanDuration) || CheckToBool(FitTransfer.TimeCoverage) || CheckToBool(FitTransfer.SegDensity) || CheckToBool(FitTransfer.WriteSegFiles) || CheckToBool(FitTransfer.WriteClusters) || CheckToBool(FitTransfer.WriteStatDurations) || CheckToBool(FitTransfer.WriteCorrFiles) || CheckToBool(FitTransfer.WriteSegFrequency) || CheckToBool(FitTransfer.MarkovSegment)))
        {
            tce.Enable(false);
            return;
        }
    ...
    void TMicroStatesFitDialog::CmOk()
    {
        ...
        if (!GoGoF.HasNoDuplicates())
        {
            ShowMessage("There are some duplicate input files, which is quite suspicious." NewLine
                        "Check your input files and please come back...",
                        FittingTitle, ShowMessageWarning);
            return;
        }
        ...
        SpatialFilterType spatialfilter = (CheckToBool(FitTransfer.SpatialFilter) && StringIsNotEmpty(FitTransfer.XyzFile)) ? SpatialFilterDefault : SpatialFilterNone;
        ...
        { // NOT computing the ESI, input shouldn't mix EEG and RIS

            if (alldatris && !allmapris || alldateeg && !allmapeeg)
            {
            ...
            if (alldatrisv && (allmapris && !allmaprisv) || (alldatris && !alldatrisv) && allmaprisv)
            {
        ...
        AtomType datatype;
        if (CheckToBool(FitTransfer.PositiveData))
            datatype = AtomTypePositive;
        else if (CheckToBool(FitTransfer.SignedData))
            datatype = AtomTypeScalar;
        else if (CheckToBool(FitTransfer.VectorData))
        ...
        if (CheckToBool(FitTransfer.AveRef))
            dataref = ReferenceAverage;
        else /*if ( CheckToBool ( FitTransfer.NoRef ) )*/
        ...
        if (templzscore && !dataallzscore)
            ShowMessage("You seemed to load some standardized templates, but not all input data files seemed standardized either!" NewLine
                        "Just to let you know that this is not the best configuration, but processing will continue anyway...",
                        FittingTitle, ShowMessageWarning);
        ...
        if (CheckToBool(FitTransfer.ShortVariableNames))
            SetFlags(outputflags, VariablesShortNames);
        else
            SetFlags(outputflags, VariablesLongNames);

        if (CheckToBool(FitTransfer.LinesAsSamples))
            SetFlags(outputflags, SheetLinesAsSamples);
        if (CheckToBool(FitTransfer.ColumnsAsFactors))
            SetFlags(outputflags, SheetColumnsAsFactors);
        if (CheckToBool(FitTransfer.OneFilePerGroup))
            SetFlags(outputflags, SaveOneFilePerGroup);
        if (CheckToBool(FitTransfer.OneFilePerVariable))
            SetFlags(outputflags, SaveOneFilePerVariable);

        if (CheckToBool(FitTransfer.WriteSegFiles) && competitive)
            SetFlags(outputflags, WriteSegFiles);
        if (CheckToBool(FitTransfer.WriteClusters))
        ...
        if (CheckToBool(FitTransfer.WriteStatDurations) && competitive)
            SetFlags(outputflags, WriteStatDurationsFiles);
        if (CheckToBool(FitTransfer.WriteCorrFiles))
            SetFlags(outputflags, WriteCorrelationFiles);
        if (CheckToBool(FitTransfer.WriteSegFrequency) && competitive)
            SetFlags(outputflags, WriteSegFrequencyFiles);

        // if ( newfiles )                                                       SetFlags ( outputflags, OwningFiles                 );  // PreProcessFiles will tell us if it created new files or not
        if (CheckToBool(FitTransfer.DeletePreProcFiles))
        ...
        bool ispreprocessing = timelyfilter || gfpnormalize;
        ...
        if (newfiles)
            SetFlags(outputflags, OwningFiles);
