...
namespace crtl
{
    ...
    void TTracksFiltersStruct::SetRectification(const char *t)
    {
        ...
        if (StringContains(t, "Absolute") || StringContains(t, "Abs"))
        {
        ...
        else if (StringContains(t, "Power") || StringContains(t, "Squared"))
        {
    ...
    {
        delete Baseline;
        delete ButterworthHigh;
        delete ValueButterworthHigh;
        delete OrderHigh;
        delete ButterworthLow;
        delete ValueButterworthLow;
        delete OrderLow;
        delete Causal;
        delete NonCausal;
        delete Notches;
        delete NotchesValues;
        delete NotchesAutoHarmonics;
        delete SpatialFilter;
        delete XyzFile;
        delete Ranking;
        delete Rectification;
        delete RectificationAbs;
        delete RectificationPower;
        delete Envelope;
        delete EnvelopeWidth;
        delete EnvelopeFreq;
        delete ThresholdKeepAbove;
        delete ThresholdKeepAboveValue;
        delete ThresholdKeepBelow;
        delete ThresholdKeepBelowValue;
        delete ValueSamplingFrequency;
        delete FilterAuxs;
        delete On;
        delete Off;
    }
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(ButterworthHigh->GetCheck()))
        {
            tce.Enable(true);
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(ButterworthLow->GetCheck()))
        {
            tce.Enable(true);
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(NonCausal->GetCheck()))
        {
            tce.Enable(true);
        }
        else
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(Rectification->GetCheck()))
        {
            tce.Enable(true);
        }
        else
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(Envelope->GetCheck()))
        {
            tce.Enable(true);
        }
        else
    ...
    void TTracksFiltersDialog::UpdateEnvelopeWidthToFreq()
    {
        ...
        if (v == 0)
            ...
            EnvelopeFreq->SetText(FloatToString(MillisecondsToFrequency(v)));
    ...
    void TTracksFiltersDialog::UpdateEnvelopeFreqToWidth()
    {
        ...
        if (v == 0)
            ...
            EnvelopeWidth->SetText(FloatToString(FrequencyToMilliseconds(v)));
    ...
    {
        if (CheckToBool(On->GetCheck()) && (CheckToBool(ButterworthHigh->GetCheck()) || CheckToBool(ButterworthLow->GetCheck())))
        {
            tce.Enable(true);
        }
        else
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(Notches->GetCheck()))
        {
            tce.Enable(true);
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(ThresholdKeepAbove->GetCheck()))
        {
            tce.Enable(true);
    ...
    {
        if (CheckToBool(On->GetCheck()) && CheckToBool(ThresholdKeepBelow->GetCheck()))
        {
            tce.Enable(true);
    ...
    {
        if (!(CheckToBool(Envelope->GetCheck()) && CheckToBool(ButterworthHigh->GetCheck())))
            return;
    ...
    void TTracksFiltersDialog::CmOKEnable(TCommandEnabler &tce)
    {
        ...
        if (CheckToBool(ButterworthHigh->GetCheck()) && !(samplf > 0 && highf > 0 && IsInsideLimits(orderh, ButterworthHighPassMinOrder, ButterworthHighPassMaxOrder) && IsEven(orderh)))
        {
            tce.Enable(false);
            return;
        }

        if (CheckToBool(ButterworthLow->GetCheck()) && !(samplf > 0 && lowf > 0 && IsInsideLimits(orderl, ButterworthLowPassMinOrder, ButterworthLowPassMaxOrder) && IsEven(orderl)))
        {
            tce.Enable(false);
            return;
        }

        if (CheckToBool(ButterworthLow->GetCheck()) && CheckToBool(ButterworthHigh->GetCheck()) && !(highf < lowf && orderh == orderl))
        {
            tce.Enable(false);
            return;
        }
        ...
        if (CheckToBool(Envelope->GetCheck()) && (samplf <= 0 || !EnvelopeWidth->GetLineLength(0)))
        {
            tce.Enable(false);
            return;
        }
