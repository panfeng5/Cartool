...
namespace crtl
{
    ...
    TMicroStatesSegFilesDialog::~TMicroStatesSegFilesDialog()
    {
        ...
        delete EpochFrom;
        delete EpochTo;
        delete ResamplingEpochs;
        delete NumResamplingEpochs;
        delete ResamplingEpochsSize;
        delete ResamplingCoverage;
        delete PresetsFiles;
        delete BaseFileName;
        delete WriteSeg;
        delete WriteMaps;
        delete WriteClusters;
        delete WriteSynth;
        delete ForceSingleFiles;
        delete BestDir;
        delete DeleteIndivDirs;
    }
    ...
    void TMicroStatesSegFilesDialog::EvPresetsChange()
    {
        ...
        if (fflags & SegPresetResampling)
        {
            ...
            // updating depends on parameter
            if (IsFlag(fflags, SegPresetPrioritySampleSize))
                SampleSizeToNumResampling();
            else if (IsFlag(fflags, SegPresetPriorityNumResampling))
                UpdateResamplingCoverage();
        }
    ...
    void TMicroStatesSegFilesDialog::AddGroupSummary(int gofi, bool updatebasefilename)
    {
        ...
        if (gof.NumFiles() == 1)
            StringAppend(buff, " File  ( ", ToFileName(gof.GetFirst()), " )");
        else
            StringAppend(buff, " Files  ( ", ToFileName(gof.GetFirst()), " .. ", ToFileName(gof.GetLast()), " )");
    ...
    {
        if (GoGoF.IsEmpty() || LockResampling || !CheckToBool(ResamplingEpochs->GetCheck()))
            return;
        ...
        if (resampling.NumData < SegmentMinNumData || resampling.SampleSize < SegmentMinSampleSize || resampling.NumResampling < SegmentMinNumResampling)
    ...
    void TMicroStatesSegFilesDialog::UpdateResamplingCoverage()
    {
        ...
        if (IsFlag(presetf.FilesFlags, SegPresetPrioritySampleSize))
        {
            ...
            if (presetf.SampleSizeSeconds)
                presetf.SampleSizeSeconds = TimeFrameToSeconds(resampling.SampleSize, SamplingFrequency);
            else if (presetf.SampleRatio)
                presetf.SampleRatio = resampling.GetRelativeSampleSize();
        }
        else if (IsFlag(presetf.FilesFlags, SegPresetPriorityNumResampling))
    ...
    {
        if (GoGoF.IsEmpty() || LockResampling || !CheckToBool(ResamplingEpochs->GetCheck()) || IsFlag(SegPresetsFiles[PresetsFiles->GetSelIndex()].FilesFlags, SegPresetPriorityNumResampling) // forbid auto-updating the number of resampling for Grand Clustering, which should remain to 100 - or to user's choice either
        )
            return;
    ...
    void TMicroStatesSegFilesDialog::UpdateSampleSize()
    {
        ...
        if (LockResampling || !CheckToBool(ResamplingEpochs->GetCheck()) || !(presetf.FilesFlags & SegPresetResampling))
            return;
    ...
    void TMicroStatesSegFilesDialog::SetBaseFilename()
    {
        ...
        if (presetfile == SegPresetFilesERP)
            StringCopy(prefix, "Seg");
        else if (presetfile == SegPresetFilesRestingStatesSubjAllData)
            StringCopy(prefix, "RS");
        else if (presetfile == SegPresetFilesRestingStatesSubjResampling)
            StringCopy(prefix, "RSResamp");
        else if (presetfile == SegPresetFilesRestingStatesGroupAllData)
            StringCopy(prefix, "GC");
        else if (presetfile == SegPresetFilesRestingStatesGroupResampling)
            StringCopy(prefix, "GCResamp");
        else
            StringCopy(prefix, "Seg");
        ...
        if (presetfile == SegPresetFilesERP || presetfile == SegPresetFilesRestingStatesSubjAllData || presetfile == SegPresetFilesRestingStatesSubjResampling)
        {

            gof.GetCommonString(match, true /*, true*/);
        }

        else if (presetfile == SegPresetFilesRestingStatesGroupAllData || presetfile == SegPresetFilesRestingStatesGroupResampling)
        {
    ...
    TMicroStatesSegParamsDialog::~TMicroStatesSegParamsDialog()
    {
        ...
        delete GfpPeaksAuto;
        delete GfpPeaksList;
        delete AnalyzeMarkers;
        delete SkipBadEpochs;
        delete SkipBadEpochsAuto;
        delete SkipBadEpochsList;
        delete SkipMarkers;
        delete SpatialFilter;
        delete XyzFile;
        delete ClipCorrelation;
        delete MinCorrelation;
        delete DualDataset;
        delete Sequentialize;
        delete MergeCorr;
        delete MergeCorrThresh;
        delete Smoothing;
        delete WindowSize;
        delete BesagFactor;
        delete RejectSmall;
    ...
    */
    ...
    void TMicroStatesSegDialog::CmToDialog(owlwparam w)
    {
        ...
        if (todialogid == IDD_SEGMENT1)
            TMicroStatesSegFilesDialog(CartoolMdiClient, IDD_SEGMENT1).Execute();
        else if (todialogid == IDD_SEGMENT2)
            TMicroStatesSegParamsDialog(CartoolMdiClient, IDD_SEGMENT2).Execute();
    }
        ...
        TMicroStatesSegDialog::CmOk()
    {
        ...
        if (CheckToBool(SegTransfer.ListEpochs))
            epochs = EpochsFromList;
        else if (CheckToBool(SegTransfer.ResamplingEpochs))
            epochs = EpochWholeTime;
        else /*CheckToBool ( SegTransfer.NoEpochs )*/
            epochs = EpochWholeTime;
        ...
        */
        ...
        SpatialFilterType spatialfilter = CheckToBool(SegTransfer.SpatialFilter) && StringIsNotEmpty(SegTransfer.XyzFile) ? SpatialFilterDefault : SpatialFilterNone;
        ...
        AtomType datatype;
        if (CheckToBool(SegTransfer.PositiveData))
            datatype = AtomTypePositive;
        else if (CheckToBool(SegTransfer.SignedData))
            datatype = AtomTypeScalar;
        else if (CheckToBool(SegTransfer.VectorData))
            datatype = AtomTypeVector;
        else
            datatype = AtomTypeScalar;

        // downgrade vectorial type to scalar if not all data are vectorial
        if (IsVector(datatype) && !allrisv)
            datatype = AtomTypePositive;
        ...
        if (CheckToBool(SegTransfer.AveRef))
            dataref = ReferenceAverage;
        else /*if ( CheckToBool ( SegTransfer.NoRef  ) )*/
        ...
        if (CheckToBool(SegTransfer.KMeans))
            clusteringmethod = ClusteringKMeans;
        else if (CheckToBool(SegTransfer.TAAHC))
            clusteringmethod = ClusteringTAAHC;
        else
            clusteringmethod = UnknownClustering;
        ...
        if (CheckToBool(SegTransfer.WriteSeg))
            SetFlags(outputflags, WriteSegFiles);
        if (CheckToBool(SegTransfer.WriteMaps))
            SetFlags(outputflags, WriteTemplatesFiles);
        if (CheckToBool(SegTransfer.WriteClusters))
        ...
        if (AllowEmptyClusterFiles)
            SetFlags(outputflags, WriteEmptyClusters); // controlled by global variable
        if (SavingNormalizedClusters)
            SetFlags(outputflags, WriteNormalizedClusters); // controlled by global variable
        if (CheckToBool(SegTransfer.WriteSynth))
            SetFlags(outputflags, WriteSyntheticFiles);

        if (CheckToBool(SegTransfer.BestDir))
            SetFlags(outputflags, CommonDirectory);
        if (CheckToBool(SegTransfer.BestDir) && CheckToBool(SegTransfer.DeleteIndivDirs))
            SetFlags(outputflags, DeleteIndivDirectories);
        ...
        bool includefilename = presetfile == SegPresetFilesRestingStatesSubjAllData || presetfile == SegPresetFilesRestingStatesSubjResampling;
        int fromchars = 0;
        int tochars = 0;
        ...
        bool ispreprocessing = timelyfilter || epochs != EpochWholeTime || gfppeaks != NoGfpPeaksDetection || badepochs != NoSkippingBadEpochs;
        ...
        {
            ShowMessage("There seems to be some duplicate base file names, according to the given parameters." NewLine NewLine
        ...
        if (gogof.NumGroups() > 1 || ispreprocessing || resampling != NoTimeResampling)
        {
            // Init gauge
            ...
            GroupGauge.AddPart(gaugegroupseggroup,
                               gogof.NumGroups(), ispreprocessing ? 05 : resampling != NoTimeResampling ? 0
                                                                                                        : 100);
            ...
            GroupGauge.AddPart(gaugegroupsegpreproc,
                               gogof.NumGroups() * 7 + gogof.NumFiles() * 2, ispreprocessing ? 45 : resampling != NoTimeResampling ? 0
                                                                                                                                   : 0);
            ...
            for (int absg = 0; absg < gogof.NumGroups(); absg++)
                ...
                GroupGauge.AddPart(gaugegroupsegprocoffset + absg,
                                   0, ispreprocessing ? 50 / (double)gogof.NumGroups() : resampling != NoTimeResampling ? 95 / (double)gogof.NumGroups()
                                                                                                                        : 0);
        ...
        for (int absg = 0; absg < gogof.NumGroups(); absg++)
        {
            ...
            owningfiles = newfiles                        // files generated by PreProcessFiles - we can own them at will
                          ...
                          || concatgof;                   // Grand Clustering intermediate concatenation - it's ours
                                                          // update auto-opening
            complimentaryopen = (epochs == EpochWholeTime || epochs == EpochsFromList && (int)epochfrom == 1
            ...
            for (int absg2 = 0; absg2 < preprocgogof.NumGroups(); absg2++)
            {
                ...
                for (int rsi = 0; rsi < (int)resampgogof; rsi++)
                {
                    ...
                    if (resampling == NoTimeResampling)
                        StringCopy(basefilename, preprocbaselist[absg2]);
                    else
                        StringCopy(basefilename, preprocbaselist[absg2], "." PostfixResampled, IntegerToString(buff, rsi + 1, NumIntegerDigits((int)resampgogof)));
                ...
                if (concatgof && dualdata)
                    DeleteFileExtended(concatfilealt);
        ...
        if (presetfile == SegPresetFilesRestingStatesGroupResampling && IsFlag(outputflags, CommonDirectory))
        {
            ...
            for (int nclusters = reqminclusters; nclusters <= reqmaxclusters; nclusters++)
            {
                ...
                if ((int)gofcluster == 1)
                {
                    ...
                    CopyFileExtended(gofcluster[0], buff, dualdata == DualRis ? SegmentationRisTemplateExt : dualdata == DualEeg ? SegmentationEegTemplateExt
                                                                                                                                 : 0);
