...
#define GetIntValue(V)           \
    GetText(buff, EditSizeText); \
    V = atoi(buff)
#define GetDoubleValue(V)        \
    GetText(buff, EditSizeText); \
    V = atof(buff)
...
#define SetDoubleValue(V) SetText(FloatToString(V))
    ...
    inline bool IsSpace(const owl::TEdit *edit)
    {
        char buff[256];
        edit->GetText(buff, 256);
        return StringIsSpace(buff);
    }
    ...
    inline int GetInteger(const owl::TEdit *edit)
    {
        char buff[EditSizeValue];
        edit->GetText(buff, EditSizeValue);
        return StringToInteger(buff);
    }
    inline double GetDouble(const owl::TEdit *edit)
    {
        char buff[EditSizeValue];
        edit->GetText(buff, EditSizeValue);
    ...
    inline void Copy(owl::TEdit *to, const owl::TEdit *from)
    {
        char buff[EditSizeText];
        from->GetText(buff, EditSizeText);
        to->SetText(buff);
    }
    ...
    inline void SetInteger(owl::TEdit *edit, int v)
    {
        char buff[EditSizeValue];
        edit->SetText(IntegerToString(buff, v));
    }
    inline void SetDouble(owl::TEdit *edit, double v)
    {
        char buff[EditSizeValue];
        edit->SetText(FloatToString(buff, v));
    }
    ...
    */
    ...
    constexpr char *BatchErrorMessage = "Batch 'Drag & Drop' is not currently available," NewLine
                                        "because your parameters don't seem to be correct!" NewLine NewLine
