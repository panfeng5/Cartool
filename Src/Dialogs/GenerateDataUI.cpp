...
namespace crtl
{
    ...
    void TCartoolMdiClient::GenerateDataUI(owlwparam w)
    {
        ...
        else
        {
            ...
            {
                segdurationmax = segdurationmin;
                fileduration = 20 * segdurationmin;
            }
            else if (numscanf == 2)
                fileduration = 20 * (segdurationmin + segdurationmax) / 2;
        ...
        if (what == GenerateEeg)
        {
            ...
            {
                ShowMessage("There seems to be a problem with the Lead Field file, which can be either:" NewLine Tab "- The file type is not recognized" NewLine Tab "- The Lead Field data type is not of vectorial type" NewLine Tab "- The file itself might be somehow corrupted" NewLine NewLine
                            "This will end your processing now, bye bye...",
                            ToFileName(leadfieldfile), ShowMessageWarning);
                return;
            }
        ...
        else if (what == GenerateRis)
        {
            ...
            if (solp.IsEmpty())
            {
                ShowMessage("There seems to be a problem with the Solution Points file!" NewLine NewLine
                            "This will end your processing now, bye bye...",
                            ToFileName(spfile), ShowMessageWarning);
                return;
            }
        }
