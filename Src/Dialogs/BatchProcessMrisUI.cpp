...
#define PreProcessMrisTitle "MRIs to process"
        ...
        void
        TCartoolMdiClient::BatchProcessMrisUI(owlwparam w)
    {
        ...
        for (int i = 0; i < (int)getfiles; i++)
        {
            ...
            if (w == CM_SKULLSTRIPPING)
            {
                ...
                if (StringContains((const char *)newfile, ".Head."))
                    StringReplace(newfile, ".Head.", ".");
                else if (StringContains((const char *)newfile, ".Full Head."))
                    StringReplace(newfile, ".Full Head.", ".");
                else if (StringContains((const char *)newfile, ".FullHead."))
                    StringReplace(newfile, ".FullHead.", ".");
                else if (StringContains((const char *)newfile, " Head "))
                    StringReplace(newfile, " Head ", "");
                else if (StringContains((const char *)newfile, "_Head"))
                    StringReplace(newfile, "_Head", "");
                else if (StringContains((const char *)newfile, "_pro"))
                    StringReplace(newfile, "_pro", "");
            }
            ...
            if (w == CM_SKULLSTRIPPING)
                StringAppend(newfile, "." InfixBrain);
            else if (w == CM_CUTBRAINSTEM)
                StringAppend(newfile, "."
                                      "NoBrainstem");
            else if (w == CM_FILTERBIASFIELD)
                StringAppend(newfile, "." InfixBiasFieldCorrection);
            else if (w == CM_FILTERSEGMENTCSF)
                StringAppend(newfile, "." InfixCsf);
            else if (w == CM_FILTERSEGMENTGREY)
                StringAppend(newfile, "." InfixGrey);
            else if (w == CM_FILTERSEGMENTWHITE)
                StringAppend(newfile, "." InfixWhite);
            else if (w == CM_FILTERSEGMENTTISSUES)
                StringAppend(newfile, "." InfixTissues);
            else if (w == CM_DOWNSAMPLEMRI)
                StringAppend(newfile, "."
                                      "Down",
                             IntegerToString(p(0)));
            else if (w == CM_HEADCLEANUP)
                StringAppend(newfile, ".", FilterPresets[FilterTypeHeadCleanup].Ext);
