    ...
    void TCartoolMdiClient::BatchAveragingFilesUI(owlwparam w)
    {
        ...
        if (tg.allrisv)
        {
            ...
            if (risvpooled)
            {
                ...
                if (!GetValueFromUser("Number of epochs to locally average:", "Pooled Vectorial Averaging", poolednumlocalavg, "6"))
                    return;

                Clipped(poolednumlocalavg, 1, numfiles);

                if (!GetValueFromUser("Number of repetitions:", "Pooled Vectorial Averaging", poolednumrepeatavg, "100"))
                    return;

                if (poolednumrepeatavg <= 0)
                    return;
            }
        }
        ...
        if (tg.alleeg
            ...
            || (tg.allris && (!tg.allrisv || risvtos)))
            BatchAveragingScalar(gof,
                                 meanfile, sdfile, 0,
                                 0, 0,
        ...
        else if (tg.allrisv)
        {
            ...
            if (risvpooled)
                BatchPoolAveragingVectorial(gof,
                                            meanfile, 0, 0,
                                            0, sphsdfile, 0,
