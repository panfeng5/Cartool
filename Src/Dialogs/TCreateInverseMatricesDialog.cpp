...
namespace crtl
{
    ...
    TCreateInverseMatricesDialog::~TCreateInverseMatricesDialog()
    {
        ...
        delete SmoothMri;
        delete ReorientMris;
        delete SagittalPlaneMri;
        delete EquatorPlaneMri;
        delete XyzPresets;
        delete XyzFile;
        delete SpPresets;
        delete NumSolPoints;
        ...
        delete ThicknessFromMri;
        delete ThicknessFromTissues;
        delete LeadFieldTargetAge;
        delete SkullConductivity;
        delete AdjustSkullMeanThickness;
        delete SkullMeanThickness;
        delete LeadFieldFile;
        delete LeadFieldSolPointsFile;
        delete InverseMN;
        delete InverseWMN;
        delete InverseDale;
        delete InverseSLoreta;
        delete InverseLoreta;
        delete InverseLaura;
        delete InverseELoreta;
        delete BaseFileName;
        delete Interactive;
        delete Automatic;
    }
    ...
    void TCreateInverseMatricesDialog::CheckXyzFile()
    {
        ...
        if (xyznumel != lfnumel)
        {
            ...
            StringCopy(buff, "Not the same amount of Electrodes:" NewLine NewLine "  - Lead Field " Tab "= ", IntegerToString(lfnumel), NewLine "  - Electrodes file " Tab "= ", IntegerToString(xyznumel), NewLine NewLine "Please check again your files!");
    ...
    void TCreateInverseMatricesDialog::SetLoadSolPointsFile(const char *file)
    {
        ...
        if (!TPoints(file).IsGrid())
        {
            ShowMessage("The given Solution Points do not seem to be regularly spaced on a grid." NewLine
                        "This will very likely cause some errors later on." NewLine NewLine
                        "Check this is really the file you intend to use!",
                        CrisTransfer.LoadSolPointsFile, ShowMessageWarning, this);
        } // ! regular
    ...
    void TCreateInverseMatricesDialog::CheckLeadFieldSolPointsFile()
    {
        ...
        if (CrisTransfer.LeadFieldPresets.GetSelIndex() == CrisLFPresetReload && CrisTransfer.SpPresets.GetSelIndex() == CrisSPPresetFromLeadField)
        {
        ...
        if (CrisTransfer.HasLeadFieldFile())
        {
            ...
            if (lfspnumsolp != lfnumsolp)
            {
                ...
                StringCopy(buff, "Not the same amount of Solution Points:" NewLine NewLine "  - Lead Field " Tab "= ", IntegerToString(lfnumsolp), NewLine "  - Solution Points " Tab "= ", IntegerToString(lfspnumsolp), NewLine NewLine "Please check again your files!");
        ...
        bool loadsps = CrisTransfer.IsLoadSPs() && CrisTransfer.HasLoadSolPointsFile();

        bool loadleadfield = CrisTransfer.IsLoadLeadField() && CrisTransfer.HasLeadFieldFile() && CrisTransfer.HasLeadFieldSolPointsFile();

        bool interpolateleadfield = loadleadfield && (computesps || loadsps);
        ...
        if (interpolateleadfield)
        {
            ...
            if (!lfsp.IsGridAligned())
            {
                ...
                ShowMessage("The given Solution Points do not seem to be regularly spaced on a grid," NewLine
                            ...
                            "This will very likely cause some errors later on." NewLine NewLine
                            "Check this is really the file you intend to use!",
                            CrisTransfer.LeadFieldSolPointsFile, ShowMessageWarning, this);
            }
        }
        else
        {
    ...
    {
        int count = CheckToBool(InverseMN->GetCheck()) + CheckToBool(InverseWMN->GetCheck()) + CheckToBool(InverseDale->GetCheck()) + CheckToBool(InverseSLoreta->GetCheck()) + CheckToBool(InverseLoreta->GetCheck()) + CheckToBool(InverseLaura->GetCheck()) + CheckToBool(InverseELoreta->GetCheck());
    ...
    void TCreateInverseMatricesDialog::SetBaseFilename()
    {
        ...
        if (CrisTransfer.HasHeadMri())
            gof.Add(CrisTransfer.HeadMriFile);
        if (CrisTransfer.HasBrainMri())
            gof.Add(CrisTransfer.BrainMriFile);
        if (CrisTransfer.HasGreyMri())
            gof.Add(CrisTransfer.GreyMriFile);
        if (gof.IsEmpty() && CrisTransfer.HasXyz())
            gof.Add(CrisTransfer.XyzFile);
        if (gof.IsEmpty() && CrisTransfer.HasLeadFieldFile())
            gof.Add(CrisTransfer.LeadFieldFile);
    ...
    void TCreateInverseMatricesDialog::EvDropFiles(TDropInfo drop)
    {
        ...
        for (int i = 0; i < (int)mrifiles; i++)
        {
            ...
            if (IsGrey(guess))
                SetMriFile(mrifiles[i], 2);
            else if (IsBrain(guess))
                SetMriFile(mrifiles[i], 1);
            else if (IsFullHead(guess))
                SetMriFile(mrifiles[i], 0);
            else if (IsRoiMask(guess))
                SetTissuesFile(mrifiles[i]);
        ...
        for (int i = 0; i < (int)lffiles; i++)
        {
            ...
            if (CrisTransfer.HasLeadFieldFile() && !((bool)spfiles || CrisTransfer.HasLeadFieldSolPointsFile()))
            {
        ...
        if ((bool)spfiles)
        {
            ...
            if (CrisTransfer.HasLeadFieldFile())
            {
                ...
                for (int i = 0; i < (int)spfiles; i++)
                {
                    ...
                    if (StringIs(spfiles[i], spfilefromlf))
                        SetLeadFieldSolPointsFile(spfiles[i]);
                    ...
                    else
                    {
                        ...
                        if (spnumsolp == lfnumsolp)
                            SetLeadFieldSolPointsFile(spfiles[i]);
                        else
                            SetLoadSolPointsFile(spfiles[i]);
                    }
                }
            } // HasLeadFieldFile
            ...
            if (CrisTransfer.HasLeadFieldSolPointsFile() && !((bool)lffiles || CrisTransfer.HasLeadFieldFile()))
            ...
            for (int i = 0; i < (int)lmfiles; i++)
            {
                ...
                if ((bool)lm.lmri)
                {
                    ...
                    foreachin(lm.lmri, iterator)
                    {
                        ...
                        if (IsGrey(guess))
                            SetMriFile(iterator, 2);
                        else if (IsBrain(guess))
                            SetMriFile(iterator, 1);
                        else if (IsFullHead(guess))
                            SetMriFile(iterator, 0);
                        else if (IsRoiMask(guess))
                            SetTissuesFile(iterator);
                ...
                if ((bool)lm.lsp)
                {
                    ...
                    foreachin(lm.lsp, iterator)
                    {
                        // set according to position
                        if (where.Y >= 600)
                            SetLeadFieldSolPointsFile(iterator());
                        else
                            SetLoadSolPointsFile(iterator());
                    }
                }
            }
        } // .lm files
        ...
        if ((bool)oldlffiles)
        {
            ...
            StringCopy(buff, "'." FILEEXT_BIN "'  files are not supported anymore as valid Lead Field input." NewLine NewLine
    ...
    void TCreateInverseMatricesDialog::CmOkEnable(TCommandEnabler &tce)
    {
        ...
        if (CrisTransfer.HasAnyMri() && (CrisTransfer.HasBrainMri() && !CrisTransfer.HasHeadMri() || CrisTransfer.HasGreyMri() && !CrisTransfer.HasBrainMri()))
        {
            tce.Enable(false);
            return;
        }
        ...
        if (CrisTransfer.IsReorientMris()
            ...
            && (!CrisTransfer.HasHeadMri() || CheckToBool(CrisTransfer.EquatorPlaneMri) && !CrisTransfer.HasBrainMri()))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsBiasField() && !CrisTransfer.HasBrainMri())
        {
            tce.Enable(false);
            return;
        }
        ...
        if (CrisTransfer.IsXyzCoregisterToMri() && !(CrisTransfer.HasXyz() && CrisTransfer.HasHeadMri()))
        {
            tce.Enable(false);
            return;
        }
        ...
        if (CrisTransfer.IsComputeSPs() && !((CrisTransfer.HasBrainMri() || CrisTransfer.HasGreyMri()) && StringToFloat(CrisTransfer.NumSolPoints) > 0))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsSameSPasLF() && !(CrisTransfer.HasLeadFieldSolPointsFile() /*&& StringToFloat ( CrisTransfer.NumSolPoints ) > 0*/))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsLoadSPs() && !(CrisTransfer.HasLoadSolPointsFile() /*&& StringToInteger( CrisTransfer.LoadSPsMaxSize ) > 0*/))
        {
            tce.Enable(false);
            return;
        }
        ...
        if (CrisTransfer.IsComputeLeadField() && !(CrisTransfer.IsThicknessFromMri() && CrisTransfer.HasHeadMri() || CrisTransfer.IsThicknessFromTissues() && CrisTransfer.HasTissuesFile()))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsComputeLeadField() && !(IsInsideLimits(StringToDouble(CrisTransfer.LeadFieldTargetAge), SkullThicknessMinAge, SkullThicknessMaxAge) && StringToDouble(CrisTransfer.SkullConductivity) > 0 && !CrisTransfer.IsAdjustSkullMeanThickness() || StringToDouble(CrisTransfer.SkullMeanThickness) > 0))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsComputeLeadField() && !(CrisTransfer.IsXyzCoregistrationDone() || CrisTransfer.IsXyzCoregisterToMri()))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsLoadLeadField() && !(CrisTransfer.HasLeadFieldFile() && CrisTransfer.HasLeadFieldSolPointsFile()))
        {
            tce.Enable(false);
            return;
        }

        if (CrisTransfer.IsInverseProcessing() && !(CrisTransfer.IsSPsProcessing() && CrisTransfer.IsLeadFieldProcessing()))
        {
            tce.Enable(false);
            return;
        }
    ...
    {
        TransferData(tdGetData);
        tce.Enable(CrisTransfer.HasBrainMri() && CrisTransfer.IsComputeSPs() || CrisTransfer.IsSameSPasLF());
    }
    ...
    {
        TransferData(tdGetData);
        tce.Enable(CrisTransfer.IsComputeLeadField() && AllLeadFieldPresets[CrisTransfer.LeadFieldPresets.GetSelIndex()].IsHuman());
    }

    void TCreateInverseMatricesDialog::CmAdjustSkullThicknessEnable(TCommandEnabler &tce)
    {
        TransferData(tdGetData);
        tce.Enable(CrisTransfer.IsComputeLeadField() && CrisTransfer.IsAdjustSkullMeanThickness() && AllLeadFieldPresets[CrisTransfer.LeadFieldPresets.GetSelIndex()].IsHuman());
    }
    ...
    {
        TransferData(tdGetData);
        tce.Enable(CrisTransfer.IsComputeLeadField() || CrisTransfer.IsLoadLeadField() && CrisTransfer.HasLeadFieldFile() && CrisTransfer.HasLeadFieldSolPointsFile());
    }
    ...
    void TCreateInverseMatricesDialog::CmOk()
    {
        ...
        if (!(CrisTransfer.HasAnyMri() || mriprocessing || xyzprocessing || spsprocessing || leadfieldprocessing || inverseprocessing))
        {

            ShowMessage("You didn't select any processing!" NewLine
                        "Thanks for coming anyway...",
                        CreatingInverseTitle, ShowMessageWarning, this);
        ...
        bool biasfield = CrisTransfer.HasBrainMri() && CrisTransfer.IsBiasField();

        bool smoothmri = CrisTransfer.HasAnyMri() && CrisTransfer.IsSmoothMri();

        bool reorientmris = CrisTransfer.HasHeadMri() // just axis permutations
                            && CrisTransfer.IsReorientMris();

        bool setsagittalplanemri = reorientmris && CrisTransfer.HasHeadMri() && CheckToBool(CrisTransfer.SagittalPlaneMri);

        bool setequatorplanemri = reorientmris // Head for initial transform, Brain for adjusting
                                  && CrisTransfer.HasHeadBrainMris() && CheckToBool(CrisTransfer.EquatorPlaneMri);
        ...
        bool xyzcoregistrationdone = CrisTransfer.IsXyzCoregistrationDone() && CrisTransfer.HasAnyMri();

        bool xyzcoregistertomrii = CrisTransfer.IsXyzCoregisterToMriI() && CrisTransfer.HasHeadMri();

        bool xyzcoregistertomriiredo = CrisTransfer.IsXyzCoregisterToMriIRedo() && CrisTransfer.HasHeadMri();

        bool xyzcoregistertomri = xyzcoregistertomrii || xyzcoregistertomriiredo;
        ...
        bool samespsaslf = CrisTransfer.IsSameSPasLF() && CrisTransfer.HasLeadFieldSolPointsFile();

        bool hassps = computesps || loadsps || samespsaslf;
        ...
        bool computegrey = !CrisTransfer.HasGreyMri() && CrisTransfer.HasBrainMri() && hassps;

        bool hasgrey = CrisTransfer.HasGreyMri() || computegrey;
        ...
        bool isthicknessfrommri = CrisTransfer.IsThicknessFromMri() && CrisTransfer.HasHeadMri();

        bool isthicknessfromtissues = CrisTransfer.IsThicknessFromTissues() && CrisTransfer.HasTissuesFile();

        bool computeleadfield = CrisTransfer.IsComputeLeadField();

        bool loadleadfield = CrisTransfer.IsLoadLeadField() && CrisTransfer.HasLeadFieldFile() && CrisTransfer.HasLeadFieldSolPointsFile();

        bool hasleadfield = computeleadfield || loadleadfield;

        bool interpolateleadfield = loadleadfield && (computesps || loadsps);

        bool getheadmodel = computeleadfield && lfpreset.IsGeometryLSMAC();
        ...
        if (loadleadfield && CrisTransfer.HasLeadFieldFile() && CrisTransfer.HasLeadFieldSolPointsFile())
        ...
        if (CrisTransfer.HasHeadMri())
            StringCopy(FileInputMriHead, CrisTransfer.HeadMriFile);
        if (CrisTransfer.HasBrainMri())
            StringCopy(FileInputMriBrain, CrisTransfer.BrainMriFile);
        if (CrisTransfer.HasGreyMri())
            StringCopy(FileInputMriGrey, CrisTransfer.GreyMriFile);
        if (isthicknessfromtissues)
            StringCopy(FileInputMriTissues, CrisTransfer.TissuesFile);
        if (CrisTransfer.IsXyzProcessing())
            StringCopy(FileInputXyz, CrisTransfer.XyzFile);

        if (samespsaslf)
            StringCopy(FileInputSpLoad, CrisTransfer.LeadFieldSolPointsFile);
        else if (loadsps)
            StringCopy(FileInputSpLoad, CrisTransfer.LoadSolPointsFile);

        if (samespsaslf || loadleadfield)
            StringCopy(FileInputSpLeadField, CrisTransfer.LeadFieldSolPointsFile);

        if (loadleadfield)
            StringCopy(FileInputLeadFieldLoad, CrisTransfer.LeadFieldFile);
        ...
        StringCopy(FileLocalRadiiLayers, BaseFileNameLocal, "."
                                                            "Boundaries.All Layers",
                   "." FILEEXT_ELS);
        ...
        StringCopy(fileradii, BaseFileNameMore, "."
                                                "Thicknesses"
                                                "." FILEEXT_EEGSEF);
        StringCopy(fileradiilayers, BaseFileNameMore, "."
                                                      "Boundaries.All Layers",
                   "." FILEEXT_ELS);

        StringCopy(filelmall, BaseFileName, "."
                                            "All",
                   "." FILEEXT_LM);
        StringCopy(filelmxyz, BaseFileName, "."
                                            "XYZ",
                   "." FILEEXT_LM);
        StringCopy(filelmis, BaseFileName, "."
                                           "IS",
                   "." FILEEXT_LM);
        StringCopy(filelmris, BaseFileName, "."
                                            "RIS",
                   "." FILEEXT_LM);
        ...
        // inverse only
        if (CrisTransfer.IsInverseProcessing() || hasgrey || hassps)
        {

            ofstream olm(TFileName(filelmis, TFilenameExtendedPath));
            // MRIs
            if (hasgrey)
                olm << FileOriginalMriGrey << NewLine;
            if (CrisTransfer.HasBrainMri())
                olm << FileOriginalMriBrain << NewLine;
            if (CrisTransfer.HasHeadMri())
                olm << FileOriginalMriHead << NewLine;
            // SPIs
            if (hassps)
                olm << FileOriginalSp << NewLine;
            // IS
            if (inversemn)
                olm << filemneis << NewLine;
            if (inversewmn)
                olm << filewmneis << NewLine;
            if (inversedale)
                olm << filedaleis << NewLine;
            if (inversesloreta)
                olm << filesloretais << NewLine;
            if (inverseloreta)
                olm << fileloretais << NewLine;
            if (inverselaura)
                olm << filelaurais << NewLine;
            if (inverseeloreta)
                olm << fileeloretais << NewLine;
        }
        else
            ClearString(filelmis);

        // MRIs only
        if ((CrisTransfer.HasHeadMri() || CrisTransfer.HasBrainMri() || hasgrey) && hassps)
        {
        ...
        if (CrisTransfer.HasBrainMri())
            olm << FileOriginalMriBrain << NewLine;
        if (isthicknessfromtissues)
            olm << FileOriginalMriTissues << NewLine;
        if (CrisTransfer.HasHeadMri())
            olm << FileOriginalMriHead << NewLine;
        // SPIs
        if (hassps)
        ...
        bool ajustskullthickness = lfpreset.IsHuman() && CrisTransfer.IsAdjustSkullMeanThickness();
        ...
        if (CrisTransfer.HasAnyMri() || isthicknessfromtissues)
        {

            Verbose.NextTopic("MRIs:");

            if (FileInputMriHead.IsNotEmpty())
                Verbose.Put("Full Head       MRI file:", FileInputMriHead);
            if (FileInputMriBrain.IsNotEmpty())
                Verbose.Put("Segmented Brain MRI file:", FileInputMriBrain);
            if (FileInputMriGrey.IsNotEmpty())
                Verbose.Put("Grey Mask       MRI file:", FileInputMriGrey);
            if (isthicknessfromtissues)
                Verbose.Put("Segmented Tissues   file:", FileInputMriTissues);
            Verbose.NextLine();
        ...
        if (CrisTransfer.IsXyzProcessing())
        {
            ...
            if (CrisTransfer.IsXyzCoregistrationDone())
                Verbose.Put("Coregistration to MRI:", "Already coregistered");
            else if (CrisTransfer.IsXyzCoregisterToMriI())
                Verbose.Put("Coregistration to MRI:", "Interactive");
            else if (CrisTransfer.IsXyzCoregisterToMriIRedo())
                Verbose.Put("Coregistration to MRI:", "Interactive");
        }
        ...
        if (hassps)
        {
            ...
            if (computesps)
            {
                ...
                if (spflags & GreyMatterLauraCheck)
                    Verbose.Put("Remove points unfit for LAURA  :", (bool)(spflags & GreyMatterLauraCheck));
                if (spflags & GreyMatterSinglePointCheck)
                    Verbose.Put("Remove isolated points         :", (bool)(spflags & GreyMatterSinglePointCheck));
            } // computesps
        ...
        if (hasleadfield)
        {
            ...
            if (computeleadfield)
            {
                ...
                if (lfpreset.IsGeometryLSMAC())
                    Verbose.Put("Lead Field Geometrical Model    :", "LSMAC (Locally Spherical Model with Anatomical Constraints)");

                if (lfpreset.IsIsotropic3ShellSphericalAry())
                    Verbose.Put("Lead Field Electromagnetic Model:", "Isotropic, 3-Shell Model, Ary spherical approximation");
                else if (lfpreset.IsIsotropicNShellSpherical())
                    Verbose.Put("Lead Field Electromagnetic Model:", "Isotropic, N-Shell Model, Exact spherical equations");
                ...
                Verbose.NextLine();
                Verbose.Put("Targeted species:", lfpreset.IsHuman() ? "Human" : lfpreset.IsMacaque() ? "Macaque monkey"
                                                                            : lfpreset.IsMouse()     ? "Mouse"
                                                                                                     : "Unknown");
                ...
                // Scalp radius
                if (lfpreset.IsGeometryLSMAC())
                    Verbose.Put("Outer absolute scalp radius is:", "Set by electrode");

                // Skull radius
                if (lfpreset.IsSkullRadiusFixedRatio())
                {
                    Verbose.Put("Skull absolute thickness is:", "Modulated by electrode");
                    Verbose.Put("Skull relative thickness is:", "Fixed");
                }
                else if (lfpreset.IsSkullRadiusModulatedRatio())
                {
                    Verbose.Put("Skull absolute thickness is:", "Fixed");
                    Verbose.Put("Skull relative thickness is:", "Modulated by electrode");
                }
                else if (isthicknessfrommri)
                {
                    Verbose.Put("Skull absolute thickness is:", "Estimated from T1 MRI");
                }
                else if (isthicknessfromtissues)
                {
                    Verbose.Put("Skull absolute thickness is:", "Computed from segmented tissues");
                }

                if (lfpreset.IsGeometryLSMAC())
                {
                    // Other radii
                    if (lfpreset.NumLayers == 4)
                    {
                    ...
                    {
                        Verbose.Put("Compact skull absolute thickness is:", "Computed from skull thickness" /*isthicknessfrommri ? "Estimated from T1 MRI" : "Estimated from segmented tissues"*/);
            ...
            else if (loadleadfield)
            {
                ...
                Verbose.NextLine();
                Verbose.Put("Lead Field processing:", interpolateleadfield ? "Interpolation" : downsampleleadfield ? "Downsampling"
                                                                                                                   : "None");
        ...
        Verbose.NextTopic("Output Files:");
        {
            ...
            if (isthicknessfrommri || isthicknessfromtissues)
            {
            ...
            if (CrisTransfer.IsInverseProcessing())
                Verbose.NextLine();
            if (inversemn)
                Verbose.Put("Matrix file Minimum Norm:", filemneis);
            if (inversewmn)
                Verbose.Put("Matrix file Weighted Minimum Norm:", filewmneis);
            if (inversedale)
                Verbose.Put("Matrix file " InfixInverseDale ":", filedaleis);
            if (inversesloreta)
                Verbose.Put("Matrix file " InfixInverseSLoreta ":", filesloretais);
            if (inverseloreta)
                Verbose.Put("Matrix file " InfixInverseLoreta ":", fileloretais);
            if (inverselaura)
                Verbose.Put("Matrix file " InfixInverseLaura ":", filelaurais);
            if (inverseeloreta)
                Verbose.Put("Matrix file " InfixInverseELoreta ":", fileeloretais);

            if (computeresmat)
            {
                Verbose.NextLine();
                if (inversemn)
                    Verbose.Put("Resolution Matrix file MN:", filemnresmat);
                if (inversewmn)
                    Verbose.Put("Resolution Matrix file WMN:", filewmnresmat);
                if (inversedale)
                    Verbose.Put("Resolution Matrix file " InfixInverseDale ":", filedaleresmat);
                if (inversesloreta)
                    Verbose.Put("Resolution Matrix file " InfixInverseSLoreta ":", filesloretaresmat);
                if (inverseloreta)
                    Verbose.Put("Resolution Matrix file " InfixInverseLoreta ":", fileloretaresmat);
                if (inverselaura)
                    Verbose.Put("Resolution Matrix file " InfixInverseLaura ":", filelauraresmat);
                if (inverseeloreta)
                    Verbose.Put("Resolution Matrix file " InfixInverseELoreta ":", fileeloretaresmat);
            }
        }
        ...
        Gauge.AddPart(gaugecimsp,
                      (computesps ? 2 : 0) + (samespsaslf || loadsps ? 4 : 0) + (hassps ? 1 : 0), 2);

        Gauge.AddPart(gaugecimheadmodel, getheadmodel ? 1 + 3 : 0, 2);
        Gauge.AddPart(gaugecimleadfield,
                      (computeleadfield ? 3 + (isthicknessfrommri ? 7 : 0) : 0) + (loadleadfield ? 3 : 0) + (leadfieldprocessing ? 1 : 0), 5);
        ...
        */
        ...
        if (CrisTransfer.HasHeadMri())
        {
            ...
            if (interactive && !GetAnswerFromUser(reorientmris ? "Is the Full Head MRI orientation detection OK (Anterior, Superior..)?" NewLine NewLine
                                                                 "(check within the MRI window)"
                                                               : "Is the head MRI in 'RAS' orientation?" NewLine
                                                                     NewLine Tab "X -> Right" NewLine Tab "Y -> Anterior" NewLine Tab "Z -> Superior" NewLine NewLine
                                                                 "(check within the MRI window)",
                                                  MriHead->GetTitle(), this))
            {

                ShowMessage("Please, manually re-orient the full MRI, then come back here." NewLine NewLine
                            "This will end your processing now, bye bye...",
                            MriHead->GetTitle(), ShowMessageWarning, this);
            ...
            if ((OriginalBrainMriOrigin - OriginalMriOrigin).Norm() > 0.001)
            {
                ...
                if (interactive)
                {
                    ...
                    ShowMessage("It seems that the Brain and the Full Head MRIs don't have the same origin!" NewLine
                                "Check both MRIs, and set a single common origin in both files." NewLine NewLine
                                "This will end your processing now, bye bye...",
                                MriBrain->GetTitle(), ShowMessageWarning, this);
                ...
                else
                {
                    ...
                    if (OriginalMriOrigin.IsNull())
                        OriginalMriOrigin = OriginalBrainMriOrigin;
                    //          else if ( OriginalBrainMriOrigin.IsNull () )    OriginalBrainMriOrigin  = OriginalMriOrigin;    // not used
                    // not null, but only one is fishy?
                    else if (!MriHead->IsValidOrigin() && MriBrain->IsValidOrigin())
                        OriginalMriOrigin = OriginalBrainMriOrigin;

                    else if (MriHead->IsValidOrigin() && !MriBrain->IsValidOrigin())
                        OriginalBrainMriOrigin = OriginalMriOrigin;

                    // reset origin, which will trigger normalization if not already set
                    else
                        OriginalMriOrigin.Reset();
                }
            }

            Gauge.Next(gaugecimmribrain, SuperGaugeUpdateTitle);

            if (interactive && !GetAnswerFromUser(reorientmris ? "Is the Brain MRI orientation detection OK (Anterior, Superior..)?" NewLine NewLine
                                                                 "(check within the MRI window)"
                                                               : "Is the brain MRI in 'RAS' orientation?" NewLine
                                                                     NewLine Tab "X -> Right" NewLine Tab "Y -> Anterior" NewLine Tab "Z -> Superior" NewLine NewLine
                                                                 "(check within the MRI window)",
                                                  MriBrain->GetTitle(), this))
            {

                ShowMessage("Please, manually re-orient the brain MRI, then come back here." NewLine NewLine
                            "This will end your processing now, bye bye...",
                            MriBrain->GetTitle(), ShowMessageWarning, this);
        ...
        if (CrisTransfer.HasGreyMri())
        {
            ...
            if (interactive && !GetAnswerFromUser(reorientmris ? "Is the Grey Mask orientation detection OK (Anterior, Superior..)?" NewLine NewLine
                                                                 "(check within the MRI window)"
                                                               : "Is the grey mask MRI in 'RAS' orientation?" NewLine
                                                                     NewLine Tab "X -> Right" NewLine Tab "Y -> Anterior" NewLine Tab "Z -> Superior" NewLine NewLine
                                                                 "(check within the MRI window)",
                                                  MriGrey->GetTitle(), this))
            {

                ShowMessage("Please, manually re-orient the grey mask MRI, then come back here." NewLine NewLine
                            "This will end your processing now, bye bye...",
                            MriGrey->GetTitle(), ShowMessageWarning, this);
        ...
        if (isthicknessfromtissues)
        {
            ...
            if (interactive && !GetAnswerFromUser(reorientmris ? "Is the Tissues volume orientation detection OK (Anterior, Superior..)?" NewLine NewLine
                                                                 "(check within the MRI window)"
                                                               : "Is the tissues volume in 'RAS' orientation?" NewLine
                                                                     NewLine Tab "X -> Right" NewLine Tab "Y -> Anterior" NewLine Tab "Z -> Superior" NewLine NewLine
                                                                 "(check within the MRI window)",
                                                  MriTissues->GetTitle(), this))
            {

                ShowMessage("Please, manually re-orient the tissues volume, then come back here." NewLine NewLine
                            "This will end your processing now, bye bye...",
                            MriTissues->GetTitle(), ShowMessageWarning, this);
        ...
        */
        ...
        {
            // estimating center?
            bool estimatecenter = reorientmris && realignmri || OriginalMriOrigin.IsNull();
        ...
        if (reorientmris)
        {
            ...
            FilterTypes filtertype = !realignmri ? FilterTypeNone : MriHead->IsMask() ? FilterTypeMedian
                                                                                      : FilterTypeNone;
            InterpolationType interpolate = !realignmri ? InterpolateNearestNeighbor : MriHead->IsMask() ? InterpolateNearestNeighbor
                                                                                                         : InterpolateLanczos3;
            int numsubsampling = !realignmri ? 1 : MriHead->IsMask() ? 1
                                                                     : 1;
            ...
            if (fullmriview)
            {
                ...
                if (!GetAnswerFromUser(reorientmris ? "Does the Sagittal and/or Equator transformation seems OK in the full MRI?" NewLine NewLine
                                                      "(check within the MRI window)"
                                                    : "Does the Sagittal and/or Equator transformation seems OK in the full MRI?" NewLine NewLine
                                                      "(check within the MRI window)",
                                       MriHead->GetTitle(), this))
                {

                    ShowMessage("Something may have fooled the Sagittal and/or Equator detection, such as:" NewLine NewLine
                                "- Artifacts in the MRI -> clean-up MRI" NewLine
                                "- Bad background threshold detection -> check for noise" NewLine
                                "- Bad orientation detection -> check orientation" NewLine NewLine
                                "This will end your processing now, bye bye...",
                                MriHead->GetTitle(), ShowMessageWarning, this);
            ...
            {
                // nothing                                     discrete values                                   continuous values
                FilterTypes filtertype = !realignmri ? FilterTypeNone : MriBrain->IsMask() ? FilterTypeMedian
                                                                                           : FilterTypeNone;
                InterpolationType interpolate = !realignmri ? InterpolateNearestNeighbor : MriBrain->IsMask() ? InterpolateNearestNeighbor
                                                                                                              : InterpolateLanczos3;
                int numsubsampling = !realignmri ? 1 : MriBrain->IsMask() ? 3
                                                                          : 1;
            ...
            {
                // nothing                                     discrete values                                   continuous values
                FilterTypes filtertype = !realignmri ? FilterTypeNone : MriGrey->IsMask() ? FilterTypeMedian
                                                                                          : FilterTypeNone;
                InterpolationType interpolate = !realignmri ? InterpolateNearestNeighbor : MriGrey->IsMask() ? InterpolateNearestNeighbor
                                                                                                             : InterpolateLanczos3;
                int numsubsampling = !realignmri ? 1 : MriGrey->IsMask() ? 3
                                                                         : 1;
            ...
            {
                // nothing                                     discrete values                                   continuous values
                FilterTypes filtertype = !realignmri ? FilterTypeNone : MriTissues->IsMask() ? FilterTypeMedian
                                                                                             : FilterTypeNone;
                InterpolationType interpolate = !realignmri ? InterpolateNearestNeighbor : MriTissues->IsMask() ? InterpolateNearestNeighbor
                                                                                                                : InterpolateLanczos3;
                int numsubsampling = !realignmri ? 1 : MriTissues->IsMask() ? 3
                                                                            : 1;
        ...
        if ((MriHead.IsOpen() || MriBrain.IsOpen()) &&
            (reorientmris && MriCenter.IsNull() || !reorientmris && OriginalMriOrigin.IsNull()))
        {
            ...
            ShowMessage("Your input MRI files together with your current parameters seem to end up with" NewLine
                        "an undefined MRI center/origin:" NewLine
                            NewLine Tab "- You could use the 'Re-Align' options to search for an origin" NewLine
                        "or" NewLine Tab "- Pick some MRIs with correctly defined origins." NewLine NewLine
                        "This will end your processing now, bye bye...",
                        CreatingInverseTitle, ShowMessageWarning, this);
        ...
        if (computegrey && MriBrain.IsOpen())
        {
            ...
            if (!greymaskok) // default is to continue anyway
                ...
                if (interactive && !GetAnswerFromUser("The Grey Matter Mask extraction might have gone wrong, do you want to proceed anyway?" NewLine NewLine
                                                      "(check the Grey Matter window)",
                                                      MriGrey->GetTitle(), this))
                    goto endprocessing;
                ...
                if (!solpoints.IsGrid())
                {
                    ...
                    if (interactive)
                    {
                        if (!GetAnswerFromUser("The given Solution Points file and the current MRI preprocessing parameters" NewLine
                                               ...
                                               "This will very likely cause some errors later on." NewLine NewLine
                                               "Do you still want to proceed with these parameters (not recommended)?",
                                               FileInputSpLoad, this))
                            goto endprocessing;
                    } // interactive
                    else
                    {
                        ShowMessage("The given Solution Points file and the current MRI preprocessing parameters" NewLine
                                    ...
                                    "This will very likely cause some errors later on." NewLine NewLine
                                    "This will end your processing now, bye bye...",
                                    FileInputSpLoad, ShowMessageWarning, this);
                        goto endprocessing;
                    }
                } // ! IsGrid
                    ...
                    if (!sizeok)
                        if (interactive)
                        {
                            if (!GetAnswerFromUser("It seems that the Solution Points and the Brain Mri do not have the same size!" NewLine
                                                   "Make sure you picked the right files." NewLine NewLine
        ...
        if (xyzprocessing)
        {
            ...
            if (interactive && !xyzcoregistrationdone && !GetAnswerFromUser("Is the Electrodes orientation detection OK (Anterior, Superior..)?" NewLine NewLine "(check within the Electrodes window)", Xyz->GetTitle(), this))
            {

                ShowMessage("Please, manually re-orient the electrodes,and come back here." NewLine NewLine
                            "This will end your processing now, bye bye...",
                            Xyz->GetTitle(), ShowMessageWarning, this);
        ...
        if (getheadmodel)
        {
            ...
            if (interactive)
            {
                ...
                // then ask the user
                if (!GetAnswerFromUser(modelok ? "Can you confirm the model is correctly fitting the upper part of the head?" NewLine NewLine
                                                 "(check within the MRI window)"
                                               : "The model might be wrong, do you still want to proceed?" NewLine NewLine
                                                 "(check within the MRI window)",
                                       MriHead->GetTitle(), this))
                {

                    ShowMessage("There seems to be a problem with the head model," NewLine
                                "try to figure out what is wrong with your data and come back here." NewLine NewLine
                                "This will end your processing now, bye bye...",
                                MriHead->GetTitle(), ShowMessageWarning, this);
                    goto endprocessing;
                }
        ...
        else if (xyzcoregistertomri)
        {
            ...
            if (xyzcoregistertomriiredo)
            { // reload an already coregistered xyz, just to refine/adjust it

                if (realignmri)
                    normtoxyz = NormToAbsOriginalMRI;
                else if (reorientmris)
                    normtoxyz = NormToAbsOriginalMRI;
                else
                    normtoxyz.SetIdentity();
            }
            ...
            { // xyzcoregistertomrii
              // Center
                ...
                if (realignmri)
                    normtoxyz = Xyz->GetStandardOrientation(RASToLocal);
                else if (reorientmris)
                    normtoxyz = NormToAbsOriginalMRI;
                else
                    normtoxyz.SetIdentity();
        ...
        if (MriHead.IsOpen() && xyzcoregistertomri)
        {
            ...
            if (!coregistrationok)
            {
                ...
                //      if ( interactive )
                ShowMessage("You cancelled the electrodes coregistration!" NewLine NewLine
                            "This will end your processing now, bye bye...",
                            Xyz->GetTitle(), ShowMessageWarning, this);
            ...
            if (interactive)
            {
                ...
                // then ask the user
                if (!GetAnswerFromUser("Are the electrodes correctly coregistered?" NewLine NewLine
                                       "(check within the MRI window)",
                                       Xyz->GetTitle(), this))
                {

                    ShowMessage("Check your input data and parameters for something wrong," NewLine
                                "and come back here." NewLine NewLine
                                "This will end your processing now, bye bye...",
                                Xyz->GetTitle(), ShowMessageWarning, this);
                    goto endprocessing;
                }
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                ...
                for (int i = 0; i < (int)sphintcsf; i++)
                    sphintcsf[i] *= TissuesRadii(i, CsfIndex, InnerRel);
                for (int i = 0; i < (int)sphintskull; i++)
                    sphintskull[i] *= TissuesRadii(i, SkullIndex, InnerRel);
                for (int i = 0; i < (int)sphextskull; i++)
                    sphextskull[i] *= TissuesRadii(i, SkullIndex, OuterRel);
                ...
                if (TissuesRadii.IsAllocated())
                {
                    ...
                    // Handy file to visualize the thicknesses as maps
                    {
                        ...
                        for (TIteratorSelectedForward ti(seltissues); (bool)ti; ++ti)
                            if (ti() != BrainIndex)
                                for (int ei = 0; ei < numel; ei++)
                                    exprad.Write(TissuesRadii(ei, ti(), ThickAbs));
                    }
                } // TissuesRadii
                ...
                if (!radiusok)
                {
                    ...
                    ShowMessage("There appear to be some major errors in the scalp or skull thicknesses estimations!" NewLine
                                "(check the verbose file for details)" NewLine NewLine
                                "This will end your processing now, bye bye...",
                                MriHead->GetTitle(), ShowMessageWarning, this);
                    ...
                    if (interactive)
                        ...
                        for (int ei = 0; ei < numel; ei++)
                        {
                            ...
                            if (TissuesRadii(ei, CsfIndex, ThickAbs) < 0 || TissuesRadii(ei, SkullIndex, ThickAbs) < 0 || TissuesRadii(ei, SkullSpongyIndex, ThickAbs) < 0 || TissuesRadii(ei, ScalpIndex, ThickAbs) < 0)
                            {

                                StringCopy(buff, "Electrode ", Xyz->GetElectrodeName(ei), " / #", IntegerToString(ei + 1), " has the wrong radii:" NewLine);

                                if (seltissues[CsfIndex])
                                    StringAppend(buff, NewLine Tab "Csf          Thickness = ", FloatToString(TissuesRadii(ei, CsfIndex, ThickAbs), 2));
                                if (seltissues[SkullIndex])
                                    StringAppend(buff, NewLine Tab "Skull        Thickness = ", FloatToString(TissuesRadii(ei, SkullIndex, ThickAbs), 2));
                                if (seltissues[SkullSpongyIndex])
                                    StringAppend(buff, NewLine Tab "Spongy Skull Thickness = ", FloatToString(TissuesRadii(ei, SkullSpongyIndex, ThickAbs), 2));
                                if (seltissues[ScalpIndex])
                                    StringAppend(buff, NewLine Tab "Scalp        Thickness = ", FloatToString(TissuesRadii(ei, ScalpIndex, ThickAbs), 2));
            ...
            else if (loadleadfield)
            {
                ...
                if (!leadfield.Set(FileInputLeadFieldLoad))
                {
                    ...
                    ShowMessage("There seems to be a problem with the Lead Field file, which can be either:" NewLine
                                    NewLine Tab "- The file type is not recognized" NewLine Tab "- The Lead Field data type is not of vectorial type" NewLine Tab "- The file itself might be somehow corrupted" NewLine NewLine
                                "This will end your processing now, bye bye...",
                                FileInputLeadFieldLoad, ShowMessageWarning, this);
                    goto endprocessing;
                }
                ...
                if (samespsaslf && loadleadfield && numsolp != numleadfieldsolpoints)
                {
                    ...
                    if (interactive)
                    {
                        ...
                        if (!GetAnswerFromUser("The given Solution Points and Lead Field files seem to have different sizes." NewLine
                                               "Please check again your input." NewLine NewLine
                                               "Do you still want to proceed (not recommended)?",
                                               FileInputSpLeadField, this))
                            goto endprocessing;
                    } // interactive
                    else
                    {
                        ShowMessage("The given Solution Points and Lead Field files seem to have different sizes." NewLine
                                    "Please check again your input." NewLine NewLine
                                    "This will end your processing now, bye bye...",
                                    FileInputSpLeadField, ShowMessageWarning, this);
                        goto endprocessing;
                    }
                } // ! regular
