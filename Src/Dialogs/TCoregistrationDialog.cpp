...
namespace crtl
{
                                       ...
                                       TMatrix44 &XyzCoregToNorm)
    {
        ...
        do
            UpdateApplication while (CoregistrationDlg != 0);
        ...
        : TBaseDialog(parent, resId)
    {
        ...
        if (usingopenview)
        {
            ...
            if (!(dynamic_cast<TElectrodesView *>(BaseView) || dynamic_cast<TVolumeView *>(BaseView)))
                BaseView = 0;
            ...
            if (BaseView == 0 && countxyzs == 1)
                BaseView = lastxyzdoc->GetViewList();
            if (MriView == 0 && countmris == 1)
                MriView = dynamic_cast<TVolumeView *>(lastmridoc->GetViewList());
        }
    ...
    TCoregistrationDialog::~TCoregistrationDialog()
    {
        ...
        delete Presets;
        delete InputSourceFile;
        delete InputTargetFile;
        delete Glue;
        delete BaseFileName;
        delete SavingAltElectrodes;
    }
    ...
    bool TCoregistrationDialog::PreProcessMsg(MSG &msg)
    {
        ...
        if (::IsDialogMessage(GetHandle(), &msg))
        {
            ...
            if (msg.message == WM_KEYDOWN)
            {
                ...
                if (Operation == '*')
                    CmAction(IDC_SCALEGLOBALPLUS);

                else if (Operation == '/')
                    CmAction(IDC_SCALEGLOBALMINUS);
                ...
                else if (Operation == 'R' && Axis)
                    CmAction(Axis == 'X'   ? (Direction == '+' ? IDC_ROTATEXPLUS : IDC_ROTATEXMINUS)
                             : Axis == 'Y' ? (Direction == '+' ? IDC_ROTATEYPLUS : IDC_ROTATEYMINUS)
                             : Axis == 'Z' ? (Direction == '+' ? IDC_ROTATEZPLUS : IDC_ROTATEZMINUS)
                                           : 0);

                else if (Operation == 'T' && Axis)
                    CmAction(Axis == 'X'   ? (Direction == '+' ? IDC_TRANSLATEXPLUS : IDC_TRANSLATEXMINUS)
                             : Axis == 'Y' ? (Direction == '+' ? IDC_TRANSLATEYPLUS : IDC_TRANSLATEYMINUS)
                             : Axis == 'Z' ? (Direction == '+' ? IDC_TRANSLATEZPLUS : IDC_TRANSLATEZMINUS)
                                           : 0);

                else if (Operation == 'S' && Axis)
                    CmAction(Axis == 'X'   ? (Direction == '+' ? IDC_SCALEXPLUS : IDC_SCALEXMINUS)
    ...
    void TCoregistrationDialog::SetProcessing(bool computemasks, bool docleanup)
    {
        ...
        { // no view given, guess from current entries
            if (IsExtensionAmong(CoregTransfer.InputSourceFile, AllCoordinatesFilesExt))
                Processing = preset;
        }
        ...
        XyzView = Processing == CoregisterXyzToMriScratch || Processing == CoregisterXyzToMriReload ? dynamic_cast<TElectrodesView *>(BaseView) : 0;
        ...
        if (Processing == CoregisterXyzToMriScratch)
        {
            ...
            Transform.SourceToTargetOrientation = mridoc->GetStandardOrientation(RASToLocal) * xyzdoc->GetStandardOrientation(LocalToRAS) * TMatrix44(-XyzCenterGeom); // translation to geometrical center
    ...
    {
        if (MriView == 0 || XyzView == 0 || IsEmpty(BaseFileName))
        {
    ...
    void TCoregistrationDialog::CmOk()
    {
        ...
        if (UndoMatrices.IsEmpty())
        {
            ...
            if (!GetAnswerFromUser("It seems you have no current transform to be applied..." NewLine NewLine
    ...
    void TCoregistrationDialog::CmAction(owlwparam op)
    {
        ...
        case IDC_RESETTRANSFORM:
            ...
            if (!(UndoMatrices.IsEmpty() && RedoMatrices.IsEmpty()) && !GetAnswerFromUser("Are you sure you really want to erase your whole transform history?", CoregistrationTitle))
                return;
        ...
        case IDC_SCALEZMINUS:
            ...
            if (op == IDC_SCALEGLOBALPLUS || op == IDC_SCALEGLOBALMINUS)
                Transform.Scale(v);
            else if (op == IDC_SCALEXPLUS || op == IDC_SCALEXMINUS)
                Transform.ScaleX(v);
            else if (op == IDC_SCALEYPLUS || op == IDC_SCALEYMINUS)
                Transform.ScaleY(v);
            else
                Transform.ScaleZ(v);
        ...
        case IDC_TRANSLATEZMINUS:
            ...
            if (op == IDC_TRANSLATEXPLUS || op == IDC_TRANSLATEXMINUS)
                Transform.TranslateX(v);
            else if (op == IDC_TRANSLATEYPLUS || op == IDC_TRANSLATEYMINUS)
        ...
        case IDC_ROTATEZMINUS:
            ...
            if (op == IDC_ROTATEXPLUS || op == IDC_ROTATEXMINUS)
                Transform.RotateX(v);
            else if (op == IDC_ROTATEYPLUS || op == IDC_ROTATEYMINUS)
                Transform.RotateY(v);
            else
                Transform.RotateZ(v);
