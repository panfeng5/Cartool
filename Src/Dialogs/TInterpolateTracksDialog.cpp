...
namespace crtl
{
    ...
    TInterpolateTracksDialog::~TInterpolateTracksDialog()
    {
        ...
        delete SurfaceSpline;
        delete SphericalSpline;
        delete ThreeDSpline;
        delete CDSpherical;
        delete Degree;
        delete FileTypes;
    ...
    void TInterpolateTracksDialog::CmBrowseXyzFileName(owlwparam w)
    {
        ...
        if (w == IDC_BROWSEFROMXYZ)
            CmFromXyzChange();
        else if (w == IDC_BROWSETOXYZ)
            CmToXyzChange();
    }
    ...
    bool TInterpolateTracksDialog::AreParametersOk()
    {
        ...
        bool fromlandmarksok = StringIsNotEmpty(InterpolateTracksTransfer.FromFront) && StringIsNotEmpty(InterpolateTracksTransfer.FromLeft) && StringIsNotEmpty(InterpolateTracksTransfer.FromTop) && StringIsNotEmpty(InterpolateTracksTransfer.FromRight) && StringIsNotEmpty(InterpolateTracksTransfer.FromRear);
        ...
        if (CheckToBool(InterpolateTracksTransfer.AnotherXyz))
        {
            ...
            bool tolandmarksok = StringIsNotEmpty(InterpolateTracksTransfer.ToFront) && StringIsNotEmpty(InterpolateTracksTransfer.ToLeft) && StringIsNotEmpty(InterpolateTracksTransfer.ToTop) && StringIsNotEmpty(InterpolateTracksTransfer.ToRight) && StringIsNotEmpty(InterpolateTracksTransfer.ToRear);
    ...
    void TInterpolateTracksDialog::CmFromXyzChange()
    {
        ...
        // 10-10 system like
        if ((DocFromXyz->GetElectrodeName("T3") || DocFromXyz->GetElectrodeName("T7")) && (DocFromXyz->GetElectrodeName("T4") || DocFromXyz->GetElectrodeName("T8")) && (DocFromXyz->GetElectrodeName("Fpz") || DocFromXyz->GetElectrodeName("Fp1") && DocFromXyz->GetElectrodeName("Fp2")) && (DocFromXyz->GetElectrodeName("Oz") || DocFromXyz->GetElectrodeName("O1") && DocFromXyz->GetElectrodeName("O2")) && DocFromXyz->GetElectrodeName("Cz"))
        {

            if (DocFromXyz->GetElectrodeName("T3"))
                FromLeft->SetText(DocFromXyz->GetElectrodeName("T3"));
            else if (DocFromXyz->GetElectrodeName("T7"))
                FromLeft->SetText(DocFromXyz->GetElectrodeName("T7"));

            if (DocFromXyz->GetElectrodeName("T4"))
                FromRight->SetText(DocFromXyz->GetElectrodeName("T4"));
            else if (DocFromXyz->GetElectrodeName("T8"))
                FromRight->SetText(DocFromXyz->GetElectrodeName("T8"));

            if (DocFromXyz->GetElectrodeName("Fpz"))
                FromFront->SetText(DocFromXyz->GetElectrodeName("Fpz"));
            else if (DocFromXyz->GetElectrodeName("Fp1"))
                FromFront->SetText("Fp1 Fp2");

            if (DocFromXyz->GetElectrodeName("Oz"))
                FromRear->SetText(DocFromXyz->GetElectrodeName("Oz"));
            else if (DocFromXyz->GetElectrodeName("O1"))
                FromRear->SetText("O1 O2");
        ...
        else if (DocFromXyz->GetElectrodeName("46") && DocFromXyz->GetElectrodeName("109") && (DocFromXyz->GetElectrodeName("15") && DocFromXyz->GetElectrodeName("18")) && DocFromXyz->GetElectrodeName("76") && (DocFromXyz->GetElectrodeName("VREF") || DocFromXyz->GetElectrodeName("REF")))
        {

            // EGI?
            if (StringIs(DocFromXyz->GetElectrodeName(/*0*/), "1") // compiler whining about overloaded function
                && StringIs(DocFromXyz->GetElectrodeName(1), "2") && StringIs(DocFromXyz->GetElectrodeName(2), "3"))
        ...
        else if (DocFromXyz->GetElectrodeName("E39") && DocFromXyz->GetElectrodeName("E115") && (DocFromXyz->GetElectrodeName("E14") && DocFromXyz->GetElectrodeName("E15") && DocFromXyz->GetElectrodeName("E21")) && DocFromXyz->GetElectrodeName("E75"))
        { // or E75 E71 E76?
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Biosemi 256
        else if (DocFromXyz->GetElectrodeName("140") && DocFromXyz->GetElectrodeName("82") && DocFromXyz->GetElectrodeName("1") && DocFromXyz->GetElectrodeName("203") && DocFromXyz->GetElectrodeName("19"))
        {
        ...
        else if (DocFromXyz->GetElectrodeName("D8") && DocFromXyz->GetElectrodeName("E9") && DocFromXyz->GetElectrodeName("A1") && DocFromXyz->GetElectrodeName("B32") && DocFromXyz->GetElectrodeName("A23"))
        {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Biosemi 128
        else if (DocFromXyz->GetElectrodeName("C17") && DocFromXyz->GetElectrodeName("D23") && DocFromXyz->GetElectrodeName("A1") && DocFromXyz->GetElectrodeName("B23") && DocFromXyz->GetElectrodeName("A23"))
        {
    ...
    void TInterpolateTracksDialog::CmToXyzChange()
    {
        ...
        // 10-10 system like
        if ((DocToXyz->GetElectrodeName("T3") || DocToXyz->GetElectrodeName("T7")) && (DocToXyz->GetElectrodeName("T4") || DocToXyz->GetElectrodeName("T8")) && (DocToXyz->GetElectrodeName("Fpz") || DocToXyz->GetElectrodeName("Fp1") && DocToXyz->GetElectrodeName("Fp2")) && (DocToXyz->GetElectrodeName("Oz") || DocToXyz->GetElectrodeName("O1") && DocToXyz->GetElectrodeName("O2")) && DocToXyz->GetElectrodeName("Cz"))
        {

            if (DocToXyz->GetElectrodeName("T3"))
                ToLeft->SetText(DocToXyz->GetElectrodeName("T3"));
            else if (DocToXyz->GetElectrodeName("T7"))
                ToLeft->SetText(DocToXyz->GetElectrodeName("T7"));

            if (DocToXyz->GetElectrodeName("T4"))
                ToRight->SetText(DocToXyz->GetElectrodeName("T4"));
            else if (DocToXyz->GetElectrodeName("T8"))
                ToRight->SetText(DocToXyz->GetElectrodeName("T8"));

            if (DocToXyz->GetElectrodeName("Fpz"))
                ToFront->SetText(DocToXyz->GetElectrodeName("Fpz"));
            else if (DocToXyz->GetElectrodeName("Fp1"))
                ToFront->SetText("Fp1 Fp2");

            if (DocToXyz->GetElectrodeName("Oz"))
                ToRear->SetText(DocToXyz->GetElectrodeName("Oz"));
            else if (DocToXyz->GetElectrodeName("O1"))
                ToRear->SetText("O1 O2");
        ...
        else if (DocToXyz->GetElectrodeName("46") && DocToXyz->GetElectrodeName("109") && (DocToXyz->GetElectrodeName("15") && DocToXyz->GetElectrodeName("18")) && DocToXyz->GetElectrodeName("76") && (DocToXyz->GetElectrodeName("VREF") || DocToXyz->GetElectrodeName("REF")))
        {

            // EGI?
            if (StringIs(DocToXyz->GetElectrodeName(/*0*/), "1") // compiler whining about overloaded function
                && StringIs(DocToXyz->GetElectrodeName(1), "2") && StringIs(DocToXyz->GetElectrodeName(2), "3"))
        ...
        else if (DocToXyz->GetElectrodeName("E39") && DocToXyz->GetElectrodeName("E115") && (DocToXyz->GetElectrodeName("E14") && DocToXyz->GetElectrodeName("E15") && DocToXyz->GetElectrodeName("E21")) && DocToXyz->GetElectrodeName("E75"))
        { // or E75 E71 E76?
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Biosemi 256
        else if (DocToXyz->GetElectrodeName("140") && DocToXyz->GetElectrodeName("82") && DocToXyz->GetElectrodeName("1") && DocToXyz->GetElectrodeName("203") && DocToXyz->GetElectrodeName("19"))
        {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Biosemi 192 tested before Biosemi 128
        else if (DocToXyz->GetElectrodeName("D8") && DocToXyz->GetElectrodeName("E9") && DocToXyz->GetElectrodeName("A1") && DocToXyz->GetElectrodeName("B32") && DocToXyz->GetElectrodeName("A23"))
        {
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Biosemi 128
        else if (DocToXyz->GetElectrodeName("C17") && DocToXyz->GetElectrodeName("D23") && DocToXyz->GetElectrodeName("A1") && DocToXyz->GetElectrodeName("B23") && DocToXyz->GetElectrodeName("A23"))
        {
    ...
    void TInterpolateTracksDialog::CmCopyLandmarks(owlwparam w)
    {
        ...
        {
            ToLeft->GetText(buff, EditSizeValue);
            FromLeft->SetText(buff);
            ToRight->GetText(buff, EditSizeValue);
            FromRight->SetText(buff);
            ToFront->GetText(buff, EditSizeValue);
            FromFront->SetText(buff);
            ToRear->GetText(buff, EditSizeValue);
            FromRear->SetText(buff);
            ToTop->GetText(buff, EditSizeValue);
            FromTop->SetText(buff);
            ToDescription->GetText(buff, EditSizeValue);
            FromDescription->SetText(buff);
        }

        else if (w == IDC_COPYFROMLANDMARKS)
        {
            FromLeft->GetText(buff, EditSizeValue);
            ToLeft->SetText(buff);
            FromRight->GetText(buff, EditSizeValue);
            ToRight->SetText(buff);
            FromFront->GetText(buff, EditSizeValue);
            ToFront->SetText(buff);
            FromRear->GetText(buff, EditSizeValue);
            ToRear->SetText(buff);
            FromTop->GetText(buff, EditSizeValue);
            ToTop->SetText(buff);
            FromDescription->GetText(buff, EditSizeValue);
            ToDescription->SetText(buff);
        }
    }
    ...
    void TInterpolateTracksDialog::ProcessCurrent(void *usetransfer, const char * /*moreinfix*/)
    {
        ...
        if ((int)fromgoodsel != fromnumpoints)
            ...
            StringCopy(buff, "Number of electrodes does not match:" NewLine "    ", IntegerToString((int)fromgoodsel), "  in EEG" NewLine "    ", IntegerToString(fromnumpoints), "  in XYZ" NewLine "Skipping this file!");

        if (EEGDoc->GetNumAuxElectrodes() != 0)

            StringCopy(buff, IntegerToString(EEGDoc->GetNumAuxElectrodes()), " auxiliaries have been found" NewLine "Skipping this file!");
