...
namespace crtl
{
    ...
    TFrequencyAnalysisDialog::~TFrequencyAnalysisDialog()
    {
        ...
        delete TimeMin;
        delete TimeMax;
        delete ClippedTimeMax;
        delete EndOfFile;
        delete NumBlocks;
        delete BlockSize;
        delete WindowOverlap0;
        delete WindowOverlap75;
        delete WindowOverlapMax;
        ...
        delete SaveFreqMax;
        delete SaveFreqStep;
        delete SaveLogInterval;
        delete SaveDecadeStep;
        delete SaveBandsValue;
        delete Presets;
        delete Fft;
        delete FftApproximation;
        delete STransform;
        delete WindowingNone;
        delete WindowingHanning;
        delete Mean;
        delete Sequence;
        ...
        delete CreateSubdir;
        delete OpenAuto;
        delete SplitByElectrode;
        delete SplitByFrequency;
        delete SplitBySpectrum;
    ...
    int TFrequencyAnalysisDialog::GetDefaultBlocksize()
    {
            ...
            if (BatchProcessing)
        {
            ...
            if (sf)
                blocksize = Clip(Round(SecondsToTimeFrame(2, sf)), MinBlockSize, MaxInitBlockSize);
            else
                blocksize = Clip(GetInteger(BlockSize), MinBlockSize, MaxInitBlockSize);
        }
        else
        {
    ...
    void TFrequencyAnalysisDialog::CmOtherRefEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(IsChecked(OtherRef) && (IsRegularFFT(CurrentPreset) || IsSTransform(CurrentPreset)));
    }
    ...
    bool TFrequencyAnalysisDialog::IsValidTime()
    {
        ...
        return (IsChecked(EndOfFile) || (numblocks > 0 && timenum > 0)) && (IsSTransform(CurrentPreset) || blocksize >= MinBlockSize /*&& IsPower2 ( blocksize )*/);
    }
    ...
    bool TFrequencyAnalysisDialog::IsValidFreq()
    {
        ...
        if (IsChecked(SaveBands) && IsSpace(SaveBandsValue))
            return false;
        ...
        if (IsChecked(SaveInterval) && (!IsInsideLimits(savefmin, savefmax, fmin, fmax) || savefmin > savefmax))
            return false;
    ...
    bool TFrequencyAnalysisDialog::IsProcessEnable()
    {
        ...
        if (IsChecked(UseXyzDoc) && IsSpace(XyzDocFile))
            return false;

        if (IsChecked(OtherRef) && IsSpace(RefList))
            return false;

        if (IsChecked(SaveBands) && IsSpace(SaveBandsValue))
            return false;

        if (!(IsChecked(FileTypeFreq) || IsChecked(SplitByElectrode) || IsChecked(SplitByFrequency) || IsChecked(SplitBySpectrum)))
            return false;
    ...
    {
        if (IsChecked(WindowOverlap0))
            return 0.00;
        if (IsChecked(WindowOverlap75))
            return 0.75;
        if (IsChecked(WindowOverlapMax))
            return blocksize ? (double)(blocksize - 1) / blocksize : 0;
    ...
    {
        if (IsSTransform(CurrentPreset))
            return timespan >= blocksize ? 1 : 0;
        else if (IsChecked(WindowOverlapMax))
            return timespan >= blocksize ? timespan - (blocksize - 1) : 0; // avoid rounding error
        else
            return timespan >= blocksize ? (int)(((double)timespan / blocksize - blocksoverlap) / (1 - blocksoverlap)) : 0;
    }

    int ComputeTimeMax(bool isstransform, bool windowoverlapmax, int timemin, int blocksize, double blocksoverlap, int numblocks)
    {
        if (isstransform)
            return timemin + blocksize - (blocksize ? 1 : 0);
        else if (windowoverlapmax)
            return timemin + blocksize + (numblocks - 1) - 1;
        else
            return timemin + blocksize * (blocksoverlap + (1 - blocksoverlap) * numblocks) - 1;
    }
    ...
    {
        if (IsSTransform(CurrentPreset) || !sf)
            return 0;
        else if (IsChecked(WindowOverlapMax))
            return TimeFrameToMilliseconds(1, sf); // avoid rounding error
        else
            return TimeFrameToMilliseconds(Truncate(blocksize * (1 - blocksoverlap)), sf);
    }
    ...
    int TFrequencyAnalysisDialog::ComputeStep(int blocksize, double blocksoverlap)
    {
        if (IsSTransform(CurrentPreset))
            ...
            return blocksize * (1 - blocksoverlap);
    }
    ...
    */
    ...
    void TFrequencyAnalysisDialog::CmNumBlocksChange()
    {
        ...
        // params uncheckables?
        if (timenum <= 0 || blocksize < MinBlockSize /*|| ! IsPower2 ( blocksize )*/
            || numblocks <= 0 || IsChecked(EndOfFile))
        {
            // if ( ! IsValidTime () ) {
    ...
    */
    ...
    void TFrequencyAnalysisDialog::CmSetAnalysis()
    {
        ...
        // set reference
        if (IsPowerMaps(CurrentPreset) || IsFFTApproximation(CurrentPreset))
        {
        ...
        // set output format
        if (IsRegularFFT(CurrentPreset) && IsIndex(transfer.WriteType, OutputAtomReal) || IsPowerMaps(CurrentPreset) || IsSTransform(CurrentPreset))
        {
    ...
    {
        if ((IsIndex(WriteType, OutputAtomComplex) || IsIndex(WriteType, OutputAtomPhase)) && IsChecked(Mean))
        {
    ...
    {
        if (IsIndex(WriteType, OutputAtomComplex) || IsIndex(WriteType, OutputAtomPhase))
        {
    ...
    void TFrequencyAnalysisDialog::CmWriteType()
    {
        ...
        if (IsIndex(WriteType, OutputAtomReal) && !IsFFTApproximation(CurrentPreset))
        {
        ...
        else if ((IsIndex(WriteType, OutputAtomNorm) || IsIndex(WriteType, OutputAtomNorm2)) && !(IsRegularFFT(CurrentPreset) || IsSTransform(CurrentPreset) || IsPowerMaps(CurrentPreset)))
        {
        ...
        // does not currently happen because CmWriteTypeEnable prevents this
        else if (IsIndex(WriteType, OutputAtomComplex) && !(IsRegularFFT(CurrentPreset) || IsSTransform(CurrentPreset) || IsPowerMaps(CurrentPreset)))
        {
    ...
    {
        tce.Enable(!(IsFFTApproximation(CurrentPreset) || CurrentPreset == FreqPresetSurfaceStransfForESI || CurrentPreset == FreqPresetIntraStransfPhase));
    }
    ...
    void TFrequencyAnalysisDialog::ProcessCurrent(void *usetransfer, const char * /*moreinfix*/)
    {
        ...
        bool openxyz = CheckToBool(transfer->UseXyzDoc) && CanOpenFile(transfer->XyzDocFile);
        ...
        if (CheckToBool(transfer->NoRef))
            ref = ReferenceAsInFile;
        else if (CheckToBool(transfer->AvgRef))
            ref = ReferenceAverage;
        else if (CheckToBool(transfer->CurrentRef))
        ...
        bool outputsequential = CheckToBool(transfer->Sequence) || (analysis == FreqAnalysisSTransform);
        ...
        bool openauto = CheckToBool(transfer->OpenAuto) && (int)BatchFileNames <= MaxFilesToOpen;
