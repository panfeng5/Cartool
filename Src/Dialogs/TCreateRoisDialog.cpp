...
namespace crtl
{
        ...
        : TBaseDialog(parent, resId)
    {
        ...
        if (!(dynamic_cast<TTracksView *>(BaseView) || dynamic_cast<TElectrodesView *>(BaseView) || dynamic_cast<TSolutionPointsView *>(BaseView) || dynamic_cast<TVolumeView *>(BaseView)))
            BaseView = 0;
    ...
    {
        delete InputFile;
        delete UseRoisMriFile;
        delete RoisMriFile;
        delete RoisLabelsFile;
        delete Automatic;
        delete Interactive;
        delete AutoTalairach;
        delete AutoAAL;
        delete AutoOtherRois;
    ...
    void TCreateRoisDialog::SetProcessing(bool docleanup)
    {
        ...
        if (CreateRoisTransfer.IsInteractive())
        {
            ...
            if (BaseView)
            { // BaseView specified will force our choice
                if (dynamic_cast<TTracksView *>(BaseView))
                    Processing = CreateRoisInteractiveTracks;
                else if (dynamic_cast<TElectrodesView *>(BaseView))
            ...
            { // no view given, guess from current entries
                if (IsExtensionAmong(CreateRoisTransfer.InputFile, AllTracksFilesExt))
                    Processing = CreateRoisInteractiveTracks;
                else if (IsExtensionAmong(CreateRoisTransfer.InputFile, AllCoordinatesFilesExt))
                    Processing = CreateRoisInteractiveXyz;
                else if (IsExtensionAmong(CreateRoisTransfer.InputFile, AllSolPointsFilesExt))
                    Processing = CreateRoisInteractiveSp;
            }
        ...
        { // automatic
            ...
            { // BaseView specified will force our choice
                ...
                if (dynamic_cast<TSolutionPointsView *>(BaseView))
                {
                    ...
                    if (CreateRoisTransfer.IsAutoSpMri() && CreateRoisTransfer.IsRoisMriFile() && IsExtensionAmong(CreateRoisTransfer.RoisMriFile, AllMriFilesExt))
                        Processing = CreateRoisTransfer.IsAutoAAL() ? CreateRoisAutomaticSpAAL
                                                                    : CreateRoisAutomaticSpMri;
                    else if (CreateRoisTransfer.IsAutoTalairach() && CanTalairach())
                        Processing = CreateRoisAutomaticSpTalairach;
                }
                // file was open, don't close when done
                if (docleanup)
                    CanCloseBaseView = false;
            ...
            { // MriView specified will force our choice
        ...
        SpView = Processing == CreateRoisInteractiveSp || Processing == CreateRoisAutomaticSpMri || Processing == CreateRoisAutomaticSpAAL || Processing == CreateRoisAutomaticSpTalairach ? dynamic_cast<TSolutionPointsView *>(BaseView) : 0;
    ...
    {
        return Taloracle.IsAllocated() && !BaseView || (BaseView && BaseView->GetGeometryTransform() && StringIs(BaseView->GetGeometryTransform()->Name, "Talairach"));
    }
    ...
    void TCreateRoisDialog::UpdateSelection(bool getselection)
    {
        ...
        else
        {
            ...
            if (CreateRoisTransfer.IsInteractive() && (EegView || XyzView || SpView))
            {
                ...
                if (EegView)
                    EegView->VnNewHighlighted(SelForRoi);
                else if (XyzView)
                    XyzView->VnNewHighlighted(SelForRoi);
                else if (SpView)
                    SpView->VnNewHighlighted(SelForRoi);
            }
    ...
    void TCreateRoisDialog::CmAddSelectionEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(CreateRoisTransfer.IsInteractive() && (EegView || XyzView || SpView) && (!Rois || Rois->GetAtomsNotSelected()->NumSet() != 0) && SelForRoi && SelForRoi->NumSet() != 0);
    }
    ...
    void TCreateRoisDialog::CmRemoveSelectionEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(CreateRoisTransfer.IsInteractive() && (EegView || XyzView || SpView) && Rois && Rois->GetNumRois());
    }
    ...
    void TCreateRoisDialog::AddRoiSummary()
    {
        ...
        {
            if (EegView)
                toroi->Selection.ToText(buff2, Names, AuxiliaryTracksNames);
            else if (XyzView)
                toroi->Selection.ToText(buff2, Names);
            else if (SpView)
                toroi->Selection.ToText(buff2, Names);
        }
    ...
    void TCreateRoisDialog::CmOkEnable(TCommandEnabler &tce)
    {
        ...
        if (CreateRoisTransfer.IsAutoTalairach() && SpView && CreateRoisTransfer.IsGenerateRoisFile() && CanTalairach())
        {
        ...
        if (CreateRoisTransfer.IsAutoSpMri() && SpView && CreateRoisTransfer.IsRoisMriFile() && (!CreateRoisTransfer.IsAutoOtherRois() || CreateRoisTransfer.IsRoisLabelsFile()) && (StringIsEmpty(CreateRoisTransfer.GenerateRoisFile) || CreateRoisTransfer.IsGenerateRoisFile()))
        {
        ...
        if (CreateRoisTransfer.IsInteractive() && (EegView || XyzView || SpView) && Rois && Rois->GetNumRois() != 0)
        {
            tce.Enable(true);
            return;
        }
    ...
    void TCreateRoisDialog::CmOk()
    {
        ...
        if (CreateRoisTransfer.IsInteractive() && (EegView || XyzView || SpView) && Rois)
        {
        ...
        else if (CreateRoisTransfer.IsAutomatic() && SpView)
        {
            ...
            if ((Processing == CreateRoisAutomaticSpMri || Processing == CreateRoisAutomaticSpAAL) && MriView)
            {
            ...
            else if (Processing == CreateRoisAutomaticSpTalairach && StringIsNotEmpty(CreateRoisTransfer.GenerateRoisFile))
            {
    ...
    void TCreateRoisDialog::EvDropFiles(TDropInfo drop)
    {
        ...
        if (((bool)tracksfiles || (bool)xyzfiles || (bool)spfiles) && !CanCloseBaseView && BaseView)
        {
            ...
            StringCopy(buff, "You are already working with file:" NewLine NewLine,
                       BaseView->BaseDoc->GetTitle(), NewLine NewLine "Close this Dialog, select another window (or none)" NewLine "if you want to work on another file.");
        ...
        if ((bool)mrifiles && !CanCloseMriView && MriView)
        {
            ...
            StringCopy(buff, "You are already working with file:" NewLine NewLine,
                       MriView->BaseDoc->GetTitle(), NewLine NewLine "Close this Dialog, select another window (or none)" NewLine "if you want to work on another file.");
