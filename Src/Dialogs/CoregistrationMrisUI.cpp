...
namespace crtl
{
    ...
    void CoregistrationSpecsType::ToVerbose(TVerboseFile &verbose) const
    {
        ...
        if (NumTranslations > 0)
            verbose.Put("Number of Translations:", NumTranslations);
        if (NumRotations > 0)
            verbose.Put("Number of Rotations:", NumRotations);
        if (NumScalings > 0)
            verbose.Put("Number of Scaling:", NumScalings);
        if (NumShearings > 0)
            verbose.Put("Number of Shearing:", NumShearings);
    };
                              ...
                              const TVolumeDoc *TargetMri)
    {
        ...
        */
        ...
        {
            if (IsEqualSizes(inclusionflags))
                Verbose.Put("Source and Target geometrical overlap:", "Exact match");
            else if (IsTargetBigger(inclusionflags))
                Verbose.Put("Source and Target geometrical overlap:", "Target including Source");
            else if (IsSourceBigger(inclusionflags))
                Verbose.Put("Source and Target geometrical overlap:", "Source including Target");
