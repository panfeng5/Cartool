...
namespace crtl
{
    ...
    TStatisticsFilesDialog::~TStatisticsFilesDialog()
    {
        ...
        delete SamplesNFiles;
        delete Samples1File;
        delete SamplesCsvFile;
        delete TimeMin;
        delete TimeMax;
        delete EndOfFile;
        delete PresetsTime;
        ...
        delete FileTypes;
        delete OutputP;
        delete Output1MinusP;
        delete OutputMinusLogP;
        delete OpenAuto;
        delete MoreOutputs;
    ...
    */
    ...
    void TStatisticsFilesDialog::AddGroupSummary(int gofi)
    {
        ...
        if (gof.GroupIndex >= 0)
        {
            if (StringIsEmpty(gof.GroupName))
                StringCopy(group, "Group ", IntegerToString(gof.GroupIndex + 1));
            else
                StringCopy(group, gof.GroupName);
        }
        else
            ClearString(group);
...
#define StatDialogPreventButtonPress \
    {                                \
        CmSamplesParams(0);          \
        SamplesCsvFile->SetFocus();  \
    }
            ...
            for (int i = 0; i < (int)lmfiles; i++)
            {
                ...
                if ((bool)lm.leeg)
                {
                    ...
                    foreachin(lm.leeg, iterator)
                        ...
                        if (!IsExtension(iterator(), FILEEXT_FREQ))
                    { // filters out frequency files
                    ...
                    if (StatTransfer.IsNFiles() && !first)
                    { // security check, in case of only frequency files
                        if (CheckGroups(GoGoF.GetLast()))
                            AddGroupSummary(GoGoF.NumGroups() - 1);
                        else
                            GoGoF.RemoveLastGroup();
    ...
    void TStatisticsFilesDialog::GuessOutputFileExtension()
    {
        ...
        if (StatTransfer.IsRois())
            StatTransfer.FileTypes.Select(PresetFileTypeDefaultEEG);
        else if (allris)
            StatTransfer.FileTypes.Select(PresetFileTypeRis);
        else if (alleph)
            StatTransfer.FileTypes.Select(PresetFileTypeEph);
        else if (allep)
            StatTransfer.FileTypes.Select(PresetFileTypeEp);
        else if (allbv)
            StatTransfer.FileTypes.Select(PresetFileTypeBV);
        else if (allsef)
            StatTransfer.FileTypes.Select(PresetFileTypeSef);
        else if (allfreq)
            StatTransfer.FileTypes.Select(PresetFileTypeSef);
    ...
    void TStatisticsFilesDialog::ReadSpreadSheet(char *filename)
    {
        ...
        else if (IsCsvStatFiles(csvtype))
        {
            ...
            if (StatTransfer.IsCsvFile() || StatTransfer.IsNFiles() && StringIs(attr, "1") || StatTransfer.Is1File() && StringIsNot(attr, "1"))
            {
        ...
        { // all other cases, like IsCsvStatFittingR
    ...
    void TStatisticsParamsDialog::SetupWindow()
    {
        ...
        if (StatTransfer.IsCsvFile())
            PresetsData->SetSelIndex(StatPresetFitting);

        else if (StatTransfer.Is1File())
        {

            if (StatTransfer.FileType == CM_STATISTICSTRACKS)
                PresetsData->SetSelIndex(StatPresetEegSurfaceSpont);
            else if (StatTransfer.FileType == CM_STATISTICSRIS)
                PresetsData->SetSelIndex(StatPresetRisScalSpont);
            else
                PresetsData->SetSelIndex(StatPresetEegSurfaceSpont);
        }
        else
        { // if ( StatTransfer.IsNFiles  () ) {
            ...
            if (StatTransfer.FileType == CM_STATISTICSTRACKS)
                PresetsData->SetSelIndex(StatPresetEegSurfaceErp);
            else if (StatTransfer.FileType == CM_STATISTICSRIS)
                PresetsData->SetSelIndex(StatPresetRisScalErp);
            else if (StatTransfer.FileType == CM_STATISTICSFREQ)
                PresetsData->SetSelIndex(StatPresetFFTPower);
            else
                PresetsData->SetSelIndex(StatPresetEegSurfaceErp);
        }
    ...
    TStatisticsParamsDialog::~TStatisticsParamsDialog()
    {
        ...
        delete NoRef;
        delete AveRef;
        delete NormalizationNone;
        delete NormalizationGfp;
        delete NormalizationGfpPaired;
        delete NormalizationGfp1TF;
        delete CheckMissingValues;
        delete CheckMissingValuesValue;
        delete NumberOfRandomization;
        delete PresetsCorrection;
        delete FDRCorrectionValue;
        delete ThresholdingPValues;
        delete ThresholdingPValuesValue;
        delete MinSignificantDuration;
        delete MinSignificantDurationValue;
    ...
    void TStatisticsParamsDialog::CmMinSignificantDurationEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(StatTransfer.IsNFiles() && (CheckToBool(ThresholdingPValues->GetCheck())                                          // user thresholded the p-values
    ...
    void TStatisticsParamsDialog::CmMinSignificantDurationValueEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(StatTransfer.IsNFiles() && (CheckToBool(ThresholdingPValues->GetCheck()) || StatTransfer.PresetsCorrection.GetSelIndex() == StatCorrPresetThresholdingPValues) && CheckToBool(MinSignificantDuration->GetCheck()));
    }
    ...
    */
    ...
    void TStatisticsParamsDialog::CmThresholdingPValuesValueEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(StatTransfer.PresetsCorrection.GetSelIndex() != StatCorrPresetThresholdingPValues && CheckToBool(StatTransfer.ThresholdingPValues));
    }
    ...
    void TStatisticsDialog::CmToDialog(owlwparam w)
    {
        ...
        if (todialogid == IDD_STATISTICS1)
            TStatisticsFilesDialog(CartoolMdiClient, IDD_STATISTICS1).Execute();
        else if (todialogid == IDD_STATISTICS2)
            TStatisticsParamsDialog(CartoolMdiClient, IDD_STATISTICS2).Execute();
    }
    ...
    void TStatisticsDialog::SamplesParams()
    {
        if (StatTransfer.Is1File() || StatTransfer.IsCsvFile())
        {
            ...
            if (StatTransfer.Is1File() && (CheckToBool(StatTransfer.NormalizationGfp) || CheckToBool(StatTransfer.NormalizationGfpPaired)) || StatTransfer.IsCsvFile() && !CheckToBool(StatTransfer.NormalizationNone))
            {
    ...
    void TStatisticsDialog::CmOkEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable(numgroups >= 2 && (twosample && !(numgroups & 1)));
    }
    ...
    void TStatisticsDialog::ProcessGroups(TGoGoF * /*gogof*/, int gofi1, int gofi2, void *usetransfer)
    {
        ...
        if (uttest)
            processings[numprocessings++] = UnpairedTTest;
        if (urand)
            processings[numprocessings++] = UnpairedRandomization;
        if (utanova)
            processings[numprocessings++] = UnpairedTAnova;
        if (pttest)
            processings[numprocessings++] = PairedTTest;
        if (prand)
            processings[numprocessings++] = PairedRandomization;
        if (ptanova)
            processings[numprocessings++] = PairedTAnova;
        ...
        if (transfer.UseXyzDoc && CanOpenFile(transfer.XyzDocFile))
        {
        ...
        bool thresholdingpvalues = CheckToBool(transfer.ThresholdingPValues) && presetcorri != StatCorrPresetThresholdingPValues; // NO p-value thresholding with this FDR correction: results are already clipped AND the results can be legally above the threshold
        ...
        bool clippingpduration = CheckToBool(transfer.MinSignificantDuration) && isnfiles && (thresholdingpvalues || presetcorri == StatCorrPresetThresholdingPValues) && significantduration > 1;
        ...
        bool ignorepolarity = CheckToBool(transfer.IgnorePolarity) && !(datatype == AtomTypePositive || datatype == AtomTypeVector);

        PolarityType polarity = ignorepolarity ? PolarityEvaluate : PolarityDirect;

        ReferenceType dataref;
        if (CheckToBool(transfer.AveRef))
        ...
        int normalize;
        if (CheckToBool(transfer.NormalizationGfp))
            normalize = IDC_RESCALE_GFP;
        else if (CheckToBool(transfer.NormalizationGfpPaired))
            normalize = IDC_RESCALE_GFP_PAIRED;
        else if (CheckToBool(transfer.NormalizationGfp1TF))
            normalize = IDC_RESCALE_GFP_1TF;
        else
            normalize = IDC_RESCALE_NONE;

        // if not same amount of files, remove the paired GFP normalization
        if (normalize == IDC_RESCALE_GFP_PAIRED && !equalsamples)
            // downgrade to single file GFP normalization
            normalize = IDC_RESCALE_GFP;

        // force no normalization
        if ((normalize == IDC_RESCALE_GFP || normalize == IDC_RESCALE_GFP_PAIRED) && !isnfiles)
        ...
        */
            ...
            for (int filei = 0; filei < gof->NumFiles(); filei++)
            {
                ...
                if (iscsvfile)
                {
                    ...
                    for (int i = 0; i < sf.GetNumAttributes(); i++)
                    {
                        ...
                        if (IsLongNames(csvtype) && StringStartsWith(attr, FitGroupNameLong) || IsShortNames(csvtype) && StringStartsWith(attr, FitGroupNameShort))
                        {
                            vari += i;
                            break;
                        }
                        ...
                        for (int tf0 = 0; tf0 < inputnumtf[gofi0]; tf0++)
                            ...
                            for (TIteratorSelectedForward ei(elsel); (bool)ei; ++ei)
                                ...
                                if (ei() == disoff)
                                    ...
                                    Data[gofi0](tf0, ei.GetIndex(), s) /= abs(rescalefactor);
                                else
                                    Data[gofi0](tf0, ei.GetIndex(), s) /= rescalefactor;
                    } // for samples
                } // if IDC_RESCALE_GFP
                        ...
                        for (int tf0 = 0; tf0 < inputnumtf[1]; tf0++)
                            ...
                            for (TIteratorSelectedForward ei(elsel); (bool)ei; ++ei)
                                ...
                                if (ei() == disoff)
                                    ...
                                    Data[gofi0 + 1](tf0, ei.GetIndex(), s) /= abs(rescalefactor);
                                else
                                    Data[gofi0 + 1](tf0, ei.GetIndex(), s) /= rescalefactor;
        ...
        if (presetsindex == PresetFileTypeEph && samplfreq > 0)
            filetype = IDC_FILETYPE_EPH;
        else if (presetsindex == PresetFileTypeEp)
            filetype = IDC_FILETYPE_EP;
        else if (presetsindex == PresetFileTypeTxt)
            filetype = IDC_FILETYPE_TXT;
        else if (presetsindex == PresetFileTypeSef)
            filetype = IDC_FILETYPE_SEF;
        else if (presetsindex == PresetFileTypeBV)
            filetype = IDC_FILETYPE_BV;
        else if (presetsindex == PresetFileTypeEdf)
            filetype = IDC_FILETYPE_EDF;
        else if (presetsindex == PresetFileTypeRis)
            filetype = IDC_FILETYPE_RIS;
        else
            filetype = IDC_FILETYPE_BV;
        ...
        if (CheckToBool(transfer.OutputP))
        {
            outputp = OutputP;
            StringCopy(PValueName, InfixP);
        }
        else if (CheckToBool(transfer.Output1MinusP))
        {
            outputp = Output1MinusP;
            StringCopy(PValueName, Infix1MinusP);
        }
        else if (CheckToBool(transfer.OutputMinusLogP))
        {
            outputp = OutputMinusLogP;
            StringCopy(PValueName, InfixLogP);
        }
        else
        {
            outputp = Output1MinusP;
        ...
        for (int proci = 0; proci < numprocessings; proci++)
        {
            ...
            if (numpairsofgroups >= 2)
                StringAppend(BaseFileName, ".", IntegerToString(gofi1 / 2 + 1, NumIntegerDigits(numpairsofgroups)));
            ...
            if (processing == UnpairedTTest)
                StringAppend(BaseFileName, "." InfixUnpaired "." InfixTTest);
            else if (processing == UnpairedRandomization)
                StringAppend(BaseFileName, "." InfixUnpaired "." InfixRand);
            else if (processing == UnpairedTAnova)
                StringAppend(BaseFileName, "." InfixUnpaired "." InfixTAnova);
            else if (processing == PairedTTest)
                StringAppend(BaseFileName, "." InfixPaired "." InfixTTest);
            else if (processing == PairedRandomization)
                StringAppend(BaseFileName, "." InfixPaired "." InfixRand);
            else if (processing == PairedTAnova)
                StringAppend(BaseFileName, "." InfixPaired "." InfixTAnova);
            ...
            {
                if (processing == UnpairedTTest)
                    verbose.Put("Test used:", "Unpaired t-test");
                else if (processing == UnpairedRandomization)
                    verbose.Put("Test used:", "Unpaired Randomization / Difference of Means");
                else if (processing == UnpairedTAnova)
                    verbose.Put("Test used:", "Unpaired TAnova");
                else if (processing == PairedTTest)
                    verbose.Put("Test used:", "Paired t-test");
                else if (processing == PairedRandomization)
                    verbose.Put("Test used:", "Paired Randomization / Mean of Differences");
                else if (processing == PairedTAnova)
                    verbose.Put("Test used:", "Paired TAnova");
                ...
                verbose.NextLine();
                verbose.Put("Testing:", rois ? "ROIs" : iscsvfile ? "Variables"
                                                                  : "Tracks");
                ...
                if (IsTAnova(processing))
                {
                    ...
                    verbose.Put("Maps / Templates Polarity:", datatype == AtomTypePositive ? "Not relevant" : ignorepolarity ? "Ignore"
                                                                                                                             : "Account");
                }

                verbose.NextLine();
                if (normalize == IDC_RESCALE_NONE)
                    verbose.Put("Data level normalization:", "None");
                else if (normalize == IDC_RESCALE_GFP)
                    verbose.Put("Data level normalization:", "Mean GFP, independently on each condition");
                else if (normalize == IDC_RESCALE_GFP_PAIRED)
                    verbose.Put("Data level normalization:", "Mean GFP, joining paired conditions together");
                else if (normalize == IDC_RESCALE_GFP_1TF)
                    verbose.Put("Data level normalization:", "GFP of each single sample");
                ...
                if (!IsTAnova(processing))
                {
                    ...
                    if (iscsvfile)
                        verbose.Put("Subtracting groups:", gof1.GroupName, " - ", gof2.GroupName);
                    else
                        verbose.Put("Subtracting groups:", "First Group - Second Group");
                }
            }
            ...
            {
                if (isnfiles)
                    verbose.Put("Data format:", "Data span across a set of files");
                else if (isonefile)
                    verbose.Put("Data format:", "Data are consecutive in 1 file");
                else if (iscsvfile)
                    verbose.Put("Data format:", "Data are contained in a .csv file");
                ...
                for (int gofi0 = 0; gofi0 < 2; gofi0++)
                {
                    ...
                    for (int filei = 0; filei < gof->NumFiles(); filei++)
                    {
                        ...
                        verbose.Put(filei ? "" : isnfiles ? "Files          :"
                                                          : "File           :",
                                    (*gof)[filei]);
                    }
            ...
            verbose.NextTopic("Output Files:");
            {
                ...
                if (iscsvfile)
                {
                    if (IsTTest(processing))
                        verbose.Put("t values transposed in a Data file:", TDataFile);
                    if (IsRandomization(processing))
                        verbose.Put("Delta values transposed in a Data file:", DDataFile);
                ...
                if (moreoutputs /*&& writenumtf > 1*/ && HasMoreOutputs(processing))
                {
                    ...
                    for (int gofi0 = 0; gofi0 < 2; gofi0++)
                    {
                        ...
                        sprintf(buff, "%s." InfixGroup "%0d." InfixSubjects ".%s", (char *)GroupBaseFileName, gofi0 + 1, IsFreqLoop() ? FILEEXT_FREQ : filetype == IDC_FILETYPE_RIS ? FILEEXT_RIS
                                                                                                                                                                                    : FILEEXT_EEGSEF);
            ...
            if (expt)
            {
                ...
                if (IsTAnova(processing))
                    expt->ElectrodesNames = TAnovaName;
                else if (outputvarnames)
                    expt->ElectrodesNames = VarNames;
            ...
            if (expd)
            {
                ...
                if (IsTAnova(processing))
                    expd->ElectrodesNames = TAnovaName;
                else if (outputvarnames)
                    expd->ElectrodesNames = VarNames;
            ...
            if (expp)
            {
                ...
                if (IsTAnova(processing))
                    expp->ElectrodesNames = TAnovaName;
                else if (outputvarnames)
                    expp->ElectrodesNames = VarNames;

                expp->Begin();
            }

            if (expt && expt->IsFileTextual())
                (ofstream &)(*expt) << StreamFormatLeft;
            if (expd && expd->IsFileTextual())
                (ofstream &)(*expd) << StreamFormatLeft;
            if (expp && expp->IsFileTextual())
                (ofstream &)(*expp) << StreamFormatLeft;
            ...
            if (writenumtf == 1 && csvoutnummaps > 0)
            {
                ...
                if (exppdata)
                {
                    ...
                    for (int v = 0; v < exppdata->NumTracks; v++)
                    {
                        ...
                        if (IsCsvStatFittingCartool(csvtype) && grepfit.Matched(VarNames[elsel.GetValue(v)], &matched))

                            exppdata->VariableNames.Add(VarNames[elsel.GetValue(v)] + StringLength(matched(0)));
                        // retrieve FIRST part as variable names
                        else if (IsCsvStatMarkov(csvtype) && grepmarkov.Matched(VarNames[elsel.GetValue(v * csvmaxmap)], &matched))
                ...
                for (int mapi = 1, v = -1; mapi <= csvmaxmap; mapi++)
                    for (int vari = 0; vari < numvars; vari++)
                    {
                        ...
                        if (!csvoutmapsel[mapi])
                        {
                            ...
                            if (exptdata)
                                exptdata->Write((float)0);
                            if (expddata)
                                expddata->Write((float)0);
                            if (exppdata)
                                exppdata->Write((float)0);
                        ...
                        if (IsCsvStatFittingCartool(csvtype))
                            ...
                            v = vari * csvmaxmap + mapi - 1; // we need to transpose the data i<->j
                        ...
                        if (exptdata)
                            exptdata->Write((float)t);
                        if (expddata)
                            expddata->Write((float)avg);
                        if (exppdata)
                            exppdata->Write((float)p);
            ...
            if (moreoutputs && HasMoreOutputs(processing) /*&& writenumtf > 1*/)
            {
                ...
                for (int gofi0 = 0; gofi0 < 2; gofi0++)
                {
                    ...
                    if (writenumvars == 1)
                    { // write all subjects as "electrode"
                    ...
                    { // write each subject one "after" the other
