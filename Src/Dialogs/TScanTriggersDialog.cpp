...
namespace crtl
{
    ...
    TScanTriggersDialog::~TScanTriggersDialog()
    {
        ...
        delete ThresholdAbove;
        delete ThresholdAboveValue;
        delete ThresholdBelow;
        delete ThresholdBelowValue;
        delete ThresholdStrict;
        delete ThresholdSlope;
        delete ScanTemplate;
        delete ScanTemplateFile;
        delete ScanTemplateThreshold;
        delete StabMin;
        delete StabMax;
        delete Gap;
        delete OneMarkerPerTrack;
        delete MergeMarkers;
        delete MergeMarkersRange;
        delete PrefixMarker;
        delete PrefixMarkerString;
        delete TrackName;
        delete NoValue;
        delete TrackValue;
        delete TrackRelativeIndex;
        delete MergedCount;
    }
    ...
    bool TScanTriggersDialog::CmProcessEnable()
    {
        ...
        if (!endoffile && (timemin > timemax || EEGDoc && timemax >= EEGDoc->GetNumTimeFrames()))
            return false;
        ...
        else if (CheckToBool(ScanTriggersTransfer.ScanThreshold))
        {
            ...
            if (!CheckToBool(ScanTriggersTransfer.ThresholdAbove) && !CheckToBool(ScanTriggersTransfer.ThresholdBelow))
            ...
            if (CheckToBool(ScanTriggersTransfer.ThresholdAbove) && CheckToBool(ScanTriggersTransfer.ThresholdBelow) && threshabove > threshbelow)
        ...
        else if (CheckToBool(ScanTriggersTransfer.ScanExtrema))
        {
            ...
            return CheckToBool(ScanTriggersTransfer.ScanMin) || CheckToBool(ScanTriggersTransfer.ScanMax);
        }
    ...
    void TScanTriggersDialog::EvMethodChanged()
    {
        ...
        if (CheckToBool(ScanTriggersTransfer.ScanStability))
            StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Stab");

        else if (CheckToBool(ScanTriggersTransfer.ScanThreshold))
        {

            if (CheckToBool(ScanTriggersTransfer.ThresholdAbove) && CheckToBool(ScanTriggersTransfer.ThresholdBelow))
                StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Between");
            else if (CheckToBool(ScanTriggersTransfer.ThresholdAbove))
                StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Above");
            else if (CheckToBool(ScanTriggersTransfer.ThresholdBelow))
                StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Below");
            else
                ClearString(ScanTriggersTransfer.PrefixMarkerString);
        }

        else if (CheckToBool(ScanTriggersTransfer.ScanTemplate))
            StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Corr");
        ...
        else if (CheckToBool(ScanTriggersTransfer.ScanExtrema))
        {
            ...
            else if (CheckToBool(ScanTriggersTransfer.ScanMin))
                StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Min");
            else if (CheckToBool(ScanTriggersTransfer.ScanMax))
                StringCopy(ScanTriggersTransfer.PrefixMarkerString, "Max");
            else
                ClearString(ScanTriggersTransfer.PrefixMarkerString);
        }
    ...
    {
        tce.Enable(CheckToBool(ScanThreshold->GetCheck()) && CheckToBool(ThresholdAbove->GetCheck()));
    }

    void TScanTriggersDialog::CmThresholdBelowEnable(TCommandEnabler &tce)
    {
        tce.Enable(CheckToBool(ScanThreshold->GetCheck()) && CheckToBool(ThresholdBelow->GetCheck()));
    }
        ...
        if (!EEGDoc->HasMultipleSessions() || EEGDoc->GetCurrentSession() == 1)
        {
            ...
            verbose.NextTopic("EEG Parameters:");
            {
                ...
                if (ref == ReferenceAsInFile)
                    ...
                    verbose.Put("Reference:", EEGDoc->GetReferenceTracks().ToText(buff, EEGDoc->GetElectrodesNames(), AuxiliaryTracksNames));
                else
        ...
        if (scanstability || scanthreshold || scanextrema)
        {
            ...
            for (int tf = timemin, tfi = 0; tf <= timemax; tf++, tfi++)
            {
                ...
                for (TIteratorSelectedForward eli(elsel); (bool)eli; ++eli, binarycode <<= 1)
                {
                    ...
                    bool update = scanstability && eegb[i][tfi] == chan[i][value] || scanthreshold && (!abovethresh || abovethresh && eegb[i][tfi] >= threshabove) && (!belowthresh || belowthresh && eegb[i][tfi] <= threshbelow);
                    ...
                    if (!update || tf == timemax)
                    {
                        ...
                        if (scanstability)
                            condition = (bool)chan[i][value]; // stability for non-null values

                        else if (scanthreshold)
                            condition = (!abovethresh || abovethresh && chan[i][value] >= threshabove) && (!belowthresh || belowthresh && chan[i][value] <= threshbelow);
                        ...
                        else if (scanextrema && tf - timemin > 2)
                        {
                            ...
                            delta1 = delta1 > 0 ? 1 : delta1 < 0 ? -1
                                                                 : 0;
                            delta2 = delta2 > 0 ? 1 : delta2 < 0 ? -1
                                                                 : 0;
                            // "laplacian of signs"
                            ...
                            if (condition)
                            {                            // we never entered the previous "if", so we are late of 1 TF, so update these fields now
                        ...
                        if (duration && timing && condition)
                        {
                            ...
                            if (smartslope)
                            { // adjust the tf to the highest slope, either before or after
                                ...
                                besttf = besttfup == besttfdown        ? besttf
                                         : bestdeltaup > bestdeltadown ? besttfup
                                                                       : besttfdown;
            ...
            for (int markeri = 0; markeri < numtemp; markeri++)
            {
                ...
                for (markeri2 = markeri; markeri2 <= numtemp; markeri2++)
                {
                    ...
                    for (int markeri3 = markeri; markeri3 < markeri2; markeri3++)
                    {
                        ...
                        if (mergedcount)
                            markercode++; // either a count
                        else
                            markercode |= newmarkers[markeri3]->Code; // or a binary combination
                    }
