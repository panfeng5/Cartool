...
namespace crtl
{
    ...
    */
    ...
    bool TTracksAveragingFilesDialog::CheckFilesCompatibility(bool silent)
    {
        ...
        for (int i = 0; i < ComboEeg->GetCount(); i++)
        {
            ...
            if (!ReadFromHeader(buff, ReadNumElectrodes, &numeleeg) || !ReadFromHeader(buff, ReadSamplingFrequency, &dsf))
            {
            ...
            else if (numeleeg1 != numeleeg)
            {
                ...
                if (!silent)
                {
                    ...
                    sprintf(buff, "Number of electrodes does not match:" NewLine Tab "EEG : %d" NewLine Tab "EEG : %d",
                            numeleeg1, numeleeg);
                    ShowMessage(buff, "Averaging Files", ShowMessageWarning);
                }
            ...
            else if (dsf != 0 && dsf1 != 0 && fabs(dsf1 - dsf) / dsf > 1e-6)
            {
                ...
                if (!silent)
                {
                    ...
                    sprintf(buff, "Sampling frequencies do not match:" NewLine Tab "%4g Hz" NewLine Tab "%4g Hz",
                            dsf1, dsf);
                    ShowMessage(buff, "Averaging Files", ShowMessageWarning);
                }
            ...
            if (somexyz && numelxyz != numeleeg)
            {
                ...
                if (!silent)
                {
                    ...
                    sprintf(buff, "Number of electrodes does not match:" NewLine Tab "XYZ : %d" NewLine Tab "EEG : %d",
                            numelxyz, numeleeg);
                    ShowMessage(buff, "Averaging Files", ShowMessageWarning);
                }
    ...
    void TTracksAveragingFilesDialog::CmAddToList()
    {
        ...
        {
            sprintf(mess, "Can not find the file:" NewLine NewLine "%s" NewLine,
                    fneeg);
    ...
    void TTracksAveragingFilesDialog::GuessTriggerType()
    {
        ...
        if (anytrigger)
            UseTriggers->SetCheck(BoolToCheck(true));
        if (anymarker)
            UseMarkers->SetCheck(BoolToCheck(true));
    }
    ...
    void TTracksAveragingFilesDialog::CmReplaceFieldEnable(TCommandEnabler &tce)
    {
        ...
        if (ComboWithFocus == ComboEeg)
            selindex = SelIndex[0];
        else if (ComboWithFocus == ComboSes)
            selindex = SelIndex[1];
        else if (ComboWithFocus == ComboTva)
            selindex = SelIndex[2];
        else
            selindex = SelIndex[3];
    ...
    void TTracksAveragingFilesDialog::EvCbnSetFocus()
    {
        ...
        if (wid == ComboEeg->GetId())
            ComboWithFocus = ComboEeg;
        else if (wid == ComboSes->GetId())
            ComboWithFocus = ComboSes;
        else if (wid == ComboTva->GetId())
            ComboWithFocus = ComboTva;
        else
            ComboWithFocus = ComboTrg;
    }
    ...
    void TTracksAveragingFilesDialog::CmBrowseEeg()
    {
        ...
        { // only one -> just put in edit field
    ...
    */
    ...
    TTracksAveragingParamsDialog::~TTracksAveragingParamsDialog()
    {
        ...
        delete AuxThreshold;
        delete AuxOverride;
        delete TriggerFixedTF;
        delete Trigger;
        delete TriggerOffset;
        delete TriggerReactionTime;
        delete TriggerOffsetValue;
        delete TriggerOffsetValueMs;
        delete TriggerFixedTFValue;
        delete TriggerFixedTFValueMs;
        delete DurationPre;
        delete DurationPreMs;
        delete DurationPost;
        delete DurationPostMs;
        delete DurationTotal;
        delete DurationTotalMs;
        delete SignedData;
        delete PositiveData;
        delete NoRef;
        delete AvgRef;
        delete AccountPolarity;
        delete IgnorePolarity;
        delete SetFilters;
        delete FilterData;
        delete FilterDisplay;
        delete BaselineCorr;
        delete BaselineCorrPre;
        delete BaselineCorrPreMs;
        delete BaselineCorrPost;
        delete BaselineCorrPostMs;
    ...
    */
    ...
    void TTracksAveragingParamsDialog::ReadParams(char *file)
    {
        ...
        do
        {
            ...
            else if (StringStartsWith(buff, "Applying average reference") || StringStartsWith(buff, "Data reference"))
            {
            ...
            {
                if (StringIs(toc, FILEEXT_EEGEP))
                    TAvgTransfer.FileTypes.Select(PresetFileTypeEp);
                else if (StringIs(toc, FILEEXT_EEGEPH))
                    TAvgTransfer.FileTypes.Select(PresetFileTypeEph);
                else if (StringIs(toc, FILEEXT_TXT))
                    TAvgTransfer.FileTypes.Select(PresetFileTypeEp);
                else if (StringIs(toc, FILEEXT_EEGSEF))
                    TAvgTransfer.FileTypes.Select(PresetFileTypeSef);
                else if (StringIs(toc, FILEEXT_EEGBV))
                    TAvgTransfer.FileTypes.Select(PresetFileTypeBV);
                else if (StringIs(toc, FILEEXT_RIS))
                    TAvgTransfer.FileTypes.Select(PresetFileTypeRis);
            }
    ...
    void TTracksAveragingParamsDialog::CmSetFiltersEnable(TCommandEnabler &tce)
    {
        ...
        if (Presets->GetSelIndex() == AvgPresetErpFreqPower || Presets->GetSelIndex() == AvgPresetErpFreqFftapprox || Presets->GetSelIndex() == AvgPresetGrandaverageFreqPower || Presets->GetSelIndex() == AvgPresetGrandaverageFreqFftapprox)
            tce.Enable(false);
        else
    ...
    {
        tce.Enable(TAvgTransfer.SamplingFrequency > 0 && StringToDouble(((TTracksAveragingParamsStruct *)GetTransferBuffer())->DurationTotal) > 0 && TAvgTransfer.Filters.HasAnyFilter() && CheckToBool(((TTracksAveragingParamsStruct *)GetTransferBuffer())->SetFilters));
    }
    ...
    void TTracksAveragingParamsDialog::CmAvgRefEnable(TCommandEnabler &tce)
    {
        TransferData(tdGetData);
        // does not make any sense of filtering frequencies!
        tce.Enable(Presets->GetSelIndex() == AvgPresetErpEegSurface || Presets->GetSelIndex() == AvgPresetGrandaverageErpSurface);
    }
    ...
    {
        delete MergeTriggers;
        delete SplitTriggers;
        delete SaveAverage;
        delete SaveSum;
        delete SaveSD;
        delete SaveSE;
        delete SaveEpochsMultiFiles;
        delete SaveEpochsSingleFile;
        delete FileTypes;
        delete SaveExcluded;
        delete SaveAux;
        delete OpenAuto;
    ...
    TTracksAveragingBaseDialog::TTracksAveragingBaseDialog(TWindow *parent, int resId) : TBaseDialog(parent, resId)
    {
    }
    ...
    void TTracksAveragingBaseDialog::CmToDialog(owlwparam w)
    {
        ...
        if (todialogid == IDD_AVERAGE1)
            TTracksAveragingFilesDialog(CartoolMdiClient, IDD_AVERAGE1).Execute();
        else if (todialogid == IDD_AVERAGE2)
            TTracksAveragingParamsDialog(CartoolMdiClient, IDD_AVERAGE2).Execute();
        else if (todialogid == IDD_AVERAGE3)
            TTracksAveragingOutputsDialog(CartoolMdiClient, IDD_AVERAGE3).Execute();
    }
    ...
    void TTracksAveragingBaseDialog::CmOkEnable(TCommandEnabler &tce)
    {
        ...
        tce.Enable((CheckToBool(TAvgTransfer.SaveAverage) || CheckToBool(TAvgTransfer.SaveSum) || CheckToBool(TAvgTransfer.SaveSD) || CheckToBool(TAvgTransfer.SaveSE)) && (CheckToBool(TAvgTransfer.MergeTriggers) || CheckToBool(TAvgTransfer.SplitTriggers) && !CheckToBool(TAvgTransfer.TriggerFixedTF)) || CheckToBool(TAvgTransfer.SaveEpochsMultiFiles) || CheckToBool(TAvgTransfer.SaveEpochsSingleFile));
    ...
    TTracksAveragingControlDialog::~TTracksAveragingControlDialog()
    {
        ...
        delete AcceptNumTF;
        delete RejectNumTF;
        delete NumTriggersAccepted;
        delete NumTriggersRejected;
        delete TracksBox;
        delete TrackName;
        delete TrackMin;
        delete TrackMax;
        ...
        delete AuxThreshold;
        delete TrackZoom;
        delete TracksToExclude;
        delete TracksExcluded;
        // delete  gauge;
    }
    ...
    void TTracksAveragingControlDialog::ProcessGroups(TGoGoF *gogof, int gofi1, int gofi2, void *usetransfer)
    {
        ...
        if (CheckToBool(transfer.TriggerFixedTF))
            triggerOrigin = EpochOriginFixedTF;
        else if (CheckToBool(transfer.Trigger))
            triggerOrigin = EpochOriginTrigger;
        else if (CheckToBool(transfer.TriggerOffset))
            triggerOrigin = EpochOriginTriggerOffset;
        else if (CheckToBool(transfer.TriggerReactionTime))
            triggerOrigin = EpochOriginTriggerReactionTime;
        else
            triggerOrigin = EpochOriginTrigger;
        ...
        if (CheckToBool(transfer.UseEvents))
            SetFlags(markertype, MarkerTypeEvent);
        if (CheckToBool(transfer.UseTriggers))
            SetFlags(markertype, MarkerTypeTrigger);
        if (CheckToBool(transfer.UseMarkers))
        ...
        AtomType datatype;
        if (CheckToBool(transfer.PositiveData))
            datatype = AtomTypePositive;
        else if (CheckToBool(transfer.SignedData))
        ...
        if (CheckToBool(transfer.AvgRef))
            dataref = ReferenceAverage;
        else /*if ( CheckToBool ( transfer.NoRef ) )*/
            dataref = ReferenceAsInFile;
        ...
        if (SaveAverage && MergeTriggers && !CanOpenFile(fileoutavg, CanOpenFileWriteAndAsk) || SaveSum && MergeTriggers && !CanOpenFile(fileoutsum, CanOpenFileWrite)
            //|| SaveEpochsMultiFiles                   && ! CanOpenFile ( fileoutepochs,       CanOpenFileWriteAndAsk )
            || SaveEpochsSingleFile && !CanOpenFile(fileout1epoch, CanOpenFileWrite) || SaveSD && MergeTriggers && !CanOpenFile(fileoutsd, CanOpenFileWrite) || SaveSE && MergeTriggers && !CanOpenFile(fileoutse, CanOpenFileWrite) || !CanOpenFile(fileoutvrb, CanOpenFileWrite) || usexyz && !CanOpenFile(fileoutxyz, CanOpenFileWrite) || usexyz && !CanOpenFile(transfer.XyzDocFile, CanOpenFileReadAndAsk))
        ...
        if (baseLineCorrPre > 0)
            baseLineCorrPre--;
        if (baseLineCorrPost < 0)
            baseLineCorrPost++;
        ...
        verbose.NextTopic("Output Files:");
        {
            ...
            if (SplitTriggers)
            {
                ...
                if (IsFreqLoop())
                    TriggerFileName += fileoutprefix + "." + FreqName + "." InfixTrigger " *";
                else
                    TriggerFileName += fileoutprefix + "." InfixTrigger " *";
                ...
                {
                    if (IsFreqLoop())
                        file = TriggerFileName + "." + InfixSum + "." + fileoutext;
                    else
                        file = TriggerFileName + "." + InfixSum + " (#epochs)" + "." + fileoutext;
        ...
        {
            if (triggerOrigin == EpochOriginFixedTF)
                verbose.Put("Epoch origin is:", "TF ", trigger1TF);
            else if (triggerOrigin == EpochOriginTrigger)
                verbose.Put("Epoch origin is:", "Trigger");
            else if (triggerOrigin == EpochOriginTriggerOffset)
                verbose.Put("Epoch origin is:", "Trigger + ", triggerOffset, " [TF]");
            else if (triggerOrigin == EpochOriginTriggerReactionTime)
                verbose.Put("Epoch origin is:", "Trigger + Reaction Time");
            ...
            if (triggerOrigin != EpochOriginFixedTF)
            {
                ...
                if (IsFlag(markertype, MarkerTypeEvent))
                    StringAppend(buff, "Events", " ");
                if (IsFlag(markertype, MarkerTypeTrigger))
                    StringAppend(buff, "Triggers", " ");
                if (IsFlag(markertype, MarkerTypeMarker))
                    StringAppend(buff, "Markers", " ");
        ...
        verbose.NextTopic("Data Type:");
        {
            if (datatype == AtomTypeScalar)
                ...
                verbose.Put("Data type:", allrisv ? "Positive Data (Norm of Vectors)" : "Positive Data");
            ...
            verbose.Put("Maps / Templates Polarity:", (datatype == AtomTypePositive || datatype == AtomTypeVector) ? "Not relevant" : ignorepolarity ? "Ignore"
                                                                                                                                                     : "Account");
        }
        ...
        for (int eegi = 0; eegi < (int)gofeeg; eegi++)
        {
            ...
            if (firsteeg)
            {
                ...
                if (ignorepolarity)
                { // exclude everything suspected of being polluted
                    CorrelationSel.Set();
            ...
            //  if ( triggerOrigin != EpochOriginFixedTF )
            //      automode    = AutoNone;
            automode = acceptall ? AutoAcceptAll : triggerOrigin != EpochOriginFixedTF ? AutoNone
                                                                                       : automode;
              ...
              // reset all codes
                ...
                for (int i = 0; i < markers.Num(); i++)
                {
                    ...
                    if (iftva)
                    {
                        ...
                        if (!iftva->eof() && (!tvacartool // always read, tva is an exhaustive list
                                              || !tvaalreadyread && IsFlag(marker.Code, (MarkerCode)TriggerInList)))
                        { // read only if a selected marker
                        ...
                        if (tvacartool)
                        { // cartool tva is a subset of the full list of markers
                        ...
                        { // ! tvacartool, always read even if not in the tested list
                    ...
                    if (triggerOrigin == EpochOriginTriggerOffset)
                        marker.From += triggerOffset;
                    else if (triggerOrigin == EpochOriginTriggerReactionTime)
                        marker.From += Round(MillisecondsToTimeFrame(tvartms, samplingfrequency));
            ...
            for (currtrigg = 0; currtrigg < markersfiltered.GetNumMarkers(); currtrigg++)
            {
                ...
                // is trigger not ok?
                if (IsFlag(marker.Code, (MarkerCode)TriggerBadEpoch) || IsFlag(marker.Code, (MarkerCode)TriggerTvaNotOk))
                {
                ...
                if (!IsInsideLimits(tf1, tf2, (long)0, numTimeFrames - 1))
                {
                    ...
                    {
                        sprintf(buff, "Trying to access file at positions  %0d  to  %0d," NewLine "while file limits are  %0d  to  %0d." NewLine "Trigger rejected.",
                ...
                for (currchannel = 0; currchannel < totalChannels; currchannel++)
                {
                    ...
                    if ((CheckEl && !AuxTracks[currchannel] && (channmin < -ElThreshold || channmax > ElThreshold)) || (CheckAux && AuxTracks[currchannel] && (channmin < -AuxThreshold || channmax > AuxThreshold)))
                ...
                if (automode != AutoNone)
                {
                    ...
                    if ((CheckEl && (regallmin < -ElThreshold || regallmax > ElThreshold)) || (CheckAux && (auxallmin < -AuxThreshold || auxallmax > AuxThreshold)))
                    {
                ...
                if (!(automode != AutoNone || (accepttrigger && marker.From <= arlimit)))
                    ...
                    do
                    {
                        ...
                        if (!CheckAux)
                        { // account for user desires to see or not the aux, when they are not tested
                            DisplayTracks -= AuxTracks & ~EegView->SelTracks;
                            DisplayTracks += AuxTracks & EegView->SelTracks;
                        }

                        switch (ButtonPressed)
                        { // test the button code
                        ...
                        case IDC_UNDOLAST:
                            ...
                            {
                                // ignore these
                                if (!IsFlag(markersfiltered[currtrigg]->Code, (MarkerCode)TriggerInList) || IsFlag(markersfiltered[currtrigg]->Code, (MarkerCode)TriggerBadEpoch) || IsFlag(markersfiltered[currtrigg]->Code, (MarkerCode)TriggerTvaNotOk))
                        ...
                        case IDCANCEL:
                            if (!GetAnswerFromUser("Are you sure you want to cancel" NewLine
            ...
            if (saveaux)
                OutputTracks += AuxTracks;
            else
                OutputTracks -= AuxTracks;

            if (saveexcl)
                OutputTracks += CheckingExcluded;
            else
                OutputTracks -= CheckingExcluded;
            ...
            if (ignorepolarity && PolarityRef.IsNotAllocated())
            {
                ...
                for (currtrigg = 0; currtrigg < markersfiltered.GetNumMarkers(); currtrigg++)
                {
                    ...
                    if (!IsFlag(marker.Code, (MarkerCode)TriggerInList) || IsFlag(marker.Code, (MarkerCode)TriggerBadEpoch) || IsFlag(marker.Code, (MarkerCode)TriggerTvaNotOk) || !IsFlag(marker.Code, (MarkerCode)TriggerAccepted))
            ...
            for (currtrigg = 0; currtrigg < markersfiltered.GetNumMarkers(); currtrigg++)
            {
                ...
                // not a good trigger?
                if (IsFlag(marker.Code, (MarkerCode)TriggerBadEpoch) || IsFlag(marker.Code, (MarkerCode)TriggerTvaNotOk) || !IsFlag(marker.Code, (MarkerCode)TriggerAccepted))
                {

                    if (IsFlag(marker.Code, (MarkerCode)TriggerBadEpoch))
                        (ofstream &)verbose << MessageEpochBadEpoch;
                    else if (IsFlag(marker.Code, (MarkerCode)TriggerTvaNotOk))
                        (ofstream &)verbose << MessageEpochBadTva;
                    else if (!IsFlag(marker.Code, (MarkerCode)TriggerAccepted))
                        (ofstream &)verbose << MessageEpochBadUser;
            ...
            if (lasteeg || StringIsNot(eegfile, gofeeg[eegi + 1]) || anyfilter)
            {
        ...
        if (ofsplitmrk.is_open())
            ofsplitmrk.close();
        if (of1epochmrk.is_open())
            of1epochmrk.close();
        ...
        if (MergeTriggers && (SaveAverage || SaveSum))
        {
            ...
            for (int t = 0; t < NumTimeFrames; t++)
                for (TIteratorSelectedForward seli(OutputTracks); (bool)seli; ++seli)
                {
                    ...
                    if (SaveAverage)
                        expavg->Write((float)(TotalSum[seli()][offsetData + t] / numtrg));
                    if (SaveSum)
                        expsum->Write((float)TotalSum[seli()][offsetData + t]);
        ...
        if (MergeTriggers && (SaveSD || SaveSE))
        {
            ...
            for (int t = 0; t < NumTimeFrames; t++)
                for (TIteratorSelectedForward seli(OutputTracks); (bool)seli; ++seli)
                {
                    ...
                    sd = sqrt(fabsl((TotalSumSqr(seli(), offsetData + t) - Square(TotalSum(seli(), offsetData + t)) / numtrg) / NonNull(numtrg - 1)));

                    se = sd / sqrt(numtrg);

                    if (SaveSD)
                        expsd->Write((float)sd);
                    if (SaveSE)
                        expse->Write((float)se);
                }
        ...
        */
        ...
        if (dosplittriggers)
        {
            ...
            for (int trig = 0; trig < alltriggers.GetNumTokens(); trig++)
            {
                ...
                if (IsFreqLoop())
                    TriggerFileName += fileoutprefix + "." + FreqName + "." InfixTrigger " " + trigname;
                else
                    TriggerFileName += fileoutprefix + "." InfixTrigger " " + trigname;
                ...
                if (SaveSum)
                {
                    if (IsFreqLoop())
                        ...
                        file = TriggerFileName + "." + InfixSum + " (" + (const char *)IntegerToString(numsplittrg) + ")" + "." + fileoutext;
                ...
                if (SaveSum)
                {
                    ...
                    if (IsFreqLoop())
                        expsum->Filename = TriggerFileName + "." + InfixSum + "." + fileoutext;
                    else
                        expsum->Filename = TriggerFileName + "." + InfixSum + " (" + (const char *)IntegerToString(numsplittrg) + ")" + "." + fileoutext;
                ...
                for (int t = 0; t < NumTimeFrames; t++)
                    for (int c = 0; c < NumElectrodes; c++)
                    {
                        ...
                        sd = sqrt(abs((SumSqr(c, t) - Square(Sum(c, t)) / numsplittrg) / NonNull(numsplittrg - 1)));

                        se = sd / sqrt(numsplittrg);

                        if (expavg)
                            expavg->Write((float)avg);
                        if (expsum)
                            expsum->Write((float)Sum(c, t));
                        if (expsd)
                            expsd->Write((float)sd);
                        if (expse)
                            expse->Write((float)se);
        ...
        {
        case 0:
            (ofstream &)verbose << Tab "Congratulation if you have read all this file (haven't you?)!" fastendl;
            break;
        case 1:
            (ofstream &)verbose << Tab "Thank you Cartool!" fastendl;
            break;
        case 2:
            (ofstream &)verbose << Tab "That's all folks!" fastendl;
            break;
        case 3:
            (ofstream &)verbose << Tab "Et voila!" fastendl;
            break;
        case 4:
            (ofstream &)verbose << Tab "Have you really understood what I've done?" fastendl;
            break;
        case 5:
            (ofstream &)verbose << Tab "Honestly, it was a real pleasure to compute this for you!" fastendl;
            break;
        case 6:
            (ofstream &)verbose << Tab "Shouldn't I have been paid for this, how computers are treated nowadays.." fastendl;
            break;
        case 7:
            (ofstream &)verbose << Tab "Wow! that's already done?!" fastendl;
            break;
        case 8:
            (ofstream &)verbose << Tab ";-)" fastendl;
            break;
        case 9:
            (ofstream &)verbose << Tab "This message will destruct itself in 10 seconds..." fastendl;
            break;
        }
        ...
        if (openauto)
        {
            ...
            if (SaveEpochsSingleFile && !IsFreqLoop() && !(SaveAverage || SaveSum))
