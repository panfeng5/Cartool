...
namespace crtl
{
    ...
    public:
        ...
        const bool IsClusters() const { return IsFlag(Flags, CRISPresetClusters); }
        bool IsEpochs() const { return IsFlag(Flags, CRISPresetEpochs); }
        bool IsCentroids() const { return IsFlag(Flags, CRISPresetCentroids); }
        const bool CanNorm() const { return IsFlag(Flags, CRISPresetNorm); }
        ...
        const bool CmGroupsAveragingEnable() const { return Code == ComputingRisPresetErpIndivMeans || IsEpochs(); } // still needs to check all groups of files are compatibles somewhere
    };
