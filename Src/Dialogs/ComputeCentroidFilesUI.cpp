    ...
    void TCartoolMdiClient::ComputeCentroidFilesUI(owlwparam w)
    {
        ...
        if (!gof.AnyTracksGroup(tg))
        {
            if (tg.noneofthese)
                ...
                ShowMessage("Files don't seem to be consistently of the same type!\nCheck again your input files...", CentroidTitle, ShowMessageWarning);
            return;
        }
        ...
        char answer = GetOptionFromUser("Type of Centroid:" NewLine Tab "(M)ean" NewLine Tab "(W)eighted Mean" NewLine Tab "Me(D)ian" NewLine Tab "Med(O)id" NewLine Tab "(E)igenvector" NewLine Tab "Ma(X)",
        ...
        bool thresholding = tg.allris
                            //                                 && IsPositive ( datatype )           // "vectorial" ranking is allowed
                            && centroidflag != MedianCentroid // Median can produce a lot of 0's, which will behave badly with our Ranking parameters
                            && ranking && GetAnswerFromUser("Thresholding input RIS data?", CentroidTitle, this);
        double threshold = 0;
        ...
        PolarityType polarity = !IsPositive(datatype) // EEG or vectorial RIS are OK
                                        && GetAnswerFromUser("Ignore polarity?", CentroidTitle, this)
                                    ? PolarityEvaluate
                                    : PolarityDirect;
        ...
        SpatialFilterType spatialfilter = (tg.alleeg // ESI has no Spatial filtering
                                           && GetAnswerFromUser("Applying Spatial filtering?", CentroidTitle, this))
                                              ? SpatialFilterDefault
                                              : SpatialFilterNone;
