...
namespace crtl
{
    ...
    TFileCalculatorDialog::~TFileCalculatorDialog()
    {
        ...
        delete Expression;
        delete BaseDir;
        delete CompoundFilenames;
        delete GenericFilenames;
        delete FileTypes;
    ...
    void TFileCalculatorDialog::GuessOutputFileExtension()
    {
        ...
        if (allris || someis)
            FileCalculatorTransfer.FileTypes.Select(PresetFileTypeRis);
        else if (alleph)
            FileCalculatorTransfer.FileTypes.Select(PresetFileTypeEph);
        else if (allep)
            FileCalculatorTransfer.FileTypes.Select(PresetFileTypeEp);
        else if (allbv)
            FileCalculatorTransfer.FileTypes.Select(PresetFileTypeBV);
        else if (allsef)
            FileCalculatorTransfer.FileTypes.Select(PresetFileTypeSef);
        else
            FileCalculatorTransfer.FileTypes.Select(PresetFileTypeDefaultEEG);
