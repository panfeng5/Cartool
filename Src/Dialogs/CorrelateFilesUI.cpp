...
namespace crtl
{
    ...
    void TCartoolMdiClient::CorrelateFilesUI()
    {
        ...
        char answer = GetOptionFromUser("Please specify the type of correlation/coherence you want:" NewLine Tab "- (S)patial Correlation between data and templates" NewLine Tab "- (T)ime-course Correlation" NewLine Tab "- P(H)ase-Intensity Coupling",
        ...
        if (!filenames1.AnyTracksGroup(tg1))
        {
            ...
            if (tg1.noneofthese)
                ...
                ShowMessage("Files don't seem to be consistently of the EEG type!", corrtitle, ShowMessageWarning);
            else if (!tg1.allfreq)
                ShowMessage("Files don't seem to be consistently of the Frequency type!", corrtitle, ShowMessageWarning);
            else if (!tg1.allris)
                ShowMessage("Files don't seem to be consistently of the RIS type!", corrtitle, ShowMessageWarning);
            else
                ShowMessage("Files don't seem to be consistently of the same type!", corrtitle, ShowMessageWarning);
        ...
        if (tg1.allfreq)
        {
            ...
            if (correlate == CorrelateTypePhaseIntCoupling && !(IsFreqTypePhase((FrequencyAnalysisType)fc1.FreqType) || IsFreqTypePositive((FrequencyAnalysisType)fc1.FreqType)))
            {
        ...
        bool spatialfilter1 = nofreqsallowed && correlate == CorrelateTypeSpatialCorrelation // right now, we cannot do a spatial filter if data are not ordered as spatial maps...
                              && tc1.NumTracks > 0 && alleeg1 && GetAnswerFromUser("Applying Spatial filtering on this first group?", corrtitle, this);
        ...
        GetFileFromUser getfiles2(correlate == CorrelateTypeSpatialCorrelation ? "Template file(s) to correlate the data with:"
                                  ...
                                  alleegerp1 ? 2 : alleegspont1 ? 3
                                               : tg1.allfreq    ? 4
                                               : tg1.allris     ? 5
                                                                : 1 /*6*/, // we can help the user by preselecting the right type of files
                                  GetFileMulti);
        ...
        if (tg2.allfreq)
        {
            ...
            if (correlate == CorrelateTypePhaseIntCoupling && !(IsFreqTypePhase((FrequencyAnalysisType)fc2.FreqType) || IsFreqTypePositive((FrequencyAnalysisType)fc2.FreqType)))
            {
                ShowMessage("Files do seem to be of Phase or Intensity types!", corrtitle, ShowMessageWarning);
                return;
            }
        ...
        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // We ask spatial filter only for tracks
        bool spatialfilter2 = nofreqsallowed && correlate == CorrelateTypeSpatialCorrelation // right now, we cannot do a spatial filter if data are not ordered as spatial maps...
                              && tc2.NumTracks > 0 && alleeg2 && GetAnswerFromUser("Applying Spatial filtering on this second group?", corrtitle, this);
        ...
        if (correlate == CorrelateTypeTimeCorrelation || correlate == CorrelateTypePhaseIntCoupling)
        {
        ...
        bool ignorepolarity = (alleeg1 || alleeg2) && correlate == CorrelateTypeSpatialCorrelation && !(IsAbsolute(datatype1) || IsAbsolute(datatype2)) && GetAnswerFromUser("Ignore polarity?", corrtitle, this);
