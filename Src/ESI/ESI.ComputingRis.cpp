...
namespace crtl
{
                      ...
                      VerboseType verbosey)
    {
        ...
        bool individualinverses = (int)inversefiles > 1 && (int)inversefiles == (Is1Group1Subject(grouplayout) ? gogof.NumGroups()
        ...
        bool computingindividualfiles = savingindividualfiles || savingepochfiles || computegroupsaverages || computegroupscentroids;

        savingzscorefactors = savingzscorefactors && backnorm != BackgroundNormalizationNone;
        ...
        // all processings:
        bool ispreprocessing = savingindividualfiles || savingepochfiles || computegroupsaverages || computegroupscentroids || savingzscorefactors;
        ...
        verbose.NextTopic("Inverse Processing:");
        {
            ...
            {
                if (IsPositive(datatypeepochs))
                    verbose.Put("Epochs data type:", "Positive Data (Norm of Vectors)");
                else if (IsVector(datatypeepochs))
                    verbose.Put("Epochs data type:", "3D Vectorial Data");
            }

            if (IsPositive(datatypefinal))
                verbose.Put("Final data type:", "Positive Data (Norm of Vectors)");
            else if (IsVector(datatypefinal))
                verbose.Put("Final data type:", "3D Vectorial Data");
            ...
            if (computegroupscentroids)
            {
                ...
                if (IsPositive(centroidsdatatype))
                    verbose.Put("Centroids data type:", "Positive Data (Norm of Vectors)");
                else if (IsVector(centroidsdatatype))
                    verbose.Put("Centroids data type:", "3D Vectorial Data");
            }
        }
            ...
            if (grouplayout == GroupsLayout1SubjAllCondAllEpochs)
            {
                ...
                for (int condi = 0; condi < splitgogof.NumGroups(); condi++)
                {
                    ...
                    if (savingindividualfiles || computegroupsaverages)
                    {
                        ...
                        if (esicase == ComputingRisPresetFreq)
                            BatchAveragingFreq(splitgogof[condi],
                                               FreqFFTComplex, PolarityDirect, // not important as long as not FreqFFTApproximation
                                               meanfile, 0,
                                               false, true);
                        else
                        { // writing vectorial, as should be
                            if (IsVector(datatypeepochs))
                                BatchAveragingVectorial(splitgogof[condi],
                                                        meanfile, 0, 0,
                                                        0, 0, 0,
                                                        false, true);
                            ...
                            //                                                                                  );
                                ...
                                BatchAveragingScalar(splitgogof[condi],
                                                     meanfile, 0, 0,
                                                     0, 0,
                                                     false, true);
                        }
                ...
                if ((savingindividualfiles || computegroupsaverages) && (dataranking || datathresholding))
        ...
        if (computegroupsaverages)
        {
            ...
            for (int absg = 0; absg < gogofpercondition.NumGroups(); absg++)
            {
                ...
                if (esicase == ComputingRisPresetFreq)
                    BatchAveragingFreq(gogofpercondition[absg],
                                       FreqFFTComplex, PolarityDirect, // not important as long as not FreqFFTApproximation
                                       meanfile, 0,
                                       false, true);
                else
                { // vectorial results allowed
                    if (IsVector(datatypefinal))
                        BatchAveragingVectorial(gogofpercondition[absg],
                                                meanfile, 0, 0,
                                                0, 0, 0,
                                                false, true);
                    ...
                    // RIS Vectorial will be straightforwardly averaged as Norm
                        ...
                        BatchAveragingScalar(gogofpercondition[absg],
                                             meanfile, 0, 0,
                                             0, 0,
                                             false, true);
                }
