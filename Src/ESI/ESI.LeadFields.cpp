    ...
    */
                                    ...
                                    TPoints &outputsolpoint, TSelection &spsrejected)
    {
        ...
        for (int outi = 0; outi < (int)outputsolpoint; outi++)
        {
            ...
            if (!(spvol.GetValueChecked(p.X, p.Y, p.Z) != 0 && spvol.GetValueChecked(p.X + 1, p.Y, p.Z) != 0 && spvol.GetValueChecked(p.X, p.Y + 1, p.Z) != 0 && spvol.GetValueChecked(p.X + 1, p.Y + 1, p.Z) != 0 && spvol.GetValueChecked(p.X, p.Y, p.Z + 1) != 0 && spvol.GetValueChecked(p.X + 1, p.Y, p.Z + 1) != 0 && spvol.GetValueChecked(p.X, p.Y + 1, p.Z + 1) != 0 && spvol.GetValueChecked(p.X + 1, p.Y + 1, p.Z + 1) != 0))
            {
    ...
    void CheckNullLeadField(const AMatrix &K, TSelection &spsrejected)
    {
        ...
        for (int sp = 0; sp < numsolp; sp++)
        {
            ...
            for (el = 0; el < numel; el++)
                ...
                if (K(el, 3 * sp) != 0 || K(el, 3 * sp + 1) != 0 || K(el, 3 * sp + 2) != 0)
            ...
            for (el = 0; el < numel; el++)
                ...
                if (IsNotAProperNumber(K(el, 3 * sp)) || IsNotAProperNumber(K(el, 3 * sp + 1)) || IsNotAProperNumber(K(el, 3 * sp + 2)))
                {
                     ...
                     double Xi, double innerskullradius, double outerskullradius)
    {
        ...
        double radius3toradius1 = radius3 > 0 ? (radialradius1 + tangentialradius1) / 2 / radius3
                                              : 1;
        ...
        const TArray1<double> &R, const TArray1<double> &sigma)
    {
        ...
        radius3toradius1 = radius3 > 0 ? (radialw * radialradius1 + tangentialw * tangentialradius1) / radius3
                                       : 0;

        moment3tomoment1 = radialw * radialmoment1 + tangentialw * tangentialmoment1;
    ...
    long double AryFn(double n, double Xi, double innerskullradius, double outerskullradius)
    {
        ...
        dn = ((n + 1) * Xi + n) * ((n * Xi / (n + 1) + 1) + (1 - Xi) * (powl(innerskullradius, 2 * n + 1) - powl(outerskullradius, 2 * n + 1))) - n * Square(1 - Xi) * powl(innerskullradius / outerskullradius, 2 * n + 1);
                     ...
                     double Xi, double innerskullradius, double outerskullradius)
    {
        ...
        for (int i = 1; i <= numlegendre; i++)
        {
            ...
            mubF += factor * powl(radius1, i - 1.0) * powl(radius3, i - 1.0) * AryFn(i, Xi, innerskullradius, outerskullradius);
                            ...
                            double Xi, double innerskullradius, double outerskullradius)
    {
        ...
        do
        {
            ...
            // choose which direction is minimum
            if (tangentialrhol < tangentialrhor)
                radius1 -= step;
            else
                radius1 += step;
        ...
        double sigma)
    {
        ...
        return (rhat * (3 * dipoledir.ScalarProduct(rhat)) - dipoledir) / (FourPi * sigma * relpos.Norm3());
    }
        ...
        double sigma)
    {
        ...
        double v = dipole.Direction.ScalarProduct(rd) / (FourPi * sigma) / 2.46; // not in actual formula, used for comparison with current N-Shell implementation
        ...
        double sigma)
    {
        ...
        double v = dipole.Direction.ScalarProduct(kev) / (FourPi * sigma * kev.Norm3()); // + cst
    ...
    */
        ...
        double sigma)
    {
        ...
        if (dipole.Position.IsNull() // dipole spot on center
            ...
            || dipole.Direction.IsAligned(dipole.Position, SingleFloatEpsilon) || electrodepos.IsAligned(dipole.Position, SingleFloatEpsilon))
        ...
        double Ur = Dr * (2 * (R * cosgamma - spradius) / L3 + (1 / (spradius * L) - 1 / (spradius * R)));

        // Tangential voltage contribution
        double Ut = Dt * cosbeta * singamma * (2 * R / L3 + (L + R) / (L * R * (R - spradius * cosgamma + L)));
...
#define PotentialIsotropicNLayersError 1e-6
        ...
        int maxterms, double convergence)
    {
        ...
        { // ComputePotentials             // dipole direction provided
        ...
        for (int n = 1; n <= maxterms; n++)
        {
            ...
            RoRe = (2 * n + 1) / (long double)n * powl(spradius, n - 1); // Ro / Re = spradius, as both electrode and SP are already in a sphere, therefor electrode radius is 1, outer SP radius is also 1, so ratio is actual SP radius
            ...
            for (int k = 0; k < numlayers - 1; k++)
            {
                ...
                p11 = n + (n + 1) * sksk1;
                p12 = (n + 1) * (sksk1 - 1) * powl(1 / R[k], 2 * n + 1);
                p21 = n * (sksk1 - 1) * powl(R[k], 2 * n + 1);
                p22 = (n + 1) + n * sksk1;
...
//    potiso ( potisosp, n - 1 )  = deltaUI;
    ...
    {
        if (lfpreset.IsUndefined() || !lfpreset.IsGeometryLSMAC() || StringIsEmpty(headfile) || StringIsEmpty(brainfile) || StringIsEmpty(xyzfile) || StringIsEmpty(spfile))
        ...
        if (mrihead.IsNotOpen() || mribrain.IsNotOpen() || xyzdoc.IsNotOpen() || xyzpoints.IsEmpty() || solpoints.IsEmpty())
    ...
    bool ComputeLeadFieldLSMAC_Segmentation(const OneLeadFieldPreset &lfpreset,
                                            ...
                                            TArray3<float> &tissuesradii)
    {
        if (lfpreset.IsUndefined() || !lfpreset.IsGeometryLSMAC() || StringIsEmpty(headfile) || StringIsEmpty(tissuesfile) || StringIsEmpty(xyzfile) || StringIsEmpty(spfile))
        ...
        if (mrihead.IsNotOpen() || mritissues.IsNotOpen() || xyzdoc.IsNotOpen() || xyzpoints.IsEmpty() || solpoints.IsEmpty())
