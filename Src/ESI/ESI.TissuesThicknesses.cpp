...
namespace crtl
{
    ...
    void AgeToSkullThicknesses_Roche(double age, double &vertex, double &lambda, double &nasionbregma)
    {
        ...
        // max at 5.50
        vertex = 0.000009446242636601540000000000 * powl(age, 5) - 0.000620319286298091000000000000 * powl(age, 4) + 0.015623870257432400000000000000 * powl(age, 3) - 0.193411281780735000000000000000 * powl(age, 2) + 1.304797848964830000000000000000 * age + 0.801749159837984000000000000000;

        // max at 6.4
        lambda = -0.000000874303233450169000000000 * powl(age, 6) + 0.000066806737937810700000000000 * powl(age, 5) - 0.002046648993681260000000000000 * powl(age, 4) + 0.032258829477856900000000000000 * powl(age, 3) - 0.287182885923529000000000000000 * powl(age, 2) + 1.599201227389930000000000000000 * age + 0.841345460863540000000000000000;

        // max at 5.05
        nasionbregma = -0.000001436028494006260000000000 * powl(age, 6) + 0.000102422066281027000000000000 * powl(age, 5) - 0.002885660844764400000000000000 * powl(age, 4) + 0.040738161856106000000000000000 * powl(age, 3) - 0.306182846374295000000000000000 * powl(age, 2) + 1.311049399323510000000000000000 * age + 1.248428210823240000000000000000;
    }
...
#define DefaultOuterSkullRelativeRadius 0.940
                           ...
                           SpatialFilterType spatialfilter, const char *filexyz)
    {
        ...
        for (int ri = 0; ri < numestimates; ri++)
            for (int ei = 0; ei < numel; ei++)
                ...
                if (radius(ri, ei))
                    stats.Add(oldradius(ri, ei) / radius(ri, ei));
                              ...
                              TArray3<float> &tissuesradii, double targetskullthickness)
    {
        ...
        for (int ei = 0; ei < numel; ei++)
            ...
            if (points[ei].Z > 0 && tissuesradii(ei, SkullIndex, ThickAbs) > 0)
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    */
    ...
    {
        if (tissuesradii.IsNotAllocated() || seltissues.IsNotAllocated() || xyzpoints.IsEmpty() || StringIsEmpty(filesurfaceall))
        ...
        for (int ei = 0; ei < (int)xyzpoints; ei++)
        {
            ...
            if (seltissues[ScalpIndex])
                pointsouterscalp[ei] = p + deltacenter;
            if (seltissues[SkullIndex])
                pointsouterskull[ei] = (p * tissuesradii(ei, SkullIndex, OuterRel)) + deltacenter;
            if (seltissues[SkullSpongyIndex])
                pointsouterskullspongy[ei] = (p * tissuesradii(ei, SkullSpongyIndex, OuterRel)) + deltacenter;
            if (seltissues[SkullSpongyIndex])
                pointsinnerskullspongy[ei] = (p * tissuesradii(ei, SkullSpongyIndex, InnerRel)) + deltacenter;
            if (seltissues[SkullIndex])
                pointsinnerskull[ei] = (p * tissuesradii(ei, SkullIndex, InnerRel)) + deltacenter;
            if (seltissues[CsfIndex])
                pointsinnercsf[ei] = (p * tissuesradii(ei, CsfIndex, InnerRel)) + deltacenter;
        }
        ...
        if (seltissues[ScalpIndex])
        {
            cluster.Points = pointsouterscalp;
            StringCopy(cluster.ClusterName, "Scalp");
            clusters.push_back(cluster);
        }
        if (seltissues[SkullIndex])
        {
            cluster.Points = pointsouterskull;
            StringCopy(cluster.ClusterName, "SkullOut");
            clusters.push_back(cluster);
        }
        if (seltissues[SkullSpongyIndex])
        {
            cluster.Points = pointsouterskullspongy;
            StringCopy(cluster.ClusterName, "SkullSpongyOut");
            clusters.push_back(cluster);
        }
        if (seltissues[SkullSpongyIndex])
        {
            cluster.Points = pointsinnerskullspongy;
            StringCopy(cluster.ClusterName, "SkullSpongyIn");
            clusters.push_back(cluster);
        }
        if (seltissues[SkullIndex])
        {
            cluster.Points = pointsinnerskull;
            StringCopy(cluster.ClusterName, "SkullIn");
            clusters.push_back(cluster);
        }
        if (seltissues[CsfIndex])
        {
            cluster.Points = pointsinnercsf;
            StringCopy(cluster.ClusterName, "CSF");
            clusters.push_back(cluster);
        }
