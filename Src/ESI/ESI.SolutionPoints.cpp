...
namespace crtl
{
        ...
        const char *filesp)
    {
        ...
        plaura(FilterParamNumNeighbors) = lauraneighborhood == Neighbors26 ? 8 : lauraneighborhood == Neighbors18 ? 6
                                                                                                                  : 2;

        ploreta(FilterParamNeighborhood) = loretaneighborhood;
        ploreta(FilterParamNumNeighbors) = loretaneighborhood == Neighbors26 ? 8 : loretaneighborhood == Neighbors18 ? 6
                                                                                                                     : 2; // reject lower than 8/6/2
        ...
        for (int numguess = 0; numguess < maxguess; numguess++)
        {
            ...
            if (spflags & (GreyMatterLauraCheck | GreyMatterLoretaCheck | GreyMatterSinglePointCheck))
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (spflags & GreyMatterLauraCheck)
                        spvol.Filter(FilterTypeLessNeighbors, plaura);
                    if (spflags & GreyMatterLoretaCheck)
                        spvol.Filter(FilterTypeLessNeighbors, ploreta);
                    if (spflags & GreyMatterSinglePointCheck)
                        spvol.Filter(FilterTypeLessNeighbors, psingle);
                }
            ...
            if (fabs((double)(numsp - numsolpointswished)) < fabs((double)(bestnumsolpoints - numsolpointswished)) && numsp >= numsolpointswished // while preferring being above the requested number of points (5009 is clearer than 4993)
            )
            {
...
#define right "R"
                                  ...
                                  TSelection &spsrejected)
    {
        ...
        int step = Round(PowerRoot((double)numspremaining / numsolpointswished * ((bool)spsrejected ? 0.85 : 1), 3.0));
        ...
        for (int spi = 0; spi < numsolp; spi++)
        {
            ...
            if (Round(downp.X) % step != 0 || Round(downp.Y) % step != 0 || Round(downp.Z) % step != 0)
