...
namespace crtl
{
...

    {
        ...
        if (!(merging == FilterTypeMedian || merging == FilterTypeMean))

            merging = FilterTypeNone;

        if (!(atomformat == AtomFormatByte || atomformat == AtomFormatFloat))

            atomformat = RisToVolumeDefaultAtomFormat;

        char fileext[32];

        if (IsFileTypeNifti(filetype))
            StringCopy(fileext, FILEEXT_MRINII);
        else if (IsFileTypeAnalyze(filetype))
            StringCopy(fileext, FILEEXT_MRIAVW_HDR);
        else
        ...
        if (spinterpol == SPInterpolation1NN)
            toip1nn = spdoc->GetInterpol1NN();
        else if (spinterpol == SPInterpolation4NN)
            toip4nn = spdoc->GetInterpol4NN();
        ...
        if (IsSolutionPointsScan(interpol))
        {
            ...
            if (interpol == VolumeInterpolationLinearRect)
                kernelradiusf = radiusf; // square    kernel
                ...
                kernelradiusf = 2.0 * radiusf; // spherical kernel - no need to boost, kernel is big enough already
        ...
        for (int blocki = 0; blocki < numsavedblocks; blocki++)
        {
            ...
            //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // generate file name
            if (outputn3d || outputn4d && firstblock)
            {
            ...
            if (merging == FilterTypeMedian)
                ris.Median(blockfromtf, blocktotf, meanrismap);
            else if (merging == FilterTypeMean)
                ris.Mean(blockfromtf, blocktotf, meanrismap);
            else
                meanrismap = ris[blockfromtf];
                    ...
                    for (xki = 0, vox.X = sp.X - kernelradiusi; xki < kerneldiameteri; xki++, vox.X++)
                        for (yki = 0, vox.Y = sp.Y - kernelradiusi; yki < kerneldiameteri; yki++, vox.Y++)
                            for (zki = 0, vox.Z = sp.Z - kernelradiusi; zki < kerneldiameteri; zki++, vox.Z++)
                            {
                                ...
                                if (grey.GetValueChecked(vox) <= mrithreshold)
                                    continue; // for exact grey mask
                                ...
                                if (interpol == VolumeInterpolationLinearRect)
                                    ...
                                    w = CubicRoot((1 - Clip(abs(sp0.X) / kernelradiusf, 0.0, 1.0)) * (1 - Clip(abs(sp0.Y) / kernelradiusf, 0.0, 1.0)) * (1 - Clip(abs(sp0.Z) / kernelradiusf, 0.0, 1.0)));
                                ...
                                else if (interpol == VolumeInterpolationCubicFastSplineSpherical)
                                {
                                    ...
                                    {
                                        dk -= 2;
                                        w = 3 * Cube(dk) - 6 * Square(dk) + 6 * dk + 4;
                                    }
                                    else
                                        w = Cube(4 - dk);
                                }
                                ...
                                if (firstblock)
                                { // storing weights is needed only once
                                    OmpAtomic
            ...
            else if (IsVoxelScan(interpol))
            {
                    ...
                    for (int li = 0; li < vol.GetLinearDim(); li++)
                {
                    ...
                    if (interpol == VolumeInterpolation1NN)
                    {
                        ...
                        if (!toip1nn->WithinBoundary(pvol))
                            continue;

                        if (toip1nn->GetValue(pvol) == UndefinedInterpolation1NN)
                            continue;
                    ...
                    else if (interpol == VolumeInterpolation4NN)
                    {
                        ...
                        if (!toip4nn->WithinBoundary(pvol))
                            continue;
                        ...
                        v = (toi4->w1 * meanrismap(toi4->i1) + toi4->w2 * meanrismap(toi4->i2) + toi4->w3 * meanrismap(toi4->i3) + toi4->w4 * meanrismap(toi4->i4)) / TWeightedPoints4SumWeights;
            ...
            else if (outputn4d && firstblock)
                expvol.MaxValue = ris.GetAbsMaxValue(fromtf, totf); // not optimal when writing integers + IsSolutionPointsScan
