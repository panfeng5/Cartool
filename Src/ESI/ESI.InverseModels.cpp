...
namespace crtl
{
    ...
    OptimizeOn
        ...
        TPointFloat
        ComputeOptimalInverseTranslation(
            const TPoints *headpoints,
            const TPoints *solpoints,
            const TPoints *xyzpoints,
            bool showprogress)
        ...
        TVector<double> &regulvalues, TStrings &regulnames)
    {
        ...
        if (IsNotAProperNumber(biggesteigen))
            biggesteigen = 1;
        ...
        char *fileresmat, char *fileresmats, char *fileresmatt)
    {
        ...
        if (!(numel > 0 && numsolp > 0 && StringIsNotEmpty(fileinverse) && (isresmat || isresmats || isresmatt) && !solpoints.IsEmpty()))
            return;
        ...
        TVerboseFile *verbose)
    {
        ...
        double wlapl6 = neighborhood == Neighbors6 ? 1.0 / 6 : neighborhood == Neighbors18 ? 1.0 / 18
                                                           : neighborhood == Neighbors26   ? 1.0 / 26
                                                                                           : 0;
        double wlapl18 = neighborhood == Neighbors18 ? 1.0 / 18 : neighborhood == Neighbors26 ? 1.0 / 26
                                                                                              : 0;
            ...
            for (int j = i + 1; j < numsolp; j++)
            {
                ...
                if (d <= neighborradius && !(postpone26 && isn26))
                {
                    ...
                    if (neighborhood == Neighbors6)
                        A1(i, j) = A1(j, i) = wlapl6;
                    else if (neighborhood == Neighbors18)
                        A1(i, j) = A1(j, i) = (isn6 ? wlapl6 : wlapl18);
                    else if (neighborhood == Neighbors26)
                        A1(i, j) = A1(j, i) = (isn6 ? wlapl6 : isn18 ? wlapl18
                                                                     : wlapl26);
        ...
        const char *filename)
    {
        ...
        gauge.AddPart(gaugeeloretapseudoinverse, regularization ? NumSavedRegularizations * (4 + ELoretaMaxIterations) : (4 + ELoretaMaxIterations), 95);
        ...
        for (int reg = 0; reg < numreg; reg++)
        {
            ...
            for (int iter = 0; iter < ELoretaMaxIterations; iter++)
            {
                ...
                if (regularization)
                    M = APseudoInverseSymmetric(KWinvKt + regulvalues[reg] * H);
                else
                    M = APseudoInverseSymmetric(KWinvKt);
                    ...
                    for (int sp3 = 0; sp3 < numsolp3; sp3 += 3)
                {
                    ...
                    {
                        sumdiff += Square(Winv(sp3, sp3) - Winvj(0, 0)) + 2 * Square(Winv(sp3, sp3 + 1) - Winvj(0, 1)) + 2 * Square(Winv(sp3, sp3 + 2) - Winvj(0, 2)) + Square(Winv(sp3 + 1, sp3 + 1) - Winvj(1, 1)) + 2 * Square(Winv(sp3 + 1, sp3 + 2) - Winvj(1, 2)) + Square(Winv(sp3 + 2, sp3 + 2) - Winvj(2, 2));

                        sumold += Square(Winv(sp3, sp3)) + 2 * Square(Winv(sp3, sp3 + 1)) + 2 * Square(Winv(sp3, sp3 + 2)) + Square(Winv(sp3 + 1, sp3 + 1)) + 2 * Square(Winv(sp3 + 1, sp3 + 2)) + Square(Winv(sp3 + 2, sp3 + 2));
                    }
        ...
        const char *filename)
    {
        ...
        { // no regularization - actually this case shouldn't be used, as there is no theoretical noise...
        ...
        for (int i = 0; i < numsolp; i++)
        {
            ...
            for (int j = i + 1; j < numsolp; j++)
            {
                ...
                if (d <= neighborradius && !(postpone26 && isn26))
                {
                    ...
                    if (neighborhood == Neighbors6 && isn6 || neighborhood == Neighbors18 && (isn6 || isn18) || neighborhood == Neighbors26 && (isn6 || isn18 || isn26))
                    {
